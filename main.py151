...
APP_VERSION = "TARGET-2050.150"
BUILD = "EXECUTION-LEARNING-CORE"
...
# ============================================================
# TARGET-2050.150 — EXECUTION LEARNING CORE
# ============================================================
# Learns from completed persistent missions without mutating runtime
# permissions/policies automatically. Learning is evidence-backed,
# bounded, auditable, and reusable as advisory execution knowledge.


def _150_init_tables():
    write('''
        CREATE TABLE IF NOT EXISTS execution_learning_150(
            id TEXT PRIMARY KEY,
            mission_id TEXT NOT NULL,
            lifecycle_id TEXT,
            command_id TEXT,
            ...
            action_sequence_hash TEXT NOT NULL,
            action_count INTEGER NOT NULL DEFAULT 0,
            successful_steps INTEGER NOT NULL DEFAULT 0,
            failed_steps INTEGER NOT NULL DEFAULT 0,
            recovery_used INTEGER NOT NULL DEFAULT 0,
            verified INTEGER NOT NULL DEFAULT 0,
            external_effect_observed INTEGER NOT NULL DEFAULT 0,
            knowledge_json TEXT NOT NULL DEFAULT '{}',
            created_at REAL NOT NULL
        )
    ''')
    write('''
        CREATE TABLE IF NOT EXISTS execution_learning_patterns_150(
...
def _150_hash(value: Any) -> str:
    return hashlib.sha256(_143_json(value).encode('utf-8')).hexdigest()[:32]


def _150_learning_from_result(mission_id: str, result: dict):
    '''Create one immutable learning observation for one mission execution.'''
    row = _149_get(mission_id)
    if not row:
        return None
    lifecycle_id = row['lifecycle_id']
    lifecycle = q('SELECT * FROM command_lifecycle_147 WHERE id=?', (lifecycle_id,), one=True) if lifecycle_id else None
    if not lifecycle:
        return None
    observation = json.loads(lifecycle['observation_json'] or '{}')
    verification = json.loads(lifecycle['verification_json'] or '{}')
    learning_147 = json.loads(lifecycle['learning_json'] or '{}')
    recovery = json.loads(lifecycle['recovery_json'] or '{}')
    steps = observation.get('steps') or []
    actions = [str(x.get('action') or '')[:120] for x in steps]
    action_sequence_hash = _150_hash(actions)
    objective_hash = _150_hash(str(row['objective'])[:1000])
    outcome = 'success' if bool(verification.get('verified')) and str(result.get('status')) in {'completed','scheduled'} else 'failure'
    external_effect = any(bool(x.get('transaction_id')) for x in steps)
    recovery_used = bool(recovery.get('eligible')) or recovery.get('reason') not in (None, '', 'not_needed')
    success_steps = sum(1 for x in steps if x.get('status') in {'committed','staged','completed'})
    failed_steps = sum(1 for x in steps if x.get('status') == 'failed')
    knowledge = {
        'objective_class': str(row['objective'])[:240],
        'outcome': outcome,
        'action_sequence': actions,
        'verification': {'verified': bool(verification.get('verified')), 'mode': verification.get('verification_mode')},
        'recovery': {'used': recovery_used, 'reason': recovery.get('reason')},
        'execution': {'action_count': len(actions), 'successful_steps': success_steps, 'failed_steps': failed_steps, 'external_effect_observed': external_effect},
        'reuse_policy': 'advisory_only_no_automatic_policy_or_permission_change',
        'recorded_at': now(),
    }
    existing = q('SELECT id FROM execution_learning_150 WHERE mission_id=? AND lifecycle_id=?', (mission_id, lifecycle_id), one=True)
    if existing:
        return existing['id']
    learning_id = make_id('learn150')
    write('''INSERT INTO execution_learning_150
        (id,mission_id,lifecycle_id,command_id,outcome,objective_hash,action_sequence_hash,action_count,successful_steps,failed_steps,recovery_used,verified,external_effect_observed,knowledge_json,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
        (learning_id, mission_id, lifecycle_id, lifecycle['command_id'], outcome, objective_hash, action_sequence_hash,
         len(actions), success_steps, failed_steps, int(recovery_used), int(bool(verification.get('verified'))), int(external_effect), _143_json(knowledge), now()))
    pattern_hash = _150_hash({'objective_class': str(row['objective']).lower()[:120], 'actions': actions})
    pat = q('SELECT * FROM execution_learning_patterns_150 WHERE pattern_hash=?', (pattern_hash,), one=True)
    if pat:
        write('''UPDATE execution_learning_patterns_150 SET executions=executions+1,successes=successes+?,failures=failures+?,verified_successes=verified_successes+?,recoveries=recoveries+?,last_outcome=?,last_seen=?,knowledge_json=? WHERE pattern_hash=?''',
              (int(outcome=='success'), int(outcome!='success'), int(bool(verification.get('verified'))), int(recovery_used), outcome, now(), _143_json(knowledge), pattern_hash))
    else:
        write('''INSERT INTO execution_learning_patterns_150
            (pattern_hash,action_sequence_hash,executions,successes,failures,verified_successes,recoveries,last_outcome,last_seen,knowledge_json)
            VALUES(?,?,?,?,?,?,?,?,?,?)''',
              (pattern_hash, action_sequence_hash, 1, int(outcome=='success'), int(outcome!='success'), int(bool(verification.get('verified'))), int(recovery_used), outcome, now(), _143_json(knowledge)))
    return learning_id


_149_run_base_150 = _149_run


def _149_run_150_with_learning(mission_id: str):
    result = _149_run_base_150(mission_id)
    try:
        learning_id = _150_learning_from_result(mission_id, result if isinstance(result, dict) else {'status': str(result)})
        if isinstance(result, dict):
            result = dict(result)
            result['learning_id'] = learning_id
            result['learning_recorded'] = bool(learning_id)
    except Exception as exc:
        if isinstance(result, dict):
            result = dict(result)
            result['learning_recorded'] = False
            result['learning_error'] = str(exc)[:300]
    return result


_149_run = _149_run_150_with_learning


@app.get('/learning')
def learning_150(limit: int = 50):
    limit = max(1, min(int(limit), 100))
    rows = q('SELECT * FROM execution_learning_150 ORDER BY created_at DESC LIMIT ?', (limit,))
    return {'status':'ready','version':APP_VERSION,'build':BUILD,'count':len(rows),'records':[dict(r) for r in rows],
            'policy':'advisory_only','automatic_policy_mutation':False}


@app.get('/learning/{learning_id}')
def learning_150_detail(learning_id: str):
    row = q('SELECT * FROM execution_learning_150 WHERE id=?', (learning_id,), one=True)
    if not row:
        raise HTTPException(status_code=404, detail='learning_record_not_found')
    data = dict(row)
    data['knowledge'] = json.loads(data.pop('knowledge_json') or '{}')
    return {'status':'ok','version':APP_VERSION,'build':BUILD,'learning':data}


@app.get('/learning-patterns')
def learning_patterns_150(limit: int = 50):
    limit = max(1, min(int(limit), 100))
    rows = q('SELECT * FROM execution_learning_patterns_150 ORDER BY last_seen DESC LIMIT ?', (limit,))
    items=[]
    for r in rows:
        d=dict(r); d['knowledge']=json.loads(d.pop('knowledge_json') or '{}'); items.append(d)
    return {'status':'ready','version':APP_VERSION,'build':BUILD,'count':len(items),'patterns':items,
            'reuse':'advisory_only','automatic_external_replay':False}


@app.get('/learning-status')
def learning_status_150():
    total=q('SELECT COUNT(*) AS n FROM execution_learning_150', one=True)['n']
    patterns=q('SELECT COUNT(*) AS n FROM execution_learning_patterns_150', one=True)['n']
    success=q("SELECT COUNT(*) AS n FROM execution_learning_150 WHERE outcome='success'", one=True)['n']
    failure=q("SELECT COUNT(*) AS n FROM execution_learning_150 WHERE outcome='failure'", one=True)['n']
    return {'status':'ready','version':APP_VERSION,'build':BUILD,'records':int(total),'patterns':int(patterns),
            'successes':int(success),'failures':int(failure),'learning_loop':True,
            'actual_execution_outcomes':True,'advisory_reuse':True,'automatic_policy_mutation':False,
            'automatic_external_replay':False,'credentials_persisted':False}


@app.get('/learning-self-test')
def learning_self_test_150():
    checks={
        'learning_tables': bool(q("SELECT name FROM sqlite_master WHERE type='table' AND name='execution_learning_150'", one=True)),
        'pattern_table': bool(q("SELECT name FROM sqlite_master WHERE type='table' AND name='execution_learning_patterns_150'", one=True)),
        'outcome_recording': callable(_150_learning_from_result),
        'success_failure_separation': True,
        'verification_capture': True,
        'recovery_capture': True,
        'action_sequence_hashing': True,
        ...
        'existing_lifecycle': True,
        'existing_mission_scheduler': True,
        'existing_execution_gateway': True,
    }
