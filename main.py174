...
APP_VERSION = "TARGET-2050.173"
BUILD = "REAL-WORLD-WORKFLOW-ORCHESTRATION-CORE"
PREVIOUS_BUILD = "TARGET-2050.172"
...
def create_workflow(steps: List[Dict[str, Any]]) -> str:
    ...
    t = now()
    with _db_lock, db() as c:
        c.execute("INSERT INTO workflows(id,status,steps_json,created_at,updated_at) VALUES(?,?,?,?,?)",
                  (wid, "queued", json.dumps(steps, ensure_ascii=False), t, t))
        for i, step in enumerate(steps):
            command_text = str(step.get("command") or step.get("objective") or "").strip()
            dep = step.get("depends_on")
            dep_i = int(dep) if dep is not None and str(dep).isdigit() else (i - 1 if i > 0 and step.get("depends_on") == "previous" else None)
            idem = str(step.get("idempotency_key") or f"{wid}:{i}:{digest(command_text)[:16]}")
            input_json = json.dumps(step.get("input"), ensure_ascii=False) if step.get("input") is not None else None
            c.execute("INSERT INTO workflow_steps(workflow_id,step_index,command,status,created_at,updated_at,input_json,idempotency_key,dependency_index) VALUES(?,?,?,?,?,?,?,?,?)",
                      (wid, i, command_text, "queued", t, t, input_json, idem, dep_i))
    workflow_event(wid, "created", {"step_count": len(steps), "orchestration": True})
    return wid
...
def _result_context(wid: str) -> Dict[str, Any]:
    with _db_lock, db() as c:
        rows=c.execute("SELECT step_index,result_json,status FROM workflow_steps WHERE workflow_id=? ORDER BY step_index",(wid,)).fetchall()
    steps={}
    for r in rows:
        value=None
        if r[1]:
        ...
        steps[str(r[0])] = {"status":r[2], "result":value}
    return {"steps":steps}
...
def _workflow_step_command(wid: str, step: Dict[str, Any]) -> str:
    raw = str(step.get("command") or step.get("objective") or "").strip()
    resolved = _resolve_templates(raw, _result_context(wid))
    return resolved
    ...
    for index in range(start_index, len(steps)):
        ...
        try:
            ...
            if dep is not None and dep != "previous":
                dep_i=int(dep)
                dep_step=workflow_step(wid,dep_i)
                if not dep_step or dep_step.get("status") != "completed":
                    update_workflow(wid,"blocked",results+[ {"step":index,"status":"blocked","depends_on":dep_i} ])
                    workflow_event(wid,"dependency_blocked",{"step":index,"depends_on":dep_i})
                    return {"workflow_id":wid,"status":"blocked","current_step":index,"results":results}
            if not objective:
                raise ValueError("each workflow step needs command or objective")
            ...
            approval=bool(plan.get("side_effect"))
            existing=workflow_step(wid,index)
            eid=existing.get("execution_id") if existing else None
            if eid:
                e=get_execution(eid)
                if e and e.get("status") == "completed":
                    results.append({"step":index,"execution_id":eid,"status":"completed","result":e.get("result_json")})
                    continue
            if not eid:
                eid=create_execution(plan.get("action_type","unknown"),plan.get("connector","none"),objective,approval,parent_id=None,payload={"workflow_id":wid,"step":index,"idempotency_key":existing.get("idempotency_key") if existing else None})
                update_workflow_step(wid,index,execution_id=eid)
            if approval:
                update_execution(eid,status="pending_approval")
                update_workflow_step(wid,index,status="waiting_approval")
                update_workflow(wid,"waiting_approval",results+[ {"step":index,"execution_id":eid,"status":"pending_approval","approval_required":True} ])
                workflow_event(wid,"approval_required",{"step":index,"execution_id":eid})
                return {"workflow_id":wid,"status":"waiting_approval","current_step":index,"execution_id":eid,"results":results}
            run_execution(eid,plan,approved=True)
            e=get_execution(eid)
            status=e.get("status") if e else "missing"
            result={"step":index,"execution_id":eid,"status":status,"result":e.get("result_json") if e else None}
            ...
            if status != "completed":
                update_workflow(wid,"uncertain" if status=="uncertain" else "failed",results)
                return {"workflow_id":wid,"status":"uncertain" if status=="uncertain" else "failed","current_step":index,"results":results}
        except Exception as ex:
            update_workflow_step(wid,index,status="failed",error=str(ex))
            workflow_event(wid,"step_failed",{"step":index,"error":str(ex)})
            update_workflow(wid,"failed",results+[ {"step":index,"status":"failed","error":str(ex)} ])
            return {"workflow_id":wid,"status":"failed","current_step":index,"results":results}
    update_workflow(wid,"completed",results)
    workflow_event(wid,"completed",{"step_count":len(steps)})
    return {"workflow_id":wid,"status":"completed","results":results}

def update_workflow(wid: str, status: str, result: Any = None) -> None:
    with _db_lock, db() as c:
        c.execute("UPDATE workflows SET status=?, result_json=?, updated_at=? WHERE id=?",
                  (status, json.dumps(result, ensure_ascii=False) if result is not None else None, now(), wid))
...
@app.get("/")
def root():
    return {"name":"AI Infinity","status":"online","version":APP_VERSION,"build":BUILD,"previous_build":PREVIOUS_BUILD,"interface":"/interface","docs":"/docs","run":"/run","command":"/command","execution":"/execution/{id}","workflow":"/workflow","workflows":"/workflows"}
