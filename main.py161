...
def result_store(execution_id: str, value: Any) -> Dict[str, Any]:
    ...
    return {"ok": True, "stored": True, "execution_id": execution_id, "bytes": len(payload.encode())}
...
def execute_adapter(eid: str, step: Dict[str,Any]) -> Dict[str,Any]:
    ...
    if sid == "result_store":
        prior = q(
            "SELECT result_json FROM execution_steps_160 WHERE execution_id=? AND step_no<? AND verified=1 ORDER BY step_no DESC LIMIT 1",
            (eid, int(step.get("step_no", 999999))),
            one=True,
        )
        if not prior:
            prior = q(
                "SELECT result_json FROM execution_steps_160 WHERE execution_id=? AND verified=1 ORDER BY step_no DESC LIMIT 1",
                (eid,),
                one=True,
            )
        value = safe_json(prior["result_json"]) if prior and prior.get("result_json") else {"message":"no_prior_verified_result"}
        return result_store(eid, value)
...
def run_execution(eid: str) -> Dict[str,Any]:
    ...
    for s in steps:
        ...
        step={"system_id":s["system_id"],"capability":s["capability"],"action":s["action"],"input":inp,"step_no":s["step_no"]}
