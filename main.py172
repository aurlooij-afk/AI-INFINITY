...
APP_VERSION = "TARGET-2050.171"
BUILD = "REAL-WORLD-ACTION-WORKFLOW-INTERFACE-CORE"
PREVIOUS_BUILD = "TARGET-2050.170"
...
def run_workflow(wid: str, steps: List[Dict[str, Any]]) -> Dict[str, Any]:
    results = []
    update = lambda status, result=None: update_workflow(wid, status, result)
    update("running")
    for index, step in enumerate(steps):
        objective = step.get("command") or step.get("objective") or ""
        try:
            plan = parse_command(objective) if objective else step
            approval = bool(plan.get("side_effect"))
            eid = create_execution(plan.get("action_type", "unknown"), plan.get("connector", "none"), objective, approval, payload={"workflow_id": wid, "step": index})
            if approval:
                result = {"step": index, "execution_id": eid, "status": "pending_approval", "approval_required": True}
                results.append(result)
                update("waiting_approval", results)
                return {"workflow_id": wid, "status": "waiting_approval", "results": results}
            run_execution(eid, plan, approved=True)
            e = get_execution(eid)
            result = {"step": index, "execution_id": eid, "status": e.get("status") if e else "missing", "result": e.get("result_json") if e else None}
            results.append(result)
            if not e or e.get("status") != "completed":
                update("failed", results)
                return {"workflow_id": wid, "status": "failed", "results": results}
        except Exception as ex:
            results.append({"step": index, "status": "failed", "error": str(ex)})
            update("failed", results)
            return {"workflow_id": wid, "status": "failed", "results": results}
