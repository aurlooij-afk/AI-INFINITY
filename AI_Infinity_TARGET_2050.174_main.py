...
APP_VERSION = "TARGET-2050.174"
BUILD = "REAL-WORLD-WORKFLOW-ORCHESTRATION-PLUS-CORE"
PREVIOUS_BUILD = "TARGET-2050.173"
...
def run_workflow(wid: str, steps: List[Dict[str, Any]], start_index: int = 0) -> Dict[str, Any]:
    ...
    while index < len(steps):
        ...
        try:
            ...
            run_execution(eid,plan,approved=True); e=get_execution(eid); status=e.get("status") if e else "missing"
            result={"step":index,"execution_id":eid,"status":status,"result":e.get("result_json") if e else None}; results.append(result)
