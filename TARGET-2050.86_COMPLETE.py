...
APP_VERSION = "TARGET-2050.86"
BUILD = "CUMULATIVE-ALL-SUCCESSFUL-VERSIONS-RATE-LIMIT-RESILIENT-RECOVERY-AND-PROVIDER-QUORUM-CORE"
...
def call_provider(name, fn, objective, retry_round=0, skip_retry=False):
    ...
    variants=list(dict.fromkeys([terms, re.sub(r"\s+"," ",objective).strip()]))
    variants=[q for q in variants if q]
    # Keep provider load bounded. The first pass uses one query; recovery uses a
    # compact query first. A failed provider is not allowed to consume the whole mission.
    ordered=variants[:2]
