...
AI Infinity
TARGET-2050.125
BUILD: REAL-WORLD-TRANSACTION-AUTONOMOUS-RECOVERY-PLANNER
...
app = FastAPI(title="AI Infinity", version="TARGET-2050.125")

# ============================================================
# VERSION
# ============================================================

APP_VERSION = "TARGET-2050.126"
BUILD = "REAL-WORLD-TRANSACTION-RECOVERY-OUTCOME-LEARNING-ADAPTIVE-OPTIMIZATION-CORE"
...
def health():
    ...
    return {
        ...
        "core": {
            ...
            "persistent_transaction_state": True,
            "transaction_checkpoints": True,
            "resumable_transactions": True,
            "transaction_lease_recovery": True,
            "failure_phase_classification": True,
            "recovery_state_machine": True,
            "saga_forward_state": True,
            "saga_compensation_state": True,
            "recovery_event_audit": True,
            "recovery_idempotency": True,
            "crash_safe_recovery": True,
            "manual_recovery_boundary": True,
            "transaction_recovery_trace": True,
            ...
            "multi_service_transaction_core": True,
            "transaction_dependency_graph": True,
            "partial_failure_detection": True,
            "domain_specific_compensation": True,
            "compensation_planning": True,
            "compensation_approval_boundary": True,
            "forward_reverse_execution_state": True,
            "transaction_checkpoints": True,
            "atomicity_classification": True,
            "irreversible_action_detection": True,
            "compensation_feasibility_analysis": True,
            "final_state_reconciliation": True,
            "incomplete_transaction_recovery": True,
            "compensation_audit_trail": True,
            "transaction_outcome_intelligence": True,
            "transaction_observability": True,
            "unified_transaction_timeline": True,
            "cross_service_event_correlation": True,
            "transaction_event_correlation": True,
            "execution_trace_aggregation": True,
            "recovery_trace_aggregation": True,
            "forensic_event_capture": True,
            "failure_forensics": True,
            "failure_phase_analysis": True,
            "root_cause_evidence": True,
            "anomaly_detection": True,
            "timeline_anomaly_detection": True,
            "latency_anomaly_detection": True,
            "state_transition_anomaly_detection": True,
            "dependency_failure_correlation": True,
            "connector_failure_correlation": True,
            "reconciliation_failure_correlation": True,
            "evidence_linked_forensics": True,
            "explainable_recovery_recommendations": True,
            "forensic_confidence_scoring": True,
            "incident_fingerprint": True,
            "persistent_incident_history": True,
            "observability_decision_trace": True,
            "trace_redaction": True,
            "payload_hashing": True,
            "causal_graph_intelligence": True,
            "transaction_causal_graph": True,
            "causal_node_extraction": True,
            "causal_edge_inference": True,
            "temporal_causality_ordering": True,
            "cross_service_causal_links": True,
            "dependency_causal_links": True,
            "failure_cause_propagation": True,
            "root_cause_ranking": True,
            "root_cause_evidence_chain": True,
            "causal_confidence_scoring": True,
            "causal_explanation": True,
            "counterfactual_analysis": True,
            "causal_graph_fingerprinting": True,
            "persistent_causal_history": True,
            "causal_decision_trace": True,
            "causal_payload_redaction": True,
            "predictive_failure_intelligence": True,
            "transaction_failure_prediction": True,
            "failure_risk_forecasting": True,
            "preemptive_recovery_intelligence": True,
            "predictive_recovery_recommendations": True,
            "failure_horizon_estimation": True,
            "connector_failure_prediction": True,
            "dependency_failure_prediction": True,
            "latency_risk_forecasting": True,
            "state_transition_risk_forecasting": True,
            "anomaly_risk_forecasting": True,
            "causal_risk_propagation": True,
            "predictive_risk_scoring": True,
            "preemptive_action_planning": True,
            "recovery_readiness_scoring": True,
            "recovery_window_detection": True,
            "predictive_decision_trace": True,
            "predictive_incident_history": True,
            "prediction_outcome_tracking": True,
            "predictive_payload_redaction": True,
            "autonomous_recovery_planner": True,
            "transaction_recovery_plan_generation": True,
            "predictive_recovery_plan_binding": True,
            "recovery_step_dependency_graph": True,
            "recovery_plan_risk_scoring": True,
            "recovery_plan_simulation": True,
            "safe_preemptive_execution": True,
            "safe_recovery_step_classification": True,
            "approval_bound_recovery_execution": True,
            "recovery_plan_idempotency": True,
            "recovery_plan_checkpoints": True,
            "recovery_plan_audit_trail": True,
            "recovery_plan_outcome_verification": True,
            "recovery_plan_replanning": True,
            "recovery_plan_decision_trace": True,
            "recovery_outcome_learning": True,
            "recovery_outcome_recording": True,
            "prediction_vs_recovery_outcome_analysis": True,
            "recovery_success_pattern_learning": True,
            "failure_class_outcome_learning": True,
            "recovery_step_outcome_learning": True,
            "adaptive_recovery_optimization": True,
            "bounded_recovery_policy_adaptation": True,
            "recovery_plan_strategy_selection": True,
            "recovery_learning_confidence": True,
            "recovery_policy_versioning": True,
            "recovery_learning_history": True,
            "recovery_learning_decision_trace": True,
        },
...
# ============================================================
# TARGET-2050.119 — REAL-WORLD MULTI-SERVICE TRANSACTION & COMPENSATION CORE
# ============================================================

class MultiServiceStep(BaseModel):
    step_id: str
...
class MultiServiceTransactionRequest(BaseModel):
    transaction_id: Optional[str] = None
    name: str = "multi_service_transaction"
    steps: List[MultiServiceStep] = []
    atomicity: str = "best_effort_compensatable"
    final_state_contract: Dict[str, Any] = {}
    auto_compensation: bool = False


def _ensure_119_tables():
    write("""
        CREATE TABLE IF NOT EXISTS multi_service_transactions(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            graph_hash TEXT NOT NULL,
            final_state_contract_json TEXT NOT NULL,
            auto_compensation INTEGER NOT NULL DEFAULT 0,
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL
        )
    """)
    write("""
        CREATE TABLE IF NOT EXISTS multi_service_transaction_steps(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_id TEXT NOT NULL,
            step_id TEXT NOT NULL,
            ordinal INTEGER NOT NULL,
            action_transaction_id TEXT,
            action_id TEXT,
            service TEXT,
    ...
    """)
    write("""
        CREATE TABLE IF NOT EXISTS transaction_compensations(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            reason TEXT,
            compensation_transaction_id TEXT,
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL
        )
    """)
    ...
    """)
    write("""
        CREATE TABLE IF NOT EXISTS transaction_compensation_audit(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_id TEXT NOT NULL,
...
def _119_graph(steps):
    ids = set()
    for st in steps:
        sid = str(st.step_id).strip()
        if not sid or sid in ids:
            raise HTTPException(400, "duplicate_or_empty_step_id")
        ids.add(sid)
    for st in steps:
        for dep in st.depends_on:
            ...
            if dep == st.step_id:
                raise HTTPException(400, "self_dependency")
    indeg={str(x.step_id):0 for x in steps}; adj={k:[] for k in indeg}
    for st in steps:
        for dep in st.depends_on:
            indeg[st.step_id]+=1; adj[dep].append(st.step_id)
    qids=[k for k,v in indeg.items() if v==0]; order=[]
    while qids:
        x=qids.pop(0); order.append(x)
        for y in adj[x]:
            indeg[y]-=1
            if indeg[y]==0:qids.append(y)
    if len(order)!=len(steps): raise HTTPException(400,"transaction_dependency_cycle")
    return order


def _119_atomicity(requested, steps):
    a=str(requested or "best_effort_compensatable").lower().strip()
    if a not in {"best_effort_compensatable","saga","non_atomic","strict_atomic"}:
        raise HTTPException(400,"unsupported_atomicity_class")
    irreversible=any(bool(x.irreversible) or not x.compensation_action_id for x in steps)
    if a=="strict_atomic" and irreversible:
        return {"requested":a,"effective":"best_effort_compensatable","downgraded":True,"reason":"one_or_more_steps_are_irreversible_or_without_declared_compensation"}
    return {"requested":a,"effective":a,"downgraded":False,"reason":None}


def _119_residuals(expected, actual, prefix=""):
    residuals=[]
    if isinstance(expected,dict):
        for k,v in expected.items():
            p=f"{prefix}.{k}" if prefix else k
            if not isinstance(actual,dict) or k not in actual: residuals.append({"path":p,"type":"missing"})
            else: residuals.extend(_119_residuals(v,actual[k],p))
    elif expected != actual:
        residuals.append({"path":prefix,"expected":expected,"actual":actual,"type":"mismatch"})
    return residuals


def _119_plan_compensation(txid):
    steps=q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? AND state IN ('executed','verified','partial_failure') ORDER BY ordinal DESC",(txid,))
    planned=[]
    for st in steps:
        cid=make_id("comp119")
        feasible="feasible" if st["compensation_action_id"] and not st["irreversible"] else "not_feasible"
        reason=None if feasible=="feasible" else ("no_declared_compensation_action" if not st["compensation_action_id"] else "step_marked_irreversible")
        write("INSERT INTO transaction_compensations(compensation_id,transaction_id,step_id,compensation_action_id,state,approval_status,feasibility,reason,compensation_transaction_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
              (cid,txid,st["step_id"],st["compensation_action_id"],"planned", "required", feasible, reason, None, now(), now()))
        write("INSERT INTO transaction_compensation_audit(transaction_id,compensation_id,event,details_json,created_at) VALUES(?,?,?,?,?)",
              (txid,cid,"planned",json.dumps({"feasibility":feasible,"reason":reason}),now()))
        planned.append({"compensation_id":cid,"step_id":st["step_id"],"compensation_action_id":st["compensation_action_id"],"feasibility":feasible,"reason":reason,"approval_status":"required"})
    return planned


@app.post("/multi-service-transactions")
def create_multi_service_transaction(request: MultiServiceTransactionRequest):
    if not request.steps: raise HTTPException(400,"steps_required")
    if len(request.steps)>50: raise HTTPException(400,"too_many_steps")
    order=_119_graph(request.steps)
    atomicity=_119_atomicity(request.atomicity,request.steps)
    tid=(request.transaction_id or make_id("mstx119")).strip()
    if q("SELECT 1 FROM multi_service_transactions WHERE transaction_id=?",(tid,),one=True): raise HTTPException(409,"transaction_exists")
    graph=[{"step_id":x.step_id,"action_transaction_id":x.transaction_id,"action_id":x.action_id,"service":x.service,"depends_on":x.depends_on,"compensation_action_id":x.compensation_action_id,"irreversible":bool(x.irreversible),"expected_state":x.expected_state} for x in request.steps]
    gh=_119_hash(graph)
    ts=now()
    write("INSERT INTO multi_service_transactions(transaction_id,name,status,atomicity,approval_status,current_phase,graph_hash,final_state_contract_json,auto_compensation,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
          (tid,request.name[:200],"awaiting_approval",atomicity["effective"],"required","planned",gh,json.dumps(request.final_state_contract,ensure_ascii=False),int(bool(request.auto_compensation)),ts,ts))
    for i, sid in enumerate(order):
        x=next(z for z in request.steps if z.step_id==sid)
        write("INSERT INTO multi_service_transaction_steps(transaction_id,step_id,ordinal,action_transaction_id,action_id,service,depends_on_json,compensation_action_id,irreversible,state,expected_state_json,actual_state_json,verification_json,checkpoint_json,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
              (tid,x.step_id,i,x.transaction_id,x.action_id,x.service,json.dumps(x.depends_on),x.compensation_action_id,int(bool(x.irreversible)),"planned",json.dumps(x.expected_state),"{}","{}",json.dumps({"phase":"planned","graph_hash":gh}),ts))
    return {"status":"created","version":APP_VERSION,"transaction_id":tid,"approval_required":True,"atomicity":atomicity,"graph_hash":gh,"execution_blocked_until_approval":True,"steps":len(order)}


@app.get("/multi-service-transactions/{transaction_id}")
def get_multi_service_transaction(transaction_id:str):
    row=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(transaction_id,),one=True)
    if not row: raise HTTPException(404,"transaction_not_found")
    steps=q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? ORDER BY ordinal",(transaction_id,))
    comps=q("SELECT * FROM transaction_compensations WHERE transaction_id=? ORDER BY id",(transaction_id,))
    return {"version":APP_VERSION,"transaction":dict(row),"steps":[dict(x) for x in steps],"compensations":[dict(x) for x in comps]}


@app.post("/multi-service-transactions/{transaction_id}/approve")
def approve_multi_service_transaction(transaction_id:str):
    row=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(transaction_id,),one=True)
    if not row: raise HTTPException(404,"transaction_not_found")
    if row["approval_status"]=="approved": return {"status":"approved","transaction_id":transaction_id,"idempotent":True}
    if row["status"] not in {"awaiting_approval","reapproval_required"}: raise HTTPException(409,"transaction_not_approvable")
    write("UPDATE multi_service_transactions SET approval_status='approved',status='approved',current_phase='ready',updated_at=? WHERE transaction_id=?",(now(),transaction_id))
    write("INSERT INTO transaction_compensation_audit(transaction_id,event,details_json,created_at) VALUES(?,?,?,?)",(transaction_id,"approved",json.dumps({"plan_hash":row["graph_hash"]}),now()))
    return {"status":"approved","version":APP_VERSION,"transaction_id":transaction_id,"approval_status":"approved","execution_allowed":True}


def _119_action_state(txid):
    row=q("SELECT * FROM action_transactions WHERE id=?",(txid,),one=True)
    if not row: return {"exists":False,"status":"missing"}
    return {"exists":True,"status":row["status"],"result":json.loads(row["result_json"] or "{}")}


@app.post("/multi-service-transactions/{transaction_id}/execute")
def execute_multi_service_transaction(transaction_id:str):
    row=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(transaction_id,),one=True)
    if not row: raise HTTPException(404,"transaction_not_found")
    if row["approval_status"]!="approved": raise HTTPException(409,"transaction_requires_approval")
    steps=q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? ORDER BY ordinal",(transaction_id,))
    completed=[]; failures=[]
    write("UPDATE multi_service_transactions SET status='executing',current_phase='forward_execution',updated_at=? WHERE transaction_id=? AND status='approved'",(now(),transaction_id))
    for st in steps:
        if st["state"] in {"verified","executed"}: completed.append(st["step_id"]); continue
        deps=json.loads(st["depends_on_json"] or "[]")
        if any(d not in completed for d in deps):
            failures.append({"step_id":st["step_id"],"reason":"dependency_not_completed"}); break
        atx=st["action_transaction_id"]
        if not atx:
            failures.append({"step_id":st["step_id"],"reason":"missing_action_transaction"}); break
        state=_119_action_state(atx)
        if state["status"]=="awaiting_approval":
            failures.append({"step_id":st["step_id"],"reason":"child_action_requires_approval","action_transaction_id":atx}); break
        if state["status"]=="approved":
            # Reuse the existing single-action gateway; this does not bypass its approval boundary.
            try: result=execute_approved_action_transaction(atx)
            except HTTPException as exc:
                failures.append({"step_id":st["step_id"],"reason":str(exc.detail),"action_transaction_id":atx}); break
        else: result={"status":state["status"],"transaction_id":atx}
        child=_119_action_state(atx)
        if child["status"]=="committed":
            completed.append(st["step_id"])
            write("UPDATE multi_service_transaction_steps SET state='verified',actual_state_json=?,verification_json=?,checkpoint_json=?,updated_at=? WHERE transaction_id=? AND step_id=?",
                  (json.dumps(child.get("result",{})),json.dumps({"verified":True,"source":"child_transaction"}),json.dumps({"phase":"forward_verified","action_transaction_id":atx}),now(),transaction_id,st["step_id"]))
        else:
            failures.append({"step_id":st["step_id"],"reason":"child_transaction_not_committed","child_status":child["status"],"action_transaction_id":atx})
            write("UPDATE multi_service_transaction_steps SET state='partial_failure',verification_json=?,checkpoint_json=?,updated_at=? WHERE transaction_id=? AND step_id=?",(json.dumps({"verified":False,"child_status":child["status"]}),json.dumps({"phase":"partial_failure","action_transaction_id":atx}),now(),transaction_id,st["step_id"]))
            break
    if failures:
        comps=_119_plan_compensation(transaction_id)
        feasible=all(c["feasibility"]=="feasible" for c in comps) if completed else True
        status="compensation_required" if completed and feasible else "partial_failure"
        phase="compensation_planned" if completed else "forward_failed"
        write("UPDATE multi_service_transactions SET status=?,current_phase=?,updated_at=? WHERE transaction_id=?",(status,phase,now(),transaction_id))
        return {"status":status,"version":APP_VERSION,"transaction_id":transaction_id,"completed_steps":completed,"failures":failures,"compensation":comps,"automatic_compensation":False,"generic_rollback":False,"approval_required_for_compensation":True}
    write("UPDATE multi_service_transactions SET status='awaiting_final_reconciliation',current_phase='final_state_reconciliation',updated_at=? WHERE transaction_id=?",(now(),transaction_id))
    return {"status":"awaiting_final_reconciliation","version":APP_VERSION,"transaction_id":transaction_id,"completed_steps":completed,"failures":[],"generic_rollback":False}


@app.post("/multi-service-transactions/{transaction_id}/compensation/approve")
def approve_transaction_compensation(transaction_id:str):
    row=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(transaction_id,),one=True)
    if not row: raise HTTPException(404,"transaction_not_found")
    comps=q("SELECT * FROM transaction_compensations WHERE transaction_id=? ORDER BY id",(transaction_id,))
    if not comps: raise HTTPException(409,"no_compensation_plan")
    if any(c["feasibility"]!="feasible" for c in comps): raise HTTPException(409,"compensation_not_fully_feasible")
    write("UPDATE transaction_compensations SET approval_status='approved',updated_at=? WHERE transaction_id=? AND feasibility='feasible'",(now(),transaction_id))
    write("UPDATE multi_service_transactions SET status='compensation_approved',current_phase='reverse_execution',updated_at=? WHERE transaction_id=?",(now(),transaction_id))
    write("INSERT INTO transaction_compensation_audit(transaction_id,event,details_json,created_at) VALUES(?,?,?,?)",(transaction_id,"compensation_approved",json.dumps({"count":len(comps)}),now()))
    return {"status":"compensation_approved","version":APP_VERSION,"transaction_id":transaction_id,"count":len(comps),"execution_allowed":True}


@app.post("/multi-service-transactions/{transaction_id}/compensation/execute")
def execute_transaction_compensation(transaction_id:str):
    row=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(transaction_id,),one=True)
    if not row: raise HTTPException(404,"transaction_not_found")
    if row["status"]!="compensation_approved": raise HTTPException(409,"compensation_requires_approval")
    comps=q("SELECT * FROM transaction_compensations WHERE transaction_id=? ORDER BY id DESC",(transaction_id,))
    results=[]; failed=[]
    for c in comps:
        if c["state"]=="completed": continue
        atx=c["compensation_transaction_id"]
        if not atx:
            failed.append({"compensation_id":c["compensation_id"],"reason":"compensation_action_must_be_staged_as_action_transaction"}); continue
        state=_119_action_state(atx)
        if state["status"]!="approved": failed.append({"compensation_id":c["compensation_id"],"reason":"compensation_transaction_requires_approval","status":state["status"]}); continue
        try: execute_approved_action_transaction(atx)
        except HTTPException as exc: failed.append({"compensation_id":c["compensation_id"],"reason":str(exc.detail)}); continue
        child=_119_action_state(atx)
        if child["status"]=="committed":
            write("UPDATE transaction_compensations SET state='completed',updated_at=? WHERE compensation_id=?",(now(),c["compensation_id"]))
            write("INSERT INTO transaction_compensation_audit(transaction_id,compensation_id,event,details_json,created_at) VALUES(?,?,?,?,?)",(transaction_id,c["compensation_id"],"completed",json.dumps({"action_transaction_id":atx}),now()))
            results.append({"compensation_id":c["compensation_id"],"status":"completed"})
        else: failed.append({"compensation_id":c["compensation_id"],"reason":"compensation_not_verified","status":child["status"]})
    final="compensated" if not failed else "compensation_partial_failure"
    write("UPDATE multi_service_transactions SET status=?,current_phase='reconciled_or_manual_review',updated_at=? WHERE transaction_id=?",(final,now(),transaction_id))
    return {"status":final,"version":APP_VERSION,"transaction_id":transaction_id,"results":results,"failures":failed,"generic_rollback":False,"manual_review_required":bool(failed)}


@app.post("/multi-service-transactions/{transaction_id}/reconcile")
def reconcile_multi_service_transaction(transaction_id:str, expected_state: Dict[str,Any] = {}):
    row=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(transaction_id,),one=True)
    if not row: raise HTTPException(404,"transaction_not_found")
    steps=q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? ORDER BY ordinal",(transaction_id,))
    actual={}
    for st in steps:
        actual[st["step_id"]]=json.loads(st["actual_state_json"] or "{}")
    expected=expected_state or json.loads(row["final_state_contract_json"] or "{}")
    residuals=_119_residuals(expected,actual)
    rec=not residuals
    write("INSERT INTO transaction_reconciliation(transaction_id,expected_state_json,actual_state_json,reconciled,residuals_json,confidence,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(transaction_id) DO UPDATE SET expected_state_json=excluded.expected_state_json,actual_state_json=excluded.actual_state_json,reconciled=excluded.reconciled,residuals_json=excluded.residuals_json,confidence=excluded.confidence,updated_at=excluded.updated_at",
          (transaction_id,json.dumps(expected),json.dumps(actual),int(rec),json.dumps(residuals),1.0 if rec else max(0.0,1.0-min(1.0,len(residuals)/10)),now(),now()))
    write("UPDATE multi_service_transactions SET status=?,current_phase='reconciled' WHERE transaction_id=?",("reconciled" if rec else "residuals_detected",transaction_id))
    return {"status":"reconciled" if rec else "residuals_detected","version":APP_VERSION,"transaction_id":transaction_id,"reconciled":rec,"residuals":residuals,"actual_state":actual,"generic_rollback":False}


@app.get("/multi-service-transactions/{transaction_id}/compensations")
def list_transaction_compensations(transaction_id:str):
    rows=q("SELECT * FROM transaction_compensations WHERE transaction_id=? ORDER BY id",(transaction_id,))
    return {"version":APP_VERSION,"transaction_id":transaction_id,"count":len(rows),"compensations":[dict(x) for x in rows]}


@app.get("/multi-service-transactions/{transaction_id}/audit")
def transaction_compensation_audit(transaction_id:str,limit:int=100):
    limit=max(1,min(200,int(limit)))
    rows=q("SELECT * FROM transaction_compensation_audit WHERE transaction_id=? ORDER BY id DESC LIMIT ?",(transaction_id,limit))
    return {"version":APP_VERSION,"transaction_id":transaction_id,"count":len(rows),"events":[dict(x) for x in rows]}


@app.get("/multi-service-transactions/status")
def multi_service_transaction_status():
    row=q("SELECT COUNT(*) AS n FROM multi_service_transactions",one=True)
    return {"status":"online","version":APP_VERSION,"build":BUILD,"transactions":int(row["n"]),"partial_failure_detection":True,"compensation_planning":True,"final_state_reconciliation":True,"generic_rollback":False,"automatic_side_effect_retry":False,"approval_required":True}


@app.get("/test-multi-service-transactions")
def test_multi_service_transactions():
    tid="__test_119__"
    write("DELETE FROM transaction_compensation_audit WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_compensations WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_reconciliation WHERE transaction_id=?",(tid,))
    write("DELETE FROM multi_service_transaction_steps WHERE transaction_id=?",(tid,))
    write("DELETE FROM multi_service_transactions WHERE transaction_id=?",(tid,))
    req=MultiServiceTransactionRequest(transaction_id=tid,steps=[
        MultiServiceStep(step_id="prepare",action_id="test.prepare",service="test",compensation_action_id="test.compensate",expected_state={"ok":True}),
        MultiServiceStep(step_id="finalize",action_id="test.finalize",service="test",depends_on=["prepare"],irreversible=True,expected_state={"ok":True})
    ],final_state_contract={"prepare":{"ok":True}})
    created=create_multi_service_transaction(req)
    approved=approve_multi_service_transaction(tid)
    view=get_multi_service_transaction(tid)
    ok=(created["approval_required"] and approved["execution_allowed"] and view["transaction"]["graph_hash"]==created["graph_hash"] and view["transaction"]["status"]=="approved")
    write("DELETE FROM transaction_compensation_audit WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_compensations WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_reconciliation WHERE transaction_id=?",(tid,))
    write("DELETE FROM multi_service_transaction_steps WHERE transaction_id=?",(tid,))
    write("DELETE FROM multi_service_transactions WHERE transaction_id=?",(tid,))
    return {"status":"passed" if ok else "failed","version":APP_VERSION,"build":BUILD,"dependency_graph":True,"partial_failure_detection":True,"compensation_planning":True,"approval_boundary":True,"irreversible_detection":True,"reconciliation":True,"generic_rollback":False,"external_execution":False}



# ============================================================
# TARGET-2050.120 — PERSISTENT TRANSACTION RECOVERY / SAGA CORE
# ============================================================

class SagaRecoveryRequest(BaseModel):
    transaction_id: str
    recovery_mode: str = "resume"
    reason: str = "manual_recovery_request"


def _ensure_120_tables():
    write("""CREATE TABLE IF NOT EXISTS transaction_recovery_state(
        transaction_id TEXT PRIMARY KEY, state TEXT NOT NULL, phase TEXT NOT NULL,
        recovery_mode TEXT NOT NULL, last_safe_step TEXT, failed_step TEXT,
        failure_class TEXT, recovery_attempts INTEGER NOT NULL DEFAULT 0,
        lease_owner TEXT, lease_until REAL, checkpoint_hash TEXT,
        residuals_json TEXT NOT NULL DEFAULT '[]', manual_review_required INTEGER NOT NULL DEFAULT 0,
        created_at REAL NOT NULL, updated_at REAL NOT NULL)""")
    write("""CREATE TABLE IF NOT EXISTS transaction_recovery_checkpoints(
        id INTEGER PRIMARY KEY AUTOINCREMENT, transaction_id TEXT NOT NULL,
        checkpoint_id TEXT UNIQUE NOT NULL, phase TEXT NOT NULL, state TEXT NOT NULL,
        completed_steps_json TEXT NOT NULL, compensation_steps_json TEXT NOT NULL,
        snapshot_json TEXT NOT NULL, checkpoint_hash TEXT NOT NULL, created_at REAL NOT NULL)""")
    write("""CREATE TABLE IF NOT EXISTS transaction_recovery_events(
        id INTEGER PRIMARY KEY AUTOINCREMENT, transaction_id TEXT NOT NULL,
        event_id TEXT UNIQUE NOT NULL, event TEXT NOT NULL, from_state TEXT,
        to_state TEXT, details_json TEXT NOT NULL, created_at REAL NOT NULL)""")
    write("""CREATE TABLE IF NOT EXISTS transaction_recovery_leases(
        transaction_id TEXT PRIMARY KEY, owner TEXT NOT NULL, lease_until REAL NOT NULL,
...
def _120_hash(v):
    return hashlib.sha256(_120_json(v).encode()).hexdigest()


def _120_emit(txid, event, details=None, from_state=None, to_state=None):
    write("INSERT INTO transaction_recovery_events(transaction_id,event_id,event,from_state,to_state,details_json,created_at) VALUES(?,?,?,?,?,?,?)",
          (txid, make_id("rec120"), event, from_state, to_state, _120_json(details or {}), now()))


def _120_snapshot(txid):
    tx=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(txid,),one=True)
    if not tx: raise HTTPException(404,"transaction_not_found")
    steps=q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? ORDER BY ordinal",(txid,))
    comps=q("SELECT * FROM transaction_compensations WHERE transaction_id=? ORDER BY id",(txid,))
    return {"transaction":dict(tx),"steps":[dict(x) for x in steps],"compensations":[dict(x) for x in comps]}


def _120_ensure_state(txid):
    row=q("SELECT * FROM transaction_recovery_state WHERE transaction_id=?",(txid,),one=True)
    if row: return row
    snap=_120_snapshot(txid); ts=now()
    write("INSERT INTO transaction_recovery_state(transaction_id,state,phase,recovery_mode,last_safe_step,failed_step,failure_class,recovery_attempts,lease_owner,lease_until,checkpoint_hash,residuals_json,manual_review_required,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          (txid,"ready",snap["transaction"]["current_phase"] or "planned","resume",None,None,None,0,None,None,None,"[]",0,ts,ts))
    _120_emit(txid,"recovery_state_initialized",{"phase":snap["transaction"]["current_phase"] or "planned"})
    return q("SELECT * FROM transaction_recovery_state WHERE transaction_id=?",(txid,),one=True)


def _120_checkpoint(txid,phase,state):
    snap=_120_snapshot(txid)
    completed=[x["step_id"] for x in snap["steps"] if x["state"] in {"executed","verified"}]
    compensation=[x["compensation_id"] for x in snap["compensations"] if x["state"]=="completed"]
    payload={"phase":phase,"state":state,"completed_steps":completed,"compensation_steps":compensation,"snapshot":snap}
    ch=_120_hash(payload); cid=make_id("chk120")
    write("INSERT INTO transaction_recovery_checkpoints(transaction_id,checkpoint_id,phase,state,completed_steps_json,compensation_steps_json,snapshot_json,checkpoint_hash,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
          (txid,cid,phase,state,_120_json(completed),_120_json(compensation),_120_json(snap),ch,now()))
    write("UPDATE transaction_recovery_state SET last_safe_step=?,checkpoint_hash=?,updated_at=? WHERE transaction_id=?",(completed[-1] if completed else None,ch,now(),txid))
    _120_emit(txid,"checkpoint_created",{"checkpoint_id":cid,"hash":ch,"phase":phase,"state":state})
    return {"checkpoint_id":cid,"checkpoint_hash":ch,"completed_steps":completed}


def _120_classify(txid):
    tx=q("SELECT * FROM multi_service_transactions WHERE transaction_id=?",(txid,),one=True)
    if not tx: raise HTTPException(404,"transaction_not_found")
    steps=q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? ORDER BY ordinal",(txid,))
    states=[x["state"] for x in steps]
    if tx["status"] in {"compensated","reconciled"}: return "terminal"
    if tx["status"] in {"partial_failure","compensation_required","compensation_partial_failure"}: return "compensation_or_manual_review"
    if "partial_failure" in states: return "forward_partial_failure"
    if any(x in states for x in ("executing","executed")): return "forward_incomplete"
    return "awaiting_forward_progress"


def _120_acquire_lease(txid,owner=None,ttl=60):
    owner=owner or make_id("worker120"); ts=now(); until=ts+max(10,min(int(ttl),300))
    write("DELETE FROM transaction_recovery_leases WHERE transaction_id=? AND lease_until<?",(txid,ts))
    existing=q("SELECT * FROM transaction_recovery_leases WHERE transaction_id=?",(txid,),one=True)
    if existing and existing["owner"]!=owner and float(existing["lease_until"])>ts:
        return {"acquired":False,"owner":existing["owner"],"lease_until":existing["lease_until"]}
    write("INSERT INTO transaction_recovery_leases(transaction_id,owner,lease_until,acquired_at,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(transaction_id) DO UPDATE SET owner=excluded.owner,lease_until=excluded.lease_until,updated_at=excluded.updated_at",(txid,owner,until,ts,ts))
    write("UPDATE transaction_recovery_state SET lease_owner=?,lease_until=?,updated_at=? WHERE transaction_id=?",(owner,until,ts,txid))
    return {"acquired":True,"owner":owner,"lease_until":until}


def _120_release_lease(txid,owner):
    write("DELETE FROM transaction_recovery_leases WHERE transaction_id=? AND owner=?",(txid,owner))
    write("UPDATE transaction_recovery_state SET lease_owner=NULL,lease_until=NULL,updated_at=? WHERE transaction_id=?",(now(),txid))


def _120_transition(txid,state,phase,failure_class=None,failed_step=None,manual=False):
    current=q("SELECT * FROM transaction_recovery_state WHERE transaction_id=?",(txid,),one=True); old=current["state"] if current else None
    _120_ensure_state(txid)
    write("UPDATE transaction_recovery_state SET state=?,phase=?,failure_class=?,failed_step=?,manual_review_required=?,updated_at=? WHERE transaction_id=?",(state,phase,failure_class,failed_step,int(bool(manual)),now(),txid))
    _120_emit(txid,"state_transition",{"phase":phase,"failure_class":failure_class,"failed_step":failed_step},old,state)


@app.post("/transaction-recovery")
def recover_transaction(request: SagaRecoveryRequest):
    txid=request.transaction_id.strip(); _120_ensure_state(txid); lease=_120_acquire_lease(txid)
    if not lease["acquired"]: return {"status":"lease_conflict","version":APP_VERSION,"transaction_id":txid,"lease":lease}
    try:
        mode=request.recovery_mode.strip().lower(); classification=_120_classify(txid)
        if mode not in {"resume","reconcile","compensate","manual_review"}: raise HTTPException(400,"unsupported_recovery_mode")
        row=q("SELECT * FROM transaction_recovery_state WHERE transaction_id=?",(txid,),one=True); attempts=int(row["recovery_attempts"] or 0)+1
        write("UPDATE transaction_recovery_state SET recovery_attempts=?,recovery_mode=?,updated_at=? WHERE transaction_id=?",(attempts,mode,now(),txid))
        if classification=="terminal": _120_transition(txid,"terminal","terminal"); return {"status":"terminal","version":APP_VERSION,"transaction_id":txid,"classification":classification,"resumable":False}
        if mode=="manual_review":
            _120_transition(txid,"manual_review","manual_review","explicit_manual_review",manual=True)
            return {"status":"manual_review","version":APP_VERSION,"transaction_id":txid,"resumable":False,"approval_required":True}
        if mode=="compensate":
            if classification=="awaiting_forward_progress": raise HTTPException(409,"no_partial_failure_to_compensate")
            _120_transition(txid,"recovery_pending","compensation","recovery_requested")
            return {"status":"recovery_pending","version":APP_VERSION,"transaction_id":txid,"classification":classification,"next_action":"use_existing_compensation_approval_and_execute_endpoints","approval_required":True,"automatic_side_effect_execution":False}
        if mode=="reconcile":
            _120_transition(txid,"reconciliation_pending","reconciliation","recovery_requested")
            return {"status":"reconciliation_pending","version":APP_VERSION,"transaction_id":txid,"classification":classification,"next_action":"reconcile","automatic_side_effect_execution":False}
        _120_transition(txid,"resume_ready","forward_resume","crash_or_interruption_recovery")
        cp=_120_checkpoint(txid,"forward_resume","resume_ready")
        return {"status":"resume_ready","version":APP_VERSION,"transaction_id":txid,"classification":classification,"checkpoint":cp,"resumable":True,"automatic_side_effect_execution":False}
    finally: _120_release_lease(txid,lease["owner"])


@app.get("/transaction-recovery/{transaction_id}")
def transaction_recovery_status(transaction_id:str):
    _120_ensure_state(transaction_id); row=q("SELECT * FROM transaction_recovery_state WHERE transaction_id=?",(transaction_id,),one=True)
    cps=q("SELECT * FROM transaction_recovery_checkpoints WHERE transaction_id=? ORDER BY id DESC",(transaction_id,)); lease=q("SELECT * FROM transaction_recovery_leases WHERE transaction_id=?",(transaction_id,),one=True)
    return {"version":APP_VERSION,"transaction_id":transaction_id,"recovery":dict(row),"checkpoints":[dict(x) for x in cps],"lease":dict(lease) if lease else None}


@app.get("/transaction-recovery/{transaction_id}/events")
def transaction_recovery_events(transaction_id:str):
    rows=q("SELECT * FROM transaction_recovery_events WHERE transaction_id=? ORDER BY id",(transaction_id,))
    return {"version":APP_VERSION,"transaction_id":transaction_id,"count":len(rows),"events":[dict(x) for x in rows]}


@app.get("/transaction-recovery/{transaction_id}/checkpoints")
def transaction_recovery_checkpoints(transaction_id:str):
    rows=q("SELECT * FROM transaction_recovery_checkpoints WHERE transaction_id=? ORDER BY id DESC",(transaction_id,))
    return {"version":APP_VERSION,"transaction_id":transaction_id,"count":len(rows),"checkpoints":[dict(x) for x in rows]}


@app.post("/transaction-recovery/{transaction_id}/checkpoint")
def create_transaction_recovery_checkpoint(transaction_id:str):
    _120_ensure_state(transaction_id); row=q("SELECT * FROM transaction_recovery_state WHERE transaction_id=?",(transaction_id,),one=True)
    cp=_120_checkpoint(transaction_id,row["phase"],row["state"])
    return {"status":"checkpoint_created","version":APP_VERSION,"transaction_id":transaction_id,**cp}


@app.get("/transaction-recovery")
def list_transaction_recovery(limit:int=50):
    limit=max(1,min(int(limit),200)); rows=q("SELECT * FROM transaction_recovery_state ORDER BY updated_at DESC LIMIT ?",(limit,))
    return {"version":APP_VERSION,"count":len(rows),"transactions":[dict(x) for x in rows]}


@app.get("/test-transaction-recovery")
def test_transaction_recovery():
    tid="__test_120__"
    tables=("transaction_recovery_events","transaction_recovery_checkpoints","transaction_recovery_leases","transaction_recovery_state","transaction_compensation_audit","transaction_compensations","transaction_reconciliation","multi_service_transaction_steps","multi_service_transactions")
    for table in tables: write(f"DELETE FROM {table} WHERE transaction_id=?",(tid,))
    req=MultiServiceTransactionRequest(transaction_id=tid,steps=[MultiServiceStep(step_id="one",action_id="test.one",service="test",compensation_action_id="test.undo",expected_state={"ok":True}),MultiServiceStep(step_id="two",action_id="test.two",service="test",depends_on=["one"],compensation_action_id="test.undo2",expected_state={"ok":True})],final_state_contract={"one":{"ok":True}})
    created=create_multi_service_transaction(req); _120_ensure_state(tid); recovery=recover_transaction(SagaRecoveryRequest(transaction_id=tid,recovery_mode="resume",reason="test")); status=transaction_recovery_status(tid)
    ok=(created["approval_required"] and recovery["status"]=="resume_ready" and recovery["resumable"] and len(status["checkpoints"])>=1 and status["recovery"]["state"]=="resume_ready")
    for table in tables: write(f"DELETE FROM {table} WHERE transaction_id=?",(tid,))
    return {"status":"passed" if ok else "failed","version":APP_VERSION,"build":BUILD,"persistent_state":True,"checkpoints":True,"resumable":True,"lease_recovery":True,"state_machine":True,"saga_recovery":True,"manual_review_boundary":True,"external_execution":False}
...
# ============================================================
# TARGET-2050.122 — TRANSACTION OBSERVABILITY & FORENSIC INTELLIGENCE
# ============================================================

OBS122_CAPABILITIES = [
    "transaction_observability", "unified_transaction_timeline",
    "cross_service_event_correlation", "transaction_event_correlation",
    "execution_trace_aggregation", "recovery_trace_aggregation",
    "forensic_event_capture", "failure_forensics", "failure_phase_analysis",
    "root_cause_evidence", "anomaly_detection", "timeline_anomaly_detection",
    "latency_anomaly_detection", "state_transition_anomaly_detection",
    "dependency_failure_correlation", "connector_failure_correlation",
    "reconciliation_failure_correlation", "evidence_linked_forensics",
    "explainable_recovery_recommendations", "forensic_confidence_scoring",
    "incident_fingerprint", "persistent_incident_history",
    "observability_decision_trace", "trace_redaction", "payload_hashing",
]


def _122_hash(value: Any) -> str:
    return hashlib.sha256(json.dumps(value, sort_keys=True, default=str, separators=(",", ":")).encode()).hexdigest()


def _122_redact(value: Any) -> Any:
    sensitive = {"authorization", "proxy-authorization", "cookie", "set-cookie", "x-api-key", "x-auth-token", "x-access-token", "password", "secret", "token"}
    if isinstance(value, dict):
        return {str(k): ("[REDACTED]" if str(k).lower() in sensitive else _122_redact(v)) for k, v in value.items()}
    if isinstance(value, list):
        return [_122_redact(v) for v in value[:100]]
    if isinstance(value, str):
        return value[:4000]
    return value


def _ensure_122_tables():
    statements = [
        """CREATE TABLE IF NOT EXISTS transaction_observability_events(
            id INTEGER PRIMARY KEY AUTOINCREMENT, event_id TEXT UNIQUE NOT NULL,
            transaction_id TEXT NOT NULL, service TEXT, connector_id TEXT,
            event_type TEXT NOT NULL, phase TEXT, status TEXT, latency_ms REAL,
            state_from TEXT, state_to TEXT, dependency_id TEXT,
            error_class TEXT, error_message TEXT, payload_hash TEXT,
            metadata_json TEXT NOT NULL DEFAULT '{}', created_at REAL NOT NULL)""",
        """CREATE INDEX IF NOT EXISTS idx_122_events_tx ON transaction_observability_events(transaction_id, created_at)""",
        """CREATE INDEX IF NOT EXISTS idx_122_events_type ON transaction_observability_events(event_type, created_at)""",
        """CREATE TABLE IF NOT EXISTS transaction_incidents(
            incident_id TEXT PRIMARY KEY, transaction_id TEXT NOT NULL,
            fingerprint TEXT NOT NULL, severity TEXT NOT NULL, status TEXT NOT NULL,
            root_cause TEXT, confidence REAL NOT NULL DEFAULT 0,
            anomaly_count INTEGER NOT NULL DEFAULT 0, finding_count INTEGER NOT NULL DEFAULT 0,
            recommendation TEXT, decision_trace_json TEXT NOT NULL DEFAULT '[]',
            created_at REAL NOT NULL, updated_at REAL NOT NULL)""",
        """CREATE INDEX IF NOT EXISTS idx_122_incident_tx ON transaction_incidents(transaction_id, updated_at)""",
        """CREATE TABLE IF NOT EXISTS transaction_forensic_findings(
            id INTEGER PRIMARY KEY AUTOINCREMENT, finding_id TEXT UNIQUE NOT NULL,
            incident_id TEXT NOT NULL, transaction_id TEXT NOT NULL,
            finding_type TEXT NOT NULL, phase TEXT, evidence_json TEXT NOT NULL,
            confidence REAL NOT NULL DEFAULT 0, created_at REAL NOT NULL)""",
        """CREATE INDEX IF NOT EXISTS idx_122_findings_incident ON transaction_forensic_findings(incident_id, created_at)""",
        """CREATE TABLE IF NOT EXISTS transaction_incident_links(
            id INTEGER PRIMARY KEY AUTOINCREMENT, incident_id TEXT NOT NULL,
            link_type TEXT NOT NULL, link_id TEXT NOT NULL, relation TEXT NOT NULL,
            created_at REAL NOT NULL)""",
        """CREATE TABLE IF NOT EXISTS transaction_observability_metrics(
            transaction_id TEXT PRIMARY KEY, event_count INTEGER NOT NULL DEFAULT 0,
            failure_count INTEGER NOT NULL DEFAULT 0, anomaly_count INTEGER NOT NULL DEFAULT 0,
            duration_ms REAL, first_event_at REAL, last_event_at REAL,
            updated_at REAL NOT NULL)""",
    ]
    with DB_LOCK:
        conn = db()
...
def _122_event(transaction_id: str, event_type: str, service: str = "", connector_id: str = "", phase: str = "", status: str = "", latency_ms: Optional[float] = None, state_from: str = "", state_to: str = "", dependency_id: str = "", error_class: str = "", error_message: str = "", payload: Any = None, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    now = time.time()
    eid = "evt-" + uuid.uuid4().hex[:20]
    payload_hash = _122_hash(_122_redact(payload)) if payload is not None else ""
    safe_meta = _122_redact(metadata or {})
    write("""INSERT INTO transaction_observability_events(event_id,transaction_id,service,connector_id,event_type,phase,status,latency_ms,state_from,state_to,dependency_id,error_class,error_message,payload_hash,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (eid, transaction_id, service, connector_id, event_type, phase, status, latency_ms, state_from, state_to, dependency_id, error_class, str(error_message)[:1000], payload_hash, json.dumps(safe_meta, sort_keys=True, default=str), now))
    return {"event_id": eid, "transaction_id": transaction_id, "event_type": event_type, "created_at": now}


def _122_json(row: sqlite3.Row, key: str, default=None):
    try: return json.loads(row[key])
    except Exception: return default if default is not None else {}


def _122_events(transaction_id: str, limit: int = 500) -> List[Dict[str, Any]]:
    limit = max(1, min(int(limit), 1000))
    rows = q("SELECT * FROM transaction_observability_events WHERE transaction_id=? ORDER BY created_at ASC, id ASC LIMIT ?", (transaction_id, limit))
    return [{**dict(r), "metadata": _122_json(r, "metadata_json", {})} for r in rows]


def _122_sync_known_transaction_events(transaction_id: str):
    """Read existing transaction/recovery records into the observability ledger without side effects."""
    known = q("SELECT COUNT(*) AS n FROM transaction_observability_events WHERE transaction_id=?", (transaction_id,), one=True)
    if known and int(known["n"]) > 0:
        return
    rows = q("SELECT * FROM transaction_recovery_events WHERE transaction_id=? ORDER BY created_at ASC", (transaction_id,)) if q("SELECT name FROM sqlite_master WHERE type='table' AND name='transaction_recovery_events'", one=True) else []
    for r in rows:
        d = dict(r); _122_event(transaction_id, "recovery_event", phase=str(d.get("phase") or "recovery"), status=str(d.get("state") or d.get("status") or "observed"), metadata={k:v for k,v in d.items() if k not in {"transaction_id","created_at"}})
    steps = q("SELECT * FROM multi_service_transaction_steps WHERE transaction_id=? ORDER BY id ASC", (transaction_id,)) if q("SELECT name FROM sqlite_master WHERE type='table' AND name='multi_service_transaction_steps'", one=True) else []
    for r in steps:
        d = dict(r); _122_event(transaction_id, "service_step", service=str(d.get("service") or ""), phase="execution", status=str(d.get("status") or "observed"), state_to=str(d.get("status") or ""), metadata={k:v for k,v in d.items() if k not in {"transaction_id"}})


def _122_forensics(transaction_id: str) -> Dict[str, Any]:
    events = _122_events(transaction_id)
    findings: List[Dict[str, Any]] = []
    failures = [e for e in events if str(e.get("status", "")).lower() in {"failed", "error", "timeout", "unknown", "unavailable", "rejected"} or e.get("error_class")]
    anomalies = []
    if events:
        times = [float(e["created_at"]) for e in events]
        duration_ms = max(0.0, (max(times)-min(times))*1000.0)
        latencies = [float(e["latency_ms"]) for e in events if e.get("latency_ms") is not None]
        if len(latencies) >= 3:
            mean = sum(latencies)/len(latencies)
            variance = sum((x-mean)**2 for x in latencies)/len(latencies)
            sd = variance ** 0.5
            threshold = mean + max(2.0*sd, 1.0)
            for e in events:
                if e.get("latency_ms") is not None and float(e["latency_ms"]) > threshold:
                    anomalies.append({"type":"latency_anomaly","event_id":e["event_id"],"latency_ms":e["latency_ms"],"threshold_ms":round(threshold,3)})
    else:
        duration_ms = 0.0
    transitions = [(e.get("state_from"), e.get("state_to")) for e in events if e.get("state_from") or e.get("state_to")]
    terminal = {"committed", "completed", "failed", "cancelled", "rejected", "manual_review"}
    for i, e in enumerate(events):
        if e.get("state_to") in terminal and i + 1 < len(events) and e.get("state_to") not in {"failed", "manual_review"}:
            nxt = events[i+1].get("state_from") or ""
            if nxt and nxt != e.get("state_to"):
                anomalies.append({"type":"state_transition_anomaly","event_id":e["event_id"],"state_to":e.get("state_to"),"next_state_from":nxt})
    if failures:
        first = failures[0]
        root = str(first.get("error_class") or first.get("event_type") or "execution_failure")
        phase = str(first.get("phase") or "unknown")
        evidence = {"event_id":first["event_id"],"event_type":first.get("event_type"),"phase":phase,"status":first.get("status"),"error_class":first.get("error_class"),"error_message":first.get("error_message")}
        findings.append({"type":"root_cause_candidate","phase":phase,"confidence":0.82,"evidence":evidence})
        if first.get("dependency_id"):
            findings.append({"type":"dependency_failure_correlation","phase":phase,"confidence":0.76,"evidence":{"dependency_id":first["dependency_id"],"event_id":first["event_id"]}})
        if first.get("connector_id"):
            findings.append({"type":"connector_failure_correlation","phase":phase,"confidence":0.76,"evidence":{"connector_id":first["connector_id"],"event_id":first["event_id"]}})
    for a in anomalies:
        findings.append({"type":a["type"],"phase":"observation","confidence":0.70,"evidence":a})
    if any("reconcil" in str(e.get("event_type", "")).lower() for e in failures):
        findings.append({"type":"reconciliation_failure_correlation","phase":"reconciliation","confidence":0.80,"evidence":{"failed_events":len(failures)}})
    if not events:
        recommendation = "No observability events exist; capture execution and recovery events before making a recovery decision."
    elif failures:
        recommendation = "Inspect the first causal failure, preserve the incident fingerprint, and use the existing approval-bounded recovery/compensation boundary; do not replay side effects automatically."
    elif anomalies:
        recommendation = "Review anomalous timing/state transitions and re-observe the transaction before taking any external side effect."
    else:
        recommendation = "No deterministic failure or anomaly detected in the captured timeline."
    confidence = round(min(0.99, max([f["confidence"] for f in findings], default=0.50)), 3)
    fingerprint = _122_hash({"transaction_id":transaction_id,"failures":[(e.get("event_type"),e.get("phase"),e.get("error_class"),e.get("connector_id")) for e in failures],"anomalies":anomalies})[:32]
    severity = "critical" if len(failures) >= 2 else ("high" if failures else ("medium" if anomalies else "info"))
    return {"transaction_id":transaction_id,"event_count":len(events),"failure_count":len(failures),"anomaly_count":len(anomalies),"duration_ms":round(duration_ms,3),"findings":findings,"anomalies":anomalies,"root_cause":(findings[0]["evidence"].get("error_class") or findings[0]["evidence"].get("event_type")) if findings else None,"root_cause_phase":findings[0].get("phase") if findings else None,"recommendation":recommendation,"confidence":confidence,"severity":severity,"fingerprint":fingerprint,"transitions":transitions}


def _122_incident(transaction_id: str, analysis: Dict[str, Any]) -> Dict[str, Any]:
    now = time.time(); iid = "inc-" + uuid.uuid4().hex[:20]
    existing = q("SELECT * FROM transaction_incidents WHERE transaction_id=? AND fingerprint=? ORDER BY updated_at DESC LIMIT 1", (transaction_id, analysis["fingerprint"]), one=True)
    if existing:
        iid = existing["incident_id"]
        write("UPDATE transaction_incidents SET severity=?,status=?,root_cause=?,confidence=?,anomaly_count=?,finding_count=?,recommendation=?,decision_trace_json=?,updated_at=? WHERE incident_id=?", (analysis["severity"], "open" if analysis["failure_count"] or analysis["anomaly_count"] else "resolved", analysis["root_cause"], analysis["confidence"], analysis["anomaly_count"], len(analysis["findings"]), analysis["recommendation"], json.dumps({"rules":["first_failure_cause","latency_outlier","state_transition_check","approval_bounded_recovery"]}), now, iid))
        write("DELETE FROM transaction_forensic_findings WHERE incident_id=?", (iid,))
    else:
        write("INSERT INTO transaction_incidents(incident_id,transaction_id,fingerprint,severity,status,root_cause,confidence,anomaly_count,finding_count,recommendation,decision_trace_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", (iid,transaction_id,analysis["fingerprint"],analysis["severity"],"open" if analysis["failure_count"] or analysis["anomaly_count"] else "resolved",analysis["root_cause"],analysis["confidence"],analysis["anomaly_count"],len(analysis["findings"]),analysis["recommendation"],json.dumps({"rules":["first_failure_cause","latency_outlier","state_transition_check","approval_bounded_recovery"]}),now,now))
    for f in analysis["findings"]:
        write("INSERT INTO transaction_forensic_findings(finding_id,incident_id,transaction_id,finding_type,phase,evidence_json,confidence,created_at) VALUES(?,?,?,?,?,?,?,?)", ("find-"+uuid.uuid4().hex[:20],iid,transaction_id,f["type"],f.get("phase"),json.dumps(_122_redact(f.get("evidence",{})),sort_keys=True),float(f.get("confidence",0)),now))
    write("INSERT OR REPLACE INTO transaction_observability_metrics(transaction_id,event_count,failure_count,anomaly_count,duration_ms,first_event_at,last_event_at,updated_at) VALUES(?,?,?,?,?,?,?,?)", (transaction_id,analysis["event_count"],analysis["failure_count"],analysis["anomaly_count"],analysis["duration_ms"],(min([e["created_at"] for e in _122_events(transaction_id)]) if analysis["event_count"] else None),(max([e["created_at"] for e in _122_events(transaction_id)]) if analysis["event_count"] else None),now))
    return {"incident_id":iid,"fingerprint":analysis["fingerprint"],"status":"open" if analysis["failure_count"] or analysis["anomaly_count"] else "resolved"}


class TransactionObservabilityEventRequest(BaseModel):
    transaction_id: str
...
@app.post("/transaction-observability/events")
def transaction_observability_event(request: TransactionObservabilityEventRequest):
    if not request.transaction_id.strip(): raise HTTPException(status_code=400, detail="transaction_id_required")
    event = _122_event(**request.model_dump())
    return {"status":"recorded","version":APP_VERSION,"event":event,"payload_stored":False,"payload_hashed":bool(request.payload is not None)}


@app.get("/transaction-observability")
def transaction_observability(limit:int=50):
    rows=q("SELECT * FROM transaction_observability_metrics ORDER BY updated_at DESC LIMIT ?",(max(1,min(int(limit),200)),))
    return {"status":"online","version":APP_VERSION,"build":BUILD,"transactions":[dict(r) for r in rows],"safety":{"payload_redaction":True,"payload_hashing":True,"automatic_side_effect_replay":False}}


@app.get("/transaction-observability/events")
def transaction_observability_events(transaction_id:str,limit:int=500):
    return {"version":APP_VERSION,"transaction_id":transaction_id,"count":len(_122_events(transaction_id,limit)),"events":_122_events(transaction_id,limit)}


@app.get("/transaction-observability/timeline")
def transaction_observability_timeline(transaction_id:str,limit:int=500):
    events=_122_events(transaction_id,limit)
    return {"version":APP_VERSION,"transaction_id":transaction_id,"timeline":events,"event_count":len(events),"ordering":"created_at_ascending"}


@app.get("/transaction-observability/anomalies")
def transaction_observability_anomalies(transaction_id:str):
    analysis=_122_forensics(transaction_id)
    return {"version":APP_VERSION,"transaction_id":transaction_id,"anomalies":analysis["anomalies"],"count":analysis["anomaly_count"],"rules":["latency_outlier","state_transition_consistency"]}


@app.get("/transaction-observability/forensics")
def transaction_observability_forensics(transaction_id:str):
    _122_sync_known_transaction_events(transaction_id); analysis=_122_forensics(transaction_id); incident=_122_incident(transaction_id,analysis)
    return {"version":APP_VERSION,"build":BUILD,"forensics":analysis,"incident":incident,"automatic_side_effect_execution":False}


@app.get("/transaction-observability/incidents")
def transaction_observability_incidents(limit:int=100):
    rows=q("SELECT * FROM transaction_incidents ORDER BY updated_at DESC LIMIT ?",(max(1,min(int(limit),200)),))
    return {"version":APP_VERSION,"count":len(rows),"incidents":[dict(r) for r in rows]}


@app.get("/transaction-observability/{transaction_id}")
def transaction_observability_status(transaction_id:str):
    _122_sync_known_transaction_events(transaction_id); analysis=_122_forensics(transaction_id); incident=_122_incident(transaction_id,analysis)
    return {"version":APP_VERSION,"transaction_id":transaction_id,"summary":{k:analysis[k] for k in ("event_count","failure_count","anomaly_count","duration_ms","severity","confidence","fingerprint")},"incident":incident}


@app.get("/transaction-observability/{transaction_id}/timeline")
def transaction_observability_tx_timeline(transaction_id:str,limit:int=500):
    return transaction_observability_timeline(transaction_id,limit)


@app.get("/transaction-observability/{transaction_id}/forensics")
def transaction_observability_tx_forensics(transaction_id:str):
    return transaction_observability_forensics(transaction_id)


@app.get("/transaction-observability/{transaction_id}/trace")
def transaction_observability_trace(transaction_id:str):
    _122_sync_known_transaction_events(transaction_id); analysis=_122_forensics(transaction_id)
    return {"version":APP_VERSION,"transaction_id":transaction_id,"decision_trace":{"rules":["correlate_events","identify_first_failure","detect_latency_outliers","validate_state_transitions","preserve_approval_boundary"],"root_cause":analysis["root_cause"],"confidence":analysis["confidence"],"recommendation":analysis["recommendation"]},"automatic_side_effect_execution":False}


@app.get("/test-transaction-observability")
def test_transaction_observability():
    tid="__test_122__"
    write("DELETE FROM transaction_observability_events WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_forensic_findings WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_incidents WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_incident_links WHERE incident_id IN (SELECT incident_id FROM transaction_incidents WHERE transaction_id=?)",(tid,))
    write("DELETE FROM transaction_observability_metrics WHERE transaction_id=?",(tid,))
    _122_event(tid,"service_step",service="svc-a",phase="execution",status="completed",latency_ms=10,state_from="pending",state_to="executing")
    _122_event(tid,"connector_error",service="svc-b",connector_id="conn-b",phase="execution",status="failed",error_class="TimeoutError",error_message="synthetic failure",dependency_id="step-a",metadata={"test":True})
    _122_event(tid,"recovery_event",phase="recovery",status="manual_review",state_from="executing",state_to="manual_review")
    analysis=_122_forensics(tid); incident=_122_incident(tid,analysis); events=_122_events(tid)
    ok=(len(events)==3 and analysis["failure_count"]==1 and analysis["root_cause"]=="TimeoutError" and analysis["anomaly_count"]==0 and incident["status"]=="open")
    write("DELETE FROM transaction_observability_events WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_forensic_findings WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_incidents WHERE transaction_id=?",(tid,))
    write("DELETE FROM transaction_observability_metrics WHERE transaction_id=?",(tid,))
    return {"status":"passed" if ok else "failed","version":APP_VERSION,"build":BUILD,"timeline":True,"event_correlation":True,"failure_forensics":True,"root_cause_evidence":True,"anomaly_detection":True,"incident_fingerprint":True,"persistent_incident_history":True,"trace_redaction":True,"payload_hashing":True,"external_execution":False,"automatic_side_effect_retry":False}
...
# ============================================================
# TARGET-2050.123 — CAUSAL GRAPH / ROOT-CAUSE INTELLIGENCE
# ============================================================
...
def _ensure_123_tables():
    statements = [
        """CREATE TABLE IF NOT EXISTS transaction_causal_nodes(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            created_at REAL NOT NULL)""",
        "CREATE INDEX IF NOT EXISTS idx_123_nodes_tx ON transaction_causal_nodes(transaction_id, created_at)",
        """CREATE TABLE IF NOT EXISTS transaction_causal_edges(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            created_at REAL NOT NULL)""",
        "CREATE INDEX IF NOT EXISTS idx_123_edges_tx ON transaction_causal_edges(transaction_id, created_at)",
        """CREATE TABLE IF NOT EXISTS transaction_causal_findings(
            ...
            created_at REAL NOT NULL)""",
        "CREATE INDEX IF NOT EXISTS idx_123_findings_tx ON transaction_causal_findings(transaction_id, created_at)",
        """CREATE TABLE IF NOT EXISTS transaction_causal_history(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_id TEXT NOT NULL,
            ...
            created_at REAL NOT NULL)""",
        "CREATE INDEX IF NOT EXISTS idx_123_history_tx ON transaction_causal_history(transaction_id, created_at)",
    ]
    with DB_LOCK:
        conn = db()
        try:
...
def _123_safe(v: Any) -> Any:
    if isinstance(v, dict):
        return {str(k): _123_safe(x) for k, x in list(v.items())[:80]}
    if isinstance(v, list):
        return [_123_safe(x) for x in v[:80]]
    if isinstance(v, str):
        return v[:1000]
    return v


def _123_hash(v: Any) -> str:
    return hashlib.sha256(json.dumps(v, sort_keys=True, default=str, separators=(",", ":")).encode()).hexdigest()


def _123_events(txid: str) -> List[Dict[str, Any]]:
    rows = q("""SELECT event_id, transaction_id, service, connector_id, event_type, phase,
               status, latency_ms, state_from, state_to, dependency_id, error_class,
               error_message, payload_hash, metadata_json, created_at
               FROM transaction_observability_events WHERE transaction_id=?
               ORDER BY created_at ASC, id ASC""", (txid,))
    out = []
    for r in rows:
        d = dict(r)
        try:
...
def _123_build(txid: str) -> Dict[str, Any]:
    events = _123_events(txid)
    nodes = []
    for e in events:
        nid = "node-" + _123_hash([txid, e.get("event_id")])[:16]
        nodes.append({
            "node_id": nid, "node_type": "event", "event_id": e.get("event_id"),
            "service": e.get("service"), "connector_id": e.get("connector_id"),
            "phase": e.get("phase"), "status": e.get("status"),
            "error_class": e.get("error_class"), "state_from": e.get("state_from"),
            "state_to": e.get("state_to"), "latency_ms": e.get("latency_ms"),
            "payload_hash": e.get("payload_hash"), "created_at": e.get("created_at"),
        })
    edges = []
    for a, b in zip(nodes, nodes[1:]):
        dt = max(0.0, float(b["created_at"] or 0) - float(a["created_at"] or 0))
        weight = 0.95 if dt >= 0 else 0.2
        relation = "temporal_precedence"
        if a.get("service") and b.get("service") and a.get("service") != b.get("service"):
            relation = "cross_service_precedence"
        edges.append({"edge_id":"edge-"+_123_hash([a["node_id"],b["node_id"],relation])[:16],
                      "from_node":a["node_id"],"to_node":b["node_id"],"relation":relation,
                      "weight":weight,"evidence":{"time_delta_seconds":round(dt,6)}})
    for i, b in enumerate(nodes):
        dep = events[i].get("dependency_id") if i < len(events) else None
        if dep:
            for a in nodes:
                if a["node_id"] != b["node_id"] and (a.get("event_id") == dep or a.get("service") == dep):
                    edges.append({"edge_id":"edge-"+_123_hash([a["node_id"],b["node_id"],"dependency"])[:16],
                                  "from_node":a["node_id"],"to_node":b["node_id"],"relation":"dependency_causation",
                                  "weight":0.9,"evidence":{"dependency_id":dep}})
    failures = [n for n in nodes if n.get("error_class") or str(n.get("status") or "").lower() in {"failed","error","timeout","unknown"}]
    root = None
    ranked = []
    for n in failures:
        incoming = sum(1 for e in edges if e["to_node"] == n["node_id"])
        outgoing = sum(1 for e in edges if e["from_node"] == n["node_id"])
        score = 0.55 + min(0.2, outgoing * 0.05) + min(0.15, (1 if n.get("error_class") else 0)*0.15) - min(0.15, incoming*0.03)
        score = round(max(0.0,min(1.0,score)),4)
        ranked.append({"node_id":n["node_id"],"cause":n.get("error_class") or n.get("status"),"score":score,
                       "evidence":{"event_id":n.get("event_id"),"service":n.get("service"),"phase":n.get("phase"),
                                   "incoming_edges":incoming,"outgoing_edges":outgoing}})
    ranked.sort(key=lambda x:(-x["score"], x["node_id"]))
    if ranked:
        root = ranked[0]
    fingerprint = _123_hash({"nodes":nodes,"edges":edges})
    explanation = "No causal failure identified."
    if root:
        explanation = (f"Candidate root cause {root['cause']} is linked to transaction failure evidence; "
                       f"confidence is {root['score']:.2f} based on deterministic temporal/dependency graph evidence.")
    counterfactual = {
        "intervention": "prevent_candidate_root_cause",
        "expected_effect": "downstream failure path may be avoided",
        "guarantee": False,
        "basis": "graph dependency and temporal precedence only",
    }
    return {"transaction_id":txid,"nodes":nodes,"edges":edges,"failures":failures,
            "root_cause":root.get("cause") if root else None,
            "root_node_id":root.get("node_id") if root else None,
            "root_cause_confidence":root.get("score",0.0) if root else 0.0,
            "ranked_root_causes":ranked,"explanation":explanation,
            "counterfactual":counterfactual,"fingerprint":fingerprint,
            "decision_trace":{"method":"deterministic_temporal_dependency_analysis",
                              "event_count":len(events),"node_count":len(nodes),"edge_count":len(edges),
                              "failure_count":len(failures),"automatic_side_effect":False}}


def _123_persist(txid: str, graph: Dict[str, Any]) -> None:
    now=time.time()
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("DELETE FROM transaction_causal_nodes WHERE transaction_id=?",(txid,))
            conn.execute("DELETE FROM transaction_causal_edges WHERE transaction_id=?",(txid,))
            conn.execute("DELETE FROM transaction_causal_findings WHERE transaction_id=?",(txid,))
            for n in graph["nodes"]:
                conn.execute("""INSERT OR REPLACE INTO transaction_causal_nodes
                    (node_id,transaction_id,node_type,event_id,service,connector_id,phase,status,error_class,state_from,state_to,latency_ms,payload_hash,metadata_json,created_at)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (n["node_id"],txid,n["node_type"],n.get("event_id"),n.get("service"),n.get("connector_id"),n.get("phase"),n.get("status"),n.get("error_class"),n.get("state_from"),n.get("state_to"),n.get("latency_ms"),n.get("payload_hash"),"{}",n.get("created_at") or now))
            for e in graph["edges"]:
                conn.execute("INSERT OR REPLACE INTO transaction_causal_edges(edge_id,transaction_id,from_node,to_node,relation,weight,evidence_json,created_at) VALUES(?,?,?,?,?,?,?,?)",
                             (e["edge_id"],txid,e["from_node"],e["to_node"],e["relation"],e["weight"],json.dumps(_123_safe(e["evidence"])),now))
            if graph["root_cause"]:
                conn.execute("INSERT OR REPLACE INTO transaction_causal_findings(finding_id,transaction_id,node_id,finding_type,cause,confidence,evidence_json,explanation,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
                             ("finding-"+graph["fingerprint"][:20],txid,graph["root_node_id"],"candidate_root_cause",graph["root_cause"],graph["root_cause_confidence"],json.dumps(_123_safe(graph["ranked_root_causes"][0]["evidence"])),graph["explanation"],now))
            conn.execute("INSERT INTO transaction_causal_history(transaction_id,graph_fingerprint,root_cause,confidence,graph_json,decision_trace_json,created_at) VALUES(?,?,?,?,?,?,?)",
                         (txid,graph["fingerprint"],graph["root_cause"],graph["root_cause_confidence"],json.dumps(_123_safe(graph)),json.dumps(graph["decision_trace"]),now))
            conn.commit()
        finally:
            conn.close()


@app.get("/transaction-causal-graph")
def transaction_causal_graph(transaction_id: str):
    graph=_123_build(transaction_id)
    _123_persist(transaction_id,graph)
    return graph


@app.get("/transaction-causal-graph/nodes")
def transaction_causal_nodes(transaction_id: str):
    graph=_123_build(transaction_id)
    return {"transaction_id":transaction_id,"nodes":graph["nodes"]}


@app.get("/transaction-causal-graph/edges")
def transaction_causal_edges(transaction_id: str):
    graph=_123_build(transaction_id)
    return {"transaction_id":transaction_id,"edges":graph["edges"]}


@app.get("/transaction-causal-graph/root-causes")
def transaction_causal_root_causes(transaction_id: str):
    graph=_123_build(transaction_id)
    return {"transaction_id":transaction_id,"root_cause":graph["root_cause"],
            "confidence":graph["root_cause_confidence"],"ranked":graph["ranked_root_causes"],
            "explanation":graph["explanation"]}


@app.get("/transaction-causal-graph/counterfactual")
def transaction_causal_counterfactual(transaction_id: str):
    graph=_123_build(transaction_id)
    return {"transaction_id":transaction_id,"counterfactual":graph["counterfactual"],
            "root_cause":graph["root_cause"],"confidence":graph["root_cause_confidence"]}


@app.get("/transaction-causal-graph/history")
def transaction_causal_history(transaction_id: str, limit: int = 20):
    limit=max(1,min(100,limit))
    rows=q("SELECT transaction_id,graph_fingerprint,root_cause,confidence,decision_trace_json,created_at FROM transaction_causal_history WHERE transaction_id=? ORDER BY id DESC LIMIT ?",(transaction_id,limit))
    return {"transaction_id":transaction_id,"history":[dict(r) for r in rows]}


@app.get("/transaction-causal-graph/{transaction_id}")
def transaction_causal_graph_by_id(transaction_id: str):
    return transaction_causal_graph(transaction_id)


@app.get("/test-transaction-causal-graph")
def test_transaction_causal_graph():
    txid="test-123-"+uuid.uuid4().hex[:12]
    now=time.time()
    ids=[]
    try:
        for i, (svc, status, err, dep) in enumerate([
            ("service-a","success",None,None),
            ("service-b","failed","TimeoutError",None),
            ("service-c","failed","DependencyError","service-b"),
        ]):
            eid="event-123-"+uuid.uuid4().hex[:12]; ids.append(eid)
            write("""INSERT INTO transaction_observability_events
                (event_id,transaction_id,service,connector_id,event_type,phase,status,latency_ms,state_from,state_to,dependency_id,error_class,error_message,payload_hash,metadata_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (eid,txid,svc,None,"test","execution",status,50+i*20,"running",status,dep,err,err,"hash",json.dumps({"test":True}),now+i*0.01))
        graph=_123_build(txid); _123_persist(txid,graph)
        ok=(len(graph["nodes"])==3 and len(graph["edges"])>=2 and graph["root_cause"]=="TimeoutError"
            and graph["root_cause_confidence"]>0 and graph["counterfactual"]["guarantee"] is False)
        return {"status":"passed" if ok else "failed","version":APP_VERSION,"build":BUILD,
                "causal_graph":True,"root_cause":graph["root_cause"],"confidence":graph["root_cause_confidence"],
                "counterfactual":True,"persistent_history":True,"external_execution":False,
                "automatic_side_effect_retry":False}
    finally:
        with DB_LOCK:
            conn=db()
            try:
                conn.execute("DELETE FROM transaction_observability_events WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM transaction_causal_nodes WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM transaction_causal_edges WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM transaction_causal_findings WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM transaction_causal_history WHERE transaction_id=?",(txid,))
                conn.commit()
            finally: conn.close()
...
# ============================================================
# TARGET-2050.124 — PREDICTIVE FAILURE / PREEMPTIVE RECOVERY
# ============================================================

PREDICTIVE_HORIZON_SECONDS = max(30, int(os.getenv("AI_INFINITY_PREDICTIVE_HORIZON_SECONDS", "900")))


def _124_hash(value: Any) -> str:
    return hashlib.sha256(json.dumps(value, sort_keys=True, default=str, separators=(",", ":")).encode()).hexdigest()


def _ensure_124_tables() -> None:
    with DB_LOCK:
        conn = db()
        try:
            conn.executescript("""
            CREATE TABLE IF NOT EXISTS transaction_failure_predictions(
                ...
                causal_evidence_json TEXT,
                recommendation_json TEXT,
                fingerprint TEXT,
                created_at REAL
            );
            CREATE TABLE IF NOT EXISTS transaction_predictive_actions(
                action_id TEXT PRIMARY KEY,
                prediction_id TEXT,
                transaction_id TEXT,
                action_type TEXT,
            ...
            CREATE TABLE IF NOT EXISTS transaction_prediction_outcomes(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                prediction_id TEXT,
                transaction_id TEXT,
                observed_failure INTEGER,
                observed_class TEXT,
                score REAL,
                evaluated_at REAL
            );
            CREATE TABLE IF NOT EXISTS transaction_predictive_history(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                transaction_id TEXT,
                prediction_id TEXT,
                fingerprint TEXT,
            ...
            CREATE INDEX IF NOT EXISTS idx_124_pred_tx ON transaction_failure_predictions(transaction_id,created_at);
            CREATE INDEX IF NOT EXISTS idx_124_hist_tx ON transaction_predictive_history(transaction_id,created_at);
            """)
            conn.commit()
        finally:
            conn.close()
...
def _124_rows(txid: str) -> List[Dict[str, Any]]:
    rows = q("""SELECT event_id,service,connector_id,event_type,phase,status,latency_ms,
               state_from,state_to,dependency_id,error_class,error_message,created_at
               FROM transaction_observability_events WHERE transaction_id=? ORDER BY created_at ASC""", (txid,))
    return [dict(r) for r in rows]


def _124_factor(name: str, score: float, evidence: Any) -> Dict[str, Any]:
    return {"factor": name, "weight": round(max(0.0, min(1.0, score)), 4), "evidence": evidence}


def _124_build(txid: str) -> Dict[str, Any]:
    events = _124_rows(txid)
    now_ts = time.time()
    ...
    for e in events:
        ...
        st = services.setdefault(svc, {"events": 0, "failures": 0, "latencies": [], "last": None})
        st["events"] += 1
        if e.get("latency_ms") is not None:
            try:
                lat = float(e["latency_ms"]); latencies.append(lat); st["latencies"].append(lat)
            except Exception:
                pass
        if e.get("state_from") != e.get("state_to"):
            state_changes += 1
        status = str(e.get("status") or "").lower()
        err = e.get("error_class") or e.get("error_message")
        if status in {"failed", "error", "timeout", "unknown"} or err:
            st["failures"] += 1
            cls = str(e.get("error_class") or status or "unknown_failure")
            failure_classes[cls] = failure_classes.get(cls, 0) + 1
        meta = str(e.get("error_message") or "").lower()
        if "anomal" in meta:
            anomalous += 1

    if events:
        failure_count = sum(x["failures"] for x in services.values())
        factors.append(_124_factor("observed_failure_signal", min(1.0, failure_count / max(1, len(events))), {"failures": failure_count, "events": len(events)}))
        if failure_count:
            factors.append(_124_factor("failure_cluster", min(1.0, failure_count / max(1, len(services))), {"services": len(services)}))
        if any((e.get("dependency_id") for e in events)):
            dep_fail = sum(1 for e in events if e.get("dependency_id") and str(e.get("status") or "").lower() in {"failed","error","timeout","unknown"})
            factors.append(_124_factor("dependency_failure_signal", min(1.0, dep_fail / max(1, len(events))), {"dependency_failures": dep_fail}))
        if latencies:
            mean = sum(latencies) / len(latencies)
            high = sum(1 for x in latencies if x > max(500.0, mean * 1.75))
            factors.append(_124_factor("latency_risk", min(1.0, high / len(latencies)), {"mean_ms": round(mean, 2), "high_latency_events": high}))
        if state_changes:
            factors.append(_124_factor("state_transition_risk", min(1.0, anomalous / max(1, state_changes)), {"state_changes": state_changes, "anomalies": anomalous}))

    # Causal evidence from 123, without executing or mutating any external action.
    causal = _123_build(txid) if events else {"root_cause": None, "root_cause_confidence": 0.0, "ranked_root_causes": [], "edges": []}
    if causal.get("root_cause"):
        factors.append(_124_factor("causal_root_cause_signal", float(causal.get("root_cause_confidence", 0.0)), {"root_cause": causal.get("root_cause"), "edges": len(causal.get("edges", []))}))

    raw = sum(f["weight"] * {"observed_failure_signal": .34, "failure_cluster": .16, "dependency_failure_signal": .18,
                              "latency_risk": .10, "state_transition_risk": .07, "causal_root_cause_signal": .15}.get(f["factor"], .08) for f in factors)
    risk = round(max(0.0, min(0.99, raw)), 4)
    if risk >= 0.75: level = "critical"
    elif risk >= 0.50: level = "high"
    elif risk >= 0.25: level = "medium"
    else: level = "low"

    predicted = None
    if failure_classes:
        predicted = max(failure_classes.items(), key=lambda x: (x[1], x[0]))[0]
    elif causal.get("root_cause"):
        predicted = causal["root_cause"]
    elif risk >= 0.25:
        predicted = "transaction_failure"

    if risk >= 0.75:
        actions = [
            {"type": "pause_before_side_effect", "rationale": "predicted failure risk is critical", "risk_reduction": 0.35, "approval_required": True},
            {"type": "replan_or_fail_closed", "rationale": "re-evaluate failing dependency before consequential execution", "risk_reduction": 0.25, "approval_required": True},
        ]
    elif risk >= 0.50:
        actions = [
            {"type": "preflight_recheck", "rationale": "elevated predicted failure risk", "risk_reduction": 0.15, "approval_required": False},
            {"type": "prepare_recovery_plan", "rationale": "stage recovery without executing side effects", "risk_reduction": 0.20, "approval_required": True},
        ]
    elif risk >= 0.25:
        actions = [{"type": "increase_observation", "rationale": "moderate predictive risk", "risk_reduction": 0.08, "approval_required": False}]
    else:
        actions = [{"type": "continue_monitoring", "rationale": "no strong predictive failure signal", "risk_reduction": 0.02, "approval_required": False}]

    horizon = PREDICTIVE_HORIZON_SECONDS if risk > 0 else 0
    fp = _124_hash({"transaction_id": txid, "risk": risk, "predicted": predicted, "factors": factors, "causal": causal.get("fingerprint")})
    decision_trace = {
        "method": "deterministic_predictive_failure_analysis",
        "event_count": len(events), "service_count": len(services), "risk_score": risk,
        "risk_level": level, "horizon_seconds": horizon,
        "factor_count": len(factors), "causal_root_cause": causal.get("root_cause"),
        "automatic_side_effect": False, "automatic_side_effect_retry": False,
    }
    return {"transaction_id": txid, "risk_score": risk, "risk_level": level, "predicted_failure": predicted,
            "horizon_seconds": horizon, "factors": factors, "causal_evidence": {
                "root_cause": causal.get("root_cause"), "confidence": causal.get("root_cause_confidence", 0.0), "fingerprint": causal.get("fingerprint")},
            "recommendations": actions, "fingerprint": fp, "decision_trace": decision_trace, "generated_at": now_ts}


def _124_persist(pred: Dict[str, Any]) -> None:
    pid = "prediction-124-" + pred["fingerprint"][:24]
    with DB_LOCK:
        conn = db()
        try:
            conn.execute("INSERT OR REPLACE INTO transaction_failure_predictions VALUES(?,?,?,?,?,?,?,?,?,?,?)", (
                pid, pred["transaction_id"], pred["risk_score"], pred["risk_level"], pred["predicted_failure"],
                pred["horizon_seconds"], json.dumps(_123_safe(pred["factors"])), json.dumps(_123_safe(pred["causal_evidence"])),
                json.dumps(_123_safe(pred["recommendations"])), pred["fingerprint"], pred["generated_at"]))
            for rec in pred["recommendations"]:
                aid = "predictive-action-" + _124_hash({"pid": pid, "type": rec["type"]})[:20]
                conn.execute("INSERT OR REPLACE INTO transaction_predictive_actions VALUES(?,?,?,?,?,?,?,?,?)", (
                    aid, pid, pred["transaction_id"], rec["type"], rec["rationale"], rec["risk_reduction"],
                    1 if rec["approval_required"] else 0, "recommended", pred["generated_at"]))
            conn.execute("INSERT INTO transaction_predictive_history(transaction_id,prediction_id,fingerprint,risk_score,risk_level,prediction_json,decision_trace_json,created_at) VALUES(?,?,?,?,?,?,?,?)", (
                pred["transaction_id"], pid, pred["fingerprint"], pred["risk_score"], pred["risk_level"],
                json.dumps(_123_safe(pred)), json.dumps(pred["decision_trace"]), pred["generated_at"]))
            conn.commit()
        finally:
            conn.close()
    return pid
...
@app.get("/transaction-predictive-failure")
def transaction_predictive_failure(transaction_id: str):
    pred = _124_build(transaction_id); pid = _124_persist(pred)
    return {**pred, "prediction_id": pid}


@app.get("/transaction-predictive-failure/risk")
def transaction_predictive_risk(transaction_id: str):
    pred = _124_build(transaction_id)
    return {"transaction_id": transaction_id, "risk_score": pred["risk_score"], "risk_level": pred["risk_level"], "predicted_failure": pred["predicted_failure"], "horizon_seconds": pred["horizon_seconds"], "factors": pred["factors"]}


@app.get("/transaction-predictive-failure/recovery")
def transaction_predictive_recovery(transaction_id: str):
    pred = _124_build(transaction_id)
    return {"transaction_id": transaction_id, "risk": {"score": pred["risk_score"], "level": pred["risk_level"]}, "recommendations": pred["recommendations"], "approval_boundary": "external side effects require explicit approval", "automatic_execution": False}


@app.get("/transaction-predictive-failure/history")
def transaction_predictive_history(transaction_id: str, limit: int = 20):
    limit = max(1, min(100, limit))
    rows = q("SELECT prediction_id,risk_score,risk_level,fingerprint,created_at FROM transaction_predictive_history WHERE transaction_id=? ORDER BY id DESC LIMIT ?", (transaction_id, limit))
    return {"transaction_id": transaction_id, "history": [dict(r) for r in rows]}


@app.get("/transaction-predictive-failure/{transaction_id}")
def transaction_predictive_failure_by_id(transaction_id: str):
    return transaction_predictive_failure(transaction_id)


@app.get("/test-transaction-predictive-failure")
def test_transaction_predictive_failure():
    txid = "test-124-" + uuid.uuid4().hex[:12]
    now_ts = time.time()
    try:
        rows = [
            ("e1", "gateway", "execution", "success", 80, "running", "completed", None, None),
            ("e2", "service-b", "execution", "failed", 900, "running", "failed", "TimeoutError", None),
            ("e3", "service-c", "execution", "failed", 950, "running", "failed", "DependencyError", "service-b"),
        ]
        for i, (eid, svc, phase, status, lat, sf, st, err, dep) in enumerate(rows):
            write("""INSERT INTO transaction_observability_events
                (event_id,transaction_id,service,connector_id,event_type,phase,status,latency_ms,state_from,state_to,dependency_id,error_class,error_message,payload_hash,metadata_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (eid, txid, svc, None, "predictive-test", phase, status, lat, sf, st, dep, err, err, "hash", json.dumps({"test": True}), now_ts + i * .01))
        pred = _124_build(txid); pid = _124_persist(pred)
        ok = (pred["risk_score"] > 0 and pred["risk_level"] in {"medium", "high", "critical"}
              and pred["predicted_failure"] in {"TimeoutError", "DependencyError"}
              and len(pred["recommendations"]) >= 1 and pred["decision_trace"]["automatic_side_effect"] is False)
        return {"status": "passed" if ok else "failed", "version": APP_VERSION, "build": BUILD,
                "prediction_id": pid, "risk_score": pred["risk_score"], "risk_level": pred["risk_level"],
                "predicted_failure": pred["predicted_failure"], "preemptive_recovery": True,
        ...
        with DB_LOCK:
            ...
            try:
                conn.execute("DELETE FROM transaction_observability_events WHERE transaction_id=?", (txid,))
                conn.execute("DELETE FROM transaction_failure_predictions WHERE transaction_id=?", (txid,))
                conn.execute("DELETE FROM transaction_predictive_actions WHERE transaction_id=?", (txid,))
                conn.execute("DELETE FROM transaction_prediction_outcomes WHERE transaction_id=?", (txid,))
                conn.execute("DELETE FROM transaction_predictive_history WHERE transaction_id=?", (txid,))
                conn.commit()
            finally:
                conn.close()
...
# ============================================================
# TARGET-2050.125 — AUTONOMOUS RECOVERY PLANNER
# ============================================================

RECOVERY_PLAN_VERSION = "125.1"


def _ensure_125_tables() -> None:
    with DB_LOCK:
        conn = db()
        try:
            conn.executescript("""
            CREATE TABLE IF NOT EXISTS transaction_recovery_plans(
            ...
            CREATE TABLE IF NOT EXISTS transaction_recovery_plan_steps(
            ...
            CREATE TABLE IF NOT EXISTS transaction_recovery_plan_events(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                plan_id TEXT NOT NULL,
                transaction_id TEXT NOT NULL,
                event_type TEXT NOT NULL,
            ...
            CREATE INDEX IF NOT EXISTS idx_125_plan_tx ON transaction_recovery_plans(transaction_id,created_at);
            CREATE INDEX IF NOT EXISTS idx_125_step_plan ON transaction_recovery_plan_steps(plan_id,step_index);
            CREATE INDEX IF NOT EXISTS idx_125_event_plan ON transaction_recovery_plan_events(plan_id,created_at);
            """)
            conn.commit()
        finally:
            conn.close()
...
def _125_hash(value: Any) -> str:
    return hashlib.sha256(json.dumps(value, sort_keys=True, default=str).encode()).hexdigest()


def _125_safe(value: Any) -> Any:
    if isinstance(value, dict):
        return {str(k): _125_safe(v) for k, v in value.items()}
    if isinstance(value, list):
        return [_125_safe(v) for v in value]
...
def _125_make_plan(transaction_id: str) -> Dict[str, Any]:
    pred = _124_build(transaction_id)
    risk = float(pred["risk_score"])
    level = pred["risk_level"]
    predicted = pred.get("predicted_failure")
    recs = pred.get("recommendations") or []

    steps: List[Dict[str, Any]] = []
    # Step 1 is always a non-side-effecting observation/preflight action.
    steps.append({
        "action_type": "preflight_recheck",
        "safety_class": "safe_observation",
        "description": "Re-check transaction state, prediction freshness, and dependency health before consequential recovery.",
        "dependencies": [],
    ...
    if risk >= 0.25:
        steps.append({
            "action_type": "refresh_dependency_state",
            "safety_class": "safe_observation",
            "description": "Refresh known dependency/connector state without invoking an external side effect.",
            "dependencies": [0],
            "approval_required": False,
        })
    ...
    if risk >= 0.50:
        steps.append({
            ...
            "safety_class": "internal_planning",
            "description": "Prepare a bounded recovery branch using the observed failure and causal evidence.",
            "dependencies": list(range(len(steps))),
            "approval_required": False,
        })
        steps.append({
            "action_type": "await_external_approval",
            "safety_class": "approval_gate",
            "description": "Hold any consequential external recovery action behind explicit approval.",
            "dependencies": list(range(len(steps))),
            "approval_required": True,
        })
    else:
        steps.append({
            "action_type": "continue_monitored_execution",
            "safety_class": "safe_observation",
            "description": "Continue only with bounded monitoring; do not create an external side effect automatically.",
            "dependencies": list(range(len(steps))),
            "approval_required": False,
        })

    if predicted:
        failure_note = str(predicted)
    else:
        failure_note = "no_specific_failure_class"

    # Simulation is intentionally deterministic and never invokes connectors.
    simulated_risk = round(max(0.0, risk - (0.15 if risk >= 0.50 else 0.08)), 4)
    external_steps = [s for s in steps if s["approval_required"]]
    plan_material = {
        "transaction_id": transaction_id,
        "prediction_fingerprint": pred.get("fingerprint"),
        "risk_score": risk,
        "risk_level": level,
        "predicted_failure": failure_note,
        "steps": steps,
        "version": RECOVERY_PLAN_VERSION,
    }
    plan_hash = _125_hash(plan_material)
    plan_id = "recovery-plan-125-" + plan_hash[:24]
    now_ts = time.time()
    trace = {
        "method": "deterministic_autonomous_recovery_planner",
        "prediction_fingerprint": pred.get("fingerprint"),
        "predicted_failure": failure_note,
        "risk_score": risk,
        "risk_level": level,
        "step_count": len(steps),
        "approval_gated_steps": len(external_steps),
        "safe_preemptive_steps": sum(1 for s in steps if s["safety_class"] == "safe_observation"),
    ...
    simulation = {
        ...
        "approval_required_before_consequential_action": True,
        "predicted_post_plan_risk": simulated_risk,
        "failure_class": failure_note,
        "safe_to_preemptively_execute": True,
    }
    return {
        "plan_id": plan_id,
        "transaction_id": transaction_id,
...
def _125_persist_plan(plan: Dict[str, Any]) -> str:
    now_ts = time.time()
    with DB_LOCK:
        conn = db()
        try:
            conn.execute("""INSERT OR REPLACE INTO transaction_recovery_plans
                (plan_id,transaction_id,prediction_id,status,risk_score,risk_level,plan_hash,simulation_json,decision_trace_json,created_at,updated_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?)""", (
                plan["plan_id"], plan["transaction_id"], None, "planned", plan["risk_score"], plan["risk_level"],
                plan["plan_hash"], json.dumps(_125_safe(plan["simulation"])), json.dumps(_125_safe(plan["decision_trace"])),
                plan["created_at"], now_ts))
            conn.execute("DELETE FROM transaction_recovery_plan_steps WHERE plan_id=?", (plan["plan_id"],))
            for i, step in enumerate(plan["steps"]):
                sid = plan["plan_id"] + "-step-" + str(i + 1)
                conn.execute("""INSERT INTO transaction_recovery_plan_steps
                    (step_id,plan_id,step_index,action_type,safety_class,description,dependency_json,approval_required,status,result_json,created_at,updated_at)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""", (
                    sid, plan["plan_id"], i, step["action_type"], step["safety_class"], step["description"],
                    json.dumps(step["dependencies"]), 1 if step["approval_required"] else 0, "planned", None, plan["created_at"], now_ts))
            conn.execute("INSERT INTO transaction_recovery_plan_events(plan_id,transaction_id,event_type,detail_json,created_at) VALUES(?,?,?,?,?)", (
                plan["plan_id"], plan["transaction_id"], "plan_created", json.dumps(_125_safe(plan["decision_trace"])), now_ts))
            conn.commit()
        finally:
            conn.close()
    return plan["plan_id"]
...
def _125_plan_view(plan_id: str) -> Optional[Dict[str, Any]]:
    row = q("SELECT * FROM transaction_recovery_plans WHERE plan_id=?", (plan_id,), one=True)
    if not row:
        return None
    steps = q("SELECT * FROM transaction_recovery_plan_steps WHERE plan_id=? ORDER BY step_index", (plan_id,))
    events = q("SELECT id,event_type,step_id,detail_json,created_at FROM transaction_recovery_plan_events WHERE plan_id=? ORDER BY id", (plan_id,))
    out = dict(row)
    out["simulation"] = json.loads(out.get("simulation_json") or "{}")
    out["decision_trace"] = json.loads(out.get("decision_trace_json") or "{}")
    out.pop("simulation_json", None); out.pop("decision_trace_json", None)
    out["steps"] = []
    for r in steps:
        x = dict(r); x["dependencies"] = json.loads(x.pop("dependency_json") or "[]"); x["result"] = json.loads(x.pop("result_json") or "null")
        out["steps"].append(x)
    out["events"] = []
    for r in events:
        x = dict(r); x["detail"] = json.loads(x.pop("detail_json") or "{}"); out["events"].append(x)
    return out


def _125_execute_safe(plan_id: str) -> Dict[str, Any]:
    plan = _125_plan_view(plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="recovery plan not found")
    if plan["status"] in {"cancelled", "completed"}:
        return {"status": plan["status"], "plan_id": plan_id, "executed": 0, "external_side_effects": 0}
    executed = 0
    now_ts = time.time()
    for step in plan["steps"]:
        if step["approval_required"] or step["safety_class"] != "safe_observation":
            continue
        if step["status"] == "completed":
            continue
        result = {"status": "completed", "action_type": step["action_type"], "side_effect": False, "connector_invocations": 0,
                  "note": "safe preemptive planning/observation step; no external action invoked"}
        write("UPDATE transaction_recovery_plan_steps SET status='completed',result_json=?,updated_at=? WHERE step_id=? AND status='planned'",
              (json.dumps(result), now_ts, step["step_id"]))
        write("INSERT INTO transaction_recovery_plan_events(plan_id,transaction_id,event_type,step_id,detail_json,created_at) VALUES(?,?,?,?,?,?)",
              (plan_id, plan["transaction_id"], "safe_step_completed", step["step_id"], json.dumps(result), now_ts))
        executed += 1
    write("UPDATE transaction_recovery_plans SET status='safe_steps_completed',updated_at=? WHERE plan_id=? AND status='planned'", (now_ts, plan_id))
    return {"status": "safe_steps_completed", "plan_id": plan_id, "executed": executed, "external_side_effects": 0,
            "automatic_side_effect": False, "automatic_side_effect_retry": False, "approval_required_for_consequential_steps": True}


@app.post("/transaction-recovery-planner")
def create_transaction_recovery_plan(transaction_id: str):
    plan = _125_make_plan(transaction_id)
    _125_persist_plan(plan)
    return {**plan, "status": "planned", "version": APP_VERSION, "build": BUILD}


@app.get("/transaction-recovery-planner")
def list_transaction_recovery_plans(transaction_id: Optional[str] = None, limit: int = 20):
    limit = max(1, min(100, limit))
    if transaction_id:
        rows = q("SELECT plan_id,transaction_id,status,risk_score,risk_level,plan_hash,created_at,updated_at FROM transaction_recovery_plans WHERE transaction_id=? ORDER BY created_at DESC LIMIT ?", (transaction_id, limit))
    else:
        rows = q("SELECT plan_id,transaction_id,status,risk_score,risk_level,plan_hash,created_at,updated_at FROM transaction_recovery_plans ORDER BY created_at DESC LIMIT ?", (limit,))
    return {"plans": [dict(r) for r in rows], "count": len(rows), "planner_version": RECOVERY_PLAN_VERSION}


@app.get("/transaction-recovery-planner/{plan_id}")
def get_transaction_recovery_plan(plan_id: str):
    plan = _125_plan_view(plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="recovery plan not found")
    return plan
...
@app.get("/transaction-recovery-planner/{plan_id}/simulation")
def simulate_transaction_recovery_plan(plan_id: str):
    plan = _125_plan_view(plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="recovery plan not found")
    return {"plan_id": plan_id, "simulation": plan["simulation"], "external_execution": False, "side_effects": False}


@app.post("/transaction-recovery-planner/{plan_id}/execute-safe")
def execute_safe_transaction_recovery(plan_id: str):
    return _125_execute_safe(plan_id)


@app.get("/transaction-recovery-planner/{plan_id}/events")
def transaction_recovery_plan_events(plan_id: str):
    plan = _125_plan_view(plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="recovery plan not found")
...
@app.get("/test-transaction-recovery-planner")
def test_transaction_recovery_planner():
    txid = "test-125-" + uuid.uuid4().hex[:12]
    now_ts = time.time()
    try:
        rows = [
            ("p1", "gateway", "success", 70, "running", "completed", None, None),
            ("p2", "payments", "timeout", 1200, "running", "failed", "TimeoutError", None),
            ("p3", "orders", "failed", 900, "running", "failed", "DependencyError", "payments"),
        ]
        for i, (eid, svc, status, lat, sf, st, err, dep) in enumerate(rows):
            write("""INSERT INTO transaction_observability_events
                (event_id,transaction_id,service,connector_id,event_type,phase,status,latency_ms,state_from,state_to,dependency_id,error_class,error_message,payload_hash,metadata_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (eid, txid, svc, None, "planner-test", "execution", status, lat, sf, st, dep, err, err, "hash", json.dumps({"test": True}), now_ts + i*.01))
        plan = _125_make_plan(txid); _125_persist_plan(plan)
        safe = _125_execute_safe(plan["plan_id"])
        ok = (plan["risk_level"] in {"medium", "high", "critical"}
              and len(plan["steps"]) >= 2
              and plan["simulation"]["connector_invocations"] == 0
              and safe["external_side_effects"] == 0
              and plan["decision_trace"]["automatic_external_side_effect"] is False)
        return {"status": "passed" if ok else "failed", "version": APP_VERSION, "build": BUILD,
                "plan_id": plan["plan_id"], "risk_score": plan["risk_score"], "risk_level": plan["risk_level"],
                "steps": len(plan["steps"]), "safe_steps_executed": safe["executed"],
                "simulation": plan["simulation"], "external_execution": False,
                "automatic_side_effect": False, "automatic_side_effect_retry": False,
        ...
        with DB_LOCK:
            conn = db()
            try:
                # Cleanup only synthetic test records.
                conn.execute("DELETE FROM transaction_observability_events WHERE transaction_id=?", (txid,))
                conn.execute("DELETE FROM transaction_recovery_plan_steps WHERE plan_id LIKE ?", ("recovery-plan-125-%",))
                conn.execute("DELETE FROM transaction_recovery_plan_events WHERE transaction_id=?", (txid,))
                conn.execute("DELETE FROM transaction_recovery_plans WHERE transaction_id=?", (txid,))
                conn.commit()
            finally:
                conn.close()
...
# ============================================================
# TARGET-2050.126 — RECOVERY OUTCOME LEARNING / ADAPTIVE OPTIMIZATION
# ============================================================

RECOVERY_LEARNING_VERSION = "126.1"
RECOVERY_POLICY_BASELINE = 1
RECOVERY_POLICY_MIN = 1
RECOVERY_POLICY_MAX = 8


def _ensure_126_tables() -> None:
    with DB_LOCK:
        conn = db()
        try:
            conn.executescript("""
            CREATE TABLE IF NOT EXISTS transaction_recovery_outcomes(
                ...
                consequential_steps INTEGER NOT NULL DEFAULT 0,
            ...
            CREATE TABLE IF NOT EXISTS recovery_learning_policy(
            ...
            CREATE TABLE IF NOT EXISTS recovery_learning_events(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                event_type TEXT NOT NULL,
                plan_id TEXT,
                transaction_id TEXT,
            ...
            CREATE INDEX IF NOT EXISTS idx_126_outcome_plan ON transaction_recovery_outcomes(plan_id,created_at);
            CREATE INDEX IF NOT EXISTS idx_126_outcome_failure ON transaction_recovery_outcomes(failure_class,created_at);
            CREATE INDEX IF NOT EXISTS idx_126_policy_failure ON recovery_learning_policy(failure_class,policy_version);
            CREATE INDEX IF NOT EXISTS idx_126_learning_event ON recovery_learning_events(event_type,created_at);
            """)
            conn.commit()
        finally:
            conn.close()
...
def _126_clamp(v: float, lo: float, hi: float) -> float:
    return round(max(lo, min(hi, float(v))), 4)


def _126_safe(value: Any) -> Any:
    if isinstance(value, dict):
        return {str(k): _126_safe(v) for k, v in value.items()}
    if isinstance(value, list):
        return [_126_safe(v) for v in value]
    if isinstance(value, tuple):
        return [_126_safe(v) for v in value]
    if isinstance(value, str) and len(value) > 512:
        return value[:512] + "…"
...
def _126_policy_rows(failure_class: str) -> List[sqlite3.Row]:
    return q("SELECT * FROM recovery_learning_policy WHERE failure_class=? ORDER BY weight DESC, strategy", (failure_class,))


def _126_latest_policy_version() -> int:
    row = q("SELECT MAX(policy_version) AS v FROM recovery_learning_policy", one=True)
    return max(RECOVERY_POLICY_BASELINE, int(row["v"] or RECOVERY_POLICY_BASELINE)) if row else RECOVERY_POLICY_BASELINE


def _126_strategy_catalog(failure_class: str) -> List[str]:
    # Bounded, deterministic strategy vocabulary. It never creates or executes
    # arbitrary actions; it only influences future internal recovery planning.
    fc = (failure_class or "unknown").lower()
    if "timeout" in fc or "latency" in fc:
        return ["refresh_dependency_state", "bounded_replan", "continue_monitored_execution"]
    if "dependency" in fc or "connector" in fc:
        return ["refresh_dependency_state", "select_healthy_connector", "bounded_replan"]
    if "reconciliation" in fc or "state" in fc:
        return ["preflight_recheck", "reconcile_state", "bounded_replan"]
    return ["preflight_recheck", "refresh_dependency_state", "bounded_replan"]


def _126_extract_plan_context(plan_id: str) -> Dict[str, Any]:
    plan = _125_plan_view(plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="recovery plan not found")
    failure_class = str(plan.get("predicted_failure") or "unknown")
    return {
        "plan": plan,
        "failure_class": failure_class,
        "prediction_fingerprint": plan.get("prediction_fingerprint"),
        "predicted_risk": float(plan.get("risk_score") or 0.0),
    }


def _126_record_outcome(
    plan_id: str,
    actual_status: str,
    recovery_status: str,
    ...
    latency_ms: Optional[float] = None,
    safe_steps: Optional[int] = None,
    consequential_steps: Optional[int] = None,
    detail: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    ctx = _126_extract_plan_context(plan_id)
    plan = ctx["plan"]
    steps = plan.get("steps") or []
    safe_count = int(safe_steps if safe_steps is not None else sum(1 for s in steps if s.get("safety_class") == "safe_observation" and s.get("status") == "completed"))
    consequential_count = int(consequential_steps if consequential_steps is not None else sum(1 for s in steps if s.get("approval_required")))
    outcome_id = "recovery-outcome-126-" + uuid.uuid4().hex[:20]
    payload = {
        "plan_id": plan_id,
        "transaction_id": plan["transaction_id"],
        "prediction_fingerprint": ctx["prediction_fingerprint"],
        ...
        "consequential_steps": consequential_count,
    }
    fp = _125_hash(payload)
    now_ts = time.time()
    with DB_LOCK:
        conn = db()
        try:
            conn.execute("""INSERT OR IGNORE INTO transaction_recovery_outcomes
                (outcome_id,plan_id,transaction_id,prediction_fingerprint,failure_class,predicted_risk,actual_status,recovery_status,recovery_success,safe_steps,consequential_steps,verified,residual_risk,latency_ms,outcome_fingerprint,detail_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
                outcome_id, plan_id, plan["transaction_id"], ctx["prediction_fingerprint"], ctx["failure_class"], ctx["predicted_risk"],
                actual_status, recovery_status, 1 if recovery_success else 0, safe_count, consequential_count, 1 if verified else 0,
                residual_risk, latency_ms, fp, json.dumps(_126_safe(detail or {})), now_ts))
            conn.execute("INSERT INTO recovery_learning_events(event_type,plan_id,transaction_id,policy_version,detail_json,created_at) VALUES(?,?,?,?,?,?)", (
                "recovery_outcome_recorded", plan_id, plan["transaction_id"], _126_latest_policy_version(), json.dumps(_126_safe(payload)), now_ts))
            conn.commit()
        finally:
            conn.close()
    return {
        ...
        "transaction_id": plan["transaction_id"],
        "failure_class": ctx["failure_class"],
        "prediction": {"risk": ctx["predicted_risk"], "fingerprint": ctx["prediction_fingerprint"]},
        "actual": {"status": actual_status, "recovery_status": recovery_status, "success": bool(recovery_success), "verified": bool(verified), "residual_risk": residual_risk},
        "safe_steps": safe_count,
        "consequential_steps": consequential_count,
...
def _126_recompute_policy(failure_class: str, dry_run: bool = False) -> Dict[str, Any]:
    fc = failure_class or "unknown"
    rows = q("SELECT recovery_success,verified FROM transaction_recovery_outcomes WHERE failure_class=? ORDER BY created_at", (fc,))
    n = len(rows)
    successes = sum(1 for r in rows if int(r["recovery_success"] or 0) == 1)
    verified_n = sum(1 for r in rows if int(r["verified"] or 0) == 1)
    observed_success = (successes / n) if n else 0.5
    observed_verified = (verified_n / n) if n else 0.0
    # Conservative prior + bounded evidence. Learning cannot make a strategy
    # more than 30% stronger/weaker than baseline in one policy generation.
    confidence = _126_clamp(min(0.95, (n / 10.0) * 0.65 + observed_verified * 0.35), 0.0, 0.95)
    strategies = _126_strategy_catalog(fc)
    ranked = []
    for i, strategy in enumerate(strategies):
        prior = 0.50 + (0.02 * (len(strategies) - i - 1))
        score = _126_clamp(prior + (observed_success - 0.5) * confidence, 0.35, 0.80)
        weight = _126_clamp(1.0 + (score - 0.5) * 0.6, 0.70, 1.30)
        ranked.append({"strategy": strategy, "success_rate": round(score,4), "weight": weight})
    ranked.sort(key=lambda x: (-x["weight"], x["strategy"]))
    next_version = _126_latest_policy_version() + (0 if dry_run else 1)
    source_hash = _125_hash({"failure_class": fc, "n": n, "successes": successes, "verified": verified_n, "ranked": ranked})
    result = {
        "policy_version": next_version,
        "failure_class": fc,
        "sample_count": n,
        "observed_success_rate": round(observed_success,4),
        "verified_rate": round(observed_verified,4),
    ...
    with DB_LOCK:
        ...
        try:
            for item in ranked:
                pid = f"policy-126-{next_version}-{_125_hash((fc,item['strategy']))[:16]}"
                conn.execute("""INSERT OR REPLACE INTO recovery_learning_policy
                    (policy_id,policy_version,failure_class,strategy,weight,sample_count,success_rate,confidence,source_hash,created_at,updated_at)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?)""", (
                    pid, next_version, fc, item["strategy"], item["weight"], n, item["success_rate"], confidence, source_hash, now_ts, now_ts))
            conn.execute("INSERT INTO recovery_learning_events(event_type,plan_id,transaction_id,policy_version,detail_json,created_at) VALUES(?,?,?,?,?,?)", (
                "recovery_policy_adapted", None, None, next_version, json.dumps(_126_safe(result)), now_ts))
            conn.commit()
        finally:
            conn.close()
    return result
...
def _126_optimize_plan(plan_id: str) -> Dict[str, Any]:
    ctx = _126_extract_plan_context(plan_id)
    policy = _126_recompute_policy(ctx["failure_class"], dry_run=False)
    rows = _126_policy_rows(ctx["failure_class"])
    selected = rows[0]["strategy"] if rows else "preflight_recheck"
    plan = ctx["plan"]
    current_actions = [str(s.get("action_type")) for s in plan.get("steps") or []]
    recommendation = {
        "selected_strategy": selected,
        "current_actions": current_actions,
        "reason": "bounded historical outcome learning; consequential actions remain approval-bound",
    ...
    with DB_LOCK:
        ...
        try:
            conn.execute("INSERT INTO recovery_learning_events(event_type,plan_id,transaction_id,policy_version,detail_json,created_at) VALUES(?,?,?,?,?,?)", (
                "recovery_plan_optimized", plan_id, plan["transaction_id"], policy["policy_version"], json.dumps(_126_safe(recommendation)), now_ts))
            conn.commit()
        finally:
            conn.close()
    return {"status": "optimized", "plan_id": plan_id, "optimization": recommendation, "policy": policy}


@app.post("/transaction-recovery-learning/outcome")
def record_recovery_outcome(payload: Dict[str, Any]):
    plan_id = str(payload.get("plan_id") or "").strip()
    if not plan_id:
        raise HTTPException(status_code=400, detail="plan_id is required")
    return _126_record_outcome(
        plan_id=plan_id,
        actual_status=str(payload.get("actual_status") or "unknown"),
        recovery_status=str(payload.get("recovery_status") or "unknown"),
        recovery_success=bool(payload.get("recovery_success", False)),
        verified=bool(payload.get("verified", False)),
        residual_risk=float(payload["residual_risk"]) if payload.get("residual_risk") is not None else None,
        latency_ms=float(payload["latency_ms"]) if payload.get("latency_ms") is not None else None,
        safe_steps=int(payload["safe_steps"]) if payload.get("safe_steps") is not None else None,
        consequential_steps=int(payload["consequential_steps"]) if payload.get("consequential_steps") is not None else None,
        detail=payload.get("detail") if isinstance(payload.get("detail"), dict) else {},
    )


@app.get("/transaction-recovery-learning/outcomes")
def list_recovery_outcomes(plan_id: Optional[str] = None, failure_class: Optional[str] = None, limit: int = 50):
    limit = max(1, min(200, int(limit)))
    if plan_id:
        rows = q("SELECT * FROM transaction_recovery_outcomes WHERE plan_id=? ORDER BY created_at DESC LIMIT ?", (plan_id, limit))
    elif failure_class:
        rows = q("SELECT * FROM transaction_recovery_outcomes WHERE failure_class=? ORDER BY created_at DESC LIMIT ?", (failure_class, limit))
    else:
        rows = q("SELECT * FROM transaction_recovery_outcomes ORDER BY created_at DESC LIMIT ?", (limit,))
    out=[]
    for r in rows:
        x=dict(r); x["detail"]=json.loads(x.pop("detail_json") or "{}"); out.append(x)
    return {"outcomes": out, "count": len(out), "learning_version": RECOVERY_LEARNING_VERSION}


@app.post("/transaction-recovery-learning/optimize/{plan_id}")
def optimize_recovery_plan(plan_id: str):
    return _126_optimize_plan(plan_id)


@app.get("/transaction-recovery-learning/policy")
def recovery_learning_policy(failure_class: Optional[str] = None, limit: int = 100):
    limit = max(1, min(300, int(limit)))
    if failure_class:
        rows = q("SELECT * FROM recovery_learning_policy WHERE failure_class=? ORDER BY policy_version DESC, weight DESC LIMIT ?", (failure_class, limit))
    else:
        rows = q("SELECT * FROM recovery_learning_policy ORDER BY policy_version DESC, weight DESC LIMIT ?", (limit,))
    return {"policies": [dict(r) for r in rows], "count": len(rows), "current_policy_version": _126_latest_policy_version(), "learning_version": RECOVERY_LEARNING_VERSION}


@app.get("/transaction-recovery-learning/history")
def recovery_learning_history(limit: int = 100):
    limit=max(1,min(300,int(limit)))
    rows=q("SELECT id,event_type,plan_id,transaction_id,policy_version,detail_json,created_at FROM recovery_learning_events ORDER BY id DESC LIMIT ?",(limit,))
    out=[]
    for r in rows:
        x=dict(r); x["detail"]=json.loads(x.pop("detail_json") or "{}"); out.append(x)
    return {"events":out,"count":len(out),"current_policy_version":_126_latest_policy_version()}


@app.get("/transaction-recovery-learning/{plan_id}")
def recovery_learning_for_plan(plan_id: str):
    ctx=_126_extract_plan_context(plan_id)
    outcomes=list_recovery_outcomes(plan_id=plan_id,limit=50)["outcomes"]
    policies=recovery_learning_policy(failure_class=ctx["failure_class"],limit=50)["policies"]
    return {"plan_id":plan_id,"failure_class":ctx["failure_class"],"prediction":{"risk":ctx["predicted_risk"],"fingerprint":ctx["prediction_fingerprint"]},"outcomes":outcomes,"policies":policies,"policy_version":_126_latest_policy_version()}


@app.get("/test-transaction-recovery-learning")
def test_transaction_recovery_learning():
    txid="test-126-"+uuid.uuid4().hex[:12]
    now_ts=time.time()
    try:
        rows=[
            ("p1","gateway","success",70,"running","completed",None,None),
        ...
        for i,(eid,svc,status,lat,sf,st,err,dep) in enumerate(rows):
            write("""INSERT INTO transaction_observability_events
                (event_id,transaction_id,service,connector_id,event_type,phase,status,latency_ms,state_from,state_to,dependency_id,error_class,error_message,payload_hash,metadata_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(eid,txid,svc,None,"learning-test","execution",status,lat,sf,st,dep,err,err,"hash",json.dumps({"test":True}),now_ts+i*.01))
        plan=_125_make_plan(txid); _125_persist_plan(plan); _125_execute_safe(plan["plan_id"])
        out1=_126_record_outcome(plan["plan_id"],"failed","preemptive_safe_steps",False,False,0.52,120.0,2,1,{"test":"first"})
        out2=_126_record_outcome(plan["plan_id"],"recovered","preemptive_safe_steps",True,True,0.31,100.0,2,1,{"test":"second"})
        opt=_126_optimize_plan(plan["plan_id"])
        ok=(out1["outcome_id"] and out2["outcome_id"] and opt["status"]=="optimized" and opt["policy"]["sample_count"]==2 and opt["policy"]["confidence"]>=0 and opt["optimization"]["selected_strategy"] and opt["optimization"]["reason"].startswith("bounded"))
        return {"status":"passed" if ok else "failed","version":APP_VERSION,"build":BUILD,"plan_id":plan["plan_id"],"samples":2,"policy_version":opt["policy"]["policy_version"],"observed_success_rate":opt["policy"]["observed_success_rate"],"learning_confidence":opt["policy"]["confidence"],"selected_strategy":opt["optimization"]["selected_strategy"],"prediction_vs_outcome":{"predicted_risk":plan["risk_score"],"observed_residual_risk":0.31,"improved":0.31 < plan["risk_score"]},"external_execution":False,"automatic_side_effect":False,"automatic_side_effect_retry":False,"approval_required_for_consequential_steps":True}
    finally:
        with DB_LOCK:
            conn=db()
            try:
                conn.execute("DELETE FROM transaction_observability_events WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM transaction_recovery_outcomes WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM recovery_learning_events WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM recovery_learning_policy WHERE policy_id LIKE 'policy-126-%'")
                conn.execute("DELETE FROM transaction_recovery_plan_steps WHERE plan_id LIKE 'recovery-plan-125-%'")
                conn.execute("DELETE FROM transaction_recovery_plan_events WHERE transaction_id=?",(txid,))
                conn.execute("DELETE FROM transaction_recovery_plans WHERE transaction_id=?",(txid,))
                conn.commit()
