...
def _f2160_selftest():
    ...
    tests=[
        ...
        {"name":"safety invariants","passed":(
            _f2160_security()["checks"].get("arbitrary_code_execution") is False and
            _f2160_security()["checks"].get("uncertain_external_replay") is False and
            _f2160_security()["checks"].get("secret_values_exposed") is False and
            all(_f2160_security()["checks"].get(k) is True for k in (
                "user_scoped_data","ssrf_boundary",
                "side_effect_authorization","idempotency_boundary",
                "dry_run_boundary","truthful_readiness"))
        )},
...
# ============================================================
# TARGET-2050.2160 — FINAL MERGED RELEASE INTEGRATION
# ============================================================
# 2110 remains the preserved production kernel; 2160 is the closure/control
# layer mounted on the same FastAPI application and SQLite data directory.
# Existing 2110 routes are intentionally retained for compatibility.
try:
    INFINITY_VERSION_2110 = FINAL2160_VERSION
    INFINITY_BUILD_2110 = FINAL2160_BUILD
    INFINITY_PREVIOUS_2110 = FINAL2160_PREVIOUS
    APP_VERSION = FINAL2160_VERSION
    BUILD = FINAL2160_BUILD
    app.version = FINAL2160_VERSION
except Exception:
    pass
...
def _final_2160_status_alias():
    return _f2160_status()

@app.get('/infinity/final/health')
def _final_2160_health_alias():
    s = _f2160_status()
    return {'status':'healthy','version':FINAL2160_VERSION,'build':FINAL2160_BUILD,
            'kernel_2110_preserved':True,'closure_2160':True,
            'database':True,'interface':True,'truthful':True,
            'capabilities':s['capabilities'],'model':s['model']}
