...
APP_VERSION = "TARGET-2050.151"
BUILD = "REAL-EXECUTION-LEARNING-REUSE-CORE"
...
# ============================================================
# TARGET-2050.151 — REAL EXECUTION LEARNING / REUSE CORE
# ============================================================
#
# Design guarantees:
# - Learning is written only from verified execution outcomes.
# - A pattern can influence strategy only after a persisted match.
# - Verification is mandatory for learning/replay eligibility.
# - External replay is restricted to public_http_get.
# - No credentials are persisted or replayed.
# - Policy mutation is bounded and evidence-backed.
# - 2050.150-era action/approval/idempotency behavior is preserved.
#

LEARNING151_MIN_MATCH = 0.72
LEARNING151_POLICY_SUCCESS_THRESHOLD = 3
LEARNING151_MAX_REPLAY_AGE = 7 * 24 * 3600
LEARNING151_MAX_PATTERNS = 500

def _init_151_tables():
    with DB_LOCK:
        conn = db()
        try:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS execution_learning_151(
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    mission_id TEXT,
                    objective TEXT NOT NULL,
            ...
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS execution_learning_patterns_151(
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS execution_policy_151(
                    id INTEGER PRIMARY KEY CHECK(id=1),
                    version INTEGER NOT NULL,
            ...
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS execution_replay_151(
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    pattern_id INTEGER,
            ...
            """)
            conn.execute("""
                INSERT OR IGNORE INTO execution_policy_151(
                    id,version,verified_successes,mutation_count,data_json,updated_at
                ) VALUES(1,1,0,0,?,?)
            """, (json.dumps({
                "retry_multiplier": 1,
                "verification_required": True,
                "external_replay": "public_http_get_only",
...
def _151_tokens(value: str) -> set:
    return {
        token for token in re.findall(r"[a-z0-9]{3,}", (value or "").lower())
        if token not in {
            "the", "and", "for", "with", "from", "that", "this",
            "into", "using", "real", "world", "task", "mission"
        }
    }


def _151_objective_key(objective: str) -> str:
    tokens = sorted(_151_tokens(objective))
    return " ".join(tokens[:80])


def _151_similarity(a: str, b: str) -> float:
    aa, bb = _151_tokens(a), _151_tokens(b)
    if not aa or not bb:
        return 0.0
    return len(aa & bb) / max(1, len(aa | bb))


def _151_target_class(action_type: str, target: str) -> str:
    if action_type == "public_http_get":
        try:
            host = (urlparse(target).hostname or "").lower()
            return "public_http_get:" + host
        except Exception:
            return "public_http_get:invalid"
    return action_type


def _151_verify_execution(action_type: str, result: dict) -> dict:
    """
    Execution verification is deliberately conservative.
    Learning never treats an unverified/uncertain outcome as success.
    """
    if not isinstance(result, dict):
        return {"verified": False, "success": False, "reason": "non_object_result"}

    if result.get("status") in {"failed", "failed_closed", "rejected"}:
        return {"verified": False, "success": False, "reason": result.get("status")}
    ...
    if action_type == "save_result":
        verified = bool(payload.get("saved") is True)
        return {
            "verified": verified,
            "success": verified,
            "reason": "persisted_result_confirmed" if verified else "save_not_confirmed",
        }

    if action_type == "public_http_get":
        code = payload.get("status_code")
        verified = bool(payload.get("ok") is True and isinstance(code, int) and 200 <= code < 300)
        return {
            "verified": verified,
            "success": verified,
            "reason": "http_2xx_confirmed" if verified else "http_result_not_verified",
        }
    ...
    return {
        "verified": verified,
        "success": verified,
        "reason": "explicit_outcome_confirmed" if verified else "outcome_not_confirmed",
    }


def _151_policy() -> dict:
    row = q("SELECT * FROM execution_policy_151 WHERE id=1", one=True)
    if not row:
        return {
            "version": 1,
...
def _151_find_pattern(objective: str, action_type: str, target: str):
    rows = q("""
        SELECT * FROM execution_learning_patterns_151
        WHERE action_type=? AND target_class=?
        ORDER BY confidence DESC, successes DESC, updated_at DESC
        LIMIT 50
    """, (action_type, _151_target_class(action_type, target)))
    best = None
    best_score = 0.0
    for row in rows:
        score = _151_similarity(objective, row["objective_key"])
        if score > best_score:
            best_score = score
            best = row
...
def _151_strategy_for(objective: str, action_type: str, target: str) -> dict:
    pattern, similarity = _151_find_pattern(objective, action_type, target)
    policy = _151_policy()
    if pattern is None or similarity < LEARNING151_MIN_MATCH:
        return {
            "pattern_reused": False,
            "strategy_changed": False,
            "similarity": round(similarity, 4),
            "strategy": {
                "retry_limit": ACTION_MAX_ATTEMPTS,
                "verification_required": True,
            },
            "pattern_id": None,
    ...
    stored = json.loads(pattern["strategy_json"] or "{}")
    strategy = {
        "retry_limit": int(stored.get("retry_limit", ACTION_MAX_ATTEMPTS)),
        "verification_required": True,
...
def _151_record_outcome(
    ...
    verification = _151_verify_execution(action_type, execution_result)
    objective_key = _151_objective_key(objective)
    success = bool(verification["verified"] and verification["success"])

    write("""
        INSERT INTO execution_learning_151(
            mission_id,objective,objective_key,action_type,target,
            outcome,verified,success,strategy_json,evidence_json,created_at
        ) VALUES(?,?,?,?,?,?,?,?,?,?,?)
    """, (
        mission_id, objective, objective_key, action_type, target,
    ...
    if success:
        existing, similarity = _151_find_pattern(objective, action_type, target)
        now_ts = time.time()
        target_class = _151_target_class(action_type, target)

        if existing and similarity >= LEARNING151_MIN_MATCH:
            uses = int(existing["uses"]) + 1
            successes = int(existing["successes"]) + 1
            failures = int(existing["failures"])
            confidence = successes / max(1, successes + failures)
            strategy_json = json.dumps({
                "retry_limit": max(1, int(strategy.get("strategy", {}).get("retry_limit", ACTION_MAX_ATTEMPTS))),
                "verification_required": True,
            })
            write("""
                UPDATE execution_learning_patterns_151
                SET uses=?,successes=?,failures=?,confidence=?,
                    strategy_json=?,last_used_at=?,updated_at=?
                WHERE id=?
            """, (
                uses, successes, failures, confidence,
                strategy_json, now_ts, now_ts, existing["id"]
            ))
            pattern = q("SELECT * FROM execution_learning_patterns_151 WHERE id=?", (existing["id"],), one=True)
        else:
            pattern_key = hashlib.sha256(
                f"{objective_key}|{action_type}|{target_class}".encode()
            ).hexdigest()
            try:
                write("""
                    INSERT INTO execution_learning_patterns_151(
                        pattern_key,objective_key,action_type,target_class,
                        uses,successes,failures,confidence,strategy_json,
            ...
            pattern = q("""
                SELECT * FROM execution_learning_patterns_151
                WHERE pattern_key=?
            """, (pattern_key,), one=True)

    # Bounded, evidence-backed policy mutation:
    # only after three independently recorded verified successes.
    if success:
        policy = _151_policy()
        total = policy["verified_successes"] + 1
        if total >= LEARNING151_POLICY_SUCCESS_THRESHOLD and policy["data"].get("retry_multiplier") == 1:
            new_data = dict(policy["data"])
            new_data["retry_multiplier"] = 2
            new_data["mutation_reason"] = "three_verified_execution_successes"
            write("""
                UPDATE execution_policy_151
                SET version=?,verified_successes=?,mutation_count=?,
            ...
            write("""
                UPDATE execution_policy_151
                SET verified_successes=?,updated_at=?
                WHERE id=1
            """, (total, time.time()))
    ...
    return {
        ...
        "pattern_reused": bool(strategy.get("pattern_reused")),
        "strategy_changed": bool(strategy.get("strategy_changed")),
        "policy": _151_policy(),
    }


def _151_replay_allowed(action_type: str, target: str) -> tuple:
    if action_type != "public_http_get":
        return False, "only_public_http_get_is_replayable"
    try:
        validate_url(target)
    except Exception as exc:
        return False, str(exc)
    if ACTION_HOST_ALLOWLIST and (urlparse(target).hostname or "").lower() not in ACTION_HOST_ALLOWLIST:
        return False, "target_not_in_action_host_allowlist"
    return True, "allowed"


class ExecutionLearning151Request(BaseModel):
    objective: str
    action_type: str = "save_result"
    target: str = "self-test"
    payload: Dict[str, Any] = {}
    mission_id: Optional[str] = None
    require_approval: bool = False


@app.get("/execution-learning-151")
def execution_learning_151_status():
    records = q("SELECT COUNT(*) AS n FROM execution_learning_151", one=True)["n"]
    patterns = q("SELECT COUNT(*) AS n FROM execution_learning_patterns_151", one=True)["n"]
    successes = q("SELECT COUNT(*) AS n FROM execution_learning_151 WHERE success=1", one=True)["n"]
    failures = q("SELECT COUNT(*) AS n FROM execution_learning_151 WHERE success=0", one=True)["n"]
    policy = _151_policy()
    return {
        "status": "ready",
        "version": APP_VERSION,
        ...
        "failures": failures,
        "learning_loop": True,
        "actual_execution_outcomes": True,
        "advisory_reuse": True,
        "automatic_policy_mutation": policy["mutation_count"] > 0,
        "automatic_external_replay": False,
        "safe_external_replay": True,
        "credentials_persisted": False,
        "verification_required": True,
        "policy": policy,
    }
...
@app.get("/execution-learning-151/patterns")
def execution_learning_151_patterns():
    rows = q("""
        SELECT id,pattern_key,objective_key,action_type,target_class,
               uses,successes,failures,confidence,strategy_json,
               last_used_at,created_at,updated_at
        FROM execution_learning_patterns_151
        ORDER BY updated_at DESC
        LIMIT ?
    """, (LEARNING151_MAX_PATTERNS,))
    return {
        "status": "ready",
        "version": APP_VERSION,
        "patterns": [
            {**dict(row), "strategy": json.loads(row["strategy_json"] or "{}")}
            for row in rows
        ],
    }


@app.get("/execution-learning-151/policy")
def execution_learning_151_policy():
    return {"status": "ready", "version": APP_VERSION, "policy": _151_policy()}


@app.get("/execution-learning-151/replays")
def execution_learning_151_replays():
    rows = q("""
        SELECT * FROM execution_replay_151
        ORDER BY created_at DESC
        LIMIT 100
    """)
    return {
...
@app.post("/execution-learning-151/run")
def execution_learning_151_run(request: ExecutionLearning151Request):
    if not request.objective.strip():
        raise HTTPException(status_code=400, detail="objective is required")

    if request.action_type not in SAFE_ACTION_ALLOWLIST:
        raise HTTPException(status_code=403, detail="action_type is not registered")

    mission_id = request.mission_id or f"learning151-{uuid.uuid4().hex[:16]}"
    strategy = _151_strategy_for(
        request.objective,
        request.action_type,
        request.target,
    )

    action_request = SafeActionRequest(
    ...
    execution = execute_safe_gateway(action_request)

    learning = _151_record_outcome(
        mission_id,
        request.objective,
        request.action_type,
        request.target,
    ...
    return {
        "status": "completed" if learning["verified"] else execution.get("status", "failed"),
        "version": APP_VERSION,
        "build": BUILD,
        "mission_id": mission_id,
        "execution": execution,
        ...
        "pattern_created": learning["pattern_created"],
        "pattern_reused": learning["pattern_reused"],
...
@app.post("/execution-learning-151/replay/{pattern_id}")
def execution_learning_151_replay(pattern_id: int, mission_id: Optional[str] = None):
    row = q("""
        SELECT * FROM execution_learning_patterns_151 WHERE id=?
    """, (pattern_id,), one=True)
    if not row:
        raise HTTPException(status_code=404, detail="pattern not found")

    allowed, reason = _151_replay_allowed(row["action_type"], row["target_class"].split(":", 1)[-1])
    if not allowed:
        raise HTTPException(status_code=403, detail=reason)

    target = row["target_class"].split(":", 1)[-1]
    replay_id = f"replay151-{uuid.uuid4().hex}"
    result = execute_safe_gateway(SafeActionRequest(
        action_type="public_http_get",
        target=target,
        payload={},
        mission_id=mission_id or replay_id,
        require_approval=False,
    ))
    verification = _151_verify_execution("public_http_get", result)
    write("""
        INSERT INTO execution_replay_151(
            pattern_id,source_mission_id,replay_id,action_type,target,
            status,verified,result_json,created_at
        ) VALUES(?,?,?,?,?,?,?,?,?)
    """, (
        pattern_id,
        mission_id,
...
@app.get("/execution-learning-151/status")
def execution_learning_151_status_alias():
    return execution_learning_151_status()


@app.get("/execution-learning-151/self-test")
def execution_learning_151_self_test():
    # Isolated deterministic self-test. It uses save_result, never an external side effect.
    suffix = uuid.uuid4().hex[:10]
    objective = f"151 learning self test {suffix} verify execution pattern reuse"
    target = "self-test"
    action = "save_result"

    first = execution_learning_151_run(ExecutionLearning151Request(
        objective=objective,
        action_type=action,
        target=target,
        payload={"self_test": suffix, "value": "first"},
    ))
    second = execution_learning_151_run(ExecutionLearning151Request(
        objective=objective,
        action_type=action,
        target=target,
        payload={"self_test": suffix, "value": "second"},
    ))

    checks = {
        "learning_loop": True,
        "execution": first.get("execution", {}).get("status") == "completed",
        "verification": first.get("verification", {}).get("verified") is True,
        "outcome_recorded": first.get("outcome_recorded") is True,
        "pattern_created": first.get("pattern_created") is True,
        "pattern_reused": second.get("pattern_reused") is True,
        "strategy_changed": second.get("strategy_changed") is True,
        "verification_on_reuse": second.get("verification", {}).get("verified") is True,
        "credentials_persisted": first.get("policy", {}).get("data", {}).get("credentials_persisted") is False,
    }
    return {
        ...
        "acceptance": {
            ...
            "outcome_recorded": first.get("outcome_recorded"),
            "pattern_created": first.get("pattern_created"),
            "pattern_reused": second.get("pattern_reused"),
            "strategy_changed": second.get("strategy_changed"),
        },
    }
