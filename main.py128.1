...
@app.post("/test-recovery-meta-learning")
def test_recovery_meta_learning():
    ...
    # Make one synthetic sample old enough to exercise deterministic decay.
    write(
        "UPDATE recovery_experiences SET created_at=?, updated_at=? WHERE id=?",
        (now() - (META_DECAY_DAYS * 86400.0), now() - (META_DECAY_DAYS * 86400.0), ids[1]),
    )
