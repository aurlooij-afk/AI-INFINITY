...
APP_VERSION = "TARGET-2050.87"
BUILD = "CUMULATIVE-ALL-SUCCESSFUL-VERSIONS-PROVIDER-QUORUM-AND-EMPIRICAL-EVIDENCE-ACQUISITION-CORE"
...
USER_AGENT = "AI-Infinity/2050.86-controlled-public-research"
...
def health():
    ...
    return {
        ...
        "research":{
            ...
            "empirical_query_adaptation":True,
            "empirical_evidence_acquisition":True,
            "evidence_graph":True,
...
def evidence_type(source):
    ...
    empirical_signals=(
        ...
        "benchmarking","test set","dataset","participants",
        "case study","field study","deployment study","real-world evaluation",
        "comparative evaluation","performance results","experimental results"
    )
    if any(x in text for x in empirical_signals):
...
def empirical_score(source, objective):
    ...
    measurement=(
        "accuracy","precision","recall","f1","success rate","completion rate",
        "failure rate","error rate","latency","throughput","tasks completed",
        "n=","sample size","participants","results show","we evaluated","we tested"
    )
    hits=sum(1 for w in strong if w in text)
    measured=sum(1 for w in measurement if w in text)
    score=min(1.0, direct*0.30 + min(hits,5)*0.12 + min(measured,4)*0.08)
...
# 2050.86 provider-quorum rules:
# - A single provider (including Crossref/Semantic Scholar) may fail or be rate-limited
#   without aborting the research mission.
# - Crossref and Crossref-alt share one provider family and never count as two families.
# - Recovery re-runs healthy/alternative providers instead of retrying only the failed
#   provider set, so one 429 cannot create a zero-source mission.
# - Rate-limited providers are recorded and skipped during the immediate recovery round;
#   they can be tried again on a later mission.
# - Verification remains truthful: no evidence means no verification.

RETRYABLE_FAILURES={"rate_limited","provider_server_error","transport_or_parse_error","timeout"}
...
def call_provider(name, fn, objective, retry_round=0, skip_retry=False):
    started=time.time()
    errors=[]
    terms=" ".join(objective_terms(objective)[:8]).strip()
    clean=re.sub(r"\s+"," ",objective).strip()
    # Research objectives about reliability need measured evidence, not merely
    # governance/theory papers. Add explicit empirical query variants while
    # preserving the original objective for recall.
    empirical_variant=(clean + " benchmark evaluation experiment performance task completion")
    measured_variant=(terms + " empirical evaluation benchmark success rate experiment")
    variants=list(dict.fromkeys([empirical_variant, measured_variant, clean, terms]))
    variants=[q for q in variants if q]
    # Bound provider load: two targeted variants per attempt. Recovery starts with
    # empirical variants so healthy providers can replace rate-limited ones.
    ordered=variants[:2]
    for i,q in enumerate(ordered):
        max_attempts=1 if retry_round>0 or skip_retry else 2
        for attempt in range(max_attempts):
            try:
                items=deduplicate_sources(fn(q))
                return {"provider":name,"status":"success","sources":len(items),
                        "latency_ms":int((time.time()-started)*1000),"query_variant":i,
                        "query":q,"retry_round":retry_round,"retries":attempt,"items":items}
            except Exception as exc:
    ...
    return {"provider":name,"status":"failed","sources":0,"error":e["error"],
            "failure_kind":e["kind"],"latency_ms":int((time.time()-started)*1000),
            "retries":max(0,len(errors)-1),"retry_round":retry_round,"items":[]}


def research(objective, recovery_round=0, failed_provider_names=None):
    # Round 0: query the complete provider pool in parallel.
    # Round 1: query alternative/healthy providers again. Rate-limited providers
    # are recorded but skipped so they cannot block recovery.
    failed=set(failed_provider_names or [])
    retryable_failed=set(failed_provider_names or [])
    results=[]; threads=[]; lock=threading.Lock()

    def worker(name,fn):
        skip=(recovery_round>0 and name in retryable_failed)
        if skip:
            return
        r=call_provider(name,fn,objective,recovery_round,skip_retry=False)
        with lock:
            results.append(r)
    ...
    # Each bounded fetch is <=6s; allow enough time for the whole quorum without
    # blocking the mission indefinitely.
    ...
    all_items=[]; events=[]
    for name,_ in PROVIDERS:
        if recovery_round>0 and name in retryable_failed:
            events.append({"provider":name,"status":"skipped","sources":0,
                           "error":"rate_limit_recovery_skip" if name in failed else "recovery_skip",
                           "failure_kind":"rate_limited" if name in failed else "recovery_skip",
                           "retry_round":recovery_round,"items":[]})
            continue
        r=by_name.get(name)
        if r is None:
            r={"provider":name,"status":"failed","sources":0,"error":"provider_timeout",
               "failure_kind":"timeout","latency_ms":8000,"items":[],"retry_round":recovery_round}
        all_items.extend(r.get("items",[]))
        events.append({k:v for k,v in r.items() if k!="items"})
    ...
    return {"sources":sources,"relevant_sources":len(sources),"provider_events":events,
            "independent_publishers":len(publisher_ids),"independent_domains":len(publisher_domains),
            "provider_count":len(provider_families),"empirical_or_evaluation_sources":empirical,
...
def execute_mission(mid, request):
    ...
    while attempts<2:
        ...
        try:
            ...
            if verification["theoretical_sources"] > verification["empirical_or_evaluation_sources"] and verification["empirical_or_evaluation_sources"] < 3:
                evidence_gaps.append("evidence_set_is_theory_heavy")
            ...
            # Provider failure is never treated as a mission-level application failure.
            # A failed provider is recorded; the evidence set determines closure.
            if failures:
                provider_note="provider_failures_tolerated; evidence_quorum_controls_closure"
            else:
                provider_note=None
            ...
            if attempts<2:
                recovery+=1
                # Do not retry only the failed providers. Recovery must use the
                # remaining provider pool so Crossref/Semantic Scholar 429s cannot
                # produce a zero-source recovery attempt. Rate-limited providers are
                # skipped for this immediate round and remain visible in telemetry.
                failed_names=[p["provider"] for p in failures if p.get("failure_kind") in RETRYABLE_FAILURES]
                if not failed_names:
...
def version_history():
    return {
        ...
        "preserved_successful_lineage":[
            ...
            "2050.80","2050.81","2050.82","2050.83","2050.84","2050.85","2050.86"
        ],
