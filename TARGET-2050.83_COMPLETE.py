...
APP_VERSION = "TARGET-2050.83"
BUILD = "CUMULATIVE-ALL-SUCCESSFUL-VERSIONS-EMPIRICAL-TRUTH-CLAIM-VERIFICATION-AND-REAL-WORLD-COMMAND-CORE"
...
def contradiction_analysis(sources):
    # Claim-level conservative screen. A contradiction is only reported when
    # strongly overlapping subject terms and explicit opposing findings coexist.
    pos_terms=("improves","effective","successful","reliable","benefit","increase","higher","outperforms","improves performance")
    neg_terms=("fails","failure","unreliable","ineffective","harm","decrease","lower","underperforms","no improvement","worse")
    records=[]
    for s in sources:
        text=(s.get("title","")+" "+s.get("abstract","")).lower()
        pos=[w for w in pos_terms if w in text]
        neg=[w for w in neg_terms if w in text]
        if pos or neg:
            records.append({
                "id":s["id"],"positive":pos[:5],"negative":neg[:5],
                "provider":s.get("provider"),"publisher":s.get("publisher")
            })
    conflicts=[]
    # Compare only records with explicit opposite polarity. This remains a
    # screening stage, not a claim that the underlying studies are semantically
    # contradictory.
    ...
    if conflicts:
        return {
            ...
            "method":"title_abstract_polarity_screen",
            "analysis_scope":"screening_only",
            "conflicts":conflicts[:20]
        }
    return {
        "count":0,
        "status":"no_explicit_polarity_conflict_detected",
        "method":"title_abstract_polarity_screen",
        "analysis_scope":"screening_only",
...
def execute_mission(mid, request):
    attempts=0; recovery=0
    event(mid,"mission_started",{"objective":request.objective})
    while attempts<2:
        attempts+=1; event(mid,"attempt_started",{"attempt":attempts})
        try:
            plan=build_plan(request.objective,request)
            event(mid,"plan_created",{"steps":plan})
            if not request.external_access or not request.research:
                result=empty_result(request.objective,attempts,recovery)
                update_mission(mid,"completed",result); event(mid,"mission_finished",{"status":"completed"}); return
            event(mid,"research_started",{"query_variants":query_variants(request.objective)})
            rr=research(request.objective); sources=rr["sources"]
            for item in sources: provenance_add(mid,item)
            claims=build_claims(sources); checkpoint(mid,"research",{"sources":len(sources)})
            contradiction=contradiction_analysis(sources)
            verification=verify(
                sources,claims,rr["independent_domains"],rr["provider_count"],
                request.verify,rr["independent_publishers"],contradiction,request.objective
            )
            graph=evidence_graph(sources,claims)
            failures=[p for p in rr["provider_events"] if p["status"]=="failed"]
            evidence_gaps=[]
            if verification["empirical_or_evaluation_sources"]<3:
                evidence_gaps.append("insufficient_direct_empirical_or_evaluation_evidence")
            if verification["independent_publishers"]<2:
                evidence_gaps.append("insufficient_publisher_independence")
            if verification["independent_provider_families"]<2:
                evidence_gaps.append("insufficient_provider_independence")
            if verification["contradiction_analysis"].get("semantic_contradiction_proof") is False:
                evidence_gaps.append("contradiction_screen_is_not_semantic_proof")
            if failures:
                evidence_gaps.append("provider_failures_present")
            action=None
            if any(x["id"]=="action" for x in plan):
                mission=get_mission(mid)
                action=action_gateway(mid,request.objective,bool(mission and mission["approved"]))
            ...
            result={
                ...
                "providers":rr["provider_events"],
                "query_terms":rr["query_terms"],
                "evidence_graph":{"nodes":len(graph["nodes"]),"links":len(graph["links"])},
                "sources":sources[:30],
                "action":action,
                "security":{
                    "private_networks_blocked":True,"waf_responses_rejected":True,
                    "unverified_responses_rejected":True,"transport_validation_required":True,
                    "research_source_integrity_validation":True,
                    "publisher_identity_validation":True,
                    "contradiction_analysis_required":True,
                    "arbitrary_code_execution":False,"permission_bypass":False
                },
                "attempts":attempts,"recovery_attempts":recovery,
                "policy_version":current_policy()["version"]
            }
            if request.remember: result["memory_key"]=remember(request.objective,result)
            event(mid,"research_completed",{
                "sources":len(sources),"relevant_sources":rr["relevant_sources"],
                "independent_publishers":rr["independent_publishers"],
                "provider_families":rr["provider_count"],
                "empirical_sources":verification["empirical_or_evaluation_sources"],
                "verified":verification["verified"],"closure":closure
            })
            # For research+verification missions, only closure=true can finish as
            # completed. A source-bearing but unverified mission is explicitly
            # needs_recovery rather than falsely reported as completed.
            if sources and (not request.verify or closure):
                update_mission(mid,"completed",result)
                event(mid,"mission_finished",{"status":"completed","closure":closure}); return
            if attempts<2:
                recovery+=1
                event(mid,"recovery_started",{"attempt":recovery,"reason":"evidence_closure_not_met","gaps":evidence_gaps})
                adaptive_upgrade("evidence-provider-recovery")
                time.sleep(0.35)
                continue
            update_mission(mid,"needs_recovery",result)
            event(mid,"mission_finished",{"status":"needs_recovery","closure":closure})
            return
        except Exception as exc:
            event(mid,"application_error",{"error":str(exc)[:500]})
            if attempts<2:
                recovery+=1; adaptive_upgrade("runtime-error-recovery"); time.sleep(0.35); continue
            result=empty_result(request.objective,attempts,recovery,str(exc)[:500])
            update_mission(mid,"needs_recovery",result); event(mid,"mission_finished",{"status":"needs_recovery"}); return
...
def version_history():
    return {
        ...
        "preserved_successful_lineage":[
            ...
            "2050.80","2050.81","2050.82","2050.83"
        ],
