...
COMMAND_EXECUTION_BRIDGE_VERSION = "141.1"
...
def _command141_self_test() -> dict:
    ...
    checks.extend([
        save_policy.get("registered") is True,
        save_policy.get("side_effects") is False,
        http_policy.get("registered") is True,
        http_policy.get("side_effects") is True,
        unknown_policy.get("registered") is False,
        preview.get("approval_required") is True,
        preview.get("external_execution") is False,
        approval_generated is True,
        approval_closed is True,
        plan.get("status") == "planned",
        plan.get("approval_required") is True,
        safe_result.get("status") in {"completed", "committed"},
        safe_result.get("action_executed") is True,
    ])
    passed = all(checks)
    return {
        "status": "passed" if passed else "failed",
