...
APP_VERSION = "TARGET-2050.172"
BUILD = "REAL-WORLD-WORKFLOW-EXECUTION-CLOSURE-CORE"
PREVIOUS_BUILD = "TARGET-2050.171"
...
def create_workflow(steps: List[Dict[str, Any]]) -> str:
    ...
    with _db_lock, db() as c:
        ...
        for i, step in enumerate(steps):
            ...
            c.execute("INSERT INTO workflow_steps(workflow_id,step_index,command,status,created_at,updated_at) VALUES(?,?,?,?,?,?)",
                      (wid, i, command_text, "queued", t, t))
    workflow_event(wid, "created", {"step_count": len(steps)})
    return wid
...
def run_workflow(wid: str, steps: List[Dict[str, Any]], start_index: int = 0) -> Dict[str, Any]:
    ...
    for index in range(start_index, len(steps)):
        step=steps[index]
        objective=str(step.get("command") or step.get("objective") or "").strip()
        try:
            plan=parse_command(objective)
            update_workflow_step(wid,index,status="running")
            ...
            if eid:
                ...
                if e and e.get("status") == "completed":
                    ...
                    continue
            if not eid:
                eid=create_execution(plan.get("action_type","unknown"),plan.get("connector","none"),objective,approval,parent_id=None,payload={"workflow_id":wid,"step":index})
