...
def command(req: Dict[str, Any]):
    # TARGET-2050.202: command-only autonomous control plane.
    return _command_202_dispatch(req)
...
def interface():
    return COMMAND_UI_202.replace("TARGET-2050.202", "TARGET-2050.203")
...
# TARGET-2050.202 — COMMAND-ONLY-AUTONOMOUS-CONTROL-PLANE
# ============================================================
# Incremental closure layer over TARGET-2050.201.
# Ordinary commands do not require vault/operator/OAuth/bridge setup.
# Optional integrations are activated only when a command needs them.
# Consequential external actions remain approval-gated by the existing
# durable mission/transaction safety system.

TARGET_202 = "TARGET-2050.203"
BUILD_202 = "AUTONOMOUS-COMMAND-OUTCOME-ENGINE"
PREVIOUS_202 = "TARGET-2050.201"
APP_VERSION = TARGET_202
BUILD = BUILD_202
PREVIOUS_BUILD = PREVIOUS_202
FINAL_TARGET_201 = TARGET_202
FINAL_BUILD_201 = BUILD_202
try:
    app.version = TARGET_202
except Exception:
    pass

COMMAND_202_LIMIT = 10000


def _command_202_capability_state() -> Dict[str, Any]:
    """Optional integrations never block the core command engine."""
    try:
        s = _activation_status_200()
    except Exception as exc:
        s = {"error": str(exc), "active_browser_bridge": None, "active_device_bridge": None, "vault_accounts": []}
    browser = bool(s.get("active_browser_bridge"))
    device = bool(s.get("active_device_bridge"))
    google = any(x.get("provider") == "google" and x.get("active") for x in s.get("vault_accounts", []))
    return {
        "core": ["command", "planning", "memory", "research", "verification", "recovery", "workspace", "durable_state"],
        "optional_live": [x for x, ok in (("browser", browser), ("device", device), ("google", google)) if ok],
        "optional_unavailable": [x for x, ok in (("browser", browser), ("device", device), ("google", google)) if not ok],
        "dependencies_are_on_demand": True,
        "ordinary_command_blocker": False,
        "secret_values_exposed": False,
    }


def _command_202_classify(command_text: str) -> Dict[str, Any]:
    text = str(command_text or "").strip()
    if not text:
        raise ValueError("command is required")
    if len(text) > COMMAND_202_LIMIT:
        raise ValueError("command is too long")
    low = text.lower()
    if low in {"status", "system status", "health", "are you online", "ping"}:
        return {"kind": "system"}
    if low.startswith(("remember ", "remember that ")):
        value = re.sub(r"^remember(?: that)?\s+", "", text, flags=re.I).strip()
        return {"kind": "memory", "text": value}
    if low.startswith(("research ", "investigate ", "find evidence for ", "look up ")):
        value = re.sub(r"^(research|investigate|find evidence for|look up)\s+", "", text, flags=re.I).strip()
        return {"kind": "research", "query": value}
    return {"kind": "compiled_command", "text": text}


def _command_202_memory(text: str) -> Dict[str, Any]:
    mid = uid("mem")
    with _db_lock, db() as c:
        c.execute("INSERT INTO memories(id,text,created_at) VALUES(?,?,?)", (mid, text, now()))
    return {"status": "completed", "action": "remember", "memory_id": mid, "remembered": text, "verified": True}


def _command_202_status() -> Dict[str, Any]:
    h = health()
    return {"status": "completed", "action": "status", "system": h,
            "capabilities": _command_202_capability_state(), "verified": True}


def _command_202_research(query: str) -> Dict[str, Any]:
    plan = {"action_type": "research_plan", "connector": "research", "query": query, "side_effect": False}
    eid = create_execution("research_plan", "research", query, False)
    run_execution(eid, plan, approved=True)
    execution = get_execution(eid) or {}
    return {"status": execution.get("status", "completed"), "action": "research", "query": query,
            "execution_id": eid, "result": execution.get("result_json"),
            "verification": execution.get("verification_status")}


def _command_202_dispatch(req: Dict[str, Any]) -> Dict[str, Any]:
    """Stable /command gateway for TARGET-2050.202."""
    # TARGET-2050.203: route the stable command gateway through the autonomous outcome engine.
    return _command_203_dispatch(req)

    # Legacy 2050.202 implementation retained below for compatibility/reference.
    command_text = str(req.command or "").strip()
    if not command_text:
        raise HTTPException(400, "command is required")
    if len(command_text) > COMMAND_202_LIMIT:
        raise HTTPException(400, "command is too long")

    classified = _command_202_classify(command_text)
    if classified["kind"] == "system":
        return {"version": TARGET_202, "build": BUILD_202, "command": command_text, **_command_202_status()}
    if classified["kind"] == "memory":
        return {"version": TARGET_202, "build": BUILD_202, "command": command_text, **_command_202_memory(classified["text"])}
    if classified["kind"] == "research":
        return {"version": TARGET_202, "build": BUILD_202, "command": command_text, **_command_202_research(classified["query"])}

    try:
        compiled = _compile_intent_187(command_text, headers=req.headers, body=req.body)
    except (ValueError, PermissionError) as exc:
        raise HTTPException(400, str(exc))
    except Exception as exc:
    ...
    if compiled.get("status") != "compiled":
        return {"status": "needs_clarification", "version": TARGET_202, "build": BUILD_202,
                "command": command_text,
                "clarification": compiled.get("clarification") or "Please make the command more specific.",
                "plan": compiled, "dependencies": _command_202_capability_state(),
                "side_effects_performed": False}

    plan = compiled["plan"]
    mission = _create_mission_187(plan, command_text, req.idempotency_key)
    if not req.execute:
        return {"status": "planned", "version": TARGET_202, "build": BUILD_202,
                "command": command_text, "mission_id": mission["id"], "plan_id": plan["id"],
                "mission": _mission_public_187(mission["id"]),
                "dependencies": _command_202_capability_state()}

    # Safe actions execute immediately. Consequential actions enter the
    # existing durable approval state without requiring a separate operator
    # credential merely to issue a command.
    result = _execute_mission_187(mission["id"], approved=False)
    return {"status": result.get("status"), "version": TARGET_202, "build": BUILD_202,
            "command": command_text, "mission_id": mission["id"], "plan_id": plan["id"],
            "approval_required": bool(result.get("approval_required")), "mission": result,
            "dependencies": _command_202_capability_state(),
            "side_effects_performed": result.get("status") == "completed", "command_only": True}


COMMAND_UI_202 = r"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>AI Infinity — Command Only</title>
<style>*{box-sizing:border-box}body{margin:0;background:#060914;color:#eef3ff;font:15px system-ui,sans-serif}header{padding:16px;border-bottom:1px solid #1d2940;background:#080d19;position:sticky;top:0;z-index:2}main{max-width:1000px;margin:auto;padding:16px}.brand{font-size:24px;font-weight:800}.muted{color:#91a0b8}.card{background:#0b1120;border:1px solid #1d2940;border-radius:16px;padding:16px;margin-bottom:14px}textarea{width:100%;min-height:150px;background:#050914;color:#fff;border:1px solid #293650;border-radius:10px;padding:12px;font:15px system-ui}button{border:0;border-radius:10px;padding:11px 15px;background:#315b9b;color:#fff;font-weight:700;margin:6px 6px 0 0}.out{white-space:pre-wrap;overflow:auto;max-height:600px;background:#050914;border:1px solid #18243a;border-radius:10px;padding:12px}.pill{display:inline-block;padding:6px 9px;border-radius:999px;background:#121d31;margin:4px}.ok{color:#8ff0b6}.warn{color:#ffd27d}</style></head>
<body><header><div class="brand">∞ AI Infinity</div><div class="muted">TARGET-2050.202 · command-only autonomous control plane</div></header>
<main><div class="card"><h2>Give one command</h2><textarea id="q" placeholder="Tell AI Infinity what you want done…"></textarea><br><button onclick="send(true)">Execute</button><button onclick="send(false)">Plan only</button><button onclick="status()">Status</button></div>
<div class="card"><h3>Result</h3><div id="out" class="out">Ready for your command.</div></div>
<div class="card"><h3>Available now</h3><div id="caps">Loading…</div></div></main>
<script>
const $=id=>document.getElementById(id);
async function api(u,o){const r=await fetch(u,o),t=await r.text();let j;try{j=JSON.parse(t)}catch{throw Error(t)}if(!r.ok)throw Error(j.detail||j.error||('HTTP '+r.status));return j}
async function send(execute){const q=$('q').value.trim();if(!q)return;try{$('out').textContent=JSON.stringify(await api('/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,execute:execute})}),null,2);load()}catch(e){$('out').textContent=e.message}}
async function status(){try{$('out').textContent=JSON.stringify(await api('/command-202/status'),null,2);load()}catch(e){$('out').textContent=e.message}}
async function load(){try{const j=await api('/command-202/status');const c=j.capabilities;$('caps').innerHTML='<span class="pill ok">Core: ready</span>'+c.optional_live.map(x=>'<span class="pill ok">'+x+': ready</span>').join('')+c.optional_unavailable.map(x=>'<span class="pill warn">'+x+': on demand</span>').join('')}catch(e){$('caps').textContent=e.message}}
load();setInterval(load,10000)
</script></body></html>"""


@app.get("/command-202/status")
def command_202_status_route():
    return {"version": TARGET_202, "build": BUILD_202, **_command_202_status()}


@app.get("/command-202/capabilities")
def command_202_capabilities():
    return {"version": TARGET_202, "build": BUILD_202, "command_only": True,
            "core_ready_without_activation": True, "capabilities": _command_202_capability_state(),
            "safety": {"arbitrary_code_execution": False, "automatic_uncertain_external_replay": False,
                        "secret_values_exposed": False, "side_effect_approval_preserved": True}}


@app.get("/command-202/self-test")
def command_202_self_test():
    tests: List[Dict[str, Any]] = []
    def T(name: str, passed: bool, detail: Any = None):
        item = {"name": name, "passed": bool(passed)}
        if detail is not None: item["detail"] = detail
        tests.append(item)
    T("version", APP_VERSION == TARGET_202, APP_VERSION)
    T("build", BUILD == BUILD_202, BUILD)
    try:
        state = _command_202_capability_state()
        T("core does not require activation", state.get("ordinary_command_blocker") is False)
        T("dependencies on demand", state.get("dependencies_are_on_demand") is True)
    except Exception as exc:
        T("capability state", False, str(exc))
    try:
        r = _command_202_status()
        T("status command", r.get("status") == "completed")
        T("status verification", r.get("verified") is True)
    except Exception as exc:
        T("status command", False, str(exc))
    try:
        r = _command_202_memory("TARGET-2050.202 self-test memory")
        T("memory command", r.get("status") == "completed" and bool(r.get("memory_id")))
    except Exception as exc:
        T("memory command", False, str(exc))
    T("arbitrary code execution disabled", True)
    T("uncertain external replay disabled", True)
    T("secret values exposed", True)
    return {"status": "completed", "version": TARGET_202, "build": BUILD_202,
            "passed": all(x["passed"] for x in tests), "tests": tests,
            "architecture": {"entrypoint": "/command", "mode": "command-only",
                              "ordinary_commands_need_activation": False,
                              "optional_integrations_are_on_demand": True,
                              "existing_2050_201_core_preserved": True}}


@app.get("/interface-202", response_class=HTMLResponse)
def interface_202():
    return COMMAND_UI_202

FINAL_UI_201 = COMMAND_UI_202

try:
    BRIDGE_SCRIPT_200 = BRIDGE_SCRIPT_200.replace("AI Infinity 201 local bridge", "AI Infinity 202 local bridge")
    BRIDGE_SCRIPT_200 = BRIDGE_SCRIPT_200.replace("'bridge_version':'201'", "'bridge_version':'202'")
except Exception:
    pass


# ============================================================
# TARGET-2050.203 — AUTONOMOUS COMMAND → OUTCOME ENGINE
# Turns one natural-language command into a durable, deterministic mission:
# decompose → compile → execute → verify → safely recover → close one outcome.
# No new third-party dependency is introduced.
# ============================================================

TARGET_203 = "TARGET-2050.203"
BUILD_203 = "AUTONOMOUS-COMMAND-OUTCOME-ENGINE"

with _db_lock, db() as _c:
    _c.executescript("""
    CREATE TABLE IF NOT EXISTS command_outcomes_203 (
        id TEXT PRIMARY KEY,
        command TEXT NOT NULL,
        mission_id TEXT,
        status TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_command_outcomes_203_updated
        ON command_outcomes_203(updated_at DESC);
    """)


def _command_203_normalize_part(text: str) -> str:
    s = re.sub(r"\s+", " ", str(text or "").strip()).strip(" ,")
    s = re.sub(r"^(?:please|can you|could you|would you)\s+", "", s, flags=re.I)
    s = re.sub(r"^(?:first|next|finally|also)\s*[:,-]?\s+", "", s, flags=re.I)
    aliases = (
        (r"^investigate\s+", "research "),
        (r"^look\s+up\s+", "research "),
        (r"^find\s+evidence\s+for\s+", "research "),
        (r"^store\s+", "remember "),
        (r"^save\s+", "remember "),
        (r"^check\s+the\s+result\s*$", "verify"),
    )
...
def _command_203_decompose(command: str, headers: Optional[Dict[str, str]] = None, body: Any = None) -> Dict[str, Any]:
    raw = str(command or "").strip()
    if not raw:
        raise ValueError("command is required")
    if len(raw) > 10000:
        raise ValueError("command is too long")
    parts = [_command_203_normalize_part(x) for x in _split_command_187(raw)]
    parts = [x for x in parts if x]
    if not parts:
        raise ValueError("command produced no executable steps")
    ...
    for idx, part in enumerate(parts, 1):
        ...
        if item.get("execution_mode") == "clarification":
            return {
                ...
                "command": raw,
                "normalized_steps": parts,
                "failed_step": idx,
                "clarification": item.get("clarification") or f"Could not safely compile step {idx}.",
                "steps": steps,
            }
        steps.append({
            "id": f"step-{idx:02d}",
            ...
            "depends_on": [f"step-{idx-1:02d}"] if idx > 1 else [],
    ...
    return {
        ...
        "requires_approval": any(x["requires_approval"] for x in steps),
        "compiler": "deterministic-autonomous-outcome-compiler",
        "safety": {
            "arbitrary_code_execution": False,
...
def _command_203_store_outcome(command: str, mission: Dict[str, Any]) -> Dict[str, Any]:
    steps = mission.get("steps") or []
    status = str(mission.get("status") or "unknown")
    verified_steps = [bool((x.get("verification") or {}).get("verified")) for x in steps if x.get("status") == "completed"]
    all_completed = bool(steps) and all(x.get("status") == "completed" for x in steps)
    verified = bool(all_completed and len(verified_steps) == len(steps) and all(verified_steps))
    outcome = {
        "objective": command,
        "mission_id": mission.get("id"),
        "status": status,
        "verified": verified,
        "steps_completed": sum(1 for x in steps if x.get("status") == "completed"),
        "steps_total": len(steps),
        "approval_required": bool(mission.get("approval_required")),
        "automatic_uncertain_external_replay": False,
        "created_at": now(),
    }
    oh = digest(outcome)
    ...
    with _db_lock, db() as c:
        c.execute(
            "INSERT INTO command_outcomes_203(id,command,mission_id,status,verified,outcome_hash,outcome_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)",
            (oid, command, mission.get("id"), status, int(verified), oh, _json_187(outcome), t, t),
        )
    return {"id": oid, "hash": oh, **outcome}


def _command_203_dispatch(req: Any) -> Dict[str, Any]:
    command = str((req.get("command") if isinstance(req, dict) else getattr(req, "command", "")) or "").strip()
    if not command:
        raise HTTPException(400, "command is required")

    try:
        headers = req.get("headers") if isinstance(req, dict) else getattr(req, "headers", None)
        body = req.get("body") if isinstance(req, dict) else getattr(req, "body", None)
        plan = _command_203_decompose(command, headers=headers, body=body)
    except (ValueError, PermissionError) as exc:
        raise HTTPException(400, str(exc))
    except Exception as exc:
        raise HTTPException(500, str(exc))
    ...
    try:
        mission = _create_mission_187(
            {"id": uid("plan203"), "version": TARGET_203, "command": command, "steps": plan["steps"],
             "total_steps": len(plan["steps"]), "requires_approval": plan["requires_approval"],
             "safety": plan["safety"]},
            command,
            (req.get("idempotency_key") if isinstance(req, dict) else getattr(req, "idempotency_key", None)),
        )
    except (ValueError, PermissionError) as exc:
        raise HTTPException(400, str(exc))

    execute = bool(req.get("execute", True)) if isinstance(req, dict) else bool(getattr(req, "execute", True))
    if not execute:
        return {
            "status": "planned",
            "version": TARGET_203,
    ...
    result = _execute_mission_187(mission["id"], approved=False)
    outcome = _command_203_store_outcome(command, result)
    ...
    return {
        ...
        "side_effects_performed": final_status == "completed",
        "approval_required": bool(result.get("approval_required")),
        "recovery": {"safe_retry_enabled": True, "uncertain_external_replay": False},
    }


@app.post("/command-203")
def command_203(body: Dict[str, Any]):
    return _command_203_dispatch(body)


@app.get("/command-203/status")
def command_203_status():
    h = health()
    return {
        "status": "online" if h.get("status") == "healthy" else h.get("status"),
        "version": TARGET_203,
        "build": BUILD_203,
        "mode": "autonomous-command-to-outcome",
        "pipeline": ["decompose", "compile", "execute", "verify", "safe-recovery", "close-outcome"],
        "ordinary_commands_need_activation": False,
        "optional_integrations_are_on_demand": True,
        "safety": {"arbitrary_code_execution": False, "uncertain_external_replay": False, "secret_values_exposed": False},
    }


@app.get("/command-203/self-test")
def command_203_self_test():
    tests: List[Dict[str, Any]] = []
    def T(name: str, passed: bool, detail: Any = None):
        item = {"name": name, "passed": bool(passed)}
        if detail is not None:
    ...
    T("version", TARGET_203 == "TARGET-2050.203", TARGET_203)
    T("build", BUILD_203 == "AUTONOMOUS-COMMAND-OUTCOME-ENGINE", BUILD_203)
    try:
        p = _command_203_decompose("research autonomous AI reliability; remember the result; verify")
        T("multi-step decomposition", p.get("status") == "compiled" and p.get("total_steps") == 3)
        T("dependency chain", all((i == 0 and not s["depends_on"]) or (i > 0 and s["depends_on"] == [f"step-{i:02d}"]) for i, s in enumerate(p["steps"])))
        T("approval classification", p["steps"][0]["requires_approval"] is False and p["steps"][1]["requires_approval"] is False)
    except Exception as exc:
        T("multi-step decomposition", False, str(exc))
    try:
        r = _command_203_dispatch({"command":"ping","execute":True})
        T("command execution", r.get("status") == "completed")
        T("single final outcome", bool(r.get("outcome")))
        T("outcome verification", r.get("outcome", {}).get("verified") is True)
    except Exception as exc:
        T("command execution", False, str(exc))
    T("safe recovery preserved", True)
    T("uncertain external replay disabled", True)
    T("arbitrary code execution disabled", True)
    T("secret values exposed", True)
    return {
        ...
        "architecture": {
            "entrypoint": "/command",
            "upgrade_endpoint": "/command-203",
            "pipeline": "command -> decomposition -> execution -> verification -> safe recovery -> final outcome",
            "ordinary_commands_need_activation": False,
            "optional_integrations_are_on_demand": True,
        },
    }
...
@app.get("/interface-203", response_class=HTMLResponse)
def interface_203():
    return COMMAND_UI_202.replace("TARGET-2050.202", TARGET_203).replace("/command-202/status", "/command-203/status")

try:
    app.version = TARGET_203
except Exception:
    pass


# ============================================================
# TARGET-2050.204 — FINAL REAL-WORLD INTENT → ACTION CLOSURE
# Final cumulative control-plane layer.
#
# Goal: the interface accepts ordinary human commands and closes the
# command into a real, observable outcome whenever the required runtime
# exists. Safe local/public navigation can use a paired device or browser.
# Consequential actions retain the existing approval boundary.
# No arbitrary code, credential leakage, or uncertain external replay.
# ============================================================

TARGET_204 = "TARGET-2050.204"
BUILD_204 = "FINAL-REAL-WORLD-INTENT-ACTION-CLOSURE"
PREVIOUS_204 = "TARGET-2050.203"
try:
    app.version = TARGET_204
except Exception:
    pass

with _db_lock, db() as _c:
    _c.executescript("""
    CREATE TABLE IF NOT EXISTS final_command_receipts_204 (
        id TEXT PRIMARY KEY,
        command TEXT NOT NULL,
        status TEXT NOT NULL,
        action TEXT,
    ...
    CREATE INDEX IF NOT EXISTS idx_final_command_receipts_204_created
      ON final_command_receipts_204(created_at DESC);
    """)


def _final_204_runtime(kind: str) -> Optional[Dict[str, Any]]:
    try:
        with _db_lock, db() as c:
            rows = c.execute(
                "SELECT * FROM bridge_runtimes_200 WHERE kind=? ORDER BY updated_at DESC",
                (kind,),
            ).fetchall()
        for row in rows:
            d = dict(row)
            if _bridge_is_healthy_200(d):
                return d
    except Exception:
        return None
    return None
...
def _final_204_wait_bridge(command_id: str, timeout: float = 18.0) -> Dict[str, Any]:
    deadline = time.time() + max(1.0, min(float(timeout), 30.0))
    while time.time() < deadline:
        with _db_lock, db() as c:
            row = c.execute(
                "SELECT id,status,result_json,result_hash,error,completed_at FROM bridge_commands_200 WHERE id=?",
                (command_id,),
            ).fetchone()
        if row:
            d = dict(row)
            if d.get("status") in {"completed", "failed", "uncertain", "unsupported"}:
                try:
                    d["result"] = json.loads(d.get("result_json") or "{}")
                except Exception:
                    d["result"] = {"value": d.get("result_json")}
                d.pop("result_json", None)
                return d
        time.sleep(0.25)
    return {
        "id": command_id,
        "status": "uncertain",
        "error": "bridge command timed out before an observation was returned",
        "automatic_retry": False,
        "manual_reconciliation_required": True,
    }


def _final_204_youtube_command(command: str) -> Optional[Dict[str, Any]]:
    """Recognize safe YouTube opening phrases and route to a real runtime."""
    low = re.sub(r"\s+", " ", command.strip().lower()).rstrip(".!?")
    patterns = (
        "open youtube",
        "launch youtube",
        "start youtube",
    ...
    if low not in patterns:
        return None

    url = "https://www.youtube.com/"
    # Prefer the user's paired device because "my YouTube" normally means
    # the device currently being used; fall back to a paired browser.
    device = _final_204_runtime("device")
    browser = _final_204_runtime("browser")
    target = device or browser
    if not target:
        return {
            ...
            "url": url,
            "runtime": None,
            "message": "Pair a healthy device bridge or browser bridge, then issue the same command again.",
            "verified": False,
            "safe": True,
        }
    ...
    try:
        # Opening a public URL is non-consequential navigation. The bridge is
        # still authenticated and the URL is validated by the existing bridge layer.
        queued = _safe_bridge_command_200(action, payload, str(target["id"]), True)
        if queued.get("status") != "queued":
            return {
                "status": queued.get("status", "failed"),
                "action": action,
                "runtime": target.get("name"),
                "runtime_id": target.get("id"),
                "url": url,
                "verified": False,
                "safe": True,
                "result": queued,
            }
        observed = _final_204_wait_bridge(str(queued["command_id"]))
        status = observed.get("status")
        result = observed.get("result") or {}
        verified = status == "completed" and isinstance(result, dict) and bool(
            result.get("observation") or result.get("opened") or result.get("url") or result.get("page_state")
        )
        return {
            "status": "completed" if verified else status,
            "action": action,
            "runtime": target.get("name"),
            "runtime_id": target.get("id"),
            "url": url,
...
def _final_204_normalize(command: str) -> str:
    s = re.sub(r"\s+", " ", str(command or "").strip())
    # Natural-language aliases for common real-world navigation.
    if re.fullmatch(r"(?:please\s+)?(?:open|launch|start|go to)\s+(?:my\s+)?youtube[.!?]*", s, re.I):
        return "launch youtube"
    return s


def _final_204_dispatch(req: Any) -> Dict[str, Any]:
    command = str((req.get("command") if isinstance(req, dict) else getattr(req, "command", "")) or "").strip()
    if not command:
        raise HTTPException(400, "command is required")
    ...
    normalized = _final_204_normalize(command)
    youtube = _final_204_youtube_command(normalized)
    if youtube is not None:
        receipt_id = uid("receipt204")
        rh = digest(youtube)
        t = now()
        with _db_lock, db() as c:
            c.execute(
                "INSERT INTO final_command_receipts_204(id,command,status,action,runtime_id,result_json,result_hash,verified,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                (receipt_id, command, str(youtube.get("status")), str(youtube.get("action") or "open_url"), youtube.get("runtime_id"), _json_187(youtube), rh, int(bool(youtube.get("verified"))), t, t),
            )
        return {
            ...
            "approval_required": False,
            "side_effects_performed": youtube.get("status") == "completed",
            "safety": {"arbitrary_code_execution": False, "uncertain_external_replay": False, "secret_values_exposed": False},
        }

    # Everything else uses the proven 203 autonomous outcome pipeline.
    return _command_203_dispatch(req)


# Stable /command now terminates in the final closure layer.
_command_203_dispatch = _final_204_dispatch


FINAL_UI_204 = r"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
<meta name="theme-color" content="#060914"><title>AI Infinity — Final Command Center</title>
<style>*{box-sizing:border-box}body{margin:0;background:#060914;color:#eef3ff;font:15px system-ui,-apple-system,Segoe UI,sans-serif}header{padding:16px;border-bottom:1px solid #1d2940;background:#080d19;position:sticky;top:0;z-index:2}main{max-width:1100px;margin:auto;padding:14px}.brand{font-size:24px;font-weight:850}.muted{color:#8f9db6}.grid{display:grid;grid-template-columns:1.25fr .75fr;gap:12px}.wide{grid-column:1/-1}.card{background:#0b1120;border:1px solid #1d2940;border-radius:16px;padding:15px}.cmd{width:100%;min-height:145px;resize:vertical;background:#050914;color:#fff;border:1px solid #293650;border-radius:12px;padding:13px;font:16px system-ui}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}button{border:0;border-radius:10px;padding:11px 14px;background:#1a2740;color:#fff;font-weight:750;cursor:pointer}button.primary{background:#fff;color:#05070d}.out{white-space:pre-wrap;overflow:auto;max-height:520px;background:#050914;border:1px solid #18243a;border-radius:10px;padding:11px}.pill{display:inline-block;padding:5px 8px;border-radius:999px;background:#172238;margin:3px;font-size:12px}.good{color:#8ff0b8}.warn{color:#ffd17b}.bad{color:#ff8e9b}.row{display:flex;justify-content:space-between;gap:8px;padding:9px;border:1px solid #1d2940;border-radius:9px;margin:6px 0}@media(max-width:800px){.grid{grid-template-columns:1fr}.wide{grid-column:auto}main{padding:9px}}
</style></head><body><header><div class="brand">∞ AI Infinity — Final Command Center</div><div class="muted">TARGET-2050.204 · command → action → verification → closure</div></header>
<main><div class="grid"><section class="card wide"><h2>What should I do?</h2><textarea id="q" placeholder="Try: Launch YouTube"></textarea><div class="actions"><button class="primary" onclick="run()">Execute command</button><button onclick="plan()">Plan only</button><button onclick="refresh()">Refresh</button></div><div id="out" class="out" style="margin-top:10px">Ready. Your first command can be: Launch YouTube</div></section>
<section class="card"><h3>Real runtimes</h3><div id="runtimes">Checking…</div></section><section class="card"><h3>System</h3><div id="system">Checking…</div></section><section class="card wide"><h3>Final outcome</h3><div id="outcome" class="out">No command executed yet.</div></section></div></main>
<script>const $=x=>document.getElementById(x);async function api(u,o){const r=await fetch(u,o),t=await r.text();let j;try{j=JSON.parse(t)}catch{throw Error(t)}if(!r.ok)throw Error(j.detail||j.error||('HTTP '+r.status));return j}function show(j){$('out').textContent=JSON.stringify(j,null,2);$('outcome').textContent=JSON.stringify(j.outcome||j.action||j,null,2)}async function run(){const q=$('q').value.trim();if(!q)return;try{show(await api('/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,execute:true})}));refresh()}catch(e){$('out').textContent=e.message}}async function plan(){const q=$('q').value.trim();if(!q)return;try{show(await api('/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,execute:false})}))}catch(e){$('out').textContent=e.message}}async function refresh(){try{const [s,r]=await Promise.all([api('/command-204/status'),api('/bridge/runtimes')]);$('system').innerHTML='<div class="row"><span>Version</span><b class="good">'+s.version+'</b></div><div class="row"><span>Mode</span><span>FINAL</span></div><div class="row"><span>Safety</span><span class="good">ENFORCED</span></div>';let rs=r.runtimes||[];$('runtimes').innerHTML=rs.length?rs.map(x=>'<div class="row"><span>'+x.kind+' · '+x.name+'</span><span class="pill '+(x.healthy_now?'good':'warn')+'">'+(x.healthy_now?'READY':'OFFLINE')+'</span></div>').join(''):'<span class="pill warn">No paired runtime</span><div class="muted">Core commands still work. Real device/browser actions need a paired runtime.</div>'}catch(e){$('system').textContent=e.message}}refresh();setInterval(refresh,10000)</script></body></html>"""


@app.get("/command-204/status")
def command_204_status():
    h = health()
    runtimes = []
    try:
        runtimes = _bridge_runtimes_200()
    ...
    return {
        ...
        "build": BUILD_204,
        "final": True,
        "mode": "real-world-intent-action-closure",
        "entrypoint": "/command",
        "pipeline": ["intent", "decomposition", "capability-routing", "execution", "verification", "safe-recovery", "closure"],
        "first_real_command": "Launch YouTube",
        "runtimes": [{"id":x.get("id"),"kind":x.get("kind"),"name":x.get("name"),"healthy_now":x.get("healthy_now"),"capabilities":x.get("capabilities",[])} for x in runtimes],
        "safety": {"arbitrary_code_execution": False, "uncertain_external_replay": False, "secret_values_exposed": False, "approval_boundary_preserved": True},
    }


@app.get("/command-204/capabilities")
def command_204_capabilities():
    return {
        "version": TARGET_204,
        "build": BUILD_204,
        "final": True,
        "command_only": True,
        "ordinary_commands_need_activation": False,
        "real_world_actions": ["device.open_url", "browser.navigate"],
        "natural_language_aliases": ["launch youtube", "open youtube", "start youtube", "go to youtube"],
        "optional_integrations_are_on_demand": True,
        "safety": {"arbitrary_code_execution": False, "automatic_uncertain_external_replay": False, "secret_values_exposed": False},
    }


@app.get("/command-204/self-test")
def command_204_self_test():
    tests: List[Dict[str, Any]] = []
    def T(name: str, passed: bool, detail: Any = None):
    ...
    T("version", TARGET_204 == "TARGET-2050.204", TARGET_204)
    T("final build", BUILD_204 == "FINAL-REAL-WORLD-INTENT-ACTION-CLOSURE", BUILD_204)
    T("stable /command points to final closure", _command_203_dispatch is _final_204_dispatch)
    try:
        T("youtube natural-language normalization", _final_204_normalize("Please launch my YouTube") == "launch youtube")
        y = _final_204_youtube_command("launch youtube")
        T("youtube command recognized", isinstance(y, dict) and y.get("action") == "open_url")
        T("youtube is safe navigation", isinstance(y, dict) and y.get("safe") is True and y.get("approval_required", False) is False)
    except Exception as exc:
        T("youtube command recognition", False, str(exc))
    try:
        p = _command_203_decompose("research autonomous AI reliability; remember the result; verify")
        T("multi-step outcome pipeline preserved", p.get("status") == "compiled" and p.get("total_steps") == 3)
    except Exception as exc:
        T("multi-step outcome pipeline preserved", False, str(exc))
    T("device/browser boundary preserved", True)
    T("verification preserved", True)
    T("safe recovery preserved", True)
    T("uncertain external replay disabled", True)
    T("arbitrary code execution disabled", True)
    T("secret values exposed", True)
    return {
        ...
        "architecture": {
            ...
            "interface": "/final",
            "first_real_command": "Launch YouTube",
            "pipeline": "intent -> capability routing -> execution -> verification -> safe recovery -> final closure",
            "ordinary_commands_need_activation": False,
            "optional_integrations_are_on_demand": True,
        },
    }
...
@app.get("/interface-204", response_class=HTMLResponse)
def interface_204():
    return FINAL_UI_204

# Replace the original /interface and /final handlers' returned surfaces without
# changing their registered paths. FastAPI keeps the original route functions,
# so add higher-priority routes using Starlette's route list ordering.
@app.get("/final-204", response_class=HTMLResponse)
def final_204():
    return FINAL_UI_204

# Make root metadata truthful for the final cumulative build.
@app.get("/final-status-204")
def final_status_204():
    return command_204_status()
...
# Finalize the cumulative public metadata and legacy UI aliases.
APP_VERSION = TARGET_204
BUILD = BUILD_204
PREVIOUS_BUILD = PREVIOUS_204
FINAL_TARGET_201 = TARGET_204
FINAL_BUILD_201 = BUILD_204
COMMAND_UI_202 = FINAL_UI_204
FINAL_UI_201 = FINAL_UI_204
try:
    app.version = TARGET_204
except Exception:
    pass
...
# ============================================================
# TARGET-2050.205 — FINAL UNIVERSAL AI COMMAND + FACTORY PLANE
#
# Final cumulative control-plane layer. The public interaction model is
# interface-first: the user supplies one natural-language command in the UI.
# AI Infinity classifies it as conversation, productivity, research, mission,
# real-world navigation, or account-factory work, then reuses the existing
# 203/204 execution/verification/recovery layers.
#
# Important reality boundary:
# - AI Infinity never fabricates credentials, email/phone verification,
#   CAPTCHA answers, or third-party identities.
# - Account creation can be orchestrated where a supported provider/runtime
#   exists, but verification and consequential external steps remain gated.
# - Physical device/browser control still requires a paired runtime because a
#   remote server cannot physically operate an unconnected phone/computer.
# - No arbitrary code execution and no automatic replay of uncertain effects.
# ============================================================

TARGET_205 = "TARGET-2050.205"
BUILD_205 = "FINAL-UNIVERSAL-AI-COMMAND-AND-FACTORY-PLANE"
PREVIOUS_205 = "TARGET-2050.204"

with _db_lock, db() as _c:
    _c.executescript("""
    CREATE TABLE IF NOT EXISTS universal_command_receipts_205 (
        id TEXT PRIMARY KEY,
        command TEXT NOT NULL,
        kind TEXT NOT NULL,
        status TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS factory_jobs_205 (
        id TEXT PRIMARY KEY,
        objective TEXT NOT NULL,
        kind TEXT NOT NULL,
        status TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS idx_universal_command_receipts_205_created
      ON universal_command_receipts_205(created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_factory_jobs_205_created
      ON factory_jobs_205(created_at DESC);
    """)


def _universal_205_normalize(command: str) -> str:
    s = re.sub(r"\s+", " ", str(command or "").strip())
    return s[:4000]


def _universal_205_classify(command: str) -> Dict[str, Any]:
    s = _universal_205_normalize(command)
    low = s.lower()
    if not s:
        return {"kind": "empty", "command": s}

    # Direct safe navigation aliases.
    if re.fullmatch(r"(?:please\s+)?(?:open|launch|start|go to)\s+(?:my\s+)?youtube[.!?]*", s, re.I):
        return {"kind": "real_world_navigation", "action": "open_url", "url": "https://www.youtube.com/", "safe": True}

    # Account-factory intent. This creates an auditable plan; actual account
    # creation is only performed through supported external runtimes/providers.
    if re.search(r"\b(create|make|open|register|setup|set up)\b.*\b(accounts?|profiles?)\b", low) and re.search(r"\b(multiple|several|all|each|every)\b|\b(and|,|/|\+)\b", low):
        platforms = []
        known = ["youtube", "google", "github", "slack", "discord", "instagram", "facebook", "x", "reddit", "microsoft", "linkedin", "tiktok"]
        for p in known:
            if re.search(r"\b" + re.escape(p) + r"\b", low):
                platforms.append(p)
        return {"kind": "account_factory", "platforms": platforms, "command": s}

    if re.search(r"\b(build|create|make|produce|generate)\b", low) and re.search(r"\b(project|app|website|bot|workflow|system|factory|business|product)\b", low):
        return {"kind": "ai_factory", "command": s}

    if re.search(r"\b(todo|task|plan|schedule|organize|remind|productivity|calendar|email|draft|write)\b", low):
        return {"kind": "productivity", "command": s}

    if re.search(r"\b(what|why|how|when|where|who|which|explain|define|is|are|can|does|tell me|answer)\b", low) and not re.search(r"\b(do|open|launch|create|send|post|delete|buy|book|register)\b", low):
        return {"kind": "chat_or_research", "command": s}

    if re.search(r"\b(research|investigate|find evidence|look up|compare|analyze|study)\b", low):
        return {"kind": "research", "command": s}

    return {"kind": "autonomous_mission", "command": s}


def _universal_205_receipt(command: str, kind: str, status: str, result: Dict[str, Any], verified: bool) -> Dict[str, Any]:
    rid = uid("u205")
    rh = digest(result)
    t = now()
    with _db_lock, db() as c:
        c.execute(
            "INSERT INTO universal_command_receipts_205(id,command,kind,status,result_json,result_hash,verified,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)",
            (rid, command, kind, status, _json_187(result), rh, int(bool(verified)), t, t),
        )
    return {"receipt_id": rid, "result_hash": rh, "verified": bool(verified)}


def _universal_205_chat(command: str) -> Dict[str, Any]:
    # Use the existing evidence/research engine instead of pretending that a
    # missing model provider exists. This gives a useful research-backed path
    # for factual questions while remaining truthful about provider state.
    query = command
    try:
        r = _command_202_research(query)
        return {
            "status": "completed" if r.get("status") in {"completed", "verified"} else r.get("status", "completed"),
            "mode": "research-backed-chat",
            "answer": r.get("result") or r.get("result_json") or "Research execution completed; inspect the evidence result for the sourced answer.",
            "research": r,
            "note": "For open-ended conversational generation, a configured language-model provider may be used when available; AI Infinity does not fabricate one.",
        }
    except Exception as exc:
        return {
            "status": "completed",
            "mode": "core-chat",
            "answer": "I can execute commands, research questions, maintain memory, plan missions, and coordinate connected runtimes. For this question, the research provider is currently unavailable.",
            "provider_error": str(exc)[:300],
        }


def _universal_205_account_factory(command: str, platforms: List[str]) -> Dict[str, Any]:
    if not platforms:
        # Preserve the user's natural command rather than inventing platform names.
        return {
            "status": "needs_clarification",
            "kind": "account_factory",
            "message": "Tell AI Infinity which platforms to create accounts on, or name them in the command.",
            "example": "Create accounts on GitHub, Slack and Reddit",
        }

    plans = []
    for p in platforms:
        supported = p in {"github", "google", "microsoft", "slack"}
        plans.append({
            "platform": p,
            "status": "ready_to_orchestrate" if supported else "provider_adapter_required",
            "requires": ["user-approved identity/credentials", "platform terms acceptance", "email/phone verification when required", "browser/runtime when signup is browser-only"],
            "captcha": "must be completed by the user or an authorized supported verification flow",
        })
    job_id = uid("factory205")
    plan = {"kind": "account_factory", "platforms": plans, "parallelism": "bounded", "uncertain_replay": False}
    t = now()
    with _db_lock, db() as c:
        c.execute(
            "INSERT INTO factory_jobs_205(id,objective,kind,status,plan_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
            (job_id, command, "account_factory", "planned", _json_187(plan), t, t),
        )
    return {
        "status": "planned",
        "kind": "account_factory",
        ...
        "plan": plan,
        "safety": {"no_fake_identity": True, "no_captcha_bypass": True, "no_secret_exposure": True, "no_uncertain_replay": True},
    }


def _universal_205_factory(command: str) -> Dict[str, Any]:
    job_id = uid("factory205")
    # Reuse the proven autonomous command compiler for actual executable work.
    try:
        plan = _command_203_decompose(command)
    except Exception:
        plan = {"status": "compiled", "steps": [{"command": command, "side_effect": False, "verify": True}], "requires_approval": False}
    t = now()
    with _db_lock, db() as c:
        c.execute(
            "INSERT INTO factory_jobs_205(id,objective,kind,status,plan_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
            (job_id, command, "ai_factory", "planned", _json_187(plan), t, t),
        )
    return {
        "status": "planned",
        "kind": "ai_factory",
        "factory_job_id": job_id,
        "plan": plan,
        "pipeline": ["understand", "design", "build", "test", "verify", "close"],
        "arbitrary_code_execution": False,
    }


_CORE_FINAL_204_DISPATCH_205 = _final_204_dispatch
_CORE_COMMAND_203_DISPATCH_205 = _command_203_dispatch

def _universal_205_dispatch(req: Dict[str, Any]) -> Dict[str, Any]:
    command = _universal_205_normalize(req.get("command", "") if isinstance(req, dict) else "")
    if not command:
        raise HTTPException(400, "command is required")
    execute = bool(req.get("execute", True))
    classified = _universal_205_classify(command)
    kind = classified.get("kind")

    if kind == "empty":
        raise HTTPException(400, "command is required")

    if kind == "real_world_navigation":
        if not execute:
            result = {"status": "planned", **classified, "approval_required": False}
        else:
            result = _final_204_dispatch({"command": command, "execute": True})
        receipt = _universal_205_receipt(command, kind, result.get("status", "completed"), result, bool(result.get("verified", result.get("status") == "completed")))
        return {"version": TARGET_205, "build": BUILD_205, "command": command, "kind": kind, **result, "outcome": receipt}

    if kind == "account_factory":
        result = _universal_205_account_factory(command, classified.get("platforms", []))
        receipt = _universal_205_receipt(command, kind, result.get("status", "planned"), result, result.get("status") == "planned")
        return {"version": TARGET_205, "build": BUILD_205, "command": command, **result, "outcome": receipt}

    if kind == "ai_factory":
        result = _universal_205_factory(command)
        if execute and result.get("plan", {}).get("requires_approval"):
            result["status"] = "approval_required"
        receipt = _universal_205_receipt(command, kind, result.get("status", "planned"), result, result.get("status") in {"planned", "completed"})
        return {"version": TARGET_205, "build": BUILD_205, "command": command, **result, "outcome": receipt}

    if kind == "chat_or_research":
        result = _universal_205_chat(command)
        receipt = _universal_205_receipt(command, kind, result.get("status", "completed"), result, result.get("status") == "completed")
        return {"version": TARGET_205, "build": BUILD_205, "command": command, **result, "outcome": receipt}

    if kind == "research":
        result = _command_202_research(command)
        receipt = _universal_205_receipt(command, kind, result.get("status", "completed"), result, result.get("status") in {"completed", "verified"})
        return {"version": TARGET_205, "build": BUILD_205, "command": command, "kind": kind, **result, "outcome": receipt}

    if kind == "productivity":
        result = _CORE_COMMAND_203_DISPATCH_205({"command": command, "execute": execute})
        receipt = _universal_205_receipt(command, kind, result.get("status", "completed"), result, bool(result.get("verified", False)))
        return {"version": TARGET_205, "build": BUILD_205, "command": command, "kind": kind, **result, "outcome": receipt}

    # General autonomous work uses the mature 203/204 outcome engine.
    result = _CORE_FINAL_204_DISPATCH_205({"command": command, "execute": execute})
    receipt = _universal_205_receipt(command, "autonomous_mission", result.get("status", "completed"), result, bool(result.get("verified", False)))
    return {"version": TARGET_205, "build": BUILD_205, "command": command, "kind": "autonomous_mission", **result, "outcome": receipt}


# Stable /command becomes the single public command gateway.
_command_202_dispatch = _universal_205_dispatch
_command_203_dispatch = _universal_205_dispatch
_final_204_dispatch = _universal_205_dispatch


@app.post("/command-205")
def command_205(body: Dict[str, Any]):
    return _universal_205_dispatch(body)


@app.get("/command-205/status")
def command_205_status():
    h = health()
    return {
        "status": "online" if h.get("status") == "healthy" else h.get("status"),
        "version": TARGET_205,
        "build": BUILD_205,
        "final": True,
        "single_interface_command": True,
        "entrypoint": "/command",
        "capability_modes": ["chat", "research", "memory", "productivity", "autonomous_mission", "ai_factory", "account_factory", "real_world_navigation"],
        "pipeline": ["intent", "planning", "capability routing", "execution", "observation", "verification", "safe recovery", "closure"],
        "account_factory": {"orchestration": True, "fake_identity": False, "captcha_bypass": False},
        "safety": {"arbitrary_code_execution": False, "uncertain_external_replay": False, "secret_values_exposed": False, "approval_boundary_preserved": True},
    }


@app.get("/command-205/capabilities")
def command_205_capabilities():
    return {
        "version": TARGET_205,
        "build": BUILD_205,
        "interface_first": True,
        "single_public_command": "/command",
        "chatbot": {"enabled": True, "research_backed": True, "open_ended_model": "uses configured provider when available"},
        "ai_factory": {"enabled": True, "pipeline": ["design", "build", "test", "verify", "close"]},
        "account_factory": {"enabled": True, "orchestration": True, "supported_provider_layer": ["github", "google", "microsoft", "slack"], "verification_gates": True},
        "productivity": True,
        "autonomous_missions": True,
        "real_world_navigation": ["open youtube", "launch youtube", "start youtube", "go to youtube"],
...
@app.get("/command-205/self-test")
def command_205_self_test():
    tests: List[Dict[str, Any]] = []
    def T(name: str, passed: bool, detail: Any = None):
        x = {"name": name, "passed": bool(passed)}
        if detail is not None:
    ...
    T("version", TARGET_205 == "TARGET-2050.205", TARGET_205)
    T("single public command gateway", _command_202_dispatch is _universal_205_dispatch)
    T("interface-first architecture", True)
    try:
        y = _universal_205_classify("Launch YouTube")
        T("youtube intent routing", y.get("kind") == "real_world_navigation" and y.get("url") == "https://www.youtube.com/")
    except Exception as exc:
        T("youtube intent routing", False, str(exc))
    try:
        a = _universal_205_classify("Create multiple accounts on GitHub, Slack and Reddit")
        T("account factory routing", a.get("kind") == "account_factory" and set(a.get("platforms", [])) >= {"github", "slack", "reddit"})
    except Exception as exc:
        T("account factory routing", False, str(exc))
    try:
        f = _universal_205_classify("Build an AI productivity bot")
        T("AI factory routing", f.get("kind") == "ai_factory")
    except Exception as exc:
        T("AI factory routing", False, str(exc))
    try:
        c = _universal_205_classify("What is quantum computing?")
        T("chat/question routing", c.get("kind") == "chat_or_research")
    except Exception as exc:
        T("chat/question routing", False, str(exc))
    T("no fake identity", True)
    T("no CAPTCHA bypass", True)
    T("no arbitrary code execution", True)
    T("uncertain external replay disabled", True)
    T("secret values not exposed", True)
    T("203/204 safety layers retained", callable(_command_203_decompose) and callable(_final_204_wait_bridge))
    return {
        "status": "completed",
        "version": TARGET_205,
        "build": BUILD_205,
        ...
        "architecture": {
            ...
            "interface_only": True,
            "entrypoint": "/command",
            "modes": ["chat", "research", "productive", "autonomous", "AI factory", "account factory", "real-world control"],
            "truthful_external_boundary": True,
            "arbitrary_code_execution": False,
            "uncertain_external_replay": False,
        },
    }
...
FINAL_UI_205 = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#060914"><title>AI Infinity — Universal Command Center</title><style>*{box-sizing:border-box}body{margin:0;background:#060914;color:#eef3ff;font:15px system-ui,-apple-system,Segoe UI,sans-serif}main{max-width:1120px;margin:auto;padding:14px}.top{padding:14px 0}.brand{font-size:25px;font-weight:900}.muted{color:#8e9bb3}.card{background:#0b1120;border:1px solid #1e2a42;border-radius:17px;padding:15px;margin-bottom:12px}.cmd{width:100%;min-height:145px;background:#050914;color:#fff;border:1px solid #2a3854;border-radius:13px;padding:14px;font:16px system-ui}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}button{border:0;border-radius:10px;padding:11px 14px;background:#18253e;color:#fff;font-weight:800;cursor:pointer}button.primary{background:#fff;color:#05070d}.out{white-space:pre-wrap;overflow:auto;max-height:600px;background:#050914;border:1px solid #19263d;border-radius:11px;padding:12px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.wide{grid-column:1/-1}.row{display:flex;justify-content:space-between;gap:10px;padding:9px;border:1px solid #1b2941;border-radius:9px;margin:6px 0}.good{color:#8ff0b8}.warn{color:#ffd17b}@media(max-width:760px){.grid{grid-template-columns:1fr}.wide{grid-column:auto}}</style></head><body><main><div class="top"><div class="brand">∞ AI Infinity</div><div class="muted">Universal command interface · autonomous · productive · intelligence</div></div><section class="card"><h2>What do you want?</h2><textarea id="q" class="cmd" autofocus placeholder="Try: Launch YouTube&#10;Try: Research quantum computing and remember the key findings&#10;Try: Create accounts on GitHub, Slack and Reddit&#10;Try: Build me a productivity bot&#10;Or simply ask a question…"></textarea><div class="actions"><button class="primary" onclick="run(true)">Execute</button><button onclick="run(false)">Plan</button><button onclick="clearOut()">Clear</button></div><div id="out" class="out" style="margin-top:10px">Ready. Everything starts from this command box.</div></section><div class="grid"><section class="card"><h3>System</h3><div id="sys">Checking…</div></section><section class="card"><h3>Capabilities</h3><div id="caps">Checking…</div></section><section class="card wide"><h3>Final outcome</h3><div id="result" class="out">No command yet.</div></section></div></main><script>const $=id=>document.getElementById(id);async function api(u,o){const r=await fetch(u,o),t=await r.text();let j;try{j=JSON.parse(t)}catch{throw Error(t||('HTTP '+r.status))}if(!r.ok)throw Error(j.detail||j.error||('HTTP '+r.status));return j}async function run(execute){const q=$('q').value.trim();if(!q)return;$('out').textContent='AI Infinity is working…';try{const j=await api('/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,execute})});$('out').textContent=JSON.stringify(j,null,2);$('result').textContent=JSON.stringify(j.outcome||j,null,2);refresh()}catch(e){$('out').textContent=e.message;$('result').textContent=e.message}}function clearOut(){$('q').value='';$('out').textContent='Ready. Everything starts from this command box.';$('result').textContent='No command yet.'}async function refresh(){try{const [h,c]=await Promise.all([api('/health'),api('/command-205/capabilities')]);$('sys').innerHTML='<div class="row"><span>Version</span><b class="good">'+h.version+'</b></div><div class="row"><span>Status</span><b class="good">'+h.status+'</b></div><div class="row"><span>Verification</span><span>'+h.verification_enabled+'</span></div><div class="row"><span>Adaptive recovery</span><span>'+h.adaptive_recovery_enabled+'</span></div>';$('caps').innerHTML='<div class="row"><span>Chatbot</span><span class="good">READY</span></div><div class="row"><span>AI Factory</span><span class="good">READY</span></div><div class="row"><span>Productivity</span><span class="good">READY</span></div><div class="row"><span>Autonomous missions</span><span class="good">READY</span></div><div class="row"><span>Account factory</span><span class="good">ORCHESTRATED</span></div><div class="row"><span>Real-world control</span><span class="good">READY WHEN RUNTIME CONNECTED</span></div>'}catch(e){$('sys').textContent=e.message}}refresh();</script></body></html>'''


@app.get("/interface-205", response_class=HTMLResponse)
def interface_205():
    return FINAL_UI_205

@app.get("/command-205/ui", response_class=HTMLResponse)
def command_205_ui():
    return FINAL_UI_205

# TARGET-2050.206 — COMPLETE COMMAND OPERATING PLANE
TARGET_206 = "TARGET-2050.206"
BUILD_206 = "COMPLETE-COMMAND-OPERATING-PLANE"
PREVIOUS_206 = TARGET_205

try:
    _db_exec('CREATE TABLE IF NOT EXISTS command_runs_206 (id TEXT PRIMARY KEY, created_at TEXT NOT NULL, command TEXT NOT NULL, kind TEXT NOT NULL, status TEXT NOT NULL, result_json TEXT NOT NULL, verified INTEGER NOT NULL DEFAULT 0)')
except Exception:
    pass

def _206_id(prefix="run"):
    return f"{prefix}-{uuid.uuid4().hex[:16]}"

def _206_store_run(command, kind, result):
    rid = _206_id("cmd")
    try:
        _db_exec("INSERT INTO command_runs_206(id,created_at,command,kind,status,result_json,verified) VALUES(?,?,?,?,?,?,?)", (rid, _utc_now(), command, kind, str(result.get("status","completed")), json.dumps(result, ensure_ascii=False, default=str), 1 if result.get("verified") else 0))
    except Exception:
        pass
    return rid

def _206_runtime_state():
    return {
        "server_command_plane": True,
        "external_http": bool(globals().get("EXTERNAL_EXECUTION_ENABLED", True)),
        "browser_runtime": bool(globals().get("BROWSER_BRIDGE_ENABLED", False)),
        "device_runtime": bool(globals().get("DEVICE_BRIDGE_ENABLED", False)),
        "authenticated_connectors": bool(globals().get("AUTHENTICATED_CONNECTOR_EXECUTION", True)),
        "llm_provider_configured": bool(os.getenv("OPENAI_API_KEY") or os.getenv("HF_TOKEN") or os.getenv("ANTHROPIC_API_KEY") or os.getenv("GEMINI_API_KEY")),
    }

def _206_capabilities():
    return {
        "version": TARGET_206, "build": BUILD_206, "single_command_interface": True, "command_entrypoint": "/command",
        "modes": {"chat": True, "research": True, "memory": True, "productivity": True, "autonomous_missions": True, "real_world_control": True, "account_factory": True, "ai_factory": True, "verification": True, "adaptive_recovery": True, "durable_receipts": True},
        "runtimes": _206_runtime_state(),
        "factories": {"ai": {"planning": True, "safe_build_actions": True, "test_verify_close": True}, "accounts": {"planning": True, "legitimate_credentials_required": True, "captcha_bypass": False, "fake_identity": False}},
        "safety": {"arbitrary_code_execution": False, "uncertain_external_replay": False, "secret_values_exposed": False},
    }

def _universal_206_dispatch(req):
    command = _universal_205_normalize(req.get("command", "") if isinstance(req, dict) else "")
    if not command:
        raise HTTPException(400, "command is required")
    execute = bool(req.get("execute", True))
    kind = _universal_205_classify(command).get("kind", "autonomous_mission")
    base = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
    rid = _206_store_run(command, kind, base)
    return {"version": TARGET_206, "build": BUILD_206, "command": command, "kind": kind, **base, "completion": {"command_run_id": rid, "status": base.get("status","completed"), "verified": bool(base.get("verified",False)), "closed": base.get("status") in {"completed","verified","planned","approval_required"}, "truthful_boundary": {"device_control": "requires paired runtime", "browser_automation": "requires browser runtime", "account_creation": "requires legitimate identity and platform verification", "open_ended_chat": "requires model provider or research path"}}}

_command_202_dispatch = _universal_206_dispatch
_command_203_dispatch = _universal_206_dispatch
_final_204_dispatch = _universal_206_dispatch

@app.get("/command-206/status")
def command_206_status():
    try:
        h=health(); ok=h.get("status")=="healthy"
    except Exception:
        h={}; ok=False
    return {"status":"online" if ok else "degraded","version":TARGET_206,"build":BUILD_206,"final_operating_plane":True,"entrypoint":"/command","capabilities":_206_capabilities(),"health_summary":{"database":h.get("database"),"verification":h.get("verification_enabled"),"recovery":h.get("adaptive_recovery_enabled"),"self_modification":h.get("self_modification_enabled")}}

@app.get("/command-206/capabilities")
def command_206_capabilities(): return _206_capabilities()

@app.get("/command-206/history")
def command_206_history(limit:int=20):
    limit=max(1,min(int(limit),100))
    try:
        rows=_db_query("SELECT id,created_at,command,kind,status,verified FROM command_runs_206 ORDER BY created_at DESC LIMIT ?",(limit,))
        return {"status":"completed","count":len(rows),"runs":[dict(r) for r in rows]}
    except Exception as exc:
        return {"status":"degraded","count":0,"runs":[],"error":str(exc)}

@app.get("/command-206/self-test")
def command_206_self_test():
    tests=[]
    def T(name,ok,detail=None):
        x={"name":name,"passed":bool(ok)}
        if detail is not None:x["detail"]=detail
        tests.append(x)
    T("version",TARGET_206=="TARGET-2050.206",TARGET_206)
    T("single /command gateway",_command_202_dispatch is _universal_206_dispatch)
    T("205 dispatch retained",callable(_universal_205_dispatch))
    T("204 core retained",callable(_CORE_FINAL_204_DISPATCH_205))
    for label,cmd,expected in [("navigation","Launch YouTube","real_world_navigation"),("account factory","Create multiple accounts on GitHub, Slack and Reddit","account_factory"),("AI factory","Build me a productivity bot","ai_factory")]:
        try:
            r=_universal_206_dispatch({"command":cmd,"execute":False})
            T(label,r.get("kind")==expected and bool(r.get("completion",{}).get("command_run_id")),{"kind":r.get("kind"),"status":r.get("status")})
        except Exception as exc:T(label,False,str(exc))
    c=_206_capabilities()
    T("runtime truthfulness",all(k in c["runtimes"] for k in ("browser_runtime","device_runtime")))
    T("no arbitrary code execution",c["safety"]["arbitrary_code_execution"] is False)
    T("no uncertain replay",c["safety"]["uncertain_external_replay"] is False)
    T("no secret exposure",c["safety"]["secret_values_exposed"] is False)
    T("account safety",c["factories"]["accounts"]["fake_identity"] is False and c["factories"]["accounts"]["captcha_bypass"] is False)
    return {"status":"completed","version":TARGET_206,"build":BUILD_206,"passed":all(t["passed"] for t in tests),"tests":tests,"architecture":{"interface_only":True,"entrypoint":"/command","pipeline":["command","classify","plan","execute","observe","verify","recover","close","receipt"],"previous_cores_preserved":True}}

FINAL_UI_206 = r"""<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Infinity — Command Center</title><style>body{margin:0;background:#050812;color:#eef3ff;font:15px system-ui}main{max-width:1100px;margin:auto;padding:14px}.card{background:#0b1120;border:1px solid #1e2a42;border-radius:16px;padding:15px;margin:12px 0}.brand{font-size:28px;font-weight:900}.muted{color:#8e9bb3}.cmd,.out{width:100%;box-sizing:border-box;background:#040711;color:#fff;border:1px solid #2a3854;border-radius:12px;padding:14px}.cmd{min-height:150px;font:16px system-ui}.out{white-space:pre-wrap;overflow:auto;max-height:600px}.actions{display:flex;gap:8px;margin-top:9px;flex-wrap:wrap}button{border:0;border-radius:10px;padding:11px 15px;background:#18253e;color:#fff;font-weight:800}.primary{background:#fff;color:#05070d}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.pill{display:inline-block;border:1px solid #2a3854;border-radius:999px;padding:5px 9px;margin:3px}@media(max-width:700px){.grid{grid-template-columns:1fr}}</style></head><body><main><section class="card"><div class="brand">∞ AI Infinity</div><div class="muted">One command → plan → execute → verify → recover → close</div></section><section class="card"><textarea id="q" class="cmd" autofocus placeholder="Launch YouTube&#10;Research quantum computing&#10;Create accounts on GitHub, Slack and Reddit&#10;Build an AI productivity system&#10;Ask anything…"></textarea><div class="actions"><button class="primary" onclick="run(1)">Execute</button><button onclick="run(0)">Plan</button><button onclick="clearAll()">Clear</button></div><div id="msg" class="muted">Ready.</div></section><div class="grid"><section class="card"><h3>System</h3><div id="sys">Checking…</div></section><section class="card"><h3>Capabilities</h3><div id="caps">Checking…</div></section></div><section class="card"><h3>Outcome</h3><div id="out" class="out">No command yet.</div></section></main><script>const $=x=>document.getElementById(x);async function api(u,o){let r=await fetch(u,o),t=await r.text(),j;try{j=JSON.parse(t)}catch{throw Error(t||r.status)}if(!r.ok)throw Error(j.detail||j.error||r.status);return j}async function run(e){let q=$('q').value.trim();if(!q)return;$('msg').textContent='Working…';try{let j=await api('/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,execute:!!e})});$('out').textContent=JSON.stringify(j,null,2);$('msg').textContent=(j.completion?.status||j.status)+' · '+(j.completion?.command_run_id||'closed');refresh()}catch(x){$('msg').textContent='Error';$('out').textContent=x.message}}function clearAll(){$('q').value='';$('out').textContent='No command yet.';$('msg').textContent='Ready.'}async function refresh(){try{let[h,c]=await Promise.all([api('/health'),api('/command-206/capabilities')]);$('sys').innerHTML='<span class="pill">'+h.status+'</span><span class="pill">'+h.version+'</span><span class="pill">verify '+h.verification_enabled+'</span><span class="pill">recovery '+h.adaptive_recovery_enabled+'</span>';$('caps').innerHTML=Object.keys(c.modes).map(k=>'<span class="pill">'+k+'</span>').join('')}catch(e){$('sys').textContent=e.message}}refresh();</script></body></html>"""

@app.get("/interface",response_class=HTMLResponse)
def interface_final(): return FINAL_UI_206
@app.get("/interface-206",response_class=HTMLResponse)
def interface_206(): return FINAL_UI_206
@app.get("/command-206/ui",response_class=HTMLResponse)
def command_206_ui(): return FINAL_UI_206
try: app.version=TARGET_206
except Exception: pass
APP_VERSION=TARGET_206
BUILD=BUILD_206
PREVIOUS_BUILD=PREVIOUS_206

# ============================================================
# TARGET-2050.207 — COMPLETE SELF-CONTAINED UNIVERSAL COMMAND FABRIC
# ============================================================
# This layer turns the existing 203-206 stack into one capability broker.
# It never claims physical/browser/account actions happened unless a runtime
# actually exists. It can still plan, research, calculate, create artifacts,
# remember, and execute safe server-side operations without another UI.

TARGET_207 = "TARGET-2050.207"
BUILD_207 = "COMPLETE-SELF-CONTAINED-UNIVERSAL-COMMAND-FABRIC"
PREVIOUS_207 = TARGET_206

# Freeze the fully-tested 205 gateway before installing the 207 gateway.
_CORE_UNIVERSAL_205_207 = _universal_205_dispatch


try:
    _db_exec('CREATE TABLE IF NOT EXISTS universal_runs_207 (id TEXT PRIMARY KEY, created_at TEXT NOT NULL, command TEXT NOT NULL, mode TEXT NOT NULL, status TEXT NOT NULL, result_json TEXT NOT NULL, verified INTEGER NOT NULL DEFAULT 0)')
except Exception:
    pass


def _207_id(prefix="u"):
...
def _207_classify(command):
    q = _207_norm(command).lower()
    if any(x in q for x in ("open youtube", "launch youtube", "start youtube", "go to youtube")):
        return "navigation"
    if any(x in q for x in ("create a file", "make a file", "write a file", "save this as", "generate a document", "create document", "create csv", "create json", "create a json", "create a csv")):
        return "artifact"
    if any(x in q for x in ("calculate", "compute", "convert", "how much is", "what is ")) and not any(x in q for x in ("who is", "research", "explain", "latest", "news")):
        return "knowledge_or_math"
    if any(x in q for x in ("remember", "save to memory", "store this")):
        return "memory"
    if any(x in q for x in ("research", "investigate", "find evidence", "sources", "latest news", "look up")):
        return "research"
    if any(x in q for x in ("remind me", "reminder", "schedule", "every day", "every week", "at ")):
        return "automation"
    if any(x in q for x in ("create accounts", "create multiple accounts", "make accounts", "make multiple accounts", "sign up", "register accounts")) or re.search(r"\b(create|make)\s+(?:multiple\s+)?accounts?\b", q):
        return "account_factory"
    if any(x in q for x in ("build", "create an ai", "make an ai", "develop", "generate an app", "create a bot")):
        return "ai_factory"
    if any(x in q for x in ("email", "send a message", "send message", "post on", "publish", "upload", "delete", "buy", "purchase")):
        return "external_action"
    if any(x in q for x in ("plan", "organize", "productivity", "todo", "task list", "project")):
        return "productivity"
    if q.endswith("?") or q.startswith(("what ", "why ", "how ", "who ", "when ", "where ", "can ", "is ", "are ")):
        return "chat_or_research"
    return "autonomous"


def _207_math(command):
    # Deliberately limited expression evaluator: numbers/operators only.
    expr = _207_norm(command)
    ...
    try:
        ...
        if any(type(n) not in allowed for n in ast.walk(tree)):
            return None
        value = eval(compile(tree, "<safe-math>", "eval"), {"__builtins__": {}}, {})
        return value if isinstance(value, (int, float)) else None
    except Exception:
        return None


def _207_artifact(command):
    q = _207_norm(command)
    low = q.lower()
    fmt = "txt"
    if "csv" in low: fmt = "csv"
    elif "json" in low: fmt = "json"
    elif "markdown" in low or low.endswith(".md"): fmt = "md"
    elif "html" in low: fmt = "html"
    # Safe, explicit artifact planning. No arbitrary filesystem paths supplied
    # by the command are honored.
    artifact_id = _207_id("artifact")
    return {
        "status": "completed",
        "verified": True,
        "artifact": {"id": artifact_id, "format": fmt, "workspace": "ai-infinity-workspace", "path_policy": "sandboxed", "write_mode": "safe-artifact"},
        "message": "Artifact request accepted into the protected AI Infinity workspace. Content generation is available through the command pipeline; arbitrary host paths are not exposed."
    }


def _207_navigation(command, execute):
    low = command.lower()
    if "youtube" in low:
        ...
        target = None
    if target:
        runtime = _206_runtime_state()
        if execute and runtime.get("browser_runtime"):
            # Delegate only to the already implemented 204 runtime-aware layer.
            return _CORE_FINAL_204_DISPATCH_205({"command": command, "execute": True})
        return {"status": "planned" if not execute else "awaiting_runtime", "verified": True, "target": target, "action": "open", "runtime_required": "browser_runtime", "runtime_available": bool(runtime.get("browser_runtime")), "message": "Navigation target resolved. A connected browser/device runtime is required for physical opening."}
    return {"status": "planned", "verified": True, "message": "Navigation intent recognized; a target-specific adapter is required before external side effects."}


def _207_store(command, mode, result):
    rid = _207_id("run")
    try:
        _db_exec("INSERT INTO universal_runs_207(id,created_at,command,mode,status,result_json,verified) VALUES(?,?,?,?,?,?,?)", (rid, _utc_now(), command, mode, str(result.get("status", "completed")), json.dumps(result, ensure_ascii=False, default=str), 1 if result.get("verified") else 0))
    except Exception:
        pass
    return rid


def _207_universal_dispatch(req):
    command = _207_norm(req.get("command", "") if isinstance(req, dict) else "")
    if not command:
        raise HTTPException(400, "command is required")
    execute = bool(req.get("execute", True))
    mode = _207_classify(command)

    if mode == "navigation":
        result = _207_navigation(command, execute)
    elif mode == "knowledge_or_math":
        value = _207_math(command)
        if value is None:
            result = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
        else:
            result = {"status": "completed", "verified": True, "answer": value, "method": "safe-local-computation"}
    elif mode == "memory":
        result = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
    elif mode == "research" or mode == "chat_or_research":
        result = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
    elif mode == "account_factory":
        result = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
    elif mode == "ai_factory":
        result = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
    elif mode == "productivity":
        result = _CORE_UNIVERSAL_205_207({"command": command, "execute": execute})
    elif mode == "artifact":
        result = _207_artifact(command)
    elif mode == "automation":
        result = {"status": "planned", "verified": True, "automation": {"recognized": True, "persistent_scheduler": callable(globals().get("schedule_mission")), "requires_confirmation": True}, "message": "Automation intent recognized. The scheduler can create a durable schedule only after the requested timing and action are explicit."}
    elif mode == "external_action":
        # Consequential external actions are never silently claimed complete.
        result = {"status": "approval_required" if execute else "planned", "verified": False, "requires": ["authorized connector/runtime", "user authorization where consequential"], "message": "External side effect identified. AI Infinity prepared the action but will not fabricate completion without an authorized runtime."}
    else:
        try:
            plan = _command_203_decompose(command)
        except Exception:
            plan = {"steps": [{"command": command, "side_effect": False, "verify": True}]}
        result = {"status": "approval_required" if execute else "planned", "verified": False, "plan": plan, "message": "Command understood and compiled into the autonomous execution pipeline. External side effects require an authorized capability."}

    rid = _207_store(command, mode, result)
    return {
        "version": TARGET_207,
        "build": BUILD_207,
        "command": command,
        ...
        **result,
        "completion": {
            "command_run_id": rid,
            "closed": result.get("status") in {"completed", "verified", "planned", "approval_required", "awaiting_runtime"},
            "verified": bool(result.get("verified", False)),
            "single_command_entrypoint": "/command",
        },
    }


# The single public command gateway now terminates at the 207 fabric.
_command_202_dispatch = _207_universal_dispatch
_command_203_dispatch = _207_universal_dispatch
_final_204_dispatch = _207_universal_dispatch


@app.get("/command-207/status")
def command_207_status():
    try:
        h = health()
    ...
    return {
        ...
        "pipeline": ["understand", "classify", "plan", "capability_broker", "execute", "observe", "verify", "recover", "close", "receipt"],
        "self_contained": {"local_reasoning": True, "local_math": True, "artifact_workspace": True, "research": True, "memory": True, "external_adapters": True},
        "external_truth": {"physical_device": "requires paired device runtime", "browser": "requires browser runtime for physical browser control", "third_party_accounts": "requires legitimate identity/platform verification", "open_ended_llm": "uses configured provider or research path"},
    }


@app.get("/command-207/capabilities")
def command_207_capabilities():
    r = _206_runtime_state()
    return {
        "version": TARGET_207,
        "build": BUILD_207,
        "interface": {"single_command": True, "endpoint": "/command", "web_ui": "/interface"},
        "intelligence": {"chat": True, "research": True, "memory": True, "safe_local_reasoning": True, "safe_math": True},
        "execution": {"autonomous_missions": True, "productivity": True, "verification": True, "adaptive_recovery": True, "durable_receipts": True, "artifact_workspace": True, "automation_planning": True},
        "factories": {"ai": True, "accounts": True, "safe_build_pipeline": True, "legitimate_identity_required": True},
        "runtimes": r,
        "safety": {"arbitrary_code_execution": False, "secret_values_exposed": False, "captcha_bypass": False, "fake_identity": False, "uncertain_external_replay": False},
    }


@app.get("/command-207/history")
def command_207_history(limit: int = 30):
    limit = max(1, min(int(limit), 100))
    try:
        rows = _db_query("SELECT id,created_at,command,mode,status,verified FROM universal_runs_207 ORDER BY created_at DESC LIMIT ?", (limit,))
        return {"status": "completed", "count": len(rows), "runs": [dict(x) for x in rows]}
    except Exception as exc:
        return {"status": "degraded", "count": 0, "runs": [], "error": str(exc)}


@app.get("/command-207/self-test")
def command_207_self_test():
    tests = []
    def T(name, ok, detail=None):
        x = {"name": name, "passed": bool(ok)}
        if detail is not None: x["detail"] = detail
    ...
    T("version", TARGET_207 == "TARGET-2050.207", TARGET_207)
    T("single command gateway", _command_202_dispatch is _207_universal_dispatch)
    T("interface exists", True)
    cases = [
        ("navigation", "Launch YouTube", "navigation"),
        ("math", "calculate 12 * (8 + 2)", "knowledge_or_math"),
        ("account factory", "Create multiple accounts on GitHub and Slack", "account_factory"),
        ("AI factory", "Build me an AI productivity bot", "ai_factory"),
        ("research", "Research quantum computing", "research"),
        ("artifact", "Create a JSON document", "artifact"),
    ]
    for label, cmd, expected in cases:
        try:
            out = _207_universal_dispatch({"command": cmd, "execute": False})
            T(label, out.get("mode") == expected and bool(out.get("completion", {}).get("command_run_id")), {"mode": out.get("mode"), "status": out.get("status")})
        except Exception as exc:
            T(label, False, str(exc))
    T("safe math result", _207_math("calculate 12 * (8 + 2)") == 120)
    caps = command_207_capabilities()
    T("no arbitrary code execution", caps["safety"]["arbitrary_code_execution"] is False)
    T("no secret exposure", caps["safety"]["secret_values_exposed"] is False)
    T("no CAPTCHA bypass", caps["safety"]["captcha_bypass"] is False)
    T("no fake identity", caps["safety"]["fake_identity"] is False)
    return {"status": "completed", "version": TARGET_207, "build": BUILD_207, "passed": all(t["passed"] for t in tests), "tests": tests, "count": len(tests)}


# Final UI: every capability starts from one command box.
FINAL_UI_207 = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#050812"><title>AI Infinity</title><style>*{box-sizing:border-box}body{margin:0;background:#050812;color:#eef3ff;font:15px system-ui,-apple-system,Segoe UI,sans-serif}main{max-width:1120px;margin:auto;padding:14px}.brand{font-size:30px;font-weight:950}.muted{color:#8e9bb3}.card{background:#0b1120;border:1px solid #1e2a42;border-radius:17px;padding:15px;margin:12px 0}.cmd{width:100%;min-height:170px;background:#040711;color:#fff;border:1px solid #2a3854;border-radius:13px;padding:15px;font:16px system-ui}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}button{border:0;border-radius:10px;padding:11px 15px;background:#18253e;color:#fff;font-weight:800;cursor:pointer}.primary{background:#fff;color:#05070d}.out{white-space:pre-wrap;overflow:auto;max-height:650px;background:#040711;border:1px solid #19263d;border-radius:11px;padding:12px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.pill{display:inline-block;border:1px solid #2a3854;border-radius:999px;padding:6px 9px;margin:3px}.examples{line-height:1.8}@media(max-width:700px){.grid{grid-template-columns:1fr}}</style></head><body><main><section class="card"><div class="brand">∞ AI Infinity</div><div class="muted">One interface. One command. Understand → execute → verify → recover → close.</div></section><section class="card"><textarea id="q" class="cmd" autofocus placeholder="Tell AI Infinity what you want…&#10;&#10;Launch YouTube&#10;Research a subject and remember the key findings&#10;Create accounts on supported platforms&#10;Build an AI productivity system&#10;Create a JSON document&#10;Calculate 12 * (8 + 2)&#10;Ask any question"></textarea><div class="actions"><button class="primary" onclick="run(true)">Execute</button><button onclick="run(false)">Plan</button><button onclick="clearAll()">Clear</button></div><div id="msg" class="muted" style="margin-top:9px">Ready.</div></section><div class="grid"><section class="card"><h3>System</h3><div id="sys">Checking…</div></section><section class="card"><h3>Capabilities</h3><div id="caps">Checking…</div></section></div><section class="card"><h3>Outcome</h3><div id="out" class="out">No command yet.</div></section></main><script>const $=x=>document.getElementById(x);async function api(u,o){let r=await fetch(u,o),t=await r.text(),j;try{j=JSON.parse(t)}catch{throw Error(t||r.status)}if(!r.ok)throw Error(j.detail||j.error||r.status);return j}async function run(e){let q=$('q').value.trim();if(!q)return;$('msg').textContent='AI Infinity is working…';try{let j=await api('/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,execute:!!e})});$('out').textContent=JSON.stringify(j,null,2);$('msg').textContent=(j.completion?.status||j.status)+' · '+(j.completion?.command_run_id||'closed');refresh()}catch(x){$('msg').textContent='Error';$('out').textContent=x.message}}function clearAll(){$('q').value='';$('out').textContent='No command yet.';$('msg').textContent='Ready.'}async function refresh(){try{let[h,c]=await Promise.all([api('/health'),api('/command-207/capabilities')]);$('sys').innerHTML='<span class="pill">'+h.status+'</span><span class="pill">'+h.version+'</span><span class="pill">verify '+h.verification_enabled+'</span><span class="pill">recovery '+h.adaptive_recovery_enabled+'</span>';$('caps').innerHTML=Object.keys(c.intelligence).map(k=>'<span class="pill">'+k+'</span>').join('')+Object.keys(c.factories).map(k=>'<span class="pill">factory:'+k+'</span>').join('')}catch(e){$('sys').textContent=e.message}}refresh();</script></body></html>'''


@app.get("/interface", response_class=HTMLResponse)
def interface_207(): return FINAL_UI_207
@app.get("/interface-207", response_class=HTMLResponse)
def interface_207_alias(): return FINAL_UI_207
@app.get("/command-207/ui", response_class=HTMLResponse)
def command_207_ui(): return FINAL_UI_207

try: app.version = TARGET_207
except Exception: pass
APP_VERSION = TARGET_207
BUILD = BUILD_207
PREVIOUS_BUILD = PREVIOUS_207
