...
app = FastAPI(title="AI Infinity", version="TARGET-2050.109")

# ============================================================
# VERSION
# ============================================================

APP_VERSION = "TARGET-2050.110"
BUILD = "ADAPTIVE-AUTONOMOUS-MISSION-ORCHESTRATION-CORE"
...
def health():
    ...
    return {
        ...
        "core": {
            ...
            "decision_trace":
                True,
            "adaptive_mission_orchestration":
                True,
            ...
            "provider_selection":
                True,
            "prediction_guided_scheduling":
                True,
            "failure_aware_execution":
                True,
            "partial_result_recovery":
                True,
            "execution_budgeting":
                True,
            "mission_deadline_control":
                True,
            "continuous_observe_decide_act":
                True,
            "execution_trace":
                True,
        },
...
def capabilities():
    ...
    return {
        ...
        "capabilities": [
            ...
            "execution-budgeting",
            "mission-deadline-control",
            "continuous-observe-decide-act",
            "execution-trace",
        ],
    }
...
# ============================================================
# TARGET-2050.110 ADAPTIVE AUTONOMOUS MISSION ORCHESTRATION
# ============================================================

ORCH_MAX_STEPS = max(1, min(50, int(os.getenv("AI_INFINITY_ORCH_MAX_STEPS", "20"))))
ORCH_MAX_ATTEMPTS = max(1, min(5, int(os.getenv("AI_INFINITY_ORCH_MAX_ATTEMPTS", "2"))))
ORCH_DEFAULT_BUDGET_SECONDS = max(5, min(3600, int(os.getenv("AI_INFINITY_ORCH_BUDGET_SECONDS", "300"))))
...
def _orch_schema():
    with DB_LOCK:
        conn = db()
        try:
            conn.execute("""CREATE TABLE IF NOT EXISTS orchestration_missions(
                id TEXT PRIMARY KEY, mission_id TEXT, objective TEXT NOT NULL,
                status TEXT NOT NULL, state_json TEXT NOT NULL DEFAULT '{}',
                budget_seconds INTEGER NOT NULL, deadline REAL, created_at REAL NOT NULL,
                updated_at REAL NOT NULL, result_json TEXT, error TEXT)""")
            conn.execute("""CREATE TABLE IF NOT EXISTS orchestration_steps(
                id TEXT PRIMARY KEY, orchestration_id TEXT NOT NULL, step_index INTEGER NOT NULL,
                title TEXT NOT NULL, action_type TEXT NOT NULL, target TEXT NOT NULL,
                payload_json TEXT NOT NULL DEFAULT '{}', dependencies_json TEXT NOT NULL DEFAULT '[]',
                risk TEXT NOT NULL DEFAULT 'low', side_effecting INTEGER NOT NULL DEFAULT 0,
                priority REAL NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'pending',
                attempts INTEGER NOT NULL DEFAULT 0, resource_id TEXT, transaction_id TEXT,
                result_json TEXT, error TEXT, created_at REAL NOT NULL, updated_at REAL NOT NULL,
                UNIQUE(orchestration_id,step_index))""")
            conn.execute("""CREATE TABLE IF NOT EXISTS orchestration_events(
                id INTEGER PRIMARY KEY AUTOINCREMENT, orchestration_id TEXT NOT NULL,
                step_id TEXT, stage TEXT NOT NULL, event TEXT NOT NULL,
                data_json TEXT, ts REAL NOT NULL)""")
            conn.execute("""CREATE TABLE IF NOT EXISTS orchestration_resources(
                id TEXT PRIMARY KEY, name TEXT NOT NULL, kind TEXT NOT NULL,
                capabilities_json TEXT NOT NULL DEFAULT '[]', cost REAL NOT NULL DEFAULT 0,
                trust REAL NOT NULL DEFAULT 0.5, latency_ms REAL NOT NULL DEFAULT 0,
                success_rate REAL NOT NULL DEFAULT 1, active INTEGER NOT NULL DEFAULT 1,
                updated_at REAL NOT NULL)""")
            conn.execute("""CREATE TABLE IF NOT EXISTS orchestration_leases(
                orchestration_id TEXT PRIMARY KEY, owner TEXT NOT NULL, lease_until REAL NOT NULL,
                updated_at REAL NOT NULL)""")
            conn.commit()
        finally:
            conn.close()
...
def _orch_json(value):
    return json.dumps(value if value is not None else {}, ensure_ascii=False, separators=(",", ":"))


def _orch_event(oid, stage, event, data=None, step_id=None):
    write("INSERT INTO orchestration_events(orchestration_id,step_id,stage,event,data_json,ts) VALUES(?,?,?,?,?,?)",
          (oid, step_id, stage, event, _orch_json(data), now()))


def _orch_prediction_adjustment():
    try:
        ctx = _world_planning_context(300)
        conf = float(ctx.get("confidence", 0) or 0)
        anomalies = ctx.get("anomalies") or []
        anomaly_penalty = min(0.35, 0.08 * len(anomalies))
        return max(0.0, min(1.0, conf)), anomaly_penalty
    except Exception:
        return 0.0, 0.0


def _orch_resource_candidates(capability):
    rows = q("SELECT * FROM orchestration_resources WHERE active=1 ORDER BY trust DESC, success_rate DESC, cost ASC, latency_ms ASC")
    out=[]
    for r in rows:
        caps=json.loads(r["capabilities_json"] or "[]")
        if not capability or capability in caps or "*" in caps:
            out.append(dict(r))
    return out


def _orch_choose_resource(action_type):
    candidates=_orch_resource_candidates(action_type)
    if not candidates:
        return None
    def score(r):
        return (float(r.get("trust",0.5))*0.45 + float(r.get("success_rate",1))*0.4
                - min(1.0,float(r.get("cost",0)))*0.1 - min(10000,float(r.get("latency_ms",0)))/10000*0.05)
    return max(candidates, key=score)


def _orch_validate_steps(steps):
    if not isinstance(steps, list) or not steps or len(steps)>ORCH_MAX_STEPS:
        raise HTTPException(400, f"steps_required_max_{ORCH_MAX_STEPS}")
    normalized=[]
    indexes=set()
    for i, raw in enumerate(steps):
        if not isinstance(raw, dict): raise HTTPException(400,"invalid_step")
        idx=int(raw.get("step_index", i))
        if idx in indexes: raise HTTPException(400,"duplicate_step_index")
        indexes.add(idx)
        deps=[int(x) for x in (raw.get("dependencies") or raw.get("depends_on") or [])]
        if idx in deps or any(x not in range(len(steps)) and x not in indexes for x in deps):
            # Forward references are allowed; final graph validation catches missing nodes.
            pass
        action=str(raw.get("action_type") or raw.get("action") or "save_result")
        if action not in SAFE_ACTION_ALLOWLIST:
            raise HTTPException(403, f"unregistered_action:{action}")
        side=bool(SAFE_ACTION_ALLOWLIST[action].get("side_effects"))
        target=str(raw.get("target") or "")
        payload=raw.get("payload") if isinstance(raw.get("payload"),dict) else (raw.get("body") if isinstance(raw.get("body"),dict) else {})
        risk=str(raw.get("risk") or ("external_side_effect" if side else "low"))
        normalized.append({"step_index":idx,"title":str(raw.get("title") or f"Step {idx+1}"),"action_type":action,
                           "target":target,"payload":payload,"dependencies":deps,"risk":risk,"side_effecting":side,
                           "priority":float(raw.get("priority",0) or 0)})
    ids={x["step_index"] for x in normalized}
    for x in normalized:
        if any(d not in ids for d in x["dependencies"]): raise HTTPException(400,"dependency_reference_missing")
    graph={x["step_index"]:x["dependencies"] for x in normalized}
    def visit(n,path):
        if n in path: raise HTTPException(400,"dependency_cycle")
        return all(visit(d,path|{n}) for d in graph.get(n,[]))
    for n in graph: visit(n,set())
    return normalized


def _orch_ready_steps(oid):
    rows=q("SELECT * FROM orchestration_steps WHERE orchestration_id=? ORDER BY priority DESC, step_index ASC",(oid,))
    ready=[]
    for r in rows:
        if r["status"]!="pending": continue
        deps=json.loads(r["dependencies_json"] or "[]")
        if all((q("SELECT status FROM orchestration_steps WHERE orchestration_id=? AND step_index=?",(oid,d),one=True) or {"status":"missing"})["status"]=="completed" for d in deps):
            ready.append(dict(r))
    return ready


def _orch_view(oid):
    m=q("SELECT * FROM orchestration_missions WHERE id=?",(oid,),one=True)
    if not m: return None
    steps=q("SELECT * FROM orchestration_steps WHERE orchestration_id=? ORDER BY step_index",(oid,))
    events=q("SELECT * FROM orchestration_events WHERE orchestration_id=? ORDER BY id DESC LIMIT 100",(oid,))
    return {"orchestration_id":oid,"mission_id":m["mission_id"],"objective":m["objective"],"status":m["status"],
            "budget_seconds":m["budget_seconds"],"deadline":m["deadline"],"state":json.loads(m["state_json"] or "{}"),
            "result":json.loads(m["result_json"] or "null"),"error":m["error"],
            "steps":[{**dict(r),"payload":json.loads(r["payload_json"] or "{}"),"dependencies":json.loads(r["dependencies_json"] or "[]"),"result":json.loads(r["result_json"] or "null")} for r in steps],
            "events":[dict(e) for e in events]}


def _orch_simulate_steps(steps):
    pred_conf, anomaly_penalty=_orch_prediction_adjustment()
    out=[]
    for s in steps:
        priority=float(s["priority"])+pred_conf*0.5-(0.3 if s["side_effecting"] else 0)-anomaly_penalty
        resource=_orch_choose_resource(s["action_type"])
        out.append({"step_index":s["step_index"],"title":s["title"],"dependencies":s["dependencies"],
                    "side_effecting":s["side_effecting"],"priority":round(priority,4),
                    "resource":resource["id"] if resource else "builtin-safe-gateway",
                    "parallel_safe":not s["side_effecting"],
                    "approval_required":s["side_effecting"],"prediction_confidence":pred_conf,
...
class OrchestrationRequest(BaseModel):
    ...
    mission_id: Optional[str] = None
    budget_seconds: int = ORCH_DEFAULT_BUDGET_SECONDS
...
def orchestrator_simulate(request: OrchestrationRequest):
    steps=_orch_validate_steps(request.steps)
    schedule=_orch_simulate_steps(steps)
    batches=[]; remaining={x["step_index"]:set(x["dependencies"]) for x in steps}
    done=set()
    while remaining:
        batch=sorted([n for n,d in remaining.items() if d.issubset(done)], key=lambda n:(-next(x["priority"] for x in steps if x["step_index"]==n),n))
        if not batch: raise HTTPException(400,"dependency_cycle")
        safe=[n for n in batch if not next(x["side_effecting"] for x in steps if x["step_index"]==n)]
        risky=[n for n in batch if n not in safe]
        batches.append({"parallel_safe_steps":safe,"approval_gated_steps":risky})
        done.update(batch)
        for n in batch: remaining.pop(n)
    return {"status":"simulated","version":APP_VERSION,"schedule":schedule,"batches":batches,
            "approval_required":any(x["side_effecting"] for x in steps),"automatic_external_replay":False}


@app.post("/orchestrator")
def create_orchestration(request: OrchestrationRequest, background_tasks: BackgroundTasks):
    steps=_orch_validate_steps(request.steps)
    budget=max(5,min(3600,int(request.budget_seconds or ORCH_DEFAULT_BUDGET_SECONDS)))
    oid=make_id("orch")
    ts=now(); deadline=ts+int(request.deadline_seconds) if request.deadline_seconds else None
    pred_conf, anomaly_penalty=_orch_prediction_adjustment()
    mission_id=request.mission_id
    if not mission_id:
        mission_id=create_mission(MissionRequest(objective=request.objective,execute=False,require_approval=False),approved=True)
    write("INSERT INTO orchestration_missions(id,mission_id,objective,status,state_json,budget_seconds,deadline,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)",
          (oid,mission_id,request.objective,"queued",_orch_json({"prediction_confidence":pred_conf,"anomaly_penalty":anomaly_penalty,"approval_required":any(x["side_effecting"] for x in steps)}),budget,deadline,ts,ts))
    for s in steps:
        p=float(s["priority"])+pred_conf*0.5-(0.3 if s["side_effecting"] else 0)-anomaly_penalty
        write("INSERT INTO orchestration_steps(id,orchestration_id,step_index,title,action_type,target,payload_json,dependencies_json,risk,side_effecting,priority,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
              (make_id("ostep"),oid,s["step_index"],s["title"],s["action_type"],s["target"],_orch_json(s["payload"]),_orch_json(s["dependencies"]),s["risk"],int(s["side_effecting"]),p,"pending",ts,ts))
    _orch_event(oid,"create","orchestration_created",{"steps":len(steps),"budget_seconds":budget,"deadline":deadline})
    if request.execute:
        background_tasks.add_task(_orch_tick,oid)
    return {"status":"accepted","orchestration_id":oid,"mission_id":mission_id,"execution_started":bool(request.execute),"approval_required":any(x["side_effecting"] for x in steps),"automatic_external_replay":False}


def _orch_acquire(oid):
    owner=uuid.uuid4().hex
    until=now()+30
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("DELETE FROM orchestration_leases WHERE lease_until<?",(now(),))
            cur=conn.execute("INSERT OR IGNORE INTO orchestration_leases(orchestration_id,owner,lease_until,updated_at) VALUES(?,?,?,?)",(oid,owner,until,now()))
            conn.commit()
            return cur.rowcount==1, owner
        finally: conn.close()


def _orch_release(oid,owner):
    write("DELETE FROM orchestration_leases WHERE orchestration_id=? AND owner=?",(oid,owner))


def _orch_budget_ok(m):
    if now()-float(m["created_at"])>float(m["budget_seconds"]): return False,"budget_exhausted"
    if m["deadline"] and now()>float(m["deadline"]): return False,"deadline_exceeded"
    return True,None


def _orch_execute_step(oid, row):
    action=row["action_type"]; target=row["target"]; payload=json.loads(row["payload_json"] or "{}")
    if bool(row["side_effecting"]):
        # Never execute an external effect from the scheduler itself. Stage only.
        try:
            staged=execute_safe_gateway(SafeActionRequest(action_type=action,target=target,payload=payload,mission_id=row["orchestration_id"],require_approval=True))
        except Exception as exc:
            return {"status":"failed","approval_required":True,"error":str(exc)[:500]}
        return {"status":"awaiting_approval","approval_required":True,"transaction_id":staged.get("transaction_id"),"staged":True,"replay_blocked":True}
    try:
        result=_execute_safe_action(action,target,payload)
        return {"status":"completed" if result.get("ok") else "failed_closed","result":result,"resource":"builtin-safe-gateway"}
    except Exception as exc:
        return {"status":"failed_closed","error":str(exc)[:500]}


def _orch_tick(oid):
    acquired,owner=_orch_acquire(oid)
    if not acquired: return _orch_view(oid)
    try:
        m=q("SELECT * FROM orchestration_missions WHERE id=?",(oid,),one=True)
        if not m: return None
        ok,reason=_orch_budget_ok(m)
        if not ok:
            write("UPDATE orchestration_missions SET status='failed',error=?,updated_at=? WHERE id=?",(reason,now(),oid)); _orch_event(oid,"scheduler","budget_stop",{"reason":reason}); return _orch_view(oid)
        write("UPDATE orchestration_missions SET status='observing',updated_at=? WHERE id=?",(now(),oid)); _orch_event(oid,"observe","world_context_refreshed")
        ready=_orch_ready_steps(oid)
        if not ready:
            pending=q("SELECT COUNT(*) n FROM orchestration_steps WHERE orchestration_id=? AND status='pending'",(oid,),one=True)["n"]
            active=q("SELECT COUNT(*) n FROM orchestration_steps WHERE orchestration_id=? AND status IN ('executing','awaiting_approval')",(oid,),one=True)["n"]
            if pending==0 and active==0:
                write("UPDATE orchestration_missions SET status='completed',result_json=?,updated_at=? WHERE id=?",(_orch_json({"completed":True}),now(),oid)); _orch_event(oid,"verify","orchestration_completed")
            return _orch_view(oid)
        pred_conf,anomaly_penalty=_orch_prediction_adjustment()
        write("UPDATE orchestration_missions SET status='deciding',state_json=?,updated_at=? WHERE id=?",(_orch_json({"prediction_confidence":pred_conf,"anomaly_penalty":anomaly_penalty,"ready_steps":len(ready)}),now(),oid))
        # Safe read-only work may run in parallel; side effects are staged one at a time.
        safe=[r for r in ready if not r["side_effecting"]]
        gated=[r for r in ready if r["side_effecting"]]
        selected=safe[:8] if safe else gated[:1]
        if safe: _orch_event(oid,"schedule","parallel_safe_batch",{"steps":[r["step_index"] for r in selected]})
        else: _orch_event(oid,"schedule","approval_gated_step",{"step":selected[0]["step_index"]})
        gated_waiting = False
        for row in selected:
            ok,reason=_orch_budget_ok(m)
            if not ok: write("UPDATE orchestration_missions SET status='failed',error=?,updated_at=? WHERE id=?",(reason,now(),oid)); break
            write("UPDATE orchestration_steps SET status='executing',attempts=attempts+1,updated_at=? WHERE id=? AND status='pending'",(now(),row["id"]))
            _orch_event(oid,"execute","step_started",{"step_index":row["step_index"]},row["id"])
            result=_orch_execute_step(oid,row)
            status=result.get("status")
            if status=="awaiting_approval":
                gated_waiting = True
                write("UPDATE orchestration_steps SET status='awaiting_approval',transaction_id=?,result_json=?,updated_at=? WHERE id=?",(result.get("transaction_id"),_orch_json(result),now(),row["id"]))
                write("UPDATE orchestration_missions SET status='awaiting_approval',updated_at=? WHERE id=?",(now(),oid))
            elif status=="completed":
                write("UPDATE orchestration_steps SET status='completed',result_json=?,updated_at=? WHERE id=?",(_orch_json(result),now(),row["id"]))
                _orch_event(oid,"observe","step_completed",{"step_index":row["step_index"]},row["id"])
            else:
                write("UPDATE orchestration_steps SET status='failed',error=?,result_json=?,updated_at=? WHERE id=?",(result.get("error") or "step_failed",_orch_json(result),now(),row["id"]))
                write("UPDATE orchestration_missions SET status='replanning',error=?,updated_at=? WHERE id=?",(result.get("error") or "step_failed",now(),oid))
                _orch_event(oid,"adapt","failure_replan_required",{"step_index":row["step_index"],"automatic_replay":False},row["id"])
                break
        else:
            if not gated_waiting:
                write("UPDATE orchestration_missions SET status='verifying',updated_at=? WHERE id=?",(now(),oid))
        return _orch_view(oid)
    finally:
        _orch_release(oid,owner)


@app.post("/orchestrator/{orchestration_id}/tick")
def orchestrator_tick(orchestration_id: str):
    if not q("SELECT id FROM orchestration_missions WHERE id=?",(orchestration_id,),one=True): raise HTTPException(404,"orchestration_not_found")
    return _orch_tick(orchestration_id)

@app.get("/orchestrator/{orchestration_id}")
def orchestrator_status(orchestration_id: str):
    view=_orch_view(orchestration_id)
    if not view: raise HTTPException(404,"orchestration_not_found")
    return {"status":"ok","orchestration":view}

@app.get("/orchestrator/{orchestration_id}/events")
def orchestrator_events(orchestration_id: str, limit: int=100):
    if not q("SELECT id FROM orchestration_missions WHERE id=?",(orchestration_id,),one=True): raise HTTPException(404,"orchestration_not_found")
    limit=max(1,min(200,int(limit or 100)))
    return {"status":"ok","events":[dict(x) for x in q("SELECT * FROM orchestration_events WHERE orchestration_id=? ORDER BY id DESC LIMIT ?",(orchestration_id,limit))]}

@app.post("/orchestrator/{orchestration_id}/resume")
def orchestrator_resume(orchestration_id: str):
    m=q("SELECT * FROM orchestration_missions WHERE id=?",(orchestration_id,),one=True)
    if not m: raise HTTPException(404,"orchestration_not_found")
    if m["status"]=="awaiting_approval":
        return {"status":"approval_required","orchestration_id":orchestration_id,"steps":_orch_view(orchestration_id)["steps"],"automatic_replay":False}
    return _orch_tick(orchestration_id)

@app.get("/orchestrator-policy")
def orchestrator_policy():
    return {"version":APP_VERSION,"build":BUILD,"max_steps":ORCH_MAX_STEPS,"max_attempts":ORCH_MAX_ATTEMPTS,
            "default_budget_seconds":ORCH_DEFAULT_BUDGET_SECONDS,"parallel_safe_execution":True,
            "dependency_aware_scheduler":True,"prediction_guided_scheduling":True,"failure_aware_execution":True,
            "partial_result_recovery":True,"mission_deadline_control":True,"continuous_observe_decide_act":True,
            "external_side_effect_approval_required":True,"automatic_side_effect_retry":False}
