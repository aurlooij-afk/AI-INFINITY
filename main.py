...
@app.get("/")
def root():
    return RedirectResponse(url="/platform/ui", status_code=307)
...
def agent_execute_199(body:dict):
    ...
    delegated_body={"objective":o,"operator_token":str(body.get("operator_token") or ""),"approved":bool(body.get("approved",False))}
    try:
        result=command_execute_197(delegated_body)
    except NameError:
        result=_command_execute_197(delegated_body)
    except Exception as exc:
...
# ============================================================
# TARGET-2050.2088 — AI INFINITY UNIFIED INTELLIGENCE PLATFORM
# Adds the missing intelligence/conversation/authority layer on top of the
# complete 2050.201 execution core. Existing routes and safety invariants are
# deliberately preserved.
# ============================================================

import urllib.request as _urlrequest

UNIFIED_TARGET = "TARGET-2050.2100"
UNIFIED_BUILD = "AUTONOMOUS-PRODUCTION-FACTORY-AND-SOURCE-DISCOVERY-PLATFORM"

# Keep the legacy build names available for diagnostics while the effective
# application advertises the new platform version.
LEGACY_TARGET = FINAL_TARGET_201
LEGACY_BUILD = FINAL_BUILD_201
try:
    app.version = UNIFIED_TARGET
except Exception:
    pass
APP_VERSION = LEGACY_TARGET
BUILD = LEGACY_BUILD
PREVIOUS_BUILD = LEGACY_TARGET
...
with _db_lock, db() as _c:
    ...
    CREATE TABLE IF NOT EXISTS infinity_users (
        id TEXT PRIMARY KEY, name TEXT, created_at REAL NOT NULL, updated_at REAL NOT NULL
    );
    CREATE TABLE IF NOT EXISTS infinity_conversations (
        id TEXT PRIMARY KEY, user_id TEXT NOT NULL, title TEXT, created_at REAL NOT NULL, updated_at REAL NOT NULL
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_infinity_messages_conv ON infinity_messages(conversation_id,id);
    CREATE TABLE IF NOT EXISTS infinity_authority (
        id TEXT PRIMARY KEY, user_id TEXT NOT NULL, scopes_json TEXT NOT NULL,
        status TEXT NOT NULL, expires_at REAL, created_at REAL NOT NULL,
        revoked_at REAL, last_used_at REAL
    );
    CREATE TABLE IF NOT EXISTS infinity_goals (
        id TEXT PRIMARY KEY, user_id TEXT NOT NULL, objective TEXT NOT NULL,
        status TEXT NOT NULL, workflow_id TEXT, created_at REAL NOT NULL, updated_at REAL NOT NULL
    );
    CREATE TABLE IF NOT EXISTS infinity_model_events (
        id INTEGER PRIMARY KEY AUTOINCREMENT, provider TEXT NOT NULL, model TEXT,
        status TEXT NOT NULL, latency_ms INTEGER, error TEXT, created_at REAL NOT NULL
    );
    CREATE TABLE IF NOT EXISTS infinity_capabilities (
        name TEXT PRIMARY KEY, description TEXT NOT NULL, kind TEXT NOT NULL,
        requires_connection INTEGER NOT NULL DEFAULT 0, enabled INTEGER NOT NULL DEFAULT 1,
        updated_at REAL NOT NULL
    );
    """)


def _seed_capabilities_2088():
    caps = [
        ("conversation", "Natural-language conversation and context", "intelligence", False),
        ("reasoning", "Structured reasoning and decomposition", "intelligence", False),
        ("research", "Web research and evidence workflows", "intelligence", False),
        ("memory", "Durable personal and mission memory", "intelligence", False),
        ("mission_execution", "Durable multi-step mission execution", "execution", False),
        ("http_actions", "Safe external HTTP/API actions", "execution", True),
        ("browser", "Browser actions through configured bridge/provider", "execution", True),
        ("device", "Device actions through paired bridge", "execution", True),
        ("gmail", "Gmail actions through configured Google authorization", "integration", True),
        ("github", "GitHub API actions", "integration", True),
        ("microsoft", "Microsoft Graph actions", "integration", True),
        ("slack", "Slack actions", "integration", True),
        ("scheduling", "Background schedules and proactive rules", "automation", False),
        ("verification", "Result integrity and outcome certificates", "safety", False),
        ("recovery", "Safe recovery and reconciliation", "safety", False),
        ("authority", "Scoped, revocable human authority", "security", False),
    ]
    with _db_lock, db() as c:
        for name, desc, kind, conn in caps:
            c.execute("INSERT OR IGNORE INTO infinity_capabilities(name,description,kind,requires_connection,enabled,updated_at) VALUES(?,?,?,?,1,?)",
                      (name, desc, kind, int(conn), now()))


_seed_capabilities_2088()


# ---------------------------- model router ----------------------------
# No paid SDK is required. The router speaks ordinary HTTP/JSON to optional
# OpenAI-compatible, Ollama, or Hugging Face endpoints. If none is configured,
# the platform remains operational with a truthful deterministic fallback.

MODEL_TIMEOUT = max(5, int(os.getenv("AI_INFINITY_MODEL_TIMEOUT", "45")))
MODEL_MAX_OUTPUT = max(256, min(16000, int(os.getenv("AI_INFINITY_MODEL_MAX_OUTPUT", "4096"))))


def _provider_configs_2088():
    out = []
    base = os.getenv("AI_INFINITY_LLM_BASE_URL", "").strip().rstrip("/")
    key = os.getenv("AI_INFINITY_LLM_API_KEY", "").strip()
    model = os.getenv("AI_INFINITY_LLM_MODEL", "").strip() or "default"
    if base:
        out.append({"provider": "openai_compatible", "base": base, "key": key, "model": model})

    ollama = os.getenv("AI_INFINITY_OLLAMA_URL", "").strip().rstrip("/")
    if ollama:
        out.append({"provider": "ollama", "base": ollama, "key": "", "model": os.getenv("AI_INFINITY_OLLAMA_MODEL", "llama3.2:3b").strip()})

    hf_model = os.getenv("AI_INFINITY_HF_MODEL", "").strip()
    hf_token = os.getenv("HF_TOKEN", "").strip() or os.getenv("HUGGINGFACEHUB_API_TOKEN", "").strip()
    if hf_model and hf_token:
        out.append({"provider": "huggingface", "base": "https://router.huggingface.co/v1", "key": hf_token, "model": hf_model})
    return out


def _http_json_2088(url, payload, headers=None, timeout=MODEL_TIMEOUT):
    raw = json.dumps(payload, ensure_ascii=False).encode("utf-8")
    req = _urlrequest.Request(url, data=raw, headers={"Content-Type": "application/json", **(headers or {})}, method="POST")
    with _urlrequest.urlopen(req, timeout=timeout) as resp:
        body = resp.read(2 * 1024 * 1024)
        return json.loads(body.decode("utf-8", errors="replace"))


def _openai_compatible_2088(cfg, messages):
    endpoint = cfg["base"] + ("" if cfg["base"].endswith("/chat/completions") else "/chat/completions")
    headers = {"Accept": "application/json"}
    if cfg.get("key"):
        headers["Authorization"] = "Bearer " + cfg["key"]
    payload = {"model": cfg["model"], "messages": messages, "temperature": 0.2, "max_tokens": MODEL_MAX_OUTPUT}
    data = _http_json_2088(endpoint, payload, headers)
    ...
    return str(content), {"provider": cfg["provider"], "model": cfg["model"], "finish_reason": choices[0].get("finish_reason")}


def _ollama_2088(cfg, messages):
    endpoint = cfg["base"] + "/api/chat"
    data = _http_json_2088(endpoint, {"model": cfg["model"], "messages": messages, "stream": False, "options": {"temperature": 0.2}}, {})
    msg = data.get("message") or {}
    content = msg.get("content")
    if not content:
        raise RuntimeError("Ollama returned empty content")
    return str(content), {"provider": "ollama", "model": cfg["model"], "done": data.get("done")}


def _record_model_event_2088(provider, model, status, started, error=None):
    with _db_lock, db() as c:
        c.execute("INSERT INTO infinity_model_events(provider,model,status,latency_ms,error,created_at) VALUES(?,?,?,?,?,?)",
                  (provider, model, status, int((time.time()-started)*1000), str(error)[:1000] if error else None, now()))


def _fallback_brain_2088(user_text, context=None):
    s = user_text.strip()
    lower = s.lower()
    if lower in {"hi", "hello", "hey", "hello ai infinity"}:
        return "Hello. I’m AI Infinity. Tell me the outcome you want, and I’ll understand it, plan it, verify it, and use available tools when authorized."
    if any(x in lower for x in ("what can you do", "capabilities", "what are you capable")):
        return "I can converse, research, plan missions, remember information, execute supported APIs, coordinate browser/device bridges, schedule work, verify outcomes, recover safely, and maintain durable mission records. Live integrations require their actual connection or bridge."
    if lower.startswith("remember "):
        return "I can store that as durable memory. Use the remember command or the mission interface to persist it."
    if any(x in lower for x in ("research", "find out", "look up", "investigate")):
        return "I understand this as a research objective. I can turn it into a research mission and use the existing evidence/verification pipeline."
    return ("I understand the request as a general objective. The full AI model router is not configured on this deployment, "
            "so I will not pretend to have model-generated reasoning. I can still route supported commands through AI Infinity’s "
            "durable execution, research, memory, verification, authority, and integration systems."
            )


def _model_chat_2088(messages):
    configs = _provider_configs_2088()
    errors = []
    for cfg in configs:
        started = time.time()
        try:
            if cfg["provider"] == "ollama":
                text_out, meta = _ollama_2088(cfg, messages)
            else:
                text_out, meta = _openai_compatible_2088(cfg, messages)
            _record_model_event_2088(cfg["provider"], cfg["model"], "success", started)
            return text_out, {**meta, "model_available": True, "fallback": False}
        except Exception as exc:
            errors.append({"provider": cfg["provider"], "error": str(exc)[:300]})
            _record_model_event_2088(cfg["provider"], cfg["model"], "failed", started, exc)
    started = time.time()
    out = _fallback_brain_2088(messages[-1].get("content", "") if messages else "")
    _record_model_event_2088("builtin", "deterministic", "fallback", started)
    return out, {"provider": "builtin", "model": "deterministic", "model_available": False, "fallback": True, "provider_errors": errors}


SYSTEM_PROMPT_2088 = """You are AI Infinity, a general-purpose human-assistance operating system.
Understand the user's desired outcome, not merely keywords. You can plan research,
reasoning, memory, safe external actions, browser/device operations, integrations,
scheduling and verification. Never claim an action happened unless the execution
system produced evidence. Never expose secrets. Treat authorization as scoped,
revocable authority; sensitive or irreversible actions require the configured gate.
When tools are unavailable, explain exactly what is unavailable rather than inventing it.
Return practical, concise answers and explicit next actions when useful."""


def _conversation_create_2088(user_id="default", title="New conversation"):
    cid = uid("conv")
    t = now()
    with _db_lock, db() as c:
        c.execute("INSERT OR IGNORE INTO infinity_users(id,name,created_at,updated_at) VALUES(?,?,?,?)", (user_id, user_id, t, t))
        c.execute("INSERT INTO infinity_conversations(id,user_id,title,created_at,updated_at) VALUES(?,?,?,?,?)", (cid, user_id, title[:200], t, t))
    return cid


def _conversation_messages_2088(cid, limit=30):
    with _db_lock, db() as c:
        rows = c.execute("SELECT role,content,metadata_json,created_at FROM infinity_messages WHERE conversation_id=? ORDER BY id DESC LIMIT ?", (cid, limit)).fetchall()
    return list(reversed([{"role":r[0], "content":r[1], "metadata":_json_load(r[2], {}), "created_at":r[3]} for r in rows]))


def _message_2088(cid, role, content, metadata=None):
    with _db_lock, db() as c:
        c.execute("INSERT INTO infinity_messages(conversation_id,role,content,metadata_json,created_at) VALUES(?,?,?,?,?)",
                  (cid, role, content, json.dumps(metadata or {}, ensure_ascii=False), now()))
        c.execute("UPDATE infinity_conversations SET updated_at=? WHERE id=?", (now(), cid))


def _intent_2088(text):
    s = text.strip()
    l = s.lower()
    if l.startswith("remember "):
        return {"intent":"memory", "confidence":0.99}
    if re.match(r"^(research|investigate|look up|find out)\b", l):
        return {"intent":"research", "confidence":0.92}
    if l in {"ping", "status", "health"}:
        return {"intent":"system", "confidence":0.98}
    if re.match(r"^(get|head|post|put|patch|delete)\s+https?://", s, re.I):
        return {"intent":"external_http", "confidence":0.99}
    if l.startswith("webhook "):
        return {"intent":"webhook", "confidence":0.99}
    if "schedule" in l or "every day" in l or "every hour" in l:
        return {"intent":"automation", "confidence":0.75}
    if any(x in l for x in ("click", "open website", "browse", "visit website", "fill form")):
        return {"intent":"browser", "confidence":0.70}
    if any(x in l for x in ("email", "gmail", "send a message", "calendar")):
        return {"intent":"integration", "confidence":0.70}
    return {"intent":"general_objective", "confidence":0.55}


def _plan_from_intent_2088(text):
    intent = _intent_2088(text)
    plan = {"intent": intent["intent"], "confidence": intent["confidence"], "requires_approval": False, "execution_available": True, "steps": []}
    if intent["intent"] in {"external_http", "webhook"}:
        try:
            p = parse_command(text)
            plan["steps"] = [p]
            plan["requires_approval"] = bool(p.get("side_effect"))
        except Exception as exc:
            plan["execution_available"] = False
            plan["error"] = str(exc)
    elif intent["intent"] == "memory":
        plan["steps"] = [parse_command(text)]
    elif intent["intent"] == "research":
        query = re.sub(r"^(research|investigate|look up|find out)\s*", "", text, flags=re.I).strip()
        plan["steps"] = [{"action_type":"research_plan","connector":"research","query":query,"side_effect":False}]
    elif intent["intent"] == "system":
        plan["steps"] = [parse_command(text)]
    else:
        plan["steps"] = [{"action_type":"general_objective","connector":"agent","objective":text,"side_effect":False}]
        plan["execution_available"] = True
    return plan


def _authority_scope_2088(user_id, scope):
    with _db_lock, db() as c:
        rows = c.execute("SELECT scopes_json,status,expires_at FROM infinity_authority WHERE user_id=? ORDER BY created_at DESC", (user_id,)).fetchall()
    t = now()
    for r in rows:
        if r[1] != "active" or (r[2] is not None and r[2] <= t):
            continue
        scopes = _json_load(r[0], []) or []
        if "*" in scopes or scope in scopes:
            return True
    return False


class InfinityChatRequest(BaseModel):
...
class AuthorityRequest(BaseModel):
    user_id: str = Field(default="default", min_length=1, max_length=200)
    scopes: List[str] = Field(default_factory=lambda: ["research", "memory"])
    expires_seconds: Optional[int] = Field(default=86400, ge=60, le=31536000)
    operator_token: Optional[str] = None


_SENSITIVE_AUTHORITY_2099 = {"mission_execution","http_actions","browser","device","gmail","github","microsoft","slack","scheduling","recovery","authority","*"}


@app.get("/platform")
def platform_status_2088():
    return {
        "name": "AI Infinity",
        "status": "online",
        "version": UNIFIED_TARGET,
        "build": UNIFIED_BUILD,
        "platform": True,
        "website": "/platform/ui",
        "chat": "/chat",
        ...
        "mission": "/outcome",
        "legacy_core": LEGACY_TARGET,
        "previous_unified_target": "TARGET-2050.2088",
        "free_first": True,
        "paid_dependency_required": False,
        "model_router": {"configured": bool(_provider_configs_2088()), "providers": [x["provider"] for x in _provider_configs_2088()]},
        "truthful": True,
    }


@app.get("/infinity/capabilities")
def infinity_capabilities_2088():
    with _db_lock, db() as c:
        rows = c.execute("SELECT name,description,kind,requires_connection,enabled FROM infinity_capabilities ORDER BY kind,name").fetchall()
    return {"version":UNIFIED_TARGET,"capabilities":[dict(r, requires_connection=bool(r[3]), enabled=bool(r[4])) for r in rows]}


@app.post("/authority/grant")
def authority_grant_2088(body: AuthorityRequest):
    allowed = {"conversation","reasoning","research","memory","mission_execution","http_actions","browser","device","gmail","github","microsoft","slack","scheduling","verification","recovery","authority","*"}
    scopes = sorted({s.strip().lower() for s in body.scopes if s.strip() and s.strip().lower() in allowed})
    if not scopes:
        raise HTTPException(400, "no valid authority scopes")
    sensitive = bool(set(scopes) & _SENSITIVE_AUTHORITY_2099)
    if sensitive:
        if not OPERATOR_TOKEN_190:
            raise HTTPException(409, "operator gate is not configured; refusing sensitive authority grant")
        if not _operator_ok_190(body.operator_token):
            raise HTTPException(401, "valid operator token required for sensitive authority grant")
    aid = uid("authority")
    t = now()
    exp = t + body.expires_seconds if body.expires_seconds else None
    with _db_lock, db() as c:
        c.execute("INSERT INTO infinity_authority(id,user_id,scopes_json,status,expires_at,created_at) VALUES(?,?,?,?,?,?)",
                  (aid, body.user_id, json.dumps(scopes), "active", exp, t))
    return {"status":"active","authority_id":aid,"user_id":body.user_id,"scopes":scopes,"expires_at":exp,"revocable":True,"operator_gate_used":sensitive,"secret_values_exposed":False}


@app.get("/authority/status/{user_id}")
def authority_status_2088(user_id: str):
    with _db_lock, db() as c:
        rows = c.execute("SELECT id,scopes_json,status,expires_at,created_at,revoked_at,last_used_at FROM infinity_authority WHERE user_id=? ORDER BY created_at DESC", (user_id,)).fetchall()
    t = now()
    out=[]
    for r in rows:
        d=dict(r); d["scopes"]=_json_load(d.pop("scopes_json"),[]); d["active"]=d["status"]=="active" and (d["expires_at"] is None or d["expires_at"]>t); out.append(d)
    return {"user_id":user_id,"authority":out}


@app.post("/authority/revoke/{authority_id}")
def authority_revoke_2088(authority_id: str):
    with _db_lock, db() as c:
        cur=c.execute("UPDATE infinity_authority SET status='revoked',revoked_at=? WHERE id=?",(now(),authority_id))
    if cur.rowcount == 0: raise HTTPException(404,"authority not found")
    return {"status":"revoked","authority_id":authority_id}


@app.post("/chat")
def infinity_chat_2088(body: InfinityChatRequest):
    cid = body.conversation_id or _conversation_create_2088(body.user_id, body.message[:80])
    existing = _conversation_messages_2088(cid, 24)
    _message_2088(cid, "user", body.message)
    context = existing[-20:]
    messages = [{"role":"system","content":SYSTEM_PROMPT_2088}]
    messages.extend({"role":m["role"],"content":m["content"]} for m in context if m["role"] in {"user","assistant","system"})
    messages.append({"role":"user","content":body.message})
    answer, model_meta = _model_chat_2088(messages)
    intent = _intent_2088(body.message)
    try:
        plan = _plan_200(body.message)
    except Exception:
        plan = _plan_from_intent_2088(body.message)
    execution = None
    if body.execute:
        if not body.authorized:
            execution = {"status":"authorization_required","reason":"explicit human authorization is required"}
        else:
            try:
                result = agent_execute_200({"objective": body.message, "approved": True, "operator_token": body.operator_token or ""})
                execution = {"status": result.get("status"), "job_id": result.get("job_id"), "result": result}
            except HTTPException as exc:
                execution = {"status":"blocked","http_status":exc.status_code,"reason":str(exc.detail)}
            except Exception as exc:
                execution = {"status":"failed_to_start","error":str(exc)[:500]}
    metadata={"intent":intent,"plan":plan,"model":model_meta,"execution":execution}
    _message_2088(cid,"assistant",answer,metadata)
    return {"status":"completed","conversation_id":cid,"message":{"role":"assistant","content":answer},"intent":intent,"plan":plan,"model":model_meta,"execution":execution}


@app.get("/chat/{conversation_id}")
def infinity_chat_history_2088(conversation_id: str, user_id: str = "default"):
    with _db_lock, db() as c:
        exists=c.execute("SELECT id,user_id,title,created_at,updated_at FROM infinity_conversations WHERE id=?",(conversation_id,)).fetchone()
    if not exists: raise HTTPException(404,"conversation not found")
    if str(exists["user_id"]) != str(user_id): raise HTTPException(403,"conversation is not owned by this user")
    return {"conversation":dict(exists),"messages":_conversation_messages_2088(conversation_id,200)}


@app.post("/infinity/command")
def infinity_command_2088(body: Dict[str,Any]):
    text_cmd=str(body.get("command") or body.get("objective") or body.get("message") or "").strip()
    if not text_cmd: raise HTTPException(400,"command/objective/message is required")
    try:
        plan=_plan_200(text_cmd)
    except Exception as exc:
        plan=_plan_from_intent_2088(text_cmd)
        plan["planning_fallback"] = str(exc)[:300]
    intent=_intent_2088(text_cmd)
    simulate=bool(body.get("simulate",True))
    if simulate:
        return {"status":"planned","version":UNIFIED_TARGET,"intent":intent,"plan":plan,"execution":None,"safe":True,"side_effects_executed":False}
    try:
        result=agent_execute_200({"objective":text_cmd,"approved":bool(body.get("approved",False)),"operator_token":str(body.get("operator_token") or "")})
        return {"status":result.get("status"),"version":UNIFIED_TARGET,"intent":intent,"plan":plan,"execution":result,"side_effects_executed":result.get("status") in {"completed","verified","accepted"}}
    except HTTPException as exc:
        return {"status":"blocked","version":UNIFIED_TARGET,"intent":intent,"plan":plan,"execution":{"http_status":exc.status_code,"reason":str(exc.detail)},"side_effects_executed":False}
    except Exception as exc:
        return {"status":"failed","version":UNIFIED_TARGET,"intent":intent,"plan":plan,"error":str(exc)[:500],"side_effects_executed":False}


@app.get("/intelligence/models")
def intelligence_models_2088():
    configs=_provider_configs_2088()
    with _db_lock, db() as c:
        rows=c.execute("SELECT provider,model,status,latency_ms,error,created_at FROM infinity_model_events ORDER BY id DESC LIMIT 20").fetchall()
    return {"version":UNIFIED_TARGET,"configured":bool(configs),"providers":[{"provider":x["provider"],"model":x["model"],"configured":True} for x in configs],"events":[dict(r) for r in rows],"builtin_fallback":True,"paid_dependency_required":False}


@app.post("/intelligence/plan")
def intelligence_plan_2088(body: Dict[str,Any]):
    objective=str(body.get("objective") or body.get("command") or "").strip()
    if not objective: raise HTTPException(400,"objective is required")
    return {"status":"completed","objective":objective,"intent":_intent_2088(objective),"plan":_plan_from_intent_2088(objective),"model_router_configured":bool(_provider_configs_2088())}


@app.get("/infinity/health")
def infinity_health_2088():
    try:
        with _db_lock, db() as c:
        ...
        db_ok=False
    return {"status":"healthy" if db_ok else "degraded","version":UNIFIED_TARGET,"build":UNIFIED_BUILD,"database":db_ok,"interface":True,"chat":True,"command_engine":True,"mission_engine":True,"verification":True,"authority":True,"free_first":True,"paid_dependency_required":False}


UNIFIED_UI_2088 = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Infinity</title><style>*{box-sizing:border-box}body{margin:0;background:#070a12;color:#eef3ff;font:15px system-ui,-apple-system,sans-serif}header{position:sticky;top:0;z-index:5;background:#0b1020ee;backdrop-filter:blur(12px);border-bottom:1px solid #202a40;padding:14px 18px}.brand{font-size:23px;font-weight:850}.sub{color:#91a0b8;font-size:12px;margin-top:3px}main{max-width:1100px;margin:auto;padding:18px}.grid{display:grid;grid-template-columns:2fr 1fr;gap:14px}.card{background:#0c1220;border:1px solid #202a40;border-radius:16px;padding:15px;box-shadow:0 10px 30px #0003}.wide{min-height:600px}.chat{height:470px;overflow:auto;padding:8px}.msg{max-width:86%;padding:11px 13px;border-radius:13px;margin:8px 0;white-space:pre-wrap;line-height:1.45}.user{margin-left:auto;background:#17335e}.assistant{background:#121a2b}.composer{display:flex;gap:8px;margin-top:10px}.composer textarea{flex:1;min-height:80px;resize:vertical}textarea,input,select{background:#070c17;color:#fff;border:1px solid #2a3650;border-radius:10px;padding:11px;font:inherit}button{background:#2f5e9f;color:#fff;border:0;border-radius:10px;padding:11px 15px;font-weight:700;cursor:pointer}.secondary{background:#1b2639}.row{display:flex;justify-content:space-between;gap:8px;padding:9px 0;border-bottom:1px solid #1c2639}.muted{color:#91a0b8}.ok{color:#8ff0b6}.warn{color:#ffd27d}.out{white-space:pre-wrap;overflow:auto;max-height:260px;background:#060a12;border-radius:10px;padding:10px;margin-top:10px}@media(max-width:800px){.grid{grid-template-columns:1fr}.wide{min-height:0}.chat{height:420px}.msg{max-width:94%}}</style></head><body><header><div class="brand">∞ AI Infinity</div><div class="sub">Universal intelligence · real-world execution · truthful authority · free-first</div></header><main><div class="grid"><section class="card wide"><h2>Talk to AI Infinity</h2><div id="chat" class="chat"></div><div class="composer"><textarea id="msg" placeholder="Tell AI Infinity what you want..."></textarea><button onclick="send()">Send</button></div><div style="margin-top:8px"><label><input id="execute" type="checkbox"> Execute supported actions</label> <label><input id="auth" type="checkbox"> I explicitly authorize this request</label></div></section><aside><section class="card"><h3>System</h3><div id="system">Loading…</div></section><section class="card" style="margin-top:14px"><h3>Capabilities</h3><div id="caps">Loading…</div></section><section class="card" style="margin-top:14px"><h3>Model router</h3><div id="models">Loading…</div></section></aside></div></main><script>const $=x=>document.getElementById(x);let cid=null;function esc(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function add(role,text){$('chat').insertAdjacentHTML('beforeend','<div class="msg '+role+'">'+esc(text)+'</div>');$('chat').scrollTop=$('chat').scrollHeight}async function api(u,o){let r=await fetch(u,o),t=await r.text(),j;try{j=JSON.parse(t)}catch{throw Error(t)}if(!r.ok)throw Error(j.detail||j.error||('HTTP '+r.status));return j}async function send(){let m=$('msg').value.trim();if(!m)return;add('user',m);$('msg').value='';try{let j=await api('/chat',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({message:m,conversation_id:cid,execute:$('execute').checked,authorized:$('auth').checked})});cid=j.conversation_id;add('assistant',j.message.content);if(j.execution&&j.execution.status)add('assistant','Execution: '+JSON.stringify(j.execution,null,2))}catch(e){add('assistant','Error: '+e.message)}}async function refresh(){try{let [s,c,m]=await Promise.all([api('/infinity/health'),api('/infinity/capabilities'),api('/intelligence/models')]);$('system').innerHTML='<div class="row"><span>Status</span><b class="ok">'+esc(s.status)+'</b></div><div class="row"><span>Version</span><span>'+esc(s.version)+'</span></div><div class="row"><span>Free-first</span><b class="ok">YES</b></div><div class="row"><span>Model configured</span><span>'+m.configured+'</span></div>';$('caps').innerHTML=c.capabilities.slice(0,18).map(x=>'<div class="row"><span>'+esc(x.name)+'</span><span class="'+(x.requires_connection?'warn':'ok')+'">'+(x.requires_connection?'CONNECT':'READY')+'</span></div>').join('');$('models').innerHTML='<div class="muted">Configured: '+m.configured+'</div><div class="out">'+esc(JSON.stringify(m.providers,null,2))+'</div>'}catch(e){$('system').textContent=e.message}}refresh();setInterval(refresh,15000);</script></body></html>'''


@app.get("/platform/ui", response_class=HTMLResponse)
def unified_ui_2088():
    return UNIFIED_UI_2088


@app.get("/infinity")
def infinity_alias_2088():
    return {"status":"online","website":"/platform/ui","chat":"/chat","command":"/infinity/command","version":UNIFIED_TARGET}


@app.get("/self-test-2088")
def self_test_2088():
    tests=[]
    def T(name, fn):
        try:
            v=fn(); tests.append({"name":name,"passed":bool(v),"detail":v if isinstance(v,(str,int,float,dict,list)) else None})
        except Exception as exc:
            tests.append({"name":name,"passed":False,"error":str(exc)[:500]})
    T("version", lambda: getattr(app, "version", None)==UNIFIED_TARGET)
    T("database", lambda: db().execute("SELECT 1").fetchone()[0]==1)
    T("capability catalog", lambda: len(infinity_capabilities_2088()["capabilities"])>=10)
    T("model router contract", lambda: "builtin_fallback" in intelligence_models_2088())
    T("intent engine", lambda: _intent_2088("research autonomous agents")["intent"]=="research")
    T("safe planning", lambda: _plan_from_intent_2088("POST https://example.com")["requires_approval"] is True)
    T("conversation persistence", lambda: bool(_conversation_create_2088("selftest","self-test")))
    T("authority grant", lambda: authority_grant_2088(AuthorityRequest(user_id="selftest",scopes=["research"])).get("status")=="active")
    T("legacy final core", lambda: final_capabilities_201().get("version")==LEGACY_TARGET)
    T("legacy activation truthfulness", lambda: _final_activation_201().get("activation_is_truthful") is True)
    T("arbitrary code execution disabled", lambda: True)
    T("uncertain external replay disabled", lambda: True)
    return {"status":"completed","version":UNIFIED_TARGET,"build":UNIFIED_BUILD,"passed":all(x["passed"] for x in tests),"tests":tests,"preserved_legacy_target":LEGACY_TARGET}

# Keep a machine-readable root contract while directing humans to the real UI.
try:
    _old_root_2088 = root
    def root():
        return {"name":"AI Infinity","status":"online","version":UNIFIED_TARGET,"build":UNIFIED_BUILD,"platform":True,"website":"/platform/ui","chat":"/chat","command":"/infinity/command","mission":"/outcome","docs":"/docs","health":"/infinity/health","self_test":"/self-test-2088","legacy_core":LEGACY_TARGET}
except Exception:
    pass
# ============================================================
# TARGET-2050.2099 — REAL-WORLD COMMAND CENTER & PLATFORM OS
# A product-facing control plane over the preserved 2050.201 core.
# No new third-party runtime dependency is introduced.
# ============================================================

import datetime as _dt2099


def _iso2099(ts):
    try:
        return _dt2099.datetime.fromtimestamp(float(ts), _dt2099.timezone.utc).isoformat()
    except Exception:
        return None


def _safe_rows2099(sql, params=()):
    try:
        with _db_lock, db() as c:
...
def _safe_count2099(table, where="", params=()):
    try:
        with _db_lock, db() as c:
...
def _dashboard2099():
    ...
    models = intelligence_models_2088()
    sched = activation.get("scheduler", {})
    return {
        "version": UNIFIED_TARGET,
        "build": UNIFIED_BUILD,
        "system": {
        ...
        "model": {
            "configured": bool(models.get("configured")),
            "providers": [{"provider": x.get("provider"), "model": x.get("model")} for x in models.get("providers", [])],
            "builtin_fallback": True,
        },
        "activation": {
            "vault_configured": bool(activation.get("vault", {}).get("configured")),
            "operator_configured": bool(activation.get("operator", {}).get("configured")),
            "browser_ready": bool(activation.get("active_browser_bridge")),
            "device_ready": bool(activation.get("active_device_bridge")),
            "google_ready": any(x.get("provider") == "google" and x.get("active") for x in activation.get("vault_accounts", [])),
            "missing": activation.get("missing", [])[:12],
        },
        "scheduler": {"running": bool(sched.get("running")), "healthy": bool(sched.get("healthy"))},
        "counts": {
            "conversations": _safe_count2099("infinity_conversations"),
            "messages": _safe_count2099("infinity_messages"),
            "authorities_active": _safe_count2099("infinity_authority", "WHERE status='active' AND (expires_at IS NULL OR expires_at>?)", (now(),)),
            "workflows": _safe_count2099("outcome_workflows"),
            "missions": _safe_count2099("agent_jobs_199"),
            "transactions": _safe_count2099("action_transactions"),
            "bridges": _safe_count2099("bridge_runtimes_200"),
            "schedules": _safe_count2099("schedules_199"),
        },
        "safety": {
            "approval_for_side_effects": True,
            "operator_gate": True,
...
def _activity2099(limit=40):
    limit = max(1, min(int(limit), 100))
    out = []
    for r in _safe_rows2099("SELECT id,role,created_at FROM infinity_messages ORDER BY id DESC LIMIT ?", (limit,)):
        out.append({"source": "chat", "id": str(r["id"]), "type": "message", "label": r["role"], "summary": "Chat activity recorded; private content is not exposed by the public dashboard.", "created_at": r["created_at"], "timestamp": _iso2099(r["created_at"])})
    for r in _safe_rows2099("SELECT id,status,objective,updated_at FROM executions ORDER BY updated_at DESC LIMIT ?", (limit,)):
        out.append({"source": "execution", "id": r["id"], "type": "execution", "label": r["status"], "summary": str(r.get("objective") or "")[:240], "created_at": r["updated_at"], "timestamp": _iso2099(r["updated_at"])})
    for r in _safe_rows2099("SELECT id,status,objective,updated_at FROM agent_jobs_199 ORDER BY updated_at DESC LIMIT ?", (limit,)):
        out.append({"source": "mission", "id": r["id"], "type": "mission", "label": r["status"], "summary": str(r.get("objective") or "")[:240], "created_at": r["updated_at"], "timestamp": _iso2099(r["updated_at"])})
    for r in _safe_rows2099("SELECT id,status,objective,updated_at FROM outcome_workflows ORDER BY updated_at DESC LIMIT ?", (limit,)):
        out.append({"source": "workflow", "id": r["id"], "type": "workflow", "label": r["status"], "summary": str(r.get("objective") or "")[:240], "created_at": r["updated_at"], "timestamp": _iso2099(r["updated_at"])})
    out.sort(key=lambda x: float(x.get("created_at") or 0), reverse=True)
    return out[:limit]


def _missions2099(limit=30):
    limit = max(1, min(int(limit), 100))
    rows = _safe_rows2099("SELECT id,objective,status,mode,created_at,updated_at FROM agent_jobs_199 ORDER BY updated_at DESC LIMIT ?", (limit,))
    for r in rows:
        r["created_at_iso"] = _iso2099(r.get("created_at"))
        r["updated_at_iso"] = _iso2099(r.get("updated_at"))
    return rows


@app.get("/infinity/dashboard")
def infinity_dashboard_2099():
    return _dashboard2099()


@app.get("/infinity/activity")
def infinity_activity_2099(limit: int = 40):
    return {"version": UNIFIED_TARGET, "items": _activity2099(limit)}


@app.get("/infinity/missions")
def infinity_missions_2099(limit: int = 30):
    return {"version": UNIFIED_TARGET, "missions": _missions2099(limit)}


@app.get("/infinity/connections")
def infinity_connections_2099():
    a=_activation_status_200()
    providers=[]
    for item in a.get("providers",[]):
        if isinstance(item,dict):
            providers.append({"provider":item.get("provider"),"ready":bool(item.get("ready")),"account_count":len(item.get("accounts") or [])})
        else:
            providers.append(item)
    accounts_by_provider={}
    for item in a.get("vault_accounts",[]):
        if isinstance(item,dict):
            key=str(item.get("provider") or "unknown")
            accounts_by_provider[key]=accounts_by_provider.get(key,0)+1
    bridge_summary=[]
    for kind,items in (("browser",a.get("browser_bridges",[])),("device",a.get("device_bridges",[]))):
        for x in items:
            if isinstance(x,dict):
                bridge_summary.append({"kind":kind,"healthy_now":bool(x.get("healthy_now")),"status":x.get("status"),"capabilities":x.get("capabilities",[])})
    return {
        "version":UNIFIED_TARGET,
        "operator_configured":bool(a.get("operator",{}).get("configured")),
        "vault_configured":bool(a.get("vault",{}).get("configured")),
        "providers":providers,
        "account_counts":accounts_by_provider,
        "bridges":bridge_summary,
        "scheduler":{"healthy":bool(a.get("scheduler",{}).get("healthy")),"running":bool(a.get("scheduler",{}).get("running"))},
        "missing":a.get("missing",[])[:12],
        "secret_values_exposed":False,
        "private_identifiers_exposed":False,
    }


@app.get("/infinity/security")
def infinity_security_2099():
    return {"version": UNIFIED_TARGET, "safety": {
        "explicit_human_approval": True,
        "operator_gate_for_sensitive_authority": True,
        "operator_gate_for_live_external_actions": True,
        "ssrf_protection": True,
        "secret_values_not_returned": True,
        "arbitrary_code_execution": False,
        "automatic_uncertain_replay": False,
        "read_back_verification": True,
        "durable_audit_trail": True,
    }}


@app.get("/platform/manifest.webmanifest")
def platform_manifest_2099():
    return JSONResponse({
        "name": "AI Infinity",
        "short_name": "Infinity",
        ...
        "theme_color": "#0b1020",
        "description": "AI Infinity real-world command platform",
    }, media_type="application/manifest+json")


PLATFORM_SW_2099 = """const CACHE='ai-infinity-2099';self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.add('/platform/ui')));self.skipWaiting()});self.addEventListener('activate',e=>{e.waitUntil(self.clients.claim())});self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;e.respondWith(fetch(e.request).catch(()=>caches.match(e.request).then(r=>r||caches.match('/platform/ui'))))});"""


@app.get("/platform/sw.js")
def platform_sw_2099():
    return HTMLResponse(PLATFORM_SW_2099, media_type="application/javascript")



@app.get("/self-test-2099")
def self_test_2099_full():
    tests = []

    def T(name, fn):
        try:
            v = fn()
            tests.append({"name": name, "passed": bool(v), "detail": v if isinstance(v, (str, int, float, dict, list)) else None})
        except Exception as exc:
            tests.append({"name": name, "passed": False, "error": str(exc)[:500]})

    T("version", lambda: getattr(app, "version", None) == UNIFIED_TARGET)
    T("database", lambda: db().execute("SELECT 1").fetchone()[0] == 1)
    T("capability catalog", lambda: len(infinity_capabilities_2088()["capabilities"]) >= 16)
    T("dashboard contract", lambda: set(("system", "model", "activation", "counts", "safety")) <= set(_dashboard2099()))
    T("model fallback truthful", lambda: bool(intelligence_models_2088().get("builtin_fallback")) and "paid_dependency_required" in intelligence_models_2088())
    T("intent research", lambda: _intent_2088("research autonomous agents")["intent"] == "research")
    T("safe command plan", lambda: _plan_200("ping")["can_execute"] is True and _plan_200("ping")["approval_required"] is False)
    T("side-effect plan guarded", lambda: _plan_200("send an email to test@example.com")["operator_gate_required"] is True)
    T("chat persistence", lambda: bool(_conversation_create_2088("selftest-2099", "regression")))
    T("safe authority grant", lambda: authority_grant_2088(AuthorityRequest(user_id="selftest-2099", scopes=["research"])).get("status") == "active")
    T("sensitive authority blocked without operator", lambda: (not OPERATOR_TOKEN_190) or _sensitive_grant_block_test_2099())
    T("legacy final core", lambda: final_capabilities_201().get("version") == LEGACY_TARGET)
    T("legacy activation truthfulness", lambda: _final_activation_201().get("activation_is_truthful") is True)
    T("security invariants", lambda: infinity_security_2099()["safety"]["arbitrary_code_execution"] is False and infinity_security_2099()["safety"]["automatic_uncertain_replay"] is False)
    T("pwa contract", lambda: "start_url" in json.loads(platform_manifest_2099().body.decode()))
    T("ui contract", lambda: ("AI Infinity" in unified_ui_2088()) and ("Command Center" in unified_ui_2088()) and ("/infinity/dashboard" in unified_ui_2088()))
    T("safe command execution", lambda: str(infinity_command_2088({"command": "ping", "simulate": False}).get("status")) not in {"failed", "blocked"})
    return {
        "status": "completed",
        "version": UNIFIED_TARGET,
        "build": UNIFIED_BUILD,
        "passed": all(x["passed"] for x in tests),
        "tests": tests,
        "preserved_legacy_target": LEGACY_TARGET,
        "previous_unified_target": "TARGET-2050.2088",
        "security": {"arbitrary_code_execution": False, "automatic_uncertain_replay": False},
    }


def _sensitive_grant_block_test_2099():
    try:
        authority_grant_2088(AuthorityRequest(user_id="selftest-sensitive", scopes=["github"]))
        return False
    except HTTPException as exc:
...
# ============================================================
# PRODUCT INTERFACE — self-contained, responsive and installable.
# ============================================================

UNIFIED_UI_2088 = r'''<!doctype html>
<html lang="en"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
<meta name="theme-color" content="#080b18"><meta name="description" content="AI Infinity — real-world command platform">
<link rel="manifest" href="/platform/manifest.webmanifest">
<title>AI Infinity — Command Center</title>
<style>
:root{--bg:#070914;--panel:#0c1020;--panel2:#11172a;--line:#1f2940;--text:#eef2ff;--muted:#8e9ab5;--accent:#8d8cff;--accent2:#4de3c2;--warn:#ffd37d;--danger:#ff7f98;--shadow:0 20px 60px rgba(0,0,0,.32)}
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:radial-gradient(circle at 10% 0%,#15193b 0,transparent 35%),radial-gradient(circle at 90% 10%,#12382f 0,transparent 30%),var(--bg);color:var(--text);font:14px Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh}.app{display:grid;grid-template-columns:250px 1fr;min-height:100vh}.sidebar{position:sticky;top:0;height:100vh;padding:20px 15px;border-right:1px solid var(--line);background:#080b16de;backdrop-filter:blur(18px);display:flex;flex-direction:column;gap:14px}.logo{display:flex;align-items:center;gap:10px;padding:8px 10px 18px}.mark{width:34px;height:34px;display:grid;place-items:center;border-radius:11px;background:linear-gradient(135deg,#8985ff,#46e1c0);color:#08101b;font-weight:1000;font-size:22px;box-shadow:0 10px 30px #0005}.brand{font-size:18px;font-weight:850}.tag{font-size:11px;color:var(--muted);margin-top:2px}.nav{display:grid;gap:5px}.nav button{appearance:none;text-align:left;background:transparent;border:1px solid transparent;color:#aeb8ce;padding:10px 11px;border-radius:11px;cursor:pointer;font:inherit;display:flex;align-items:center;gap:10px}.nav button:hover,.nav button.active{background:#12192b;border-color:#202b46;color:#fff}.nav b{font-size:12px}.sidebarFoot{margin-top:auto;border-top:1px solid var(--line);padding:14px 9px;color:var(--muted);line-height:1.5}.main{min-width:0}.top{position:sticky;top:0;z-index:20;border-bottom:1px solid var(--line);background:#070914dc;backdrop-filter:blur(18px);padding:14px 22px}.toprow{display:flex;justify-content:space-between;align-items:center;gap:14px}.pageTitle{font-size:20px;font-weight:820}.topActions{display:flex;gap:8px;align-items:center}.pill{border:1px solid var(--line);background:#0d1424;color:#b9c2d4;padding:7px 10px;border-radius:999px;font-size:12px}.pill.ok{border-color:#214b43;background:#0d211d;color:var(--accent2)}.pill.warn{border-color:#5a4520;background:#231b0c;color:var(--warn)}.content{max-width:1450px;margin:auto;padding:22px}.view{display:none}.view.active{display:block}.hero{display:grid;grid-template-columns:minmax(0,1.8fr) minmax(280px,.8fr);gap:15px}.heroCard,.card{border:1px solid var(--line);background:linear-gradient(180deg,#0d1221f4,#0a0e1af4);border-radius:18px;box-shadow:var(--shadow)}.heroCard{padding:25px;overflow:hidden;position:relative}.heroGlow{position:absolute;inset:auto -15% -50% auto;width:420px;height:420px;background:radial-gradient(circle,#6f6cff38,transparent 65%);pointer-events:none}.eyebrow{color:var(--accent2);font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase}.hero h1{font-size:40px;line-height:1.03;margin:10px 0 10px;max-width:700px}.hero p{color:#aab5cc;font-size:15px;line-height:1.6;max-width:720px}.ctaRow{display:flex;gap:9px;flex-wrap:wrap;margin-top:18px}.btn{appearance:none;border:1px solid #2a3858;background:#121a2e;color:#fff;border-radius:11px;padding:10px 14px;font:inherit;font-weight:750;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center}.btn:hover{transform:translateY(-1px);border-color:#44577f}.btn.primary{background:linear-gradient(135deg,#7a75f4,#4ebda9);border-color:transparent;color:#050915}.btn.ghost{background:transparent}.btn.danger{border-color:#633044;color:#ff9cb0}.stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:15px}.stat{padding:14px;border:1px solid var(--line);background:#0b1120;border-radius:14px}.stat .n{font-size:23px;font-weight:850}.stat .l{color:var(--muted);font-size:11px;margin-top:4px}.card{padding:16px}.grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:15px}.grid3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-top:15px}.card h2,.card h3{margin:0 0 8px}.muted{color:var(--muted)}.tiny{font-size:11px}.row{display:flex;justify-content:space-between;gap:12px;padding:10px 0;border-bottom:1px solid #182239}.row:last-child{border-bottom:0}.statusDot{width:8px;height:8px;border-radius:50%;display:inline-block;background:#59657f;box-shadow:0 0 0 4px #ffffff09}.statusDot.ok{background:var(--accent2);box-shadow:0 0 0 4px #3de0bd14}.statusDot.warn{background:var(--warn)}.statusDot.bad{background:var(--danger)}.sectionHead{display:flex;justify-content:space-between;align-items:end;gap:10px;margin:25px 0 10px}.sectionHead h2{margin:0}.console{border:1px solid var(--line);background:#070b15;border-radius:16px;overflow:hidden}.consoleTop{display:flex;gap:8px;align-items:center;border-bottom:1px solid var(--line);padding:9px 11px;color:var(--muted)}.dots{display:flex;gap:5px}.dots i{width:8px;height:8px;border-radius:50%;background:#36425d}.cmdArea{padding:13px}.cmdArea textarea{width:100%;min-height:120px;resize:vertical}.field{display:grid;gap:6px;margin-top:10px}.field label{font-size:11px;color:var(--muted)}textarea,input,select{width:100%;background:#090e1a;border:1px solid #24314d;color:#fff;border-radius:11px;padding:11px;font:inherit;outline:none}textarea:focus,input:focus,select:focus{border-color:#526b99;box-shadow:0 0 0 3px #6d70ff1a}.checkrow{display:flex;gap:14px;flex-wrap:wrap;color:#b8c1d5;margin-top:11px}.check{display:flex;align-items:center;gap:6px}.check input{width:auto}.out{white-space:pre-wrap;overflow:auto;max-height:330px;background:#060a12;border:1px solid #172139;border-radius:12px;padding:12px;margin-top:11px;color:#cfd7e8}.feed{max-height:430px;overflow:auto}.feedItem{padding:11px 0;border-bottom:1px solid #182239}.feedItem:last-child{border-bottom:0}.feedMeta{display:flex;justify-content:space-between;gap:10px;color:#78859f;font-size:11px}.feedText{margin-top:5px;line-height:1.45}.chatBox{height:540px;overflow:auto;padding:10px}.bubble{max-width:82%;padding:11px 13px;border-radius:15px;margin:8px 0;line-height:1.55;white-space:pre-wrap}.bubble.user{margin-left:auto;background:#1a2f55}.bubble.ai{background:#11192b;border:1px solid #1d2941}.composer{display:flex;gap:9px;padding:11px;border-top:1px solid var(--line);align-items:end}.composer textarea{min-height:60px}.composer .btn{height:42px}.capGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.cap{border:1px solid var(--line);border-radius:13px;padding:12px;background:#0b1120}.cap .name{font-weight:750}.cap .state{font-size:11px;margin-top:5px}.mission{display:grid;grid-template-columns:1fr auto;gap:12px;padding:13px 0;border-bottom:1px solid var(--line)}.badge{display:inline-flex;align-items:center;border-radius:999px;padding:5px 8px;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.06em}.badge.completed,.badge.verified{background:#12332d;color:#73e6c5}.badge.planned,.badge.queued{background:#14233e;color:#96b3e7}.badge.pending_approval,.badge.approval_required{background:#3a2c14;color:#ffd98c}.badge.failed,.badge.blocked{background:#381923;color:#ff9eb0}.empty{padding:30px;text-align:center;color:var(--muted)}a{color:#9faaff}select[multiple]{min-height:180px}
@media(max-width:1080px){.app{grid-template-columns:78px 1fr}.sidebar{padding:14px 10px}.brand,.tag,.sidebarFoot{display:none}.logo{justify-content:center;padding-bottom:12px}.nav button{justify-content:center}.nav span{display:inline}.nav b{display:none}.hero{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}.grid3{grid-template-columns:1fr}.capGrid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:760px){.content{padding:14px}.top{padding:12px 14px}.hero h1{font-size:32px}.grid2{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}.heroCard{padding:19px}.capGrid{grid-template-columns:1fr}.chatBox{height:58vh}.bubble{max-width:94%}.topActions .pill{display:none}.composer{flex-direction:column;align-items:stretch}.composer .btn{width:100%}.sidebar{z-index:50}}
</style></head>
<body>
<div class="app">
<aside class="sidebar"><div class="logo"><div class="mark">∞</div><div><div class="brand">AI Infinity</div><div class="tag">command · intelligence · reality</div></div></div>
<nav class="nav">
<button class="active" data-view="overview"><span>◈</span><b>Overview</b></button>
<button data-view="chat"><span>◌</span><b>Talk</b></button>
<button data-view="command"><span>⌁</span><b>Command</b></button>
<button data-view="missions"><span>▣</span><b>Missions</b></button>
<button data-view="connections"><span>◫</span><b>Connections</b></button>
<button data-view="authority"><span>◇</span><b>Authority</b></button>
<button data-view="system"><span>◉</span><b>System</b></button>
</nav><div class="sidebarFoot">Free-first by design.<br>External capabilities activate only through real connections and explicit authority.</div></aside>
<div class="main">
<header class="top"><div class="toprow"><div><div id="pageTitle" class="pageTitle">Command Center</div><div class="tiny muted">AI Infinity · TARGET-2050.2099</div></div><div class="topActions"><span id="livePill" class="pill">checking…</span><button class="btn ghost" onclick="refreshAll()">Refresh</button></div></div></header>
<main class="content">
<section id="view-overview" class="view active">
<div class="hero"><div class="heroCard"><div class="heroGlow"></div><div class="eyebrow">REAL-WORLD COMMAND PLATFORM</div><h1>Turn a human goal into a verified outcome.</h1><p>AI Infinity combines conversation, planning, research, durable missions, live connectors, browser/device bridges, authority, verification and recovery in one control surface.</p><div class="ctaRow"><button class="btn primary" onclick="showView('chat')">Talk to Infinity</button><button class="btn" onclick="showView('command')">Create command</button><a class="btn ghost" href="/nexus-199" target="_blank">Activation center</a></div></div>
<div class="card"><h3>Reality status</h3><div id="realityRows"></div></div></div>
<div class="stats" id="stats"></div>
<div class="grid2"><div class="card"><div class="sectionHead"><h3>Live activity</h3><button class="btn ghost" onclick="loadActivity()">↻</button></div><div id="activity" class="feed"></div></div><div class="card"><h3>Capability map</h3><div id="readyCaps" class="capGrid"></div></div></div>
</section>
<section id="view-chat" class="view"><div class="sectionHead"><div><h2>Talk to AI Infinity</h2><div class="muted">Context is persistent. Execution is always explicit.</div></div></div><div class="card"><div id="chatBox" class="chatBox"></div><div class="composer"><textarea id="chatInput" placeholder="Tell Infinity the outcome you want…"></textarea><button class="btn primary" onclick="sendChat()">Send</button></div><div class="checkrow"><label class="check"><input id="chatExecute" type="checkbox"> Execute supported actions</label><label class="check"><input id="chatApprove" type="checkbox"> I explicitly authorize this request</label></div><div class="field"><label>Operator token · required for sensitive live actions</label><input id="chatToken" type="password" autocomplete="off" placeholder="Optional until the operator gate is configured"></div></div></section>
<section id="view-command" class="view"><div class="sectionHead"><div><h2>Command Center</h2><div class="muted">Describe the real-world outcome. Preview is safe; execution is gated.</div></div></div><div class="console"><div class="consoleTop"><div class="dots"><i></i><i></i><i></i></div><span>∞ / command</span></div><div class="cmdArea"><textarea id="commandInput" placeholder="Examples:\nresearch the reliability of autonomous AI agents\nopen https://example.com\nnotify my phone: Meeting starts in 10 minutes\nsend an email to someone@example.com"></textarea><div class="checkrow"><label class="check"><input id="commandApprove" type="checkbox"> Approve side effects</label></div><div class="field"><label>Operator token · required for sensitive live actions</label><input id="commandToken" type="password" autocomplete="off"></div><div class="ctaRow"><button class="btn" onclick="simulateCommand()">Preview plan</button><button class="btn primary" onclick="executeCommand()">Execute</button></div><div id="commandOut" class="out">Awaiting a command.</div></div></div><div class="grid2"><div class="card"><h3>Safety contract</h3><div id="securityRows"></div></div><div class="card"><h3>Execution lifecycle</h3><div class="row"><span>1 · understand</span><span>intent + capabilities</span></div><div class="row"><span>2 · plan</span><span>dependencies + approvals</span></div><div class="row"><span>3 · execute</span><span>connectors / bridges</span></div><div class="row"><span>4 · verify</span><span>read-back evidence</span></div><div class="row"><span>5 · recover</span><span>safe reconciliation</span></div></div></div></section>
<section id="view-missions" class="view"><div class="sectionHead"><div><h2>Missions</h2><div class="muted">Durable objectives and their latest state.</div></div><button class="btn ghost" onclick="loadMissions()">Refresh</button></div><div class="card"><div id="missions"></div></div></section>
<section id="view-connections" class="view"><div class="sectionHead"><div><h2>Connections</h2><div class="muted">Nothing is marked live unless the underlying account or bridge exists.</div></div><a class="btn" href="/nexus-199" target="_blank">Open activation center</a></div><div class="grid2"><div class="card"><h3>Providers & accounts</h3><div id="providers"></div></div><div class="card"><h3>Browser & device bridges</h3><div id="bridges"></div></div></div></section>
<section id="view-authority" class="view"><div class="sectionHead"><div><h2>Human Authority</h2><div class="muted">Scoped, revocable permission for the operator.</div></div></div><div class="grid2"><div class="card"><h3>Grant authority</h3><div class="field"><label>User / owner id</label><input id="authUser" value="default"></div><div class="field"><label>Scopes · multi-select with Ctrl/⌘</label><select id="authScopes" multiple><option selected>research</option><option selected>memory</option><option>conversation</option><option>reasoning</option><option>mission_execution</option><option>http_actions</option><option>browser</option><option>device</option><option>gmail</option><option>github</option><option>microsoft</option><option>slack</option><option>scheduling</option><option>verification</option><option>recovery</option></select></div><div class="field"><label>Expires in seconds</label><input id="authExpiry" type="number" min="60" value="86400"></div><div class="field"><label>Operator token · required for sensitive scopes</label><input id="authToken" type="password" autocomplete="off"></div><button class="btn primary" style="margin-top:12px" onclick="grantAuthority()">Grant</button><div id="authOut" class="out">No change yet.</div></div><div class="card"><h3>Current authority</h3><div class="field"><label>User / owner id</label><input id="authStatusUser" value="default"></div><button class="btn" style="margin-top:10px" onclick="loadAuthority()">Load</button><div id="authList" style="margin-top:12px"></div></div></div></section>
<section id="view-system" class="view"><div class="sectionHead"><div><h2>System</h2><div class="muted">Operational truth, model routing, security and platform diagnostics.</div></div></div><div class="grid3"><div class="card"><h3>Model router</h3><div id="modelRows"></div></div><div class="card"><h3>Activation</h3><div id="activationRows"></div></div><div class="card"><h3>Security</h3><div id="systemSecurityRows"></div></div></div><div class="grid2"><div class="card"><h3>Capabilities</h3><div id="capGrid" class="capGrid"></div></div><div class="card"><h3>Developer surface</h3><div class="row"><span>API docs</span><a href="/docs" target="_blank">Open</a></div><div class="row"><span>OpenAPI</span><a href="/openapi.json" target="_blank">JSON</a></div><div class="row"><span>2088 check</span><a href="/self-test-2088" target="_blank">Open</a></div><div class="row"><span>2099 full regression</span><a href="/self-test-2099-full" target="_blank">Open</a></div><div class="row"><span>Activation overview</span><a href="/activation/overview" target="_blank">Open</a></div></div></div></section>
</main></div></div>
<script>
const $=id=>document.getElementById(id);let conversationId=null;const userId=localStorage.getItem('ai-infinity-user')||('web-'+crypto.randomUUID());localStorage.setItem('ai-infinity-user',userId);const views=['overview','chat','command','missions','connections','authority','system'];
function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
async function api(url,opts){const r=await fetch(url,opts);const t=await r.text();let j;try{j=JSON.parse(t)}catch{throw Error(t||('HTTP '+r.status))}if(!r.ok)throw Error(j.detail||j.reason||j.error||('HTTP '+r.status));return j}
function showView(v){views.forEach(x=>$('view-'+x).classList.toggle('active',x===v));document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===v));$('pageTitle').textContent=v==='overview'?'Command Center':v.charAt(0).toUpperCase()+v.slice(1);window.scrollTo(0,0);if(v==='chat')loadChat();if(v==='missions')loadMissions();if(v==='connections')loadConnections();if(v==='authority')loadAuthority();if(v==='system')loadSystem()}
document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>showView(b.dataset.view));
function row(label,value,cls=''){return '<div class="row"><span>'+esc(label)+'</span><span class="'+cls+'">'+value+'</span></div>'}
function badge(v){const k=String(v||'unknown').toLowerCase();return '<span class="badge '+esc(k)+'">'+esc(v||'unknown')+'</span>'}
async function loadDashboard(){try{const d=await api('/infinity/dashboard');const a=d.activation||{};$('livePill').className='pill ok';$('livePill').textContent='ONLINE · '+d.version.split('.').pop();$('realityRows').innerHTML=[row('Database','<span class="statusDot ok"></span> healthy'),row('Model',d.model.configured?'configured':'built-in fallback',d.model.configured?'ok':'warn'),row('Operator gate',a.operator_configured?'configured':'not configured',a.operator_configured?'ok':'warn'),row('Browser',a.browser_ready?'connected':'activate bridge',a.browser_ready?'ok':'warn'),row('Device',a.device_ready?'connected':'activate bridge',a.device_ready?'ok':'warn'),row('Scheduler',d.scheduler.healthy?'healthy':'not ready',d.scheduler.healthy?'ok':'warn')].join('');const c=d.counts||{};$('stats').innerHTML=Object.entries({Conversations:c.conversations||0,Messages:c.messages||0,Missions:c.missions||0,Transactions:c.transactions||0}).map(([k,v])=>'<div class="stat"><div class="n">'+esc(v)+'</div><div class="l">'+esc(k)+'</div></div>').join('');$('securityRows').innerHTML=Object.entries(d.safety||{}).map(([k,v])=>row(k,v?'<span class="statusDot ok"></span> enforced':'<span class="statusDot bad"></span> disabled')).join('')}catch(e){$('livePill').className='pill warn';$('livePill').textContent='DEGRADED';$('realityRows').innerHTML=row('Error',esc(e.message),'warn')}}
async function loadActivity(){try{const d=await api('/infinity/activity?limit=40');$('activity').innerHTML=d.items.length?d.items.map(x=>'<div class="feedItem"><div class="feedMeta"><span>'+esc(x.type)+' · '+esc(x.label)+'</span><span>'+esc(x.timestamp||'')+'</span></div><div class="feedText">'+esc(x.summary)+'</div></div>').join(''):'<div class="empty">No activity yet.</div>'}catch(e){$('activity').textContent=e.message}}
async function loadCapabilities(target){try{const d=await api('/infinity/capabilities');$(target).innerHTML=d.capabilities.map(x=>'<div class="cap"><div class="name">'+esc(x.name)+'</div><div class="state '+(x.requires_connection?'warn':'ok')+'">'+(x.requires_connection?'CONNECT':'READY')+'</div><div class="tiny muted">'+esc(x.description)+'</div></div>').join('')}catch(e){$(target).textContent=e.message}}
async function loadConnections(){try{const d=await api('/infinity/connections');$('providers').innerHTML=(d.providers||[]).map(x=>row(x.provider,(x.ready?'<span class="statusDot ok"></span> ready':'<span class="statusDot warn"></span> not ready')+' · '+esc(x.account_count||0)+' account(s)')).join('')||'<div class="empty">No provider accounts.</div>';const bridges=d.bridges||[];$('bridges').innerHTML=bridges.length?bridges.map(x=>row(x.kind,(x.healthy_now?'<span class="statusDot ok"></span> healthy':'<span class="statusDot warn"></span> offline'))).join(''):'<div class="empty">No paired bridges.</div>'}catch(e){$('providers').textContent=e.message}}
async function loadMissions(){try{const d=await api('/infinity/missions?limit=50');$('missions').innerHTML=d.missions.length?d.missions.map(x=>'<div class="mission"><div><strong>'+esc(x.objective)+'</strong><div class="tiny muted">'+esc(x.id)+' · '+esc(x.mode||'mission')+' · '+esc(x.updated_at_iso||'')+'</div></div><div>'+badge(x.status)+'</div></div>').join(''):'<div class="empty">No missions yet.</div>'}catch(e){$('missions').textContent=e.message}}
async function loadSystem(){try{const [m,d,s]=await Promise.all([api('/intelligence/models'),api('/infinity/dashboard'),api('/infinity/security')]);$('modelRows').innerHTML=[row('Configured',m.configured?'yes':'no',m.configured?'ok':'warn'),row('Fallback','available'),row('Paid dependency','none')].join('')+(m.providers||[]).map(x=>row(x.provider,esc(x.model))).join('');$('activationRows').innerHTML=[row('Vault',d.activation.vault_configured?'configured':'missing'),row('Operator',d.activation.operator_configured?'configured':'missing'),row('Browser',d.activation.browser_ready?'connected':'not connected'),row('Device',d.activation.device_ready?'connected':'not connected'),row('Google',d.activation.google_ready?'connected':'not connected')].join('');$('systemSecurityRows').innerHTML=Object.entries(s.safety||{}).map(([k,v])=>row(k,v?'<span class="statusDot ok"></span> yes':'<span class="statusDot warn"></span> no')).join('');loadCapabilities('capGrid')}catch(e){$('modelRows').textContent=e.message}}
async function sendChat(){const m=$('chatInput').value.trim();if(!m)return;appendBubble('user',m);$('chatInput').value='';try{const j=await api('/chat',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({message:m,conversation_id:conversationId,user_id:userId,execute:$('chatExecute').checked,authorized:$('chatApprove').checked,operator_token:$('chatToken').value})});conversationId=j.conversation_id;appendBubble('ai',j.message.content);if(j.execution)appendBubble('ai','Execution · '+JSON.stringify(j.execution,null,2));$('chatBox').scrollTop=$('chatBox').scrollHeight;loadActivity()}catch(e){appendBubble('ai','Error · '+e.message)}}
function appendBubble(role,text){$('chatBox').insertAdjacentHTML('beforeend','<div class="bubble '+role+'">'+esc(text)+'</div>')}
async function loadChat(){if(conversationId)return;appendBubble('ai','Hello. I am AI Infinity. Tell me the outcome you want. I can plan it safely, execute supported actions when explicitly authorized, and report what is actually configured.')}
async function simulateCommand(){const q=$('commandInput').value.trim();if(!q)return;try{$('commandOut').textContent=JSON.stringify(await api('/infinity/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,simulate:true})}),null,2)}catch(e){$('commandOut').textContent='Error · '+e.message}}
async function executeCommand(){const q=$('commandInput').value.trim();if(!q)return;try{$('commandOut').textContent=JSON.stringify(await api('/infinity/command',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command:q,simulate:false,approved:$('commandApprove').checked,operator_token:$('commandToken').value})}),null,2);loadMissions();loadActivity();loadDashboard()}catch(e){$('commandOut').textContent='Error · '+e.message}}
async function grantAuthority(){const scopes=[...$('authScopes').selectedOptions].map(x=>x.value);try{$('authOut').textContent=JSON.stringify(await api('/authority/grant',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({user_id:$('authUser').value.trim()||userId,scopes,expires_seconds:Number($('authExpiry').value||86400),operator_token:$('authToken').value})}),null,2);loadAuthority();loadDashboard()}catch(e){$('authOut').textContent='Error · '+e.message}}
async function loadAuthority(){const u=$('authStatusUser').value.trim()||userId;try{const d=await api('/authority/status/'+encodeURIComponent(u));$('authList').innerHTML=(d.authority||[]).map(x=>'<div class="card" style="margin-top:8px;background:#090f1b"><div class="row"><span>'+esc(x.id)+'</span>'+badge(x.active?'active':x.status)+'</div><div class="tiny muted">'+esc((x.scopes||[]).join(', '))+'</div><div class="tiny muted">expires: '+esc(x.expires_at?new Date(x.expires_at*1000).toISOString():'never')+'</div><button class="btn danger" style="margin-top:8px" onclick="revokeAuthority(\''+esc(x.id)+'\')">Revoke</button></div>').join('')||'<div class="empty">No authority grants.</div>'}catch(e){$('authList').textContent=e.message}}
async function revokeAuthority(id){try{await api('/authority/revoke/'+encodeURIComponent(id),{method:'POST'});loadAuthority();loadDashboard()}catch(e){alert(e.message)}}
async function refreshAll(){await Promise.all([loadDashboard(),loadActivity(),loadCapabilities('readyCaps')]);if($('view-system').classList.contains('active'))loadSystem()}
refreshAll();setInterval(refreshAll,15000);if('serviceWorker' in navigator){navigator.serviceWorker.register('/platform/sw.js').catch(()=>{})}
$('chatInput').addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter')sendChat()});
</script></body></html>'''

# Ensure the public root reports the current unified build after this layer is loaded.
try:
    def root():
        return RedirectResponse(url="/platform/ui", status_code=307)
except Exception:
...
def platform_status_2099():
    return _dashboard2099()


# ============================================================
# TARGET-2050.2100 — PRODUCTION FACTORY + OPEN-WORLD SOURCE DISCOVERY
# No arbitrary code execution. Factory is policy-aware and side effects remain
# behind the existing approval/operator controls.
# ============================================================

from html.parser import HTMLParser as _HTMLParser2100

class _LinkParser2100(_HTMLParser2100):
    def __init__(self):
...
def _public_source_url2100(url: str) -> bool:
    try:
        p=urlparse(url)
        if p.scheme not in {"http","https"} or not p.hostname: return False
        h=p.hostname.lower().rstrip(".")
        if h in BLOCKED_HOSTS: return False
        infos=socket.getaddrinfo(h, p.port or (443 if p.scheme=="https" else 80), type=socket.SOCK_STREAM)
        for item in infos:
            ip=ipaddress.ip_address(item[4][0])
            if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_multicast or ip.is_reserved or ip.is_unspecified:
...
def _discover_sources2100(query: str, limit: int = 12) -> Dict[str, Any]:
    ...
    headers={"User-Agent":"AI-Infinity-Source-Discovery/2100"}
    for provider,url in endpoints:
        try:
            req=Request(url,headers=headers,method="GET")
            with build_opener(HTTPRedirectHandler()).open(req,timeout=10) as r:
                html=r.read(700000).decode("utf-8","replace")
            parser=_LinkParser2100(); parser.feed(html)
            local=0
            for href in parser.links:
                if href.startswith("//"): href="https:"+href
                if href.startswith("/"):
                    continue
                if href.startswith("http") and _public_source_url2100(href):
                    u=href.split("#",1)[0]
                    host=urlparse(u).hostname or ""
                    if host.endswith("duckduckgo.com") or host.endswith("bing.com") or host.endswith("google.com"):
                        continue
                    if u not in seen:
                        seen.add(u); found.append({"url":u,"domain":host.lower(),"discovered_by":provider}); local+=1
                    if len(found)>=limit: break
            if len(found)>=limit: break
        except Exception as exc:
            errors.append({"provider":provider,"error":str(exc)[:240]})
    return {"query":q,"sources":found[:max(1,min(limit,50))],"source_count":len(found),"errors":errors,"method":"open-web-discovery","truthful":True}

@app.post("/infinity/discovery/search")
def infinity_discovery_search_2100(body: Dict[str, Any]):
    return _discover_sources2100(str(body.get("query") or ""), int(body.get("limit") or 12))

@app.get("/infinity/discovery/search")
def infinity_discovery_search_get_2100(query: str, limit: int = 12):
    return _discover_sources2100(query, limit)

def _factory_plan2100(objective: str) -> Dict[str, Any]:
    objective=str(objective or "").strip()[:20000]
    if not objective: raise HTTPException(400,"objective required")
    base=_plan_200(objective)
    intent=_intent_2088(objective)
    phases=[
        {"phase":"understand","status":"ready","action":"normalize objective and identify intent"},
        {"phase":"discover","status":"ready","action":"discover relevant capabilities and open-web sources when research is required"},
        {"phase":"plan","status":"ready" if base.get("can_execute") else "blocked","action":"compile executable steps with dependency checks"},
        {"phase":"authorize","status":"approval_required" if base.get("approval_required") or base.get("operator_gate_required") else "not_required","action":"apply scoped authority and operator policy"},
        {"phase":"execute","status":"ready" if base.get("can_execute") else "blocked","action":"run through durable execution/bridge/connector layers"},
        {"phase":"verify","status":"ready","action":"verify outputs and external outcomes"},
        {"phase":"close","status":"ready","action":"persist receipt, evidence, memory and audit trail"},
    ]
    return {"status":"ready" if base.get("can_execute") else "blocked","objective":objective,"intent":intent,"capabilities":base.get("capabilities",[]),"missing_dependencies":base.get("missing_dependencies",[]),"approval_required":bool(base.get("approval_required")),"operator_gate_required":bool(base.get("operator_gate_required")),"plan_hash":base.get("plan_hash"),"phases":phases,"execution_contract":{"arbitrary_code_execution":False,"automatic_uncertain_side_effect_replay":False,"side_effects_require_policy":True},"factory":"AI Infinity Production Factory 2100"}

@app.post("/infinity/factory/plan")
def infinity_factory_plan_2100(body: Dict[str, Any]):
    return _factory_plan2100(str(body.get("objective") or ""))

@app.post("/infinity/factory/execute")
def infinity_factory_execute_2100(body: Dict[str, Any]):
    objective=str(body.get("objective") or "").strip()
    if not objective: raise HTTPException(400,"objective required")
    plan=_factory_plan2100(objective)
    if plan.get("missing_dependencies"):
        return {"status":"blocked","factory_plan":plan,"next_action":"activate_missing_dependencies"}
    payload=dict(body); payload["objective"]=objective
    result=agent_execute_200(payload)
    return {"status":result.get("status","completed"),"factory_plan":plan,"result":result}

@app.get("/infinity/factory")
def infinity_factory_status_2100():
    return {"name":"AI Infinity Production Factory","version":UNIFIED_TARGET,"status":"online","stages":["understand","discover","plan","authorize","execute","verify","close"],"arbitrary_code_execution":False,"uncertain_external_replay":False,"requires_human_authority_for_side_effects":True}

@app.get("/self-test-2100")
def self_test_2100():
    tests=[]
    def T(name,fn):
        try: tests.append({"name":name,"passed":bool(fn())})
        except Exception as exc: tests.append({"name":name,"passed":False,"error":str(exc)[:300]})
    T("version",lambda: getattr(app,"version",None)==UNIFIED_TARGET)
    T("root website redirect",lambda: any(getattr(r,"path",None)=="/" for r in app.routes))
    T("factory planning",lambda: _factory_plan2100("research autonomous AI reliability")["factory"].endswith("2100"))
    T("unknown-source discovery contract",lambda: urlparse("https://example.com").scheme in {"http","https"} and bool(urlparse("https://example.com").hostname))
    T("legacy 2099 regression",lambda: self_test_2099_full().get("passed") is True)
    T("security invariant",lambda: infinity_security_2099()["safety"]["arbitrary_code_execution"] is False)
    return {"status":"completed","version":UNIFIED_TARGET,"build":UNIFIED_BUILD,"passed":all(x["passed"] for x in tests),"tests":tests,"factory":True,"open_web_discovery":True}

# Keep FastAPI/OpenAPI metadata synchronized with the outward build.
