...
app = FastAPI(title="AI Infinity", version="TARGET-2050.102")

# ============================================================
# VERSION
# ============================================================

APP_VERSION = "TARGET-2050.102"
BUILD = "REAL-WORLD-COMMAND-EXECUTION-TRANSACTION-SAFETY-CORE"
...
def real_world_command(
    ...
    """Stage an external command. Never executes an external side effect."""
    ...
    payload = {"method": method, "body": request.body or {}, "headers": headers}
    ...
    write("""INSERT INTO action_log(mission_id,action,status,details_json,ts,action_type,target,idempotency_key,result_json) VALUES(?,?,?,?,?,?,?,?,?)""",
          (request.mission_id, "external_action", "awaiting_approval",
           json.dumps({"approval_required": True, "action_hash": action_hash, "checkpoint": "pre_approval"}),
    ...
    return {
        ...
        "execution_path": "stage -> explicit approval -> execute",
        "transaction": transaction_public_view(row),
    }
...
def execute_approved_action_transaction(transaction_id: str):
    """Final execution boundary for an approved external transaction."""
    row=q("SELECT * FROM action_transactions WHERE id=?", (transaction_id,), one=True)
    if not row: raise HTTPException(status_code=404, detail="action transaction not found")
    if row["status"] == "awaiting_approval": raise HTTPException(status_code=409, detail="execution_requires_approval")
    if row["status"] in {"committed","failed_closed","rejected"}:
        return {"status":row["status"], "transaction_id":transaction_id, "idempotent_replay":True,
                "transaction":transaction_public_view(row)}
    if row["status"] != "approved": raise HTTPException(status_code=409, detail="transaction_not_executable_in_current_state")
    ...
    if not updated: raise HTTPException(status_code=409, detail="execution_claim_lost")
    ...
    write("INSERT INTO action_snapshots(transaction_id,snapshot_json,created_at) VALUES(?,?,?)",
          (transaction_id,json.dumps({"checkpoint":"pre_execution","action_hash":final_hash,"target":target,"method":normalized["method"]}),now()))
    write("""INSERT INTO action_log(mission_id,action,status,details_json,ts,action_type,target,idempotency_key,result_json) VALUES(?,?,?,?,?,?,?,?,?)""",
          (executing["mission_id"],"external_action","executing",json.dumps({"approval_used":True,"checkpoint":"pre_execution","action_hash":final_hash}),
    ...
    try:
        ...
        result={"ok":False,"outcome":"unknown_remote_outcome","error":str(exc)[:500],"replay_blocked":True}
    status_code=int(result.get("status_code",0) or 0)
    reached=status_code>0
    http_success=200 <= status_code < 300
    unknown=result.get("outcome") in {"unknown_remote_outcome","not_executed"}
    final_status="failed_closed" if unknown or not http_success else "committed"
    outcome={**result,"transport_reached_target":reached,"http_success":http_success,
             "business_outcome_verified":False,"verification_scope":"transport_and_http_status_only",
             "replay_blocked":bool(result.get("replay_blocked",False) or unknown)}

    write("""UPDATE action_transactions SET status=?,result_json=?,error=?,updated_at=? WHERE id=? AND status='executing'""",
          (final_status,json.dumps(outcome,ensure_ascii=False),None if final_status=="committed" else (outcome.get("error") or outcome.get("outcome")),now(),transaction_id))
    write("INSERT INTO action_snapshots(transaction_id,snapshot_json,created_at) VALUES(?,?,?)",
          (transaction_id,json.dumps({"checkpoint":"post_execution","action_hash":final_hash,"outcome":outcome},ensure_ascii=False),now()))
    write("""INSERT INTO action_log(mission_id,action,status,details_json,ts,action_type,target,idempotency_key,result_json) VALUES(?,?,?,?,?,?,?,?,?)""",
          (executing["mission_id"],"external_action",final_status,json.dumps({"approval_used":True,"post_execution_verification":{
              "transport_reached_target":reached,"http_success":http_success,"business_outcome_verified":False},"replay_blocked":outcome["replay_blocked"]}),
           now(),executing["action"],target,executing["idempotency_key"],json.dumps(outcome,ensure_ascii=False)))
    row=q("SELECT * FROM action_transactions WHERE id=?",(transaction_id,),one=True)
    return {"status":"completed" if final_status=="committed" else "failed","transaction_id":transaction_id,
            "approved":True,"approval_used":True,"action_executed":reached,"action_verified":http_success,
            "external_side_effects":reached,"outcome":outcome.get("outcome"),"result":outcome,
            "replay_blocked":outcome["replay_blocked"],"transaction":transaction_public_view(row)}
