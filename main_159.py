...
APP_VERSION = "TARGET-2050.159"
BUILD = "AUTONOMOUS-SAFETY-APPROVAL-LAYER-CORE"
...
# ============================================================
# TARGET-2050.159 — AUTONOMOUS SAFETY / APPROVAL LAYER
# ============================================================
# Additive safety decision layer over the verified 158 adaptation engine.
# Safety decisions are persistent, explainable, approval-aware, and
# immutable with respect to core safety invariants. This layer NEVER
# executes an action itself and cannot bypass the existing action gateway.

SAFETY_159_VERSION = 1
...
def _159_init():
    ...
    CREATE TABLE IF NOT EXISTS safety_decisions_159(
        ...
        verification_required INTEGER NOT NULL DEFAULT 1,
        adaptation_policy_id TEXT,
        adaptation_policy_version INTEGER,
    ...
    """)
    write("""
    CREATE TABLE IF NOT EXISTS safety_events_159(
        id TEXT PRIMARY KEY,
...
def _159_tokens(text: str) -> set[str]:
    return set(re.findall(r"[a-z0-9_]+", (text or "").lower()))


def _159_risk(command: str, route: str = "general", external_side_effect: bool = False) -> tuple[str, float, list[str]]:
    tokens = _159_tokens(command)
    score = 0.12
    reasons = []
    if route == "research":
        score += 0.04
        ...
        score += 0.34
        reasons.append("side_effecting_action_route")
    if external_side_effect:
        score += 0.28
        reasons.append("external_side_effect")
    high = {"send","post","publish","submit","delete","remove","update","create","transfer","purchase","pay","deploy","webhook"}
    critical = {"password","credential","token","secret","private_key","wire","bank","money","irreversible","destroy"}
    matches_high = sorted(tokens & high)
    matches_critical = sorted(tokens & critical)
    if matches_high:
        score += min(0.20, 0.05 * len(matches_high))
        reasons.append("high_impact_terms:" + ",".join(matches_high[:5]))
    if matches_critical:
        score += 0.30
        if external_side_effect:
            score = max(score, 0.82)
        reasons.append("sensitive_or_irreversible_terms:" + ",".join(matches_critical[:5]))
    ...
    if score >= 0.80:
        ...
        level = "low"
    if not reasons:
        reasons.append("baseline_risk_assessment")
    return level, score, reasons


def _159_route(command: str) -> str:
    # Reuse the proven 154/158 routing when available.
    try:
        return str(_orch_route(command))
    except Exception:
        tokens = _159_tokens(command)
        if tokens & {"send","post","publish","submit","delete","update","create","webhook"}:
            return "action"
        if tokens & {"verify","validate","proof","check"}:
            return "verification"
        if tokens & {"research","search","evidence","study"}:
            return "research"
        return "general"


def _159_adaptation(command: str) -> dict:
    try:
        selected = _158_select_policy(command)
        policy = selected.get("policy", {})
        return {
            "policy_id": policy.get("policy_id"),
            "policy_version": policy.get("version"),
            "score": float(policy.get("score", 0.5)),
            "verification_required": bool(selected.get("verification_required", True)),
            "safe_fallback": bool(selected.get("safe_fallback", True)),
            "approval_boundary": bool(policy.get("policy", {}).get("approval_boundary", True)),
        }
    except Exception:
        return {"policy_id": None, "policy_version": None, "score": 0.5, "verification_required": True, "safe_fallback": True, "approval_boundary": True}


def _159_decide(command: str, external_side_effect: bool = False, explicit_approval: bool = False, authorization: str = "evaluate") -> dict:
    _159_init()
    command = str(command or "").strip()[:4000]
    if not command:
        raise HTTPException(status_code=400, detail="command_required")
    route = _159_route(command)
    level, score, reasons = _159_risk(command, route, external_side_effect)
    adaptation = _159_adaptation(command)
    if adaptation["score"] < 0.30:
        score = min(1.0, score + 0.10)
        reasons.append("low_historical_adaptation_confidence")
        if score >= 0.60:
            level = "high"
        elif score >= 0.35:
            level = "medium"
    # Core rule: every external side effect requires explicit approval;
    # high/critical risk also escalates even if a caller asks for approval.
    approval_required = bool(external_side_effect or level in {"high", "critical"})
    escalation_required = bool(level == "critical" or (level == "high" and not explicit_approval))
    verification_required = True
    if adaptation["verification_required"]:
        verification_required = True
    if not adaptation["approval_boundary"]:
        approval_required = True
        escalation_required = True
        reasons.append("adaptation_safety_invariant_forced_approval")
    if authorization == "execute" and approval_required and not explicit_approval:
        decision_auth = "denied_pending_approval"
    elif level == "critical" and not explicit_approval:
        decision_auth = "escalate"
    elif approval_required and not explicit_approval:
        decision_auth = "approval_required"
    else:
        decision_auth = "authorized_for_gateway_only"
    decision_id = make_id("safety")
    ts = now()
    safety = {
        "arbitrary_code_execution": False,
        "unrestricted_network_access": False,
        ...
        "approval_boundary": True,
        "verification_required": True,
        "gateway_execution_only": True,
    }
    write("INSERT INTO safety_decisions_159(id,command,route,risk_level,risk_score,approval_required,approval_mode,authorization,escalation_required,verification_required,adaptation_policy_id,adaptation_policy_version,reasons_json,safety_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          (decision_id,command,route,level,score,int(approval_required),"explicit_user_approval" if approval_required else "none",decision_auth,int(escalation_required),1,adaptation["policy_id"],adaptation["policy_version"],json.dumps(reasons),json.dumps(safety),ts,ts))
    write("INSERT INTO safety_events_159(id,decision_id,event_type,from_state,to_state,verified,success,details_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
          (make_id("safety-event"),decision_id,"decision_created",None,decision_auth,0,0,json.dumps({"risk_level":level,"risk_score":score,"reasons":reasons}),ts))
    return {"decision_id":decision_id,"command":command,"route":route,"risk_level":level,"risk_score":score,"approval_required":approval_required,"escalation_required":escalation_required,"authorization":decision_auth,"verification_required":True,"adaptation":adaptation,"reasons":reasons,"safety":safety}


def _159_decision_payload(row) -> dict:
    return {
        "decision_id":row["id"],"command":row["command"],"route":row["route"],
        "risk_level":row["risk_level"],"risk_score":round(float(row["risk_score"]),4),
        "approval_required":bool(row["approval_required"]),"approval_mode":row["approval_mode"],
        "authorization":row["authorization"],"escalation_required":bool(row["escalation_required"]),
        "verification_required":bool(row["verification_required"]),
        "adaptation_policy_id":row["adaptation_policy_id"],"adaptation_policy_version":row["adaptation_policy_version"],
        "reasons":json.loads(row["reasons_json"] or "[]"),"safety":json.loads(row["safety_json"] or "{}"),
    }


def _159_record_post_action(decision_id: str, verified: bool, success: bool, details: dict | None = None) -> dict:
    _159_init()
    row=q("SELECT * FROM safety_decisions_159 WHERE id=?",(decision_id,),one=True)
    if not row:
        return {"updated":False,"reason":"safety_decision_not_found"}
    verified=bool(verified); success=bool(success)
    outcome="verified_success" if verified and success else "verified_failure" if verified else "unverified"
    write("INSERT INTO safety_events_159(id,decision_id,event_type,from_state,to_state,verified,success,details_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
          (make_id("safety-event"),decision_id,"post_action_verification",row["authorization"],outcome,int(verified),int(success),json.dumps(details or {}),now()))
    new_auth = "verified_success" if verified and success else "verified_failure" if verified else row["authorization"]
    write("UPDATE safety_decisions_159 SET authorization=?,updated_at=? WHERE id=?",(new_auth,now(),decision_id))
    # Feed verified outcome back into the 158 adaptation engine when possible.
    adaptation_update=None
    try:
        if row["adaptation_policy_id"]:
            usage=q("SELECT id FROM adaptation_usage_158 WHERE policy_id=? ORDER BY created_at DESC LIMIT 1",(row["adaptation_policy_id"],),one=True)
            if usage:
                adaptation_update=_158_adapt_outcome(usage["id"],verified,success,{"reason":"159_post_action_verification","decision_id":decision_id})
    except Exception as exc:
        adaptation_update={"updated":False,"reason":str(exc)[:200]}
    return {"updated":True,"decision_id":decision_id,"outcome":outcome,"authorization":new_auth,"adaptation_update":adaptation_update,"verification_required":True}


def safety_layer_status_159():
    _159_init()
    decisions=q("SELECT COUNT(*) n FROM safety_decisions_159",one=True)
    escalations=q("SELECT COUNT(*) n FROM safety_decisions_159 WHERE escalation_required=1",one=True)
    approvals=q("SELECT COUNT(*) n FROM safety_decisions_159 WHERE approval_required=1",one=True)
    events=q("SELECT COUNT(*) n FROM safety_events_159",one=True)
    verified=q("SELECT COUNT(*) n FROM safety_events_159 WHERE event_type='post_action_verification' AND verified=1",one=True)
    return {"status":"ready","version":APP_VERSION,"build":BUILD,
            "safety":{"continuous_evaluation":True,"dynamic_risk":True,"dynamic_approval":True,"authorization":True,"escalation":True,"post_action_verification":True,"adaptation_aware":True,"persistent_audit":True,"safe_fallback":True,"verification_required":True,"credential_non_persistence":True,"arbitrary_code_execution":False,"unrestricted_network_access":False,"approval_boundary":True},
            "counts":{"decisions":int(decisions["n"]),"approval_required":int(approvals["n"]),"escalations":int(escalations["n"]),"events":int(events["n"]),"verified_post_actions":int(verified["n"])},
            "preserved_158_contract":adaptation_engine_status_158()["adaptation"]}


@app.get("/159-status")
def status_159():
    st=safety_layer_status_159()
    return {"status":"ready","version":APP_VERSION,"build":BUILD,"previous_build":"TARGET-2050.158 CONTINUOUS-ADAPTATION-ENGINE-CORE","safety":st["safety"],"counts":st["counts"],"preserved_158_contract":st["preserved_158_contract"]}


@app.post("/safety/evaluate")
def safety_evaluate_159(request: RealCommandInterfaceRequest155):
    return {"status":"evaluated","version":APP_VERSION,"build":BUILD,**_159_decide(request.command,False,False,"evaluate")}


@app.post("/safety/authorize")
def safety_authorize_159(request: dict):
    command=str(request.get("command") or "").strip()
    external=bool(request.get("external_side_effect",False))
    explicit=bool(request.get("explicit_approval",False))
    return {"status":"evaluated","version":APP_VERSION,"build":BUILD,**_159_decide(command,external,explicit,"execute")}


@app.post("/safety/post-action")
def safety_post_action_159(request: dict):
    decision_id=str(request.get("decision_id") or "").strip()
    if not decision_id: raise HTTPException(status_code=400,detail="decision_id_required")
    return {"status":"recorded","version":APP_VERSION,"build":BUILD,**_159_record_post_action(decision_id,bool(request.get("verified",False)),bool(request.get("success",False)),request.get("details") if isinstance(request.get("details"),dict) else {})}


@app.get("/safety/self-test")
def safety_self_test_159():
    _159_init(); checks={}
    low=_159_decide("research and summarize public evidence about autonomous agents")
    checks["continuous_safety_evaluation"]=bool(low.get("decision_id"))
    checks["dynamic_risk"]=low.get("risk_level") in {"low","medium","high","critical"}
    action=_159_decide("send a webhook to publish the verified result",True,False,"execute")
    checks["dynamic_approval"]=action.get("approval_required") is True
    checks["authorization_decision"]=action.get("authorization") in {"approval_required","denied_pending_approval","escalate"}
    critical=_159_decide("transfer money using a secret credential",True,False,"execute")
    checks["high_risk_escalation"]=critical.get("escalation_required") is True and critical.get("risk_level")=="critical"
    checks["persistent_decision"]=bool(q("SELECT id FROM safety_decisions_159 WHERE id=?",(low["decision_id"],),one=True))
    post=_159_record_post_action(low["decision_id"],True,True,{"self_test":True})
    checks["post_action_verification"]=post.get("outcome")=="verified_success"
    checks["adaptation_aware"]=bool(low.get("adaptation",{}).get("policy_id"))
    checks["audit_persistence"]=int(q("SELECT COUNT(*) n FROM safety_events_159 WHERE decision_id=?",(low["decision_id"],),one=True)["n"])>=2
    st=safety_layer_status_159()
    checks["credential_non_persistence"]=st["safety"]["credential_non_persistence"] is True
    checks["no_arbitrary_code"]=st["safety"]["arbitrary_code_execution"] is False
    checks["no_unrestricted_network"]=st["safety"]["unrestricted_network_access"] is False
    checks["approval_boundary"]=st["safety"]["approval_boundary"] is True
    preserved=st["preserved_158_contract"]
    checks["preserved_158_contract"]=all(preserved.get(k) is True for k in ["continuous","persistent","outcome_driven","verified_adaptation","policy_versioning","genome_integration","capability_adaptation","safe_fallback","verification_required","credential_non_persistence"]) and preserved.get("arbitrary_code_execution") is False and preserved.get("unrestricted_network_access") is False and preserved.get("approval_boundary") is True
    return {"status":"passed" if all(checks.values()) else "failed","version":APP_VERSION,"build":BUILD,"failed_checks":[k for k,v in checks.items() if not v],"checks":checks,"sample":{"decision_id":low["decision_id"],"action_decision_id":action["decision_id"],"critical_decision_id":critical["decision_id"]}}


@app.get("/safety/{decision_id}")
def safety_get_159(decision_id: str):
    row=q("SELECT * FROM safety_decisions_159 WHERE id=?",(decision_id,),one=True)
    if not row: raise HTTPException(status_code=404,detail="safety_decision_not_found")
    events=q("SELECT * FROM safety_events_159 WHERE decision_id=? ORDER BY created_at DESC",(decision_id,))
    return {"status":"ready","version":APP_VERSION,"build":BUILD,"decision":_159_decision_payload(row),"events":[dict(e) for e in events]}
