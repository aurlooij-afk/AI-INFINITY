...
APP_VERSION="TARGET-2050.179"
BUILD="REAL-WORLD-ACTION-STATE-AND-IDEMPOTENCY-CORE"
PREVIOUS_BUILD="TARGET-2050.178"
...
def init_db():
    with LOCK,conn() as c:
        ...
        CREATE TABLE IF NOT EXISTS action_idempotency(idempotency_key TEXT PRIMARY KEY,request_hash TEXT NOT NULL,execution_id TEXT,status TEXT NOT NULL,result_json TEXT,created_at REAL,updated_at REAL);
        CREATE INDEX IF NOT EXISTS idx_action_idempotency_execution ON action_idempotency(execution_id);
...
def action_state(key):
    if not key:return None
    with LOCK,conn() as c:r=c.execute("SELECT * FROM action_idempotency WHERE idempotency_key=?",(key,)).fetchone()
    if not r:return None
    d=dict(r); d["result"]=json.loads(d.pop("result_json") or "null"); return d

def reserve_action(key,request_hash):
    if not key:return {"reused":False,"record":None}
    old=action_state(key)
    if old:
        if old["request_hash"]!=request_hash: raise HTTPException(409,"idempotency key reused with different request")
        return {"reused":True,"record":old}
    t=now()
    try:
        with LOCK,conn() as c:c.execute("INSERT INTO action_idempotency(idempotency_key,request_hash,status,created_at,updated_at) VALUES(?,?,?,?,?)",(key,request_hash,"reserved",t,t))
    except sqlite3.IntegrityError:
        old=action_state(key)
        if old and old["request_hash"]==request_hash:return {"reused":True,"record":old}
        raise HTTPException(409,"idempotency key conflict")
    return {"reused":False,"record":None}

def bind_action(key,eid,status="running"):
    if key:
        with LOCK,conn() as c:c.execute("UPDATE action_idempotency SET execution_id=?,status=?,updated_at=? WHERE idempotency_key=?",(eid,status,now(),key))

def close_action(key,status,result=None):
    if key:
        with LOCK,conn() as c:c.execute("UPDATE action_idempotency SET status=?,result_json=?,updated_at=? WHERE idempotency_key=?",(status,dumps(result) if result is not None else None,now(),key))

def recover_action_state():
    cutoff=now()-max(30,int(os.getenv("AI_INFINITY_ACTION_STALE_SECONDS","120")))
    with LOCK,conn() as c:c.execute("UPDATE action_idempotency SET status='uncertain',updated_at=? WHERE status IN ('reserved','running') AND updated_at<?",(now(),cutoff))

class RunRequest(BaseModel):
    command: Optional[str]=None; objective: Optional[str]=None; execute: bool=True; require_approval: bool=True; idempotency_key: Optional[str]=None
...
def health():
    ...
    return {'status':'healthy','version':APP_VERSION,'build':BUILD,'database':'ready','policy_version':1,'external_execution_enabled':True,'verification_enabled':True,'adaptive_recovery_enabled':True,'self_modification_enabled':True,'intent_router_enabled':True,'result_closure_enabled':True,'adaptive_decision_enabled':True,'workflow_resume_enabled':True,'action_state_enabled':True,'idempotency_enforced':True,'duplicate_action_suppression':True,'execution_count':counts[0],'workflow_count':counts[1],'memory_count':counts[2]}
@app.get('/status')
def status():return health()
@app.get('/capabilities')
def capabilities():return {'capabilities':['real_world_commands','http_actions','approval_gate','ssrf_protection','result_closure','durable_execution','adaptive_decisions','workflow_dependencies','workflow_resume','workflow_idempotency','persistent_context','memory','action_state','idempotency_enforcement','duplicate_action_suppression','restart_safe_state']}
...
def command(req:RunRequest):
    ...
    reservation=reserve_action(key,digest({'command':cmd,'plan':plan,'require_approval':req.require_approval})) if key else {'reused':False}
    if reservation.get('reused'):
        rec=reservation['record']; eid=rec.get('execution_id')
        return {'execution_id':eid,'status':rec['status'],'plan':plan,'idempotent_reuse':True,'action_state':rec}
    eid=execute_plan(plan,objective=cmd)
    bind_action(key,eid,get_exec(eid)['status'])
    current=get_exec(eid)
    if current['status']=='completed':close_action(key,'completed',current.get('result'))
    elif current['status'] in {'uncertain','failed','rejected'}:close_action(key,current['status'],current.get('result'))
    return {'execution_id':eid,'status':current['status'],'plan':plan,'idempotent_reuse':False,'action_state':action_state(key) if key else None}
...
@app.get('/action-state/{idempotency_key}')
def get_action_state(idempotency_key:str):
    recover_action_state(); r=action_state(idempotency_key)
    if not r:raise HTTPException(404,'idempotency key not found')
    return {'status':'ready','action_state':r}

@app.post('/action-state/recover')
def action_state_recover():
    recover_action_state(); return {'status':'completed','recovered':'stale action states','action_state_enabled':True}
...
def self_test():
    ...
    def idem_test():
        k='__selftest_179__'; h=digest({'command':'ping','test':179}); reserve_action(k,h); x=reserve_action(k,h)
        if not x.get('reused'):raise Exception('duplicate not suppressed')
        try: reserve_action(k,'different'); raise Exception('conflict accepted')
        except HTTPException as e:
            if e.status_code!=409:raise
        with LOCK,conn() as c:c.execute('DELETE FROM action_idempotency WHERE idempotency_key=?',(k,))
    chk('action idempotency enforcement',idem_test)
