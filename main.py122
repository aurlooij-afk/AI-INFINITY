...
APP_VERSION = "TARGET-2050.121"
BUILD = "REAL-WORLD-TRANSACTION-CONSISTENCY-GLOBAL-STATE-RECONCILIATION-CORE"
...
def health():
    ...
    return {
        ...
        "core": {
            ...
            "safe_action_gateway":
                True,
            "global_transaction_state":
                True,
            "cross_service_state_snapshots":
                True,
            ...
            "cross_service_reconciliation":
                True,
            "cross_service_conflict_detection":
                True,
            "authoritative_source_selection":
                True,
            "stale_state_resolution":
                True,
            "reconciliation_planning":
                True,
            "post_recovery_consistency_verification":
                True,
            "persistent_consistency_history":
                True,
            "transaction_consistency_scoring":
                True,
            "consistency_decision_trace":
                True,
            "action_provenance":
                True,
            "action_idempotency":
...
def capabilities():
    ...
    return {
        ...
        "capabilities": [
            ...
            "persistent-consistency-history",
            "transaction-consistency-scoring",
            "consistency-decision-trace",
        ],
    }
...
# ============================================================
# TARGET-2050.121 — GLOBAL TRANSACTION CONSISTENCY / RECONCILIATION
# ============================================================

CONSISTENCY_POLICY_VERSION = "121.1"
_CONSISTENCY_STATUSES = {"observed", "drifted", "reconciled", "conflicted", "manual_review"}


def _ensure_121_tables():
    write("""
        CREATE TABLE IF NOT EXISTS transaction_state_snapshots (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_id TEXT NOT NULL,
            ...
            source_health REAL NOT NULL DEFAULT 1.0,
            freshness_seconds REAL NOT NULL DEFAULT 0.0,
            observation_status TEXT NOT NULL DEFAULT 'observed'
        )
    """)
    write("CREATE INDEX IF NOT EXISTS idx_121_snap_tx ON transaction_state_snapshots(transaction_id, observed_at DESC)")
    write("""
        CREATE TABLE IF NOT EXISTS transaction_consistency_invariants (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_id TEXT NOT NULL,
            name TEXT NOT NULL,
    ...
    """)
    write("""
        CREATE TABLE IF NOT EXISTS transaction_consistency_history (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_id TEXT NOT NULL,
            status TEXT NOT NULL,
            ...
            authoritative_source_json TEXT NOT NULL DEFAULT '{}',
    ...
    """)
    write("""
        CREATE TABLE IF NOT EXISTS transaction_reconciliation_plans (
            id TEXT PRIMARY KEY,
            transaction_id TEXT NOT NULL,
            plan_hash TEXT NOT NULL,
            status TEXT NOT NULL,
    ...
    """)
    write("CREATE INDEX IF NOT EXISTS idx_121_hist_tx ON transaction_consistency_history(transaction_id, created_at DESC)")
    write("CREATE INDEX IF NOT EXISTS idx_121_plans_tx ON transaction_reconciliation_plans(transaction_id, created_at DESC)")
...
def _121_hash(value):
    return hashlib.sha256(_121_json(value).encode("utf-8")).hexdigest()


def _121_norm_state(value):
    if not isinstance(value, dict):
        return {"value": value}
    # Do not persist credentials or cookie material from observed state.
    blocked = {"authorization", "proxy-authorization", "cookie", "set-cookie", "x-api-key", "password", "secret", "token"}
    return {str(k): v for k, v in value.items() if str(k).lower() not in blocked}


def _121_state_rows(transaction_id):
    return q("SELECT * FROM transaction_state_snapshots WHERE transaction_id=? ORDER BY observed_at DESC, id DESC", (transaction_id,))


def _121_latest_by_service(transaction_id):
    rows = _121_state_rows(transaction_id)
    out = {}
    for r in rows:
        if r["service"] not in out:
            out[r["service"]] = r
    return out


def _121_observe(transaction_id, service, state, source_priority=0, source_health=1.0, observed_at=None):
    service = str(service).strip()
    if not service:
        raise HTTPException(400, "service_required")
    state = _121_norm_state(state)
    ts = float(observed_at if observed_at is not None else now())
    freshness = max(0.0, now() - ts)
    h = _121_hash(state)
    write("""INSERT INTO transaction_state_snapshots(transaction_id,service,state_json,state_hash,observed_at,source_priority,source_health,freshness_seconds,observation_status) VALUES(?,?,?,?,?,?,?,?,?)""",
          (transaction_id, service, _121_json(state), h, ts, int(source_priority), max(0.0, min(float(source_health), 1.0)), freshness, "observed"))
    return {"service": service, "state": state, "state_hash": h, "observed_at": ts, "source_priority": int(source_priority), "source_health": max(0.0, min(float(source_health), 1.0)), "freshness_seconds": freshness}


def _121_authority(transaction_id):
    latest = _121_latest_by_service(transaction_id)
    groups = {}
    for service, row in latest.items():
        state = json.loads(row["state_json"] or "{}")
        for key, value in state.items():
            groups.setdefault(key, []).append((service, value, row))
    selected = {}
    conflicts = []
    for key, items in groups.items():
        ranked = sorted(items, key=lambda x: (-int(x[2]["source_priority"]), -float(x[2]["source_health"]), float(x[2]["freshness_seconds"]), x[0]))
        winner = ranked[0]
        selected[key] = {"service": winner[0], "value": winner[1], "source_priority": int(winner[2]["source_priority"]), "freshness_seconds": float(winner[2]["freshness_seconds"])}
        values = {_121_json(x[1]) for x in items}
        if len(values) > 1:
            conflicts.append({"field": key, "sources": [{"service": x[0], "value": x[1]} for x in items], "selected": winner[0]})
    return selected, conflicts


def _121_invariant_ok(global_state, rule):
    ...
    if field not in global_state:
        return False, "missing_field"
    actual = global_state[field]
    if op == "equals": ok = actual == expected
    elif op == "not_equals": ok = actual != expected
    elif op == "in": ok = actual in (expected if isinstance(expected, list) else [])
    elif op == "exists": ok = field in global_state
    else: return False, "unsupported_operator"
    return bool(ok), "ok" if ok else "value_mismatch"


def _121_evaluate(transaction_id, expected_state=None):
    latest = _121_latest_by_service(transaction_id)
    selected, conflicts = _121_authority(transaction_id)
    global_state = {k: v["value"] for k, v in selected.items()}
    expected_state = _121_norm_state(expected_state or {})
    drift = []
    for key, expected in expected_state.items():
        if key not in global_state:
            drift.append({"field": key, "kind": "missing", "expected": expected})
        elif global_state[key] != expected:
            drift.append({"field": key, "kind": "value_mismatch", "expected": expected, "actual": global_state[key]})
    inv_rows = q("SELECT * FROM transaction_consistency_invariants WHERE transaction_id=? ORDER BY id", (transaction_id,))
    invariant_results = []
    for r in inv_rows:
        rule = json.loads(r["rule_json"] or "{}")
        ok, reason = _121_invariant_ok(global_state, rule)
        invariant_results.append({"name": r["name"], "version": r["version"], "passed": ok, "reason": reason, "rule": rule})
        if not ok:
            drift.append({"field": rule.get("field", ""), "kind": "invariant_failed", "invariant": r["name"], "reason": reason})
    service_count = max(1, len(latest))
    score = max(0.0, min(1.0, 1.0 - (len(drift) + len(conflicts)) / max(1, len(expected_state) + len(inv_rows) + service_count)))
    status = "conflicted" if conflicts else ("drifted" if drift else "observed")
    return {"global_state": global_state, "authoritative_sources": selected, "drift": drift, "conflicts": conflicts, "invariants": invariant_results, "consistency_score": round(score, 4), "status": status}


def _121_record_history(transaction_id, evaluation, plan=None, before_hash="", after_hash="", decision=None):
    trace = decision or {"policy_version": CONSISTENCY_POLICY_VERSION, "authority_rule": "priority -> health -> freshness -> service_name", "automatic_side_effect_execution": False}
    write("""INSERT INTO transaction_consistency_history(transaction_id,status,consistency_score,drift_count,conflict_count,authoritative_source_json,plan_json,before_hash,after_hash,decision_trace_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
          (transaction_id, evaluation["status"], evaluation["consistency_score"], len(evaluation["drift"]), len(evaluation["conflicts"]), _121_json(evaluation["authoritative_sources"]), _121_json(plan or {}), before_hash, after_hash, _121_json(trace), now()))


def _121_plan(transaction_id, evaluation):
    actions = []
    for d in evaluation["drift"]:
        if d.get("kind") in {"missing", "value_mismatch"}:
            source = evaluation["authoritative_sources"].get(d.get("field", ""), {})
            if source:
                actions.append({"type": "state_reconciliation", "field": d["field"], "target_service": "", "source_service": source["service"], "desired_value": source["value"], "requires_external_approval": True})
    for c in evaluation["conflicts"]:
        actions.append({"type": "conflict_resolution", "field": c["field"], "selected_source": c["selected"], "requires_external_approval": True})
    plan = {"transaction_id": transaction_id, "policy_version": CONSISTENCY_POLICY_VERSION, "actions": actions, "approval_required": bool(actions), "automatic_execution": False}
    pid = make_id("recon121")
    ph = _121_hash(plan)
    write("INSERT INTO transaction_reconciliation_plans(id,transaction_id,plan_hash,status,actions_json,approval_required,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
          (pid, transaction_id, ph, "awaiting_approval" if actions else "no_action", _121_json(actions), 1 if actions else 0, now(), now()))
    return {"plan_id": pid, "plan_hash": ph, **plan}


class ConsistencySnapshotRequest(BaseModel):
    transaction_id: str
    service: str
    state: dict = {}
...
class ConsistencyInvariantRequest(BaseModel):
    transaction_id: str
    name: str
    rule: dict
    version: str = CONSISTENCY_POLICY_VERSION
...
@app.post("/transaction-consistency/snapshots")
def transaction_consistency_snapshot(request: ConsistencySnapshotRequest):
    return {"status": "observed", "version": APP_VERSION, "transaction_id": request.transaction_id, "snapshot": _121_observe(request.transaction_id, request.service, request.state, request.source_priority, request.source_health, request.observed_at)}


@app.get("/transaction-consistency/snapshots")
def transaction_consistency_snapshots(transaction_id: str, limit: int = 100):
    limit = max(1, min(int(limit), 500)); rows = q("SELECT * FROM transaction_state_snapshots WHERE transaction_id=? ORDER BY observed_at DESC,id DESC LIMIT ?", (transaction_id, limit))
    return {"version": APP_VERSION, "transaction_id": transaction_id, "count": len(rows), "snapshots": [dict(r) for r in rows]}


@app.post("/transaction-consistency/invariants")
def transaction_consistency_invariant(request: ConsistencyInvariantRequest):
    name = request.name.strip()
    if not name: raise HTTPException(400, "invariant_name_required")
    if not isinstance(request.rule, dict) or not request.rule.get("field"): raise HTTPException(400, "valid_rule_required")
    write("INSERT INTO transaction_consistency_invariants(transaction_id,name,rule_json,version,created_at) VALUES(?,?,?,?,?)", (request.transaction_id, name, _121_json(request.rule), request.version, now()))
    return {"status": "registered", "version": APP_VERSION, "transaction_id": request.transaction_id, "name": name, "policy_version": request.version}


@app.get("/transaction-consistency/invariants")
def transaction_consistency_invariants(transaction_id: str):
    rows = q("SELECT * FROM transaction_consistency_invariants WHERE transaction_id=? ORDER BY id", (transaction_id,))
    return {"version": APP_VERSION, "transaction_id": transaction_id, "count": len(rows), "invariants": [dict(r) for r in rows]}


@app.post("/transaction-consistency/drift")
def transaction_consistency_drift(request: ConsistencyEvaluateRequest):
    evaluation = _121_evaluate(request.transaction_id, request.expected_state)
    _121_record_history(request.transaction_id, evaluation)
    return {"status": "passed" if not evaluation["drift"] and not evaluation["conflicts"] else evaluation["status"], "version": APP_VERSION, "transaction_id": request.transaction_id, **evaluation}


@app.get("/transaction-consistency/drift")
def transaction_consistency_drift_get(transaction_id: str):
    evaluation = _121_evaluate(transaction_id, {})
    return {"version": APP_VERSION, "transaction_id": transaction_id, **evaluation}


@app.get("/transaction-consistency/conflicts")
def transaction_consistency_conflicts(transaction_id: str):
    evaluation = _121_evaluate(transaction_id, {})
    return {"version": APP_VERSION, "transaction_id": transaction_id, "count": len(evaluation["conflicts"]), "conflicts": evaluation["conflicts"], "authoritative_sources": evaluation["authoritative_sources"]}


@app.post("/transaction-consistency/reconciliation")
def transaction_consistency_reconciliation(request: ConsistencyEvaluateRequest):
    evaluation = _121_evaluate(request.transaction_id, request.expected_state)
    plan = _121_plan(request.transaction_id, evaluation)
    before = _121_hash(evaluation["global_state"])
    _121_record_history(request.transaction_id, evaluation, plan, before_hash=before, decision={"policy_version": CONSISTENCY_POLICY_VERSION, "decision": "plan_only", "approval_required": plan["approval_required"], "automatic_side_effect_execution": False})
    return {"status": "planned", "version": APP_VERSION, "transaction_id": request.transaction_id, "evaluation": evaluation, "reconciliation": plan, "external_execution": False, "approval_required": plan["approval_required"]}


@app.get("/transaction-consistency/reconciliation")
def transaction_consistency_reconciliation_get(transaction_id: str):
    rows = q("SELECT * FROM transaction_reconciliation_plans WHERE transaction_id=? ORDER BY created_at DESC", (transaction_id,))
    return {"version": APP_VERSION, "transaction_id": transaction_id, "count": len(rows), "plans": [dict(r) for r in rows], "automatic_execution": False}


@app.get("/transaction-consistency/history")
def transaction_consistency_history(transaction_id: str, limit: int = 50):
    limit=max(1,min(int(limit),200)); rows=q("SELECT * FROM transaction_consistency_history WHERE transaction_id=? ORDER BY created_at DESC LIMIT ?",(transaction_id,limit))
    return {"version":APP_VERSION,"transaction_id":transaction_id,"count":len(rows),"history":[dict(r) for r in rows]}


@app.get("/transaction-consistency/{transaction_id}")
def transaction_consistency(transaction_id: str):
    evaluation = _121_evaluate(transaction_id, {})
    return {"version": APP_VERSION, "transaction_id": transaction_id, "policy_version": CONSISTENCY_POLICY_VERSION, **evaluation}


@app.post("/transaction-consistency/{transaction_id}/verify")
def transaction_consistency_verify(transaction_id: str, request: ConsistencyEvaluateRequest):
    if request.transaction_id != transaction_id: raise HTTPException(400, "transaction_id_mismatch")
    evaluation = _121_evaluate(transaction_id, request.expected_state)
    status = "verified" if not evaluation["drift"] and not evaluation["conflicts"] and all(x["passed"] for x in evaluation["invariants"]) else "manual_review"
    _121_record_history(transaction_id, evaluation, decision={"policy_version": CONSISTENCY_POLICY_VERSION, "decision": "post_recovery_verification", "status": status, "automatic_side_effect_execution": False})
    return {"status": status, "version": APP_VERSION, "transaction_id": transaction_id, "consistency_score": evaluation["consistency_score"], "drift": evaluation["drift"], "conflicts": evaluation["conflicts"], "invariants": evaluation["invariants"], "post_recovery_consistency_verified": status == "verified"}


@app.get("/transaction-consistency/sources")
def transaction_consistency_sources(transaction_id: str):
    selected, conflicts = _121_authority(transaction_id)
    return {"version": APP_VERSION, "transaction_id": transaction_id, "authoritative_sources": selected, "conflicts": conflicts, "selection_policy": "explicit_priority -> health -> freshness -> deterministic_service_name"}


@app.get("/test-transaction-consistency")
def test_transaction_consistency():
    tid="__test_121__"
    for table in ("transaction_state_snapshots","transaction_consistency_invariants","transaction_consistency_history","transaction_reconciliation_plans"):
        write(f"DELETE FROM {table} WHERE transaction_id=?", (tid,))
    _121_observe(tid,"service_a",{"status":"complete","amount":100,"version":1},source_priority=10,source_health=1.0)
    _121_observe(tid,"service_b",{"status":"complete","amount":90,"version":1},source_priority=5,source_health=1.0)
    transaction_consistency_invariant(ConsistencyInvariantRequest(transaction_id=tid,name="status_complete",rule={"field":"status","op":"equals","value":"complete"}))
    ev=_121_evaluate(tid,{"status":"complete","amount":100,"version":1})
    plan=_121_plan(tid,ev)
    verification=_121_evaluate(tid,{"status":"complete","amount":100,"version":1})
    ok=(len(ev["conflicts"])>=1 and len(ev["drift"])==0 and plan["approval_required"] is True and verification["consistency_score"] < 1.0)
    for table in ("transaction_state_snapshots","transaction_consistency_invariants","transaction_consistency_history","transaction_reconciliation_plans"):
        write(f"DELETE FROM {table} WHERE transaction_id=?", (tid,))
    return {"status":"passed" if ok else "failed","version":APP_VERSION,"build":BUILD,"global_transaction_state":True,"state_snapshots":True,"invariants":True,"drift_detection":True,"conflict_detection":True,"authoritative_source_selection":True,"reconciliation_planning":True,"post_recovery_verification":True,"persistent_history":True,"consistency_scoring":True,"approval_boundary":True,"external_execution":False,"generic_rollback":False}
