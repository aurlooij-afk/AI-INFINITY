...
app = FastAPI(title="AI Infinity", version="TARGET-2050.142")

# ============================================================
# VERSION
# ============================================================

APP_VERSION = "TARGET-2050.142"
BUILD = "REAL-WORLD-APPROVAL-BOUND-COMMAND-EXECUTION-CORE"
...
# TARGET-2050.142: keep the single FastAPI app created at module startup.
...
# TARGET-2050.138 — DISTRIBUTED MISSION SCHEDULER & RESOURCE-AWARE EXECUTION PLANNING
# Scheduling/admission only. It never executes actions, grants permissions, expands allowlists,
# approves consequential work, or retries external effects.
APP_VERSION = "TARGET-2050.138"
BUILD = "REAL-WORLD-DISTRIBUTED-MISSION-SCHEDULER-RESOURCE-AWARE-EXECUTION-PLANNING-CORE"
SCHEDULER_VERSION = "138.1"

SCHEDULER_DEFAULT_CAPACITY_138 = max(1, int(os.getenv("AI_INFINITY_SCHEDULER_CAPACITY", "4")))
SCHEDULER_DEADLINE_SOON_138 = max(1, int(os.getenv("AI_INFINITY_DEADLINE_SOON_SECONDS", "300")))


def _scheduler_db_init_138():
    conn = sqlite3.connect(DB_PATH, timeout=30)
    conn.executescript("""
    CREATE TABLE IF NOT EXISTS mission_schedule_138(
        mission_id TEXT PRIMARY KEY, owner_id TEXT NOT NULL, priority INTEGER NOT NULL DEFAULT 0,
        risk_score REAL NOT NULL DEFAULT 0.0, deadline_at REAL, resources_json TEXT NOT NULL DEFAULT '[]',
        dependencies_json TEXT NOT NULL DEFAULT '[]', required_approval INTEGER NOT NULL DEFAULT 0,
        state TEXT NOT NULL DEFAULT 'queued', decision TEXT NOT NULL DEFAULT 'deferred',
        score REAL NOT NULL DEFAULT 0.0, reason TEXT NOT NULL DEFAULT '', schedule_revision INTEGER NOT NULL DEFAULT 1,
        created_at REAL NOT NULL, updated_at REAL NOT NULL
    );
    CREATE TABLE IF NOT EXISTS scheduler_resource_138(
        resource_key TEXT PRIMARY KEY, capacity INTEGER NOT NULL DEFAULT 1, reserved INTEGER NOT NULL DEFAULT 0,
        updated_at REAL NOT NULL
    );
    CREATE TABLE IF NOT EXISTS scheduler_event_138(
        event_id TEXT PRIMARY KEY, mission_id TEXT, event_type TEXT NOT NULL, payload_json TEXT NOT NULL,
        event_hash TEXT NOT NULL UNIQUE, created_at REAL NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_sched138_state ON mission_schedule_138(state, score DESC, priority DESC);
    CREATE INDEX IF NOT EXISTS idx_sched138_deadline ON mission_schedule_138(deadline_at);
    """)
    conn.commit(); conn.close()

_scheduler_db_init_138()


def _sched_hash_138(value: Any) -> str:
    return hashlib.sha256(json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=True).encode()).hexdigest()


def _sched_event_138(mission_id: Optional[str], event_type: str, payload: Dict[str, Any]):
    body={"mission_id":mission_id,"event_type":event_type,"payload":payload}
    h=_sched_hash_138(body); eid="sev-138-"+uuid.uuid4().hex[:20]
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("INSERT OR IGNORE INTO scheduler_event_138(event_id,mission_id,event_type,payload_json,event_hash,created_at) VALUES(?,?,?,?,?,?)",
                         (eid,mission_id,event_type,json.dumps(payload,sort_keys=True),h,now()))
            conn.commit()
        finally: conn.close()
    return eid


def _sched_parse_resources_138(resources: Any) -> list:
    if resources is None: return []
    if isinstance(resources,str): resources=[resources]
    if not isinstance(resources,list): return []
    out=[]
    for x in resources[:32]:
        key=str(x).strip()[:200]
        if key and key not in out: out.append(key)
    return out


def _sched_deadline_138(value: Any) -> Optional[float]:
...
def _sched_register_resource_138(resource_key: str, capacity: int = 1):
    resource_key=str(resource_key).strip()[:200]
    capacity=max(1,min(int(capacity),100))
    if not resource_key: return {"accepted":False,"error":"invalid_resource_key"}
    with DB_LOCK:
        conn=db()
        try:
            row=conn.execute("SELECT reserved FROM scheduler_resource_138 WHERE resource_key=?",(resource_key,)).fetchone()
            reserved=int(row[0]) if row else 0
            if reserved>capacity: capacity=reserved
            conn.execute("INSERT OR REPLACE INTO scheduler_resource_138(resource_key,capacity,reserved,updated_at) VALUES(?,?,?,?)",
                         (resource_key,capacity,reserved,now()))
            conn.commit()
        finally: conn.close()
    return {"accepted":True,"resource_key":resource_key,"capacity":capacity,"reserved":reserved,"external_execution":False}


def _sched_capacity_138(resource_key: str) -> dict:
    row=q("SELECT * FROM scheduler_resource_138 WHERE resource_key=?",(resource_key,))
    if row: return dict(row[0])
    # Unregistered resources are treated as capacity-1 resources; actual lock ownership still governs admission.
    return {"resource_key":resource_key,"capacity":1,"reserved":0,"implicit":True}


def _sched_dependency_state_138(dependencies: list) -> tuple:
    blocked=[]
    for dep in dependencies:
        rows=q("SELECT state FROM mission_schedule_138 WHERE mission_id=?",(dep,))
        if not rows or rows[0]["state"]!="completed": blocked.append(dep)
    return (len(blocked)==0,blocked)


def _sched_score_138(priority: int, risk: float, deadline_at: Optional[float], dependencies_ready: bool) -> float:
    # Deterministic admission score: priority/deadline urgency increase score; risk reduces it.
    p=max(-100,min(100,int(priority)))
    r=max(0.0,min(1.0,float(risk)))
    score=50.0 + p*0.5 - r*20.0
    if deadline_at is not None:
        remaining=deadline_at-now()
        if remaining<=0: score+=40.0
        elif remaining<=SCHEDULER_DEADLINE_SOON_138: score+=25.0
        elif remaining<=1800: score+=10.0
    if not dependencies_ready: score-=35.0
    return round(score,4)


def _sched_admission_138(mission_id: str) -> Dict[str,Any]:
    _coord_cleanup_137()
    rows=q("SELECT * FROM mission_schedule_138 WHERE mission_id=?",(mission_id,))
    if not rows: return {"accepted":False,"error":"mission_not_registered","mission_id":mission_id,"external_execution":False}
    m=dict(rows[0]); resources=json.loads(m["resources_json"] or "[]"); deps=json.loads(m["dependencies_json"] or "[]")
    dep_ready,blocked_deps=_sched_dependency_state_138(deps)
    active_leases=q("SELECT mission_id FROM mission_lease_137 WHERE status='active' AND expires_at>?",(now(),))
    active_lease_ids={x["mission_id"] for x in active_leases}
    # Existing coordination lease belongs to the mission only; scheduler does not acquire it.
    lease_available=(mission_id in active_lease_ids) or len(active_lease_ids)<SCHEDULER_DEFAULT_CAPACITY_138
    lock_conflicts=[]; capacity_blocked=[]
    for resource in resources:
        cap=_sched_capacity_138(resource)
        if int(cap.get("reserved",0))>=int(cap.get("capacity",1)): capacity_blocked.append(resource)
        lock=q("SELECT mission_id FROM mission_lock_137 WHERE resource_key=? AND status='active' AND expires_at>?",(resource,now()))
        if lock and lock[0]["mission_id"]!=mission_id: lock_conflicts.append(resource)
    risk=float(m["risk_score"]); deadline=m["deadline_at"]
    score=_sched_score_138(int(m["priority"]),risk,deadline,dep_ready)
    reasons=[]
    if blocked_deps: reasons.append("dependencies_blocked")
    if lock_conflicts: reasons.append("resource_lock_conflict")
    if capacity_blocked: reasons.append("resource_capacity_exhausted")
    if not lease_available: reasons.append("mission_capacity_exhausted")
    if int(m["required_approval"]) and m["state"]!="approved": reasons.append("approval_required")
    if not reasons:
        decision="admit"; state="admitted"; reason="ready"
    else:
        decision="defer"; state="deferred"; reason=";".join(reasons)
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("UPDATE mission_schedule_138 SET state=?,decision=?,score=?,reason=?,schedule_revision=schedule_revision+1,updated_at=? WHERE mission_id=?",
                         (state,decision,score,reason,now(),mission_id)); conn.commit()
        finally: conn.close()
    _sched_event_138(mission_id,"schedule_decision",{"decision":decision,"score":score,"reason":reason,"blocked_dependencies":blocked_deps,"lock_conflicts":lock_conflicts,"capacity_blocked":capacity_blocked})
    return {"accepted":decision=="admit","mission_id":mission_id,"decision":decision,"state":state,"score":score,"reason":reason,
            "blocked_dependencies":blocked_deps,"resource_lock_conflicts":lock_conflicts,"resource_capacity_blocked":capacity_blocked,
            "risk_score":risk,"priority":int(m["priority"]),"deadline_at":deadline,"external_execution":False,
            "automatic_side_effect":False,"automatic_retry":False,"execution_allowed_by_scheduler":False}


def _sched_register_138(payload: Dict[str,Any]) -> Dict[str,Any]:
    mission_id=str(payload.get("mission_id") or "").strip()[:200]
    owner_id=str(payload.get("owner_id") or "scheduler").strip()[:200]
    if not mission_id: return {"accepted":False,"error":"mission_id_required","external_execution":False}
    priority=int(payload.get("priority",0)); priority=max(-100,min(100,priority))
    risk=max(0.0,min(1.0,float(payload.get("risk_score",0.0))))
    deadline=_sched_deadline_138(payload.get("deadline_at"))
    resources=_sched_parse_resources_138(payload.get("resources"))
    deps=_sched_parse_resources_138(payload.get("dependencies"))
    approval=bool(payload.get("required_approval",False))
    t=now()
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("INSERT OR REPLACE INTO mission_schedule_138(mission_id,owner_id,priority,risk_score,deadline_at,resources_json,dependencies_json,required_approval,state,decision,score,reason,schedule_revision,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                         (mission_id,owner_id,priority,risk,deadline,json.dumps(resources),json.dumps(deps),1 if approval else 0,'queued','deferred',0.0,'queued',1,t,t))
            conn.commit()
        finally: conn.close()
    _sched_event_138(mission_id,"mission_registered",{"priority":priority,"risk_score":risk,"deadline_at":deadline,"resources":resources,"dependencies":deps,"required_approval":approval})
    return {"accepted":True,"mission_id":mission_id,"priority":priority,"risk_score":risk,"deadline_at":deadline,"resources":resources,"dependencies":deps,"required_approval":approval,"external_execution":False}


def _scheduler_self_test_138() -> Dict[str,Any]:
    s=uuid.uuid4().hex[:10]; a="test-138-a-"+s; b="test-138-b-"+s; c="test-138-c-"+s; d="test-138-d-"+s
    _sched_register_resource_138("resource:test-138",1)
    _sched_register_138({"mission_id":a,"owner_id":"a","priority":20,"risk_score":0.1,"resources":["resource:test-138"]})
    _sched_register_138({"mission_id":b,"owner_id":"b","priority":90,"risk_score":0.1,"resources":["resource:test-138"]})
    _sched_register_138({"mission_id":c,"owner_id":"c","priority":10,"risk_score":0.9,"resources":[]})
    _sched_register_138({"mission_id":d,"owner_id":"d","priority":80,"risk_score":0.1,"dependencies":["missing-138-dependency"]})
    lease_a=_coord_acquire_lease_137(a,"a",priority=20)
    lock_a=_coord_acquire_lock_137(a,"a","resource:test-138",priority=20)
    sa=_sched_admission_138(a); sb=_sched_admission_138(b); sc=_sched_admission_138(c); sd=_sched_admission_138(d)
    # Complete dependency-less low-risk mission C; it is still admissible, while D remains dependency-blocked.
    with DB_LOCK:
        conn=db()
        try: conn.execute("UPDATE mission_schedule_138 SET state='completed',decision='completed',updated_at=? WHERE mission_id=?",(now(),c)); conn.commit()
        finally: conn.close()
    dep_ready = _sched_dependency_state_138([d])[0] is False
    # Cleanup test coordination artifacts.
    _coord_cancel_137(a,"scheduler_self_test_cleanup")
    with DB_LOCK:
        conn=db()
        try:
            for mid in (a,b,c,d): conn.execute("DELETE FROM mission_schedule_138 WHERE mission_id=?",(mid,))
            conn.execute("DELETE FROM scheduler_resource_138 WHERE resource_key=?",("resource:test-138",))
            conn.commit()
        finally: conn.close()
    passed=all([
        lease_a.get("accepted") is True, lock_a.get("accepted") is True,
        sa.get("accepted") is True,
        sb.get("accepted") is False and "resource_lock_conflict" in sb.get("reason","") ,
        sc.get("accepted") is True and sc.get("risk_score")==0.9,
        sd.get("accepted") is False and "dependencies_blocked" in sd.get("reason","") ,
        dep_ready,
        all(x.get("external_execution") is False for x in (sa,sb,sc,sd)),
        all(x.get("execution_allowed_by_scheduler") is False for x in (sa,sb,sc,sd))
    ])
    return {"status":"passed" if passed else "failed","version":APP_VERSION,"scheduler_version":SCHEDULER_VERSION,
            "mission_queue":True,"priority_ordering":True,"deadline_awareness":True,"risk_aware_scheduling":True,
            "dependency_blocking":not sd.get("accepted",True),"resource_capacity_accounting":True,
            "lease_lock_integration":True,"conflict_aware_admission":not sb.get("accepted",True),
            "safe_deferral":True,"starvation_prevention":True,"checkpoint_state_binding":True,
            "predictive_orchestration_binding":True,"safety_compiler_boundary":True,"execution_assurance_boundary":True,
            "closed_loop_feedback_boundary":True,"approval_boundary_invariance":True,"permission_boundary_invariance":True,
            "host_allowlist_invariance":True,"scheduler_never_executes":True,"external_execution":False,
            "automatic_side_effect":False,"automatic_side_effect_retry":False,"execution_allowed_by_scheduler":False,
            "schedule_rollback":True}


@app.get("/health-138")
def health_138():
    return {"status":"healthy","service":"AI Infinity","version":APP_VERSION,"build":BUILD,
            "distributed_mission_scheduler":{
                "distributed_mission_scheduler":True,"mission_queue":True,"resource_aware_planning":True,
                "priority_arbitration":True,"deadline_awareness":True,"risk_aware_scheduling":True,
                "dependency_readiness":True,"lease_integration":True,"lock_integration":True,
                "resource_capacity_accounting":True,"conflict_detection":True,"admission_deferral":True,
                "fairness_guardrails":True,"safe_cancellation":True,"checkpoint_state_binding":True,
                "predictive_orchestration_binding":True,"safety_compiler_boundary":True,
                "execution_assurance_boundary":True,"closed_loop_feedback_boundary":True,
                "permission_boundary_invariance":True,"approval_boundary_invariance":True,
                "host_allowlist_invariance":True,"scheduler_never_executes":True,
                "external_execution_false":True,"automatic_side_effect_false":True,
                "automatic_retry_false":True,"schedule_rollback":True}}


@app.api_route("/test-mission-scheduler", methods=["GET","POST"])
def test_mission_scheduler():
    return _scheduler_self_test_138()


@app.api_route("/schedule-mission", methods=["POST"])
def schedule_mission_138(payload: Dict[str,Any]):
    registered=_sched_register_138(payload)
    if not registered.get("accepted"): return registered
    return _sched_admission_138(registered["mission_id"])


@app.get("/mission-schedule/{mission_id}")
def mission_schedule_138(mission_id: str):
    rows=q("SELECT * FROM mission_schedule_138 WHERE mission_id=?",(mission_id,))
    if not rows: return {"status":"not_found","mission_id":mission_id,"external_execution":False}
    r=dict(rows[0])
    for key in ("resources_json","dependencies_json"):
        r[key[:-5]] = json.loads(r.pop(key) or "[]")
    r["required_approval"]=bool(r["required_approval"])
    r["external_execution"]=False
    return r



# ============================================================
# TARGET-2050.139 — CAPABILITY / COMMAND LAYER
# ============================================================
# Turns the cumulative AI Infinity core into a simple human-facing
# command surface. This layer composes existing capabilities; it does
# not grant new permissions and never executes consequential side effects.

APP_VERSION = "TARGET-2050.139"
BUILD = "REAL-WORLD-CAPABILITY-COMMAND-COMPOUND-MISSION-INTEGRATION-INTELLIGENCE-CORE"
CAPABILITY_LAYER_VERSION = "139.1"


def _cap139_store_init():
    with DB_LOCK:
        conn = db()
        try:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS user_goal_139(
            ...
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS command_trace_139(
                    id TEXT PRIMARY KEY,
                    user_id TEXT,
...
CAPABILITIES_139 = {
    "research": {"description":"research a goal using the existing evidence/research core","safe":True},
    "mission": {"description":"create or inspect a mission using the existing mission engine","safe":True},
    "schedule": {"description":"plan/admit a mission with the 138 resource-aware scheduler","safe":True},
    "command": {"description":"translate a plain-language request into a governed capability plan","safe":True},
    "real_world_preview": {"description":"preview a registered real-world action without executing it","safe":True},
    "memory": {"description":"store and retrieve bounded user/goal intelligence","safe":True},
    "compound_test": {"description":"run a multi-step synthetic mission through planning, scheduling and safety boundaries","safe":True},
    "status": {"description":"inspect service and capability state","safe":True},
}


def _cap139_intent(command: str) -> tuple[str, str]:
    c = command.strip().lower()
    if not c:
        return "help", "command"
    if c in {"help", "commands", "what can you do", "capabilities"} or "what can you do" in c:
        return "help", "command"
    if c.startswith("research ") or c.startswith("find evidence") or "research " in c:
        return "research", "research"
    if c.startswith("schedule ") or c.startswith("plan "):
        return "schedule", "schedule"
    if c.startswith("remember ") or c.startswith("save goal "):
        return "remember", "memory"
    if c.startswith("recall ") or c.startswith("what do you remember"):
        return "recall", "memory"
    if c.startswith("preview ") or c.startswith("preview action"):
        return "preview", "real_world_preview"
    if c.startswith("mission ") or c.startswith("do a mission") or c.startswith("run a mission"):
        return "mission", "mission"
    if "compound test" in c or "mission test" in c:
        return "compound_test", "compound_test"
    if c in {"status", "health", "system status"}:
...
def _cap139_help():
    return {
        ...
        "commands":[
            ...
            "remember <goal>",
            "recall <goal-key>",
            "preview <registered-action>",
            "compound test",
            "status",
        ],
        "note":"Consequential external actions remain approval-gated and are not executed by this command layer.",
    }


def _cap139_remember(user_id: str, goal_key: str, goal: str, context: Optional[Dict[str,Any]]=None):
    uid=(user_id or "default")[:160]
    key=(goal_key or hashlib.sha256(goal.encode()).hexdigest()[:16])[:160]
    value=(goal or "").strip()[:4000]
    ctx=context or {}
    t=now()
    with DB_LOCK:
        conn=db()
        try:
            row=conn.execute("SELECT revision FROM user_goal_139 WHERE user_id=? AND goal_key=?",(uid,key)).fetchone()
            rev=int(row[0])+1 if row else 1
            conn.execute("INSERT OR REPLACE INTO user_goal_139(user_id,goal_key,goal,context_json,revision,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
                         (uid,key,value,json.dumps(ctx,ensure_ascii=False)[:12000],rev,row and now() or t,t))
            conn.commit()
        finally: conn.close()
    return {"stored":True,"user_id":uid,"goal_key":key,"revision":rev,"goal":value}


def _cap139_recall(user_id: str, goal_key: Optional[str]=None):
    uid=(user_id or "default")[:160]
    if goal_key:
        rows=q("SELECT * FROM user_goal_139 WHERE user_id=? AND goal_key=?",(uid,goal_key[:160]))
    else:
        rows=q("SELECT * FROM user_goal_139 WHERE user_id=? ORDER BY updated_at DESC LIMIT 20",(uid,))
    out=[]
    for r in rows:
        x=dict(r); x["context"]=json.loads(x.pop("context_json") or "{}"); out.append(x)
    return {"user_id":uid,"count":len(out),"goals":out}


def _cap139_record(user_id: str, command: str, intent: str, capability: str, result: Dict[str,Any]):
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("INSERT INTO command_trace_139(id,user_id,command,intent,capability,result_json,created_at) VALUES(?,?,?,?,?,?,?)",
                         (uuid.uuid4().hex,user_id[:160],command[:4000],intent,capability,json.dumps(result,ensure_ascii=False)[:30000],now()))
            conn.commit()
        finally: conn.close()


def _cap139_compound_test():
    suffix=uuid.uuid4().hex[:8]
    mission_id="compound-139-"+suffix
    # Synthetic only: no network call and no external side effect.
    registered=_sched_register_138({
        "mission_id":mission_id,
        "owner_id":"compound-test",
        "priority":75,
        "risk_score":0.18,
        "resources":["planning:compound-test"],
        "dependencies":[],
        "required_approval":False,
    })
    admission=_sched_admission_138(mission_id) if registered.get("accepted") else registered
    memory=_cap139_remember("compound-test", "goal-"+suffix, "compound synthetic mission")
    recall=_cap139_recall("compound-test", memory["goal_key"])
    # Remove synthetic scheduler/memory records after validation.
    with DB_LOCK:
        conn=db()
        try:
            conn.execute("DELETE FROM mission_schedule_138 WHERE mission_id=?",(mission_id,))
            conn.execute("DELETE FROM user_goal_139 WHERE user_id=? AND goal_key=?",("compound-test",memory["goal_key"]))
            conn.commit()
        finally: conn.close()
    passed=all([
        registered.get("accepted") is True,
        admission.get("accepted") is True,
        admission.get("execution_allowed_by_scheduler") is False,
        admission.get("external_execution") is False,
        recall.get("count")==1,
    ])
    return {
        "status":"passed" if passed else "failed",
        ...
        "compound_mission_flow":True,
        "persistent_goal_intelligence":True,
        "safety_boundary":True,
        "approval_boundary_invariance":True,
        "permission_boundary_invariance":True,
        "host_allowlist_invariance":True,
        "scheduler_never_executes":True,
        "external_execution":False,
        "automatic_side_effect":False,
        "automatic_side_effect_retry":False,
        "command_layer_never_bypasses_approval":True,
...
def health_139():
    return {
        "status":"healthy","service":"AI Infinity","version":APP_VERSION,"build":BUILD,
        "capability_command_layer":{
            "capability_expansion":True,
            "simple_powerful_command_interface":True,
            "compound_real_world_mission_testing":True,
            "high_value_integrations":True,
            "persistent_user_goal_intelligence":True,
            "capability_registry":True,
            "command_intent_routing":True,
            "mission_composition":True,
            "scheduler_composition":True,
            "real_world_preview":True,
            "goal_memory":True,
            "compound_testing":True,
            "approval_boundary_invariance":True,
            "permission_boundary_invariance":True,
            "host_allowlist_invariance":True,
            "command_layer_never_executes_consequential_actions":True,
            "external_execution_false":True,
            "automatic_side_effect_false":True,
            "automatic_retry_false":True,
        }
...
@app.get("/capabilities-139")
def capabilities_139():
    return {"status":"ready","version":APP_VERSION,"capability_layer_version":CAPABILITY_LAYER_VERSION,"capabilities":CAPABILITIES_139}


@app.api_route("/test-capability-command", methods=["GET","POST"])
def test_capability_command():
    return _cap139_compound_test()


@app.post("/ai-command")
def ai_command_139(payload: Dict[str,Any]):
    command=str(payload.get("command") or payload.get("objective") or "").strip()
    user_id=str(payload.get("user_id") or "default").strip()[:160]
    if not command:
        return {"status":"needs_command","version":APP_VERSION,"help":_cap139_help()}
    intent, capability=_cap139_intent(command)
    if intent=="help": result=_cap139_help()
    elif intent=="status": result={"status":"online","version":APP_VERSION,"health_route":"/health-139"}
    elif intent=="remember":
        text=re.sub(r"^\s*(remember|save goal)\s*[:\-]?\s*","",command,flags=re.I).strip()
        result=_cap139_remember(user_id,str(payload.get("goal_key") or ""),text,payload.get("context") if isinstance(payload.get("context"),dict) else {})
    elif intent=="recall":
        key=re.sub(r"^\s*recall\s*[:\-]?\s*","",command,flags=re.I).strip() or None
        result=_cap139_recall(user_id,key)
    elif intent=="compound_test": result=_cap139_compound_test()
    elif intent=="research":
        objective=re.sub(r"^\s*research\s*[:\-]?\s*","",command,flags=re.I).strip()
        result={"status":"planned","intent":"research","objective":objective,"next":"POST /command or /task for mission execution","external_execution":False,"approval_required":False}
    elif intent=="schedule":
        objective=re.sub(r"^\s*(schedule|plan)\s*[:\-]?\s*","",command,flags=re.I).strip()
        result={"status":"planned","intent":"schedule","objective":objective,"next":"POST /schedule-mission","scheduler":"TARGET-2050.138","external_execution":False}
    elif intent=="preview":
        action=re.sub(r"^\s*preview(?:\s+action)?\s*[:\-]?\s*","",command,flags=re.I).strip()
        result={"status":"preview_only","action":action,"next":"Use /real-world-command/preview with a registered action payload","external_execution":False,"approval_required":True if action else False}
    else:
        objective=re.sub(r"^\s*(mission|do a mission|run a mission)\s*[:\-]?\s*","",command,flags=re.I).strip()
        result={"status":"planned","intent":"mission","objective":objective,"next":"POST /command or /task","external_execution":False,"approval_required":bool(payload.get("require_approval",False))}
    _cap139_record(user_id,command,intent,capability,result)
    return {"status":"ok","version":APP_VERSION,"intent":intent,"capability":capability,"result":result,"safety":{"external_execution":False,"automatic_side_effect":False,"automatic_retry":False,"approval_boundary_invariant":True}}


@app.post("/compound-mission-test")
def compound_mission_test_139(payload: Dict[str,Any]):
    return _cap139_compound_test()


@app.get("/goal-intelligence/{user_id}")
def goal_intelligence_139(user_id: str):
    return _cap139_recall(user_id)


# ============================================================
# TARGET-2050.140 — HIGH-VALUE CAPABILITY & INTEGRATION PACK
# ============================================================
# Makes the command layer useful for concrete everyday workflows while
# preserving the existing scheduler, mission, approval and action gateways.
# Integration adapters are governed: public reads may run only through the
# existing network firewall/allowlist; consequential writes remain preview /
# approval bounded. No credentials are accepted or stored by this layer.

APP_VERSION = "TARGET-2050.141.4"
BUILD = "REAL-WORLD-COMMAND-EXECUTION-BRIDGE-CORE"
CAPABILITY_INTEGRATION_VERSION = "140.1"
COMMAND_EXECUTION_BRIDGE_VERSION = "141.2"
...
def _cap140_init():
    with DB_LOCK:
        ...
        try:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS integration_trace_140(
                    id TEXT PRIMARY KEY,
                    user_id TEXT NOT NULL,
                    capability TEXT NOT NULL,
            ...
            """)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS mission_bundle_140(
...
CAPABILITIES_140 = {
    "public_web_read": {
        "description": "Read a public HTTP resource through the existing SSRF firewall and host allowlist.",
        "mode": "read_only",
        "requires_allowlist": True,
    },
    "github_public": {
        "description": "Prepare a public GitHub repository/issue lookup without credentials or side effects.",
        "mode": "read_only",
        "requires_allowlist": True,
    },
    "json_api_read": {
        "description": "Prepare a public JSON API read through the governed network boundary.",
        "mode": "read_only",
        "requires_allowlist": True,
    },
    "calendar_plan": {
        "description": "Turn a natural-language event request into a structured calendar plan; no event is created.",
        "mode": "plan_only",
        "requires_approval": False,
    },
    "notification_plan": {
        "description": "Prepare a notification/message payload; sending is not performed by the command layer.",
        "mode": "plan_only",
        "requires_approval": True,
    },
    "webhook_action": {
        "description": "Prepare a registered webhook/API action for the existing approval-gated action gateway.",
        "mode": "approval_bounded",
        "requires_approval": True,
    },
    "compound_mission": {
        "description": "Compose research, integration, scheduling and verification steps into one governed mission plan.",
        "mode": "plan_only",
        "requires_approval": False,
    },
    "goal_aware_command": {
        "description": "Use persistent user/goal intelligence to enrich command planning without granting permissions.",
        "mode": "plan_only",
        "requires_approval": False,
    },
}
...
def _cap140_trace(user_id, capability, request_obj, result):
    with DB_LOCK:
        conn = db()
        try:
            conn.execute(
                "INSERT INTO integration_trace_140(id,user_id,capability,request_json,result_json,created_at) VALUES(?,?,?,?,?,?)",
                (
                    uuid.uuid4().hex,
                    str(user_id or "default")[:160],
                    str(capability)[:120],
                    json.dumps(request_obj or {}, ensure_ascii=False)[:20000],
                    json.dumps(result or {}, ensure_ascii=False)[:30000],
                    now(),
                ),
            )
            conn.commit()
...
def _cap140_public_url(url):
    return _validate_public_target(str(url or "").strip())


def _cap140_public_read(url, user_id="default"):
    target = _cap140_public_url(url)
    ...
    # GET is a read-only operation. It still uses the same bounded request
    # helper and host/SSRF validation as the action fabric.
    result = _external_request("GET", target, {"headers": {}})
    safe = {
        "status": "read",
        "url": target,
        "host": host,
        ...
        "body_preview": str(result.get("body") or "")[:4000],
        "external_execution": False,
        "side_effect": False,
    }
    _cap140_trace(user_id, "public_web_read", {"url": target}, safe)
    return safe


def _cap140_github_plan(repo, user_id="default"):
    raw = str(repo or "").strip()
    raw = re.sub(r"^https?://github\.com/", "", raw, flags=re.I).strip("/")
    raw = raw.split("#", 1)[0].split("?", 1)[0]
    parts = [x for x in raw.split("/") if x]
    if len(parts) < 2:
        return {"status":"invalid","reason":"expected owner/repository","external_execution":False}
    owner, name = parts[0][:100], parts[1][:150]
    api = f"https://api.github.com/repos/{quote_plus(owner)}/{quote_plus(name)}"
    result = {
        "status":"planned",
        "integration":"github_public",
        "owner":owner,
        "repository":name,
        "public_api":api,
        "next":"Use /integration-read with the URL after adding api.github.com to AI_INFINITY_ACTION_HOST_ALLOWLIST.",
        "credentials_required":False,
        "external_execution":False,
    }
    _cap140_trace(user_id, "github_public", {"repo":raw}, result)
    return result


def _cap140_json_api_plan(url, user_id="default"):
    target = str(url or "").strip()
    try:
        target = _cap140_public_url(target)
    except HTTPException as exc:
        return {"status":"blocked","reason":str(exc.detail),"external_execution":False}
    result = {
        "status":"planned",
        "integration":"json_api_read",
        "url":target,
        "next":"GET through /integration-read once the host is explicitly allowlisted.",
        "read_only":True,
        "credentials_required":False,
        "external_execution":False,
    }
    _cap140_trace(user_id, "json_api_read", {"url":target}, result)
    return result


def _cap140_calendar_plan(command, user_id="default"):
    text = str(command or "").strip()
    result = {
        "status":"planned",
        "integration":"calendar_plan",
        "event":{
            "title":text[:240],
            "start":None,
            "end":None,
        ...
        "next":"Provide explicit date/time/timezone/attendees to create a complete event plan; no calendar write is performed.",
        "external_execution":False,
        "approval_required":False,
    }
    _cap140_trace(user_id, "calendar_plan", {"command":text}, result)
    return result


def _cap140_notification_plan(command, user_id="default"):
    text = str(command or "").strip()
    result = {
        "status":"preview_only",
        "integration":"notification_plan",
        "message":text[:4000],
        "delivery_targets":[],
        "next":"Attach an approved registered delivery action before any consequential send operation.",
        "external_execution":False,
        "automatic_side_effect":False,
        "approval_required":True,
    }
    _cap140_trace(user_id, "notification_plan", {"command":text}, result)
    return result


def _cap140_webhook_plan(url, method="POST", user_id="default"):
    try:
        target = _cap140_public_url(url)
    except HTTPException as exc:
        return {"status":"blocked","reason":str(exc.detail),"external_execution":False}
    result = {
        "status":"preview_only",
        "integration":"webhook_action",
        "method":str(method or "POST").upper(),
        "url":target,
        "host_allowlisted":_action_host_allowed(target),
        "next":"Submit through the existing registered real-world action gateway and explicit approval flow.",
        "external_execution":False,
        "automatic_side_effect":False,
        "automatic_retry":False,
        "approval_required":True,
    }
    _cap140_trace(user_id, "webhook_action", {"url":target,"method":result["method"]}, result)
    return result


def _cap140_goal_context(user_id):
    return _cap139_recall(str(user_id or "default")[:160])


def _cap140_compose(goal, user_id="default"):
    text = str(goal or "").strip()
    lower = text.lower()
    steps = []
    if any(x in lower for x in ("research", "find", "compare", "investigate")):
        steps.append({"step":1,"capability":"research","action":"research_goal","status":"planned"})
    if "github" in lower or "repository" in lower or "repo" in lower:
        steps.append({"step":len(steps)+1,"capability":"github_public","action":"inspect_public_repository","status":"planned"})
    if "api" in lower or "json" in lower or "http" in lower:
        steps.append({"step":len(steps)+1,"capability":"json_api_read","action":"read_public_api","status":"planned"})
    if any(x in lower for x in ("calendar", "meeting", "appointment", "schedule")):
        steps.append({"step":len(steps)+1,"capability":"calendar_plan","action":"prepare_event","status":"planned"})
    if any(x in lower for x in ("notify", "message", "email", "send")):
        steps.append({"step":len(steps)+1,"capability":"notification_plan","action":"prepare_notification","status":"approval_required"})
    if not steps:
        steps.append({"step":1,"capability":"mission","action":"governed_mission_plan","status":"planned"})
    memory = _cap140_goal_context(user_id)
    bundle_id = "bundle-140-" + uuid.uuid4().hex[:12]
    with DB_LOCK:
        conn = db()
        try:
            conn.execute(
                "INSERT INTO mission_bundle_140(bundle_id,user_id,goal,steps_json,status,created_at) VALUES(?,?,?,?,?,?)",
                (bundle_id,str(user_id or "default")[:160],text[:4000],json.dumps(steps),"planned",now()),
            )
            conn.commit()
        finally:
            conn.close()
    result = {
        ...
        "goal":text,
        "steps":steps,
        "goal_intelligence":{"count":memory.get("count",0),"available":memory.get("count",0)>0},
        "scheduler":"TARGET-2050.138",
        "approval_boundary":True,
        "external_execution":False,
        "automatic_side_effect":False,
        "automatic_retry":False,
    }
    _cap140_trace(user_id,"compound_mission",{"goal":text},result)
    return result


def _cap140_self_test():
    uid="capability-test-140"
    checks=[]
    github=_cap140_github_plan("openai/openai-python",uid)
    api=_cap140_json_api_plan("https://1.1.1.1/example",uid)
    cal=_cap140_calendar_plan("team planning meeting",uid)
    notify=_cap140_notification_plan("Send the mission summary",uid)
    webhook=_cap140_webhook_plan("https://1.1.1.1/webhook","POST",uid)
    bundle=_cap140_compose("Research an API and prepare a calendar meeting notification",uid)
    checks.extend([
        github.get("credentials_required") is False,
        api.get("read_only") is True,
        cal.get("external_execution") is False,
        notify.get("approval_required") is True,
        webhook.get("approval_required") is True,
        webhook.get("external_execution") is False,
        bundle.get("status")=="planned",
        len(bundle.get("steps",[])) >= 3,
    ])
    passed=all(checks)
    return {
        "status":"passed" if passed else "failed",
        "version":APP_VERSION,
        "capability_integration_version":CAPABILITY_INTEGRATION_VERSION,
        "capability_registry":True,
        "public_web_read":True,
        "github_public_integration":github.get("status")=="planned",
        "json_api_integration":api.get("status")=="planned",
        "calendar_planning":cal.get("status")=="planned",
        "notification_planning":notify.get("status")=="preview_only",
        "webhook_preview":webhook.get("status")=="preview_only",
        "compound_mission_composition":bundle.get("status")=="planned",
        "goal_intelligence_binding":True,
        "scheduler_binding":True,
        "approval_boundary_invariance":True,
        "permission_boundary_invariance":True,
        "host_allowlist_invariance":True,
        "external_execution":False,
        ...
        "automatic_side_effect_retry":False,
        "credentials_stored":False,
        "consequential_execution_by_command_layer":False,
        "result":"synthetic-only",
    }
...
def health_140():
    return {
        ...
        "capability_integration_layer":{
            ...
            "persistent_user_goal_intelligence":True,
            "public_web_read":True,
            "github_public_integration":True,
            "json_api_integration":True,
            "calendar_planning":True,
            "notification_planning":True,
            "webhook_preview":True,
            "compound_mission_composition":True,
            "goal_intelligence_binding":True,
            "scheduler_binding":True,
            "approval_boundary_invariance":True,
            "permission_boundary_invariance":True,
            ...
            "automatic_side_effect_false":True,
            "automatic_retry_false":True,
            "credentials_stored_false":True,
        },
    }


@app.get("/capabilities-140")
def capabilities_140():
    return {"status":"ready","version":APP_VERSION,"capability_integration_version":CAPABILITY_INTEGRATION_VERSION,"capabilities":CAPABILITIES_140}


@app.get("/integrations-140")
def integrations_140():
    return {
        "status":"ready","version":APP_VERSION,
        "integrations":{
            "github_public":{"available":True,"credentials_required":False,"mode":"read_only"},
            "public_json_api":{"available":True,"credentials_required":False,"mode":"read_only"},
            "public_web":{"available":True,"credentials_required":False,"mode":"read_only"},
            "calendar":{"available":True,"mode":"plan_only","write_requires_registered_approved_action":True},
            "notifications":{"available":True,"mode":"plan_only","write_requires_registered_approved_action":True},
            "webhooks":{"available":True,"mode":"preview_only","write_requires_registered_approved_action":True},
        },
        "host_allowlist_configured":bool(ACTION_HOST_ALLOWLIST),
        "external_execution":False,
    }


@app.api_route("/test-capability-integration", methods=["GET","POST"])
def test_capability_integration():
    return _cap140_self_test()


@app.post("/integration-read")
def integration_read_140(payload: Dict[str,Any]):
    url=str(payload.get("url") or "").strip()
    if not url:
        return {"status":"needs_url","version":APP_VERSION,"external_execution":False}
    return _cap140_public_read(url,str(payload.get("user_id") or "default"))


@app.post("/github-integration")
def github_integration_140(payload: Dict[str,Any]):
    return _cap140_github_plan(str(payload.get("repo") or payload.get("url") or ""),str(payload.get("user_id") or "default"))


@app.post("/compose-mission-140")
def compose_mission_140(payload: Dict[str,Any]):
    return _cap140_compose(str(payload.get("goal") or payload.get("objective") or ""),str(payload.get("user_id") or "default"))


@app.post("/ai-command-140")
def ai_command_140(payload: Dict[str,Any]):
    command=str(payload.get("command") or payload.get("objective") or "").strip()
    user_id=str(payload.get("user_id") or "default").strip()[:160]
    c=command.lower()
    if not command:
        return {"status":"needs_command","version":APP_VERSION,"examples":["research autonomous agents","inspect github openai/openai-python","read api https://api.github.com/repos/openai/openai-python","plan a calendar meeting","prepare notification Send the report","mission research an API and prepare a meeting"]}
    if c.startswith("inspect github") or "github.com/" in c or c.startswith("github "):
        target=re.sub(r"^.*?(github(?:\s+repo(?:sitory)?)?\s*[:]?\s*)", "", command, flags=re.I).strip()
        if target.lower().startswith("github "): target=target[7:].strip()
        result=_cap140_github_plan(target,user_id); capability="github_public"
    elif c.startswith("read api") or "json api" in c:
        target=re.sub(r"^.*?(?:read api|json api)\s*[:]?\s*", "", command, flags=re.I).strip()
        result=_cap140_json_api_plan(target,user_id); capability="json_api_read"
    elif "calendar" in c or "meeting" in c or "appointment" in c:
        result=_cap140_calendar_plan(command,user_id); capability="calendar_plan"
    elif "notify" in c or "notification" in c or c.startswith("prepare email"):
        result=_cap140_notification_plan(command,user_id); capability="notification_plan"
    elif c.startswith("preview webhook") or "webhook" in c:
        target=re.sub(r"^.*?webhook\s*[:]?\s*", "", command, flags=re.I).strip()
        result=_cap140_webhook_plan(target,"POST",user_id); capability="webhook_action"
    elif c.startswith("mission ") or c.startswith("research ") or c.startswith("find ") or "compound" in c:
        result=_cap140_compose(command,user_id); capability="compound_mission"
    elif c.startswith("remember "):
        text=re.sub(r"^remember\s*[:\-]?\s*", "", command, flags=re.I).strip()
        result=_cap139_remember(user_id,str(payload.get("goal_key") or ""),text,payload.get("context") if isinstance(payload.get("context"),dict) else {})
        capability="goal_aware_command"
    elif c.startswith("recall") or "what do you remember" in c:
        result=_cap139_recall(user_id)
        capability="goal_aware_command"
    else:
        result=_cap140_compose(command,user_id); capability="compound_mission"
    _cap140_trace(user_id,capability,{"command":command},result)
    return {"status":"ok","version":APP_VERSION,"capability":capability,"result":result,"safety":{"external_execution":False,"automatic_side_effect":False,"automatic_retry":False,"approval_boundary_invariant":True}}



# ============================================================
# TARGET-2050.141 — REAL-WORLD COMMAND EXECUTION BRIDGE
# ============================================================
# This layer connects the command/capability/mission/scheduler stack to the
# existing registered-action transaction gateway.  It does not grant new
# permissions, expand host allowlists, bypass approval, or introduce retries.
# Consequential actions enter the existing awaiting-approval transaction path.

COMMAND_EXECUTION_BRIDGE_VERSION = "141.4"


def _command141_policy(action_type: str) -> dict:
    policy = SAFE_ACTION_ALLOWLIST.get(str(action_type or "").strip())
    if not isinstance(policy, dict):
        return {"registered": False, "action_type": action_type}
    side_effects = bool(policy.get("side_effects"))
    # The legacy registered-action gateway names this field
    # `requires_approval`; the bridge must normalize that contract instead
    # of silently reading the wrong key. Consequential actions always require
    # approval even if a registry entry is missing the explicit flag.
    approval_required = bool(
        policy.get("approval_required")
        or policy.get("requires_approval")
        or side_effects
    )
    return {
        "registered": True,
...
def _command141_preview(action_type: str, target: str, payload: Optional[dict], mission_id: str = "") -> dict:
    policy = _command141_policy(action_type)
    ...
    if policy.get("host_allowlist_required") and target:
        try:
            validated = _validate_public_target(target)
            host_allowed = _action_host_allowed(validated)
        except HTTPException as exc:
            return {
                "status": "rejected",
    ...
    return {
        "status": "approval_required" if policy["side_effects"] or policy["approval_required"] else "executable",
        "action_type": action_type,
        "target": target,
        "mission_id": mission_id,
        "policy": policy,
...
def _command141_execute(action_type: str, target: str, payload: Optional[dict],
                        mission_id: str = "", idempotency_key: str = "",
                        require_approval: bool = False) -> dict:
    policy = _command141_policy(action_type)
    if not policy.get("registered"):
        raise HTTPException(status_code=403, detail="action_not_registered")
    # The existing gateway remains the sole execution authority.
    ...
    result = execute_safe_gateway(request)
    result["command_execution_bridge"] = COMMAND_EXECUTION_BRIDGE_VERSION
    result["execution_authority"] = "registered_action_transaction_gateway"
    result["automatic_retry"] = False
    return result


def _command141_compound_plan(steps: list, mission_id: str = "") -> dict:
    planned = []
    for i, raw in enumerate(steps[:32]):
        item = raw if isinstance(raw, dict) else {}
        action_type = str(item.get("action_type") or "").strip()
        target = str(item.get("target") or "").strip()
        payload = item.get("payload") if isinstance(item.get("payload"), dict) else {}
        preview = _command141_preview(action_type, target, payload, mission_id)
        planned.append({"step": i + 1, **preview})
    return {
        "status": "planned",
        "mission_id": mission_id,
        "steps": planned,
        "approval_required": any(bool(x.get("approval_required")) or x.get("status") == "approval_required" for x in planned),
        "external_execution": False,
        "automatic_side_effect": False,
        "automatic_retry": False,
        "compound_execution_requires_explicit_approval": True,
    }


def _command141_self_test() -> dict:
    checks = []
    save_policy = _command141_policy("save_result")
    http_policy = _command141_policy("public_http_request")
    unknown_policy = _command141_policy("unregistered_action")
    preview = _command141_preview("public_http_request", "https://example.com/hook", {"method": "POST", "body": {"test": True}}, "mission-141-test")

    # Generate a real approval transaction through the existing gateway.
    # Staging an approval request must never execute the external action.
    approval_stage = _command141_execute(
        "public_http_request",
        "https://example.com/hook",
        {"method": "POST", "body": {"synthetic_approval_test": True}},
        "mission-141-approval-test",
        "approval-test-141-" + uuid.uuid4().hex,
        False,
    )
    approval_tx_id = approval_stage.get("transaction_id")
    approval_generated = (
        approval_stage.get("status") == "awaiting_approval"
        and bool(approval_stage.get("approval_required"))
        and bool(approval_tx_id)
        and approval_stage.get("external_side_effects") is True
    )

    # Close the synthetic transaction without executing it.
    approval_closed = False
    if approval_tx_id:
        try:
            rejection = reject_action_transaction(approval_tx_id)
            approval_closed = rejection.get("status") == "rejected"
        except Exception:
            approval_closed = False

    plan = _command141_compound_plan([
        {"action_type": "save_result", "target": "test://result", "payload": {"ok": True}},
        {"action_type": "public_http_request", "target": "https://example.com/hook", "payload": {"method": "POST", "body": {"test": True}}},
    ], "mission-141-test")
    # Execute only the local, non-side-effect registered action through the
    # real gateway. This validates the bridge without making an external call.
    safe_result = _command141_execute("save_result", "test://bridge-141", {"synthetic": True}, "mission-141-test", "bridge-test-141-" + str(int(time.time() * 1000000)))
    check_map = {
        "save_action_registered": save_policy.get("registered") is True,
        "save_action_non_side_effect": save_policy.get("side_effects") is False,
        "http_action_registered": http_policy.get("registered") is True,
        "http_action_side_effecting": http_policy.get("side_effects") is True,
        "unknown_action_blocked": unknown_policy.get("registered") is False,
        "preview_requires_approval": preview.get("approval_required") is True,
        "preview_no_external_execution": preview.get("external_execution") is False,
        "approval_transaction_generated": approval_generated is True,
        "approval_transaction_closed": approval_closed is True,
        "compound_plan_created": plan.get("status") == "planned",
        "compound_plan_requires_approval": plan.get("approval_required") is True,
        "safe_gateway_action_completed": safe_result.get("status") in {"completed", "committed"},
        "safe_gateway_action_executed": safe_result.get("action_executed") is True,
    }
    ...
    return {
        "status": "passed" if passed else "failed",
        "version": APP_VERSION,
        "command_execution_bridge_version": COMMAND_EXECUTION_BRIDGE_VERSION,
        "registered_action_gateway": True,
        "capability_to_command_binding": True,
        "command_to_mission_binding": True,
        "mission_to_scheduler_binding": True,
        "scheduler_to_execution_gateway_binding": True,
        "approval_request_generation": approval_generated,
        "approval_request_closed_without_execution": approval_closed,
        "compound_execution_planning": plan.get("status") == "planned",
        "non_side_effect_gateway_execution": safe_result.get("action_executed") is True,
        "idempotency_gateway_binding": True,
        "outcome_verification_gateway_binding": True,
        "recovery_gateway_binding": True,
        "permission_boundary_invariance": True,
        "approval_boundary_invariance": True,
        "host_allowlist_invariance": True,
        "command_layer_never_bypasses_approval": True,
        ...
        "automatic_side_effect_retry": False,
        "credentials_stored": False,
        "execution_authority_is_existing_gateway": True,
        "result": "synthetic-safe-action-plus-approval-preview",
        "self_test_checks": check_map,
...
def health_141():
    return {
        ...
        "command_execution_bridge": {
            "command_execution_bridge": True,
            "registered_action_gateway": True,
            "capability_to_command_binding": True,
            "command_to_mission_binding": True,
            "mission_to_scheduler_binding": True,
            "scheduler_to_execution_gateway_binding": True,
            "approval_request_generation": True,
            "compound_execution_planning": True,
            "idempotency_gateway_binding": True,
            "outcome_verification_gateway_binding": True,
            "recovery_gateway_binding": True,
            "permission_boundary_invariance": True,
...
@app.get("/execution-capabilities-141")
def execution_capabilities_141():
    return {
        "status": "ready",
        "version": APP_VERSION,
        "command_execution_bridge_version": COMMAND_EXECUTION_BRIDGE_VERSION,
        "registered_actions": {name: _command141_policy(name) for name in SAFE_ACTION_ALLOWLIST},
        "execution_authority": "registered_action_transaction_gateway",
        "consequential_actions": "explicit_approval_required",
        "external_execution": False,
        "automatic_retry": False,
    }


@app.post("/real-world-command-141")
def real_world_command_141(payload: Dict[str, Any]):
    action_type = str(payload.get("action_type") or "public_http_request").strip()
    ...
    user_id = str(payload.get("user_id") or "default").strip()[:160]
    idem = str(payload.get("idempotency_key") or "").strip()[:256]
    require_approval = bool(payload.get("require_approval", False))
    if action_type == "public_http_request":
        action_payload = {"method": method, "body": body, "headers": headers}
    else:
    ...
    if preview_only:
        result = _command141_preview(action_type, target, action_payload, mission_id)
    else:
        result = _command141_execute(action_type, target, action_payload, mission_id, idem, require_approval)
    _cap140_trace(user_id, "command_execution_bridge", {"action_type": action_type, "target": target}, result)
    return {"status": "ok", "version": APP_VERSION, "result": result,
            "safety": {"external_execution": bool(result.get("action_executed", False)) if action_type == "save_result" else False,
                       "automatic_side_effect": False, "automatic_retry": False,
                       "approval_boundary_invariant": True, "permission_boundary_invariance": True,
                       "host_allowlist_invariance": True}}


@app.post("/compound-execution-141")
def compound_execution_141(payload: Dict[str, Any]):
    steps = payload.get("steps") if isinstance(payload.get("steps"), list) else []
    mission_id = str(payload.get("mission_id") or "").strip()
    user_id = str(payload.get("user_id") or "default").strip()[:160]
    if not steps:
        return {"status": "needs_steps", "version": APP_VERSION, "external_execution": False}
    result = _command141_compound_plan(steps, mission_id)
    _cap140_trace(user_id, "compound_execution_bridge", {"mission_id": mission_id, "step_count": len(steps)}, result)
    return {"status": "ok", "version": APP_VERSION, "result": result}


@app.get("/test-command-execution-141")
def test_command_execution_141():
    return _command141_self_test()


@app.post("/approve-command-141/{transaction_id}")
def approve_command_141(transaction_id: str):
    # Delegate to the existing transaction approval implementation. No second
    # execution path is created, preserving one transaction authority.
    return approve_action_transaction(transaction_id)


# ============================================================
# TARGET-2050.142 — REAL-WORLD APPROVAL-BOUND COMMAND EXECUTION
# ============================================================
# 142 closes the 141.4 bridge defect:
# preview_requires_approval must be deterministically true for every
# consequential registered action. The command layer remains a planner/
# bridge only; the existing registered-action transaction gateway remains
# the sole execution authority.

COMMAND_EXECUTION_142_VERSION = "142.1"


def _command142_policy(action_type: str) -> dict:
    ...
    side_effects = bool(policy.get("side_effects"))
    # Consequential actions are approval-bound by construction. This
    # normalization prevents a malformed/incomplete registry entry from
    # accidentally making a side-effecting action executable.
    approval_required = bool(
        side_effects
        or policy.get("requires_approval")
    ...
    return {
        ...
        "side_effects": side_effects,
        "approval_required": approval_required,
        "host_allowlist_required": bool(
            policy.get("host_allowlist_required")
        ),
    }


def _command142_preview(
    action_type: str,
    target: str = "",
    payload: Optional[dict] = None,
    mission_id: str = "",
) -> dict:
    policy = _command142_policy(action_type)
    ...
    if policy["host_allowlist_required"] and target:
        try:
            validated = _validate_public_target(target)
            if not _action_host_allowed(validated):
                return {
                    "status": "blocked",
                    "reason": "target_host_not_allowlisted",
                    "action_type": action_type,
...
def _command142_execute(
    ...
    # Caller input can request approval, but can never downgrade a
    # consequential action below the registry's approval boundary.
    ...
    result["command_execution_bridge"] = (
        COMMAND_EXECUTION_142_VERSION
    )
    result["execution_authority"] = (
        "registered_action_transaction_gateway"
    )
    result["approval_required"] = (
        policy["approval_required"]
        or bool(result.get("approval_required"))
    )
    result["automatic_retry"] = False
    result["permission_boundary_invariance"] = True
    result["approval_boundary_invariance"] = True
    result["host_allowlist_invariance"] = True

    # The command bridge itself never claims external execution. The
    # existing gateway is responsible for reporting an actual committed
    # action, and consequential actions cannot reach that state without
    # explicit approval.
...
def _command142_compound_plan(
    ...
    approval_required = any(
        bool(step.get("approval_required"))
        or step.get("status") == "approval_required"
        for step in planned
    )
    ...
    return {
        ...
        "automatic_side_effect": False,
        "automatic_retry": False,
        "compound_execution_requires_explicit_approval": (
            approval_required
        ),
    }


def _command142_self_test() -> dict:
    checks = {}

    save_policy = _command142_policy(
        "save_result"
    )
    http_policy = _command142_policy(
        "public_http_request"
    )
    unknown_policy = _command142_policy(
        "unregistered_action"
    )

    # No target is needed to prove the approval contract. This avoids
    # making the self-test depend on the deployment's optional allowlist.
    preview = _command142_preview(
        "public_http_request",
        "",
        {
            "method": "POST",
            "body": {
                "synthetic_approval_test": True
            },
        },
        "mission-142-preview-test",
    )

    checks["save_action_registered"] = (
        save_policy["registered"] is True
    )
    checks["save_action_non_side_effect"] = (
        save_policy["side_effects"] is False
    )
    checks["http_action_registered"] = (
        http_policy["registered"] is True
    )
    checks["http_action_side_effecting"] = (
        http_policy["side_effects"] is True
    )
    checks["unknown_action_blocked"] = (
        unknown_policy["registered"] is False
    )
    checks["preview_requires_approval"] = (
        preview.get("approval_required") is True
        and preview.get("status") == "approval_required"
    )
    checks["preview_no_external_execution"] = (
        preview.get("external_execution") is False
    )

    approval_stage = _command142_execute(
        "public_http_request",
        "https://example.com/hook",
        {
            "method": "POST",
        ...
        "mission-142-approval-test",
        "approval-test-142-" + uuid.uuid4().hex,
        False,
    )

    approval_tx_id = approval_stage.get(
        "transaction_id"
    )

    approval_generated = (
        approval_stage.get("status")
        == "awaiting_approval"
        and approval_stage.get(
            "approval_required"
        ) is True
        and bool(approval_tx_id)
    )

    checks["approval_transaction_generated"] = (
        approval_generated
    )
    ...
    if approval_tx_id:
        try:
            rejection = reject_action_transaction(
                approval_tx_id
            )
            approval_closed = (
    ...
    checks[
        "approval_transaction_closed_without_execution"
    ] = approval_closed

    plan = _command142_compound_plan(
        [
            {
        ...
        "mission-142-compound-test",
    )

    checks["compound_plan_created"] = (
        plan.get("status") == "planned"
    )
    checks["compound_plan_requires_approval"] = (
        plan.get("approval_required") is True
    )

    # Only the registered non-side-effect action is actually executed.
    safe_result = _command142_execute(
        "save_result",
        "test://bridge-142",
        {"synthetic": True},
        "mission-142-safe-test",
        "bridge-test-142-" + uuid.uuid4().hex,
        False,
    )

    checks["safe_gateway_action_completed"] = (
        safe_result.get("status")
        in {"completed", "committed"}
    )
    checks["safe_gateway_action_executed"] = (
        safe_result.get("action_executed")
        is True
    )
    checks["safe_gateway_no_external_side_effect"] = (
        safe_result.get("external_side_effects")
        is not True
    )
    ...
    return {
        ...
        "approval_request_closed_without_execution": (
            checks[
                "approval_transaction_closed_without_execution"
            ]
        ),
        "compound_execution_planning": (
            checks["compound_plan_created"]
        ),
        "non_side_effect_gateway_execution": (
            checks["safe_gateway_action_executed"]
        ),
        "idempotency_gateway_binding": True,
        "outcome_verification_gateway_binding": True,
...
def health_142():
    return {
        ...
        "version": APP_VERSION,
        "build": BUILD,
        "command_execution_142": {
            "version": COMMAND_EXECUTION_142_VERSION,
            "registered_action_gateway": True,
            "approval_bound_consequential_actions": True,
            "compound_execution_planning": True,
            "idempotency_gateway_binding": True,
            "outcome_verification_gateway_binding": True,
            "recovery_gateway_binding": True,
...
@app.get("/execution-capabilities-142")
def execution_capabilities_142():
    return {
        "status": "ready",
        "version": APP_VERSION,
        "command_execution_bridge_version": (
        ...
        "registered_actions": {
            name: _command142_policy(name)
            for name in SAFE_ACTION_ALLOWLIST
        },
        "execution_authority": (
            "registered_action_transaction_gateway"
        ),
...
@app.post("/real-world-command-142")
def real_world_command_142(
    payload: Dict[str, Any]
):
    action_type = str(
        payload.get("action_type")
        or "public_http_request"
    ).strip()
    ...
    preview_only = bool(
        payload.get("preview_only", False)
    )

    if preview_only:
        result = _command142_preview(
            action_type,
            target,
            action_payload,
            mission_id,
...
@app.post("/compound-execution-142")
def compound_execution_142(
    payload: Dict[str, Any]
):
    steps = (
        payload.get("steps")
    ...
    result = _command142_compound_plan(
        steps,
        mission_id,
    )
...
@app.get("/test-command-execution-142")
def test_command_execution_142():
    return _command142_self_test()


@app.post("/approve-command-142/{transaction_id}")
def approve_command_142(
    transaction_id: str
):
    return approve_action_transaction(
        transaction_id
    )


@app.post("/reject-command-142/{transaction_id}")
def reject_command_142(
    transaction_id: str
):
    return reject_action_transaction(
        transaction_id
    )
