...
def test_recovery_meta_learning():
    ...
    # Validate the decay mechanism independently of freshly-created synthetic rows.
    # Fresh test samples are intentionally near age 0, so they cannot reliably
    # demonstrate decay. A deterministic old timestamp verifies the actual rule.
    synthetic_old_timestamp = now() - (META_DECAY_DAYS * 2.0 * 86400.0)
    synthetic_decay_factor = _meta_decay(synthetic_old_timestamp)
    stale = synthetic_decay_factor < 1.0
    ...
    return {
        ...
        "synthetic_decay_factor": synthetic_decay_factor,
...
APP_VERSION = "TARGET-2050.128"
BUILD = "REAL-WORLD-TRANSACTION-RECOVERY-META-LEARNING-CROSS-MISSION-INTELLIGENCE-CORE"
